...
# ---------------------------------------------------------------------------
REFUSED=(
  "screens/a.css|.foo { font-family: var(--f-mono); }"
  "screens/a.css|.foo { font: 12px/1 \"Geist Mono\", monospace; }"
  "screens/a.css|.foo, code { font-family: var(--f-mono); }"
  ...
  "screens/a.css|pre { .label { font-family: var(--f-mono); } }"
  "screens/a.css|:root { --code-face: var(--f-mono); }"
...
plant "$CLEAN" screens/b.css '.foo code { font-family: var(--f-mono); }
pre.code-block:hover { font-family: monospace; }
.panel > .code-block { font-family: var(--f-mono); }
...
if [[ "$FAILURES" -ne 0 ]]; then
  ...
  echo "must leave code, pre, samp, .code-block and the --f-mono token alone." >&2
  exit 1
fi
