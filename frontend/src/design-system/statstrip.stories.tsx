...
// THE LINE FORM: the same readings as one ruled line rather than a plate of
// tiles, for a page whose readings frame the work under them (the Brief). The
// figure and its name share a baseline, the receipt is withheld, and the whole
// slot is the door — the word in the foot is not drawn, and the slot lights
// under the pointer to say so.
export const AsALine: Story = {
  render: () => (
    <StatStrip shape="line">
      <StatCard
        label="Urgent"
        value="6"
        tone="warn"
      ...
      <StatCard
        ...
        detail="all prepared"
        onOpen={() => {}}
        openLabel="View today's meetings"
      />
      <StatCard label="Leads to contact" value="0" detail="none assigned" />
      <StatCard
        label="Deal value needing attention"
        value="€384.5k"
        onOpen={() => {}}
        openLabel="Review deal work"
      />
      <StatCard
        label="Reviews needed"
        value="9"
        detail="proposals and record checks"
        onOpen={() => {}}
        openLabel="Review decisions"
      />
    </StatStrip>
  ),
};
