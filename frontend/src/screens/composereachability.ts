...
//
// Both answers come off the contact's 360, which is why they belong together:
// the composer holds that payload already, and two hooks reaching for the same
// view would ask for it twice.

// WHICH OF THESE ADDRESSES IS KNOWN NOT TO ARRIVE. The contact page badges an
...
// own. That payload comes back under the SAME key the contact page fetches under,
...
// learned a contact — a deal timeline has no single one — is handed nothing and
// warns about nothing.
//
...
//
// Free-typed addresses with no contact context stay unwarned on purpose (#3160):
...
// the contact the row's own timeline names: `["contact", contactId]` is the same
// query key the 360 screen already fetches under, so this rides its cache
// instead of opening a second request. A caller that never learned a contactId
// (e.g. a deal timeline, which has no single contact to check) gets the
...
export function useChannelReachable(
  ...
  if (!isChannel || contactId == null) return true;
  ...
  // Telegram-reachable contact's rows whatever carried the conversation — the
  ...
  return (contact.data?.reachability ?? []).some(
