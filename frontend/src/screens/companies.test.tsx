...
describe("CompanyScreen — edit with If-Match (P-1)", () => {
  it("PATCHes /companies/{id} with If-Match:<version> and only update fields", async () => {
    ...
    // The company fixture carries no domains, so an edit that doesn't touch them
...
// B7: the edit modal's repeatable domains field replace-sets the company's live
...
describe("mapCompanyUpdate — domains change detection (P1)", () => {
  ...
  it("sends [] when every domain row is removed (clear all)", () => {
    const body = mapCompanyUpdate({ display_name: "X" }, { domains: [] }, [
      dom("a.test", true),
    ]);
...
// B5: the Firmographics & legal card renders the company's confirmed profile
...
describe("CompanyScreen — archive (P-3)", () => {
  it("opens a confirm, DELETEs /companies/{id} on confirm, and navigates to the list", async () => {
    let deleted = false;
...
describe("CompanyScreen — merge into target (P-2)", () => {
  ...
  it("searches, excludes the source row, and merges into the picked target", async () => {
    ...
    stubFetch(async (url, method, request) => {
      if (method === "POST" && url.includes("/companies/o-1/merge")) {
...
describe("CompanyScreen — hierarchy roll-up in the rail (P-7)", () => {
  ...
  ])("discloses %i account(s) the viewer cannot read", async (hidden, said) => {
    stubFetch(
      async (url) => {
        if (url.includes("/activities")) {
          return jsonResponse({ data: [] });
...
describe("CompanyScreen — relationship kinds by scope (P-5)", () => {
  it("offers company↔company kinds (not deal_stakeholder) from a company and POSTs counterparty_company_id", async () => {
    const user = userEvent.setup();
    let posted: unknown = null;
    stubFetch(async (url, method, request) => {
      if (method === "POST" && url.includes("/relationships")) {
...
describe("CompanyScreen — the record's history", () => {
  ...
  it("opens the full history from the overflow menu", async () => {
    stubFetch(async (url) => {
      if (url.includes("/records/company/o-1/history")) {
        return jsonResponse({
          data: [
            {
              id: "h1",
...
describe("CompanyScreen — next-step suggestions", () => {
  it("leads each suggestion with the reason the rule fired, and cites the record", async () => {
    const three60 = { ...company360, suggestions: [stalledSuggestion] };
  ...
  it("dismisses by fingerprint and leaves the row for the server to remove", async () => {
    ...
    stubFetch(
      ...
      { company360: { ...company360, suggestions: [stalledSuggestion] } },
    );
    renderSuggestionsFor({ ...company360, suggestions: [stalledSuggestion] });
...
// `UpdateCompanyRequest.owner_id` cannot carry "unassign" — a null is
...
describe("CompanyScreen — the timeline filter does not follow you", () => {
  it("returns to All when another company is opened", async () => {
    stubFetch(async (url) =>
      /\/companies\/o-\d$/.test(new URL(url).pathname)
...
describe("CompanyScreen — the Partner tab is scoped to the account being read", () => {
  ...
  function stubTwoCompanies() {
    stubFetch(async (url) => {
      ...
      if (pathname.endsWith("/companies/o-1")) {
        return jsonResponse(partnerCompany);
      }
      if (pathname.endsWith("/companies/o-2")) {
        return jsonResponse(nonPartnerCompany);
      }
      return emptyPage();
    });
  }
  ...
  function renderTwoCompanies() {
    const client = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
    client.setQueryData(["company", "o-1"], partnerCompany);
    client.setQueryData(["company", "o-2"], nonPartnerCompany);
    const page = (id: string) => (
      <QueryClientProvider client={client}>
        <LocaleProvider initial="en">
          <CompanyScreen id={id} />
  ...
  it("does not carry the Partner tab to an account with no programme", async () => {
    stubTwoCompanies();
    const { rerender, page } = renderTwoCompanies();
