...
/** useCompanyGraph reads the account's one-hop connections. */
export function useCompanyGraph(id: string, enabled = true) {
  return useQuery({
    enabled,
    ...
    staleTime: 60_000,
    queryKey: ["company-graph", id],
    queryFn: async () => {
      const { data, error } = await api.GET("/companies/{id}/graph", {
        params: { path: { id } },
      });
      if (error || !data) {
