...
// click starts (or joins — idempotent per company+url) a background crawl of the
...
function DeepReadCard({ companyId }: Readonly<{ companyId: string }>) {
  const t = useT();
...
// P-7: the company hierarchy roll-up (weighted pipeline, current-quarter
...
// zero-filled figure.
function HierarchyRollupCard({ companyId }: Readonly<{ companyId: string }>) {
  const t = useT();
  const { locale } = useLocale();
  const recordZone = useRecordZone();
...
export function CompanyScreen({ id }: Readonly<{ id: string }>) {
  ...
  // one the reader saw.
  useAcknowledgeCompanyView(id, view.data?.state === "ready");
...
function CompanyRecord({
  ...
  company: Company;
  view: { data?: Company360Result; isPending: boolean; isError: boolean };
  tab: CompanyTab;
...
function CompanyRecordBody({
  ...
  return (
    <>
      ...
      {!overlay && tab === "partner" && <PartnerTab companyId={company.id} />}
...
function CompanyOverviewStack({
  ...
  const scan = useAccountScan(company.id, !overlay && view !== undefined);
      ...
      tools={
        <>
          ...
          <Panel title={t("co.tools.title")}>
            <PanelBody>
              <CustomFieldsCard object="company" record={company} />
              <HierarchyRollupCard companyId={company.id} />
              ...
              {!offerOnOverview && <DeepReadCard companyId={company.id} />}
              ...
              <TechnicalProfileCard companyId={company.id} />
            </PanelBody>
          </Panel>
        </>
      }
