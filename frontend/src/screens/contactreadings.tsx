...
import { owedPromises, owedPromisesTruncated } from "./contactowed";
...
// same span the rail's own overall reading turns on (contactrail.tsx), so the
// card and the rail cannot disagree about when silence became a problem.

export function ContactReadings({
  view,
  onOpenTab,
}: Readonly<{
  view: Contact360;
...
function MoveCard({
  ...
  const basis = (
    <FactList
      facts={[
        {
          key: "in",
          term: t("contact.strip.lastInbound"),
          value: relativeDays(inbound, t, locale, asOf),
        },
        {
          key: "out",
          term: t("contact.strip.lastOutbound"),
  ...
  if (!inbound && !outbound) {
    return (
      <StatCard
        label={label}
        value={t("contact.readings.neverSpoke")}
        {...basisProps}
      />
    );
  }
    ...
    <StatCard
      ...
      detail={
        inbound
          ...
          : t("contact.readings.neverReplied")
      }
...
function reciprocityFact(
  ...
  return [
    {
      key: "exchanged",
      term: t("contact.strip.reciprocity"),
      value: t("contact.strip.inOut", {
...
function PromisesCard({
  ...
  const label = t("contact.readings.promises");
  if (withheld) {
    return withheldCard(label, t);
  }
  ...
  // about the same contact, on a record whose only open promise was a task.
  ...
  if (ours.length === 0) {
    return (
      <StatCard
        ...
        detail={t("contact.readings.nothingOwed")}
      />
    );
  }
...
function DealCard({
  ...
  const label = t("contact.readings.deal");
  if (withheld) {
    return withheldCard(label, t);
  }
  ...
  if (!deal) {
    return (
      <StatCard
        onOpen={onOpen}
        label={label}
        value={t("contact.strip.noOpenDeal")}
      />
    );
  }
  const priced =
        ...
        view.commercial?.role ? (
          <FactList
            facts={[
              {
                key: "role",
                term: t("contact.page.buyingRole"),
...
function MeetingCard({
  ...
  const label = t("contact.strip.nextMeeting");
  if (withheld) {
    return withheldCard(label, t);
  }
  ...
  if (!meeting) {
    return (
      <StatCard
        onOpen={onOpen}
        label={label}
        value={t("contact.strip.noMeeting")}
      />
    );
  }
  return (
