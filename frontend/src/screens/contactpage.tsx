...
const TAB_LABEL_KEYS: Readonly<Record<ContactTab, MessageKey>> = {
...
const COMPOSER_INTENT_KEYS: Readonly<Record<string, MessageKey>> = {
  agenda: "contact.composer.intentAgenda",
  reply: "contact.composer.intentReply",
  deliver_commitment: "contact.composer.intentCommitment",
  follow_up: "contact.composer.intentFollowUp",
};
...
function ProfileLink({ href }: Readonly<{ href: string }>) {
  const t = useT();
  const label = t("contact.page.linkedin");
  if (!linkedinUrl(href)) {
    return label;
  }
  return (
...
/**
 * ContactTabPanel is what the tab bar chose, and nothing else: the page above
...
// standalone function rather than a closure inside ContactPageV2: the switch's
...
function runContactMomentAction(
  ...
  switch (destination.surface) {
    ...
    case "task":
      ...
      return;
    default:
      // Every surface ContactMomentDestination documents today is handled
...
export function ContactPageV2({
  id,
  tab,
}: Readonly<{ id: string; tab: ContactTab }>) {
  ...
  // the same of the same row (contactrail.tsx); this is the header's and the
  // tabs' copy of the answer, and the one sentence both point at.
  const readOnlyReasonId = useId();
  const readOnlyReason = useRecordWriteRefusal("contact", view.data?.contact, {
    archived: t("contact.rail.archivedReadOnly"),
  ...
  // `runContactMomentAction`, a standalone function rather than a closure here
      ...
      <ContactWriteTo contactId={id} onWrite={() => openComposer("")}>
        <div className="record-sheet">
          ...
          >
            ...
            <BriefQueue closeLabel={t("common.close")} />
          </RecordView>
        </div>
 ...
 * composers ContactMailDrawer replaced. The drawer leads with mail whenever the
...
function ContactWriteTo({
  ...
  const writeTo: WriteTo = (target) => {
    if (target.entityType === "contact" && target.entityId === contactId) {
      onWrite();
      return;
    }
    shell?.(target);
 ...
 * composer: an address is something a contact can type, and a stray
...
function ContactMailDrawer({
  ...
  contactId: string;
  view: Contact360;
  // Which of the contact's addresses a FIRST message goes to. A contact carries
...
// side. Split out the way ContactMailDrawer is above, so this drawer's own
// branching stays out of ContactPageV2's cognitive weight.
function ContactActivityDrawer({
  contactId,
  drawer,
  onClose,
}: Readonly<{ contactId: string; drawer: Drawer; onClose: () => void }>) {
