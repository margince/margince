...
function fields(allow: Parameters<typeof meRoute>[0]) {
  return () => {
    ...
    return (
      <StoryProviders>
        <EnrichedFields contactId="p-1" view={view} />
      </StoryProviders>
    );
  };
}

const MAY_CORRECT = { contact: ["update"] as const };
...
// verdicts are a write, so a seat without `contact:update` gets neither button.
