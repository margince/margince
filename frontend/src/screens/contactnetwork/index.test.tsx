...
import { ContactNetworkTab } from "./index";
...
>;
type Route = NonNullable<ContactGraph["routes"]>[number];
  ...
  render(
    <QueryClientProvider client={client}>
      <LocaleProvider initial={locale}>
        <ContactNetworkTab contactId={CONTACT} view={beside.view} />
      </LocaleProvider>
    </QueryClientProvider>,
  );
  return { seatAnswered };
...
describe("a route that cannot be asked for", () => {
  const taken: NonNullable<ContactGraph["routes"]>[number] = {
    route_id: "direct:1",
    route_type: "direct",
  ...
  it("is named the same way wherever it is drawn", async () => {
    ...
    const label = en["contact.intro.unavailable"];
    await waitFor(() => {
  ...
  it("offers no button that would answer 409", async () => {
    ...
    await screen.findByText(en["contact.intro.unavailable"]);
    expect(
      screen.queryByRole("button", {
        name: en["contact.intro.askFirstName"].replace(
          "{name}",
...
describe("several ways in", () => {
  const direct: NonNullable<ContactGraph["routes"]>[number] = {
    route_id: "direct:1",
    route_type: "direct",
    via_user_id: "018f3a1b-0000-7000-8000-0000000000a1",
    via_display_name: "Sofia Meier",
  ...
  const viaContact: NonNullable<ContactGraph["routes"]>[number] = {
    ...direct,
    route_id: "through:1",
  ...
  it("counts the colleagues and says how many go through a contact", async () => {
    ...
    expect(screen.getAllByText(en["contact.band.moderate"]).length).toBe(1);
  });
  ...
  it("opens the ask drawer from an alternative's own button", async () => {
    ...
    await user.click(
      await screen.findByRole("button", {
        name: en["contact.intro.askFirstName"].replace("{name}", "Lena Hoff"),
      }),
    );
    expect(
      await screen.findByText(
...
describe("the verdict and its evidence", () => {
  ...
  it("cites the receipts and splits the count by who sent it", async () => {
    ...
    await screen.findByText(
      en["contact.intro.verdictDirect"].replace("{name}", "Sofia Meier"),
    );
    expect(screen.getByText(en["contact.intro.lastYesterday"])).toBeTruthy();
    expect(
      screen.getByText(
        en["contact.intro.evidenceFrom"]
          .replace("{count}", "4")
    ...
    await user.click(
      screen.getByRole("button", {
        name: en["contact.intro.askFirstName"].replace("{name}", "Sofia Meier"),
      }),
    );
    expect(
      await screen.findByText(
  ...
  it("names the colleague a route goes through, and says the counts are pooled", async () => {
    ...
    await screen.findByText(
      en["contact.intro.verdictVia"]
        .replace("{name}", "Lena Hoff")
        .replace("{through}", "Philipp Königs"),
    );
    expect(
      screen.getByText(en["contact.intro.lastDays"].replace("{days}", "6")),
    ).toBeTruthy();
    expect(screen.getByText(en["contact.graph.countsOnly"])).toBeTruthy();
  ...
  it("does not call unanswered sends a correspondence", async () => {
    ...
    await screen.findByText(
      en["contact.intro.verdictOneSided"].replace("{name}", "Martin Weber"),
    );
    expect(screen.getByText(en["contact.intro.factOneSided"])).toBeTruthy();
    expect(screen.getByText(en["contact.intro.lastNever"])).toBeTruthy();
  });
});
...
describe("the handoff with an ask open", () => {
  ...
  it("says the colleague owes an answer, and by when", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getAllByText(
          en["contact.intro.handoffOwner"].replace("{name}", "Sofia Meier"),
        ).length,
      ).toBe(2);
    });
    ...
    expect(
      screen.getByText(
        `${en["contact.intro.stepCurrent"]} · ${en["contact.intro.stepAwaitingAnswer"]}`,
      ),
    ).toBeTruthy();
  });
  ...
  it("hands the move back to the requester once the colleague has said yes", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getAllByText(
          en["contact.intro.handoffOwner"].replace("{name}", "Demo Admin"),
        ).length,
      ).toBe(2);
    });
    ...
    expect(
      screen.getByText(
        `${en["contact.intro.stepDone"]} · ${en["contact.intro.stateAccepted"]}`,
      ),
    ).toBeTruthy();
  });
});
...
describe("what changed lately", () => {
  ...
  it("flags the newest change as the reason to act", async () => {
    ...
    expect(
      screen.getByText(
        en["contact.change.warmed"]
          .replace("{from}", en["contact.band.weak"])
          .replace("{to}", en["contact.band.strong"]),
      ),
    ).toBeTruthy();
  });
  ...
  it("says nothing moved rather than drawing an empty list", async () => {
    ...
    expect(
      await screen.findByText(en["contact.network.noMoments"]),
    ).toBeTruthy();
...
describe("a peer the graph observed but nothing has recorded", () => {
  ...
  function withPeer(): ContactGraph {
    return {
      contact_id: CONTACT,
      nodes: [anchor, peer],
      edges: [
  ...
  it("can be selected on the map and offers the write", async () => {
    ...
    expect(
      await screen.findByRole("button", {
        name: en["contact.graph.recordWorksWith"].replace("{name}", "Rui Peer"),
      }),
    ).toBeTruthy();
  });
...
//
// Nothing in the graph read excludes the contact doing the reading, so on a
...
describe("the reader's own route", () => {
  ...
  const mine: NonNullable<ContactGraph["routes"]>[number] = {
  ...
  const colleague: NonNullable<ContactGraph["routes"]>[number] = {
    ...mine,
    route_id: "direct:2",
  ...
  function reachedByReader(
    routes: NonNullable<ContactGraph["routes"]>,
  ): ContactGraph {
    return {
      contact_id: CONTACT,
      nodes: [anchor],
  ...
  it("is written in the second contact and offers no introduction", async () => {
    const { seatAnswered } = renderTab(reachedByReader([mine]));

    expect(
      await screen.findByText(en["contact.intro.verdictDirectYou"]),
    ).toBeTruthy();
    expect(screen.getByText(en["contact.intro.heroYou"])).toBeTruthy();
    expect(screen.getByText(en["contact.intro.heroDirectYou"])).toBeTruthy();
    ...
    // "Demo Admin" about the same contact.
    expect(
      screen.getByText(
        en["contact.intro.evidenceFromYou"].replace("{count}", "5"),
      ),
    ).toBeTruthy();

    // The server refuses an ask whose introducer is the contact making it, so
    ...
    expect(
      screen.queryByRole("button", {
        name: en["contact.intro.askFirstName"].replace("{name}", "Demo Admin"),
      }),
    ).toBeNull();
    expect(screen.getByText(en["contact.intro.ownRouteNoAsk"])).toBeTruthy();
  });
  ...
  it("says who corresponds with whom in the second contact on the list too", async () => {
    ...
    expect(
      await screen.findByText(en["contact.graph.routeDirectYou"]),
    ).toBeTruthy();
  });
});
...
describe("using a route from the picture", () => {
  ...
  // A node in the drawing, which names its lane after the contact — anchored so
  ...
  function drawnGraph(viaUserId: string, name: string): ContactGraph {
    return {
      contact_id: CONTACT,
      nodes: [
        anchor,
  ...
  it("opens the ask for the colleague the node draws", async () => {
    ...
    await user.click(
      await screen.findByRole("button", {
        name: en["contact.intro.useThisRoute"],
      }),
    );
