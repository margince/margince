...
// The contact graph, drawn as the shared relationship map.
//
...
// this contact, and how warmly — would be two answers a reader has to reconcile.
//
...
/**
 * mapModelFromContactGraph turns one graph payload into the map's model.
 *
 ...
 * of contact around them — the others at their company, and the ones they are
 ...
 */
export function mapModelFromContactGraph(
  graph: Graph,
  copy: MapCopy,
): RelationshipMapModel {
  ...
  const mapNodes: MapNode[] = [
    ...
    ...account.map((n) => contactNode(n, "contact")),
    ...peers.map((n) => contactNode(n, "contact")),
    ...(anchor ? [contactNode(anchor, "contact")] : []),
  ];

  // Left: our colleagues. Centre: the contact themselves, because THEY are the
  // hub this whole picture is about. Right: the other contacts at their company,
  ...
  const lanes: MapLane[] = [
    ...
    // Under their company rather than beside it: both lanes hold contacts around
...
function contactNode(n: GraphNode, kind: "contact"): MapNode {
