        ...
        <div className="pn-verdict-main">
          ...
          <div className="pn-hero">
            ...
            <div className="pn-hero-who">
              <strong>
                {mine ? t("contact.intro.heroYou") : route.via_display_name}
              </strong>
          ...
          {/* The ask sits with the move, not in a band under both halves:
              ...
              The server refuses an ask whose introducer is the contact making
          ...
          {blocked ? null : mine ? (
            <p className="pn-counts">{t("contact.intro.ownRouteNoAsk")}</p>
          ) : (
            <p className="pn-verdict-actions">
              <Button variant="primary" onClick={() => onAsk(route)}>
                {t("contact.intro.askFirstName", {
                  name: route.via_display_name,
                })}
              </Button>
            </p>
 ...
 *
 * Three sentences for three shapes of route, in either contact. A one-sided
...
function verdict(route: RouteCandidate, mine: boolean, t: Translate): string {
  ...
  if (through) {
    return mine
      ? t("contact.intro.verdictViaYou", { through })
      : t("contact.intro.verdictVia", { name, through });
  }
  if (route.evidence.two_way) {
    return mine
      ? t("contact.intro.verdictDirectYou")
      : t("contact.intro.verdictDirect", { name });
  }
  return mine
    ? t("contact.intro.verdictOneSidedYou")
    : t("contact.intro.verdictOneSided", { name });
}

// The chain's second line, which continues the name above it — so it is a verb
// phrase in both contacts and never a sentence of its own.
function heroLine(route: RouteCandidate, mine: boolean, t: Translate): string {
  const through = route.through_display_name;
  if (through) {
    return mine
      ? t("contact.intro.heroIndirectYou", { through })
      : t("contact.intro.heroIndirect", { through });
  }
  return mine
    ? t("contact.intro.heroDirectYou")
    : t("contact.intro.heroDirect");
}
...
function EvidencePlate({
  ...
  return (
    <div className="pn-evidence">
      <Eyebrow as="h3">{t("contact.intro.evidenceEyebrow")}</Eyebrow>
    ...
    <>
      ...
      <span className="pn-split-legend t-caption">
        ...
        <span className="pn-split-key-out">
          {mine
            ? t("contact.intro.evidenceFromYou", {
...
function lastContactReading(
  ...
  switch (last.kind) {
    case "never":
      return t("contact.intro.lastNever");
    case "today":
      return t("contact.intro.lastToday");
    case "yesterday":
      return t("contact.intro.lastYesterday");
    case "days":
      return t("contact.intro.lastDays", {
