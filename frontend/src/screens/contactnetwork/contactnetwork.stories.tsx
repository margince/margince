...
const sofia: ContactGraph["nodes"][number] = {
  id: `user:${SOFIA}`,
  type: "colleague",
  group: "direct",
  label: "Sofia Meier",
...
// which edge put her in the graph (contactgraphaccount.go), so an indirect-only
// story that left her `direct` would put her in the wrong lane of the map and
// describe a payload the server cannot produce.
const sofiaOnAccount: ContactGraph["nodes"][number] = {
  ...sofia,
  group: "account",
};

const martin: ContactGraph["nodes"][number] = {
  id: `user:${MARTIN}`,
  type: "colleague",
  group: "direct",
  label: "Martin Weber",
...
// through.
const philipp: ContactGraph["nodes"][number] = {
  id: `contact:${PHILIPP}`,
  type: "contact",
  group: "account",
...
const rui: ContactGraph["nodes"][number] = {
  id: `contact:${RUI}`,
  type: "contact",
  group: "peer",
  label: "Rui Almeida",
...
// (contactgraph.go) — so a fixture carrying one without the other describes a
...
function stub(
  payload: ContactGraph,
  asks: IntroRequest[] = [],
  allow: Parameters<typeof meRoute>[0] = READS_AND_ASKS,
) {
  installFetchStub({
    "GET /me": meRoute(allow),
    "GET /contacts/{id}/graph": () => jsonResponse(payload),
    [`GET /contacts/${CONTACT}/graph`]: () => jsonResponse(payload),
    [`GET /contacts/${CONTACT}/intro-requests`]: () => jsonResponse(asks),
  });
}
...
function render(
  ...
  return (
    <StoryProviders>
      <ContactNetworkTab contactId={CONTACT} view={view} />
    </StoryProviders>
  );
}
...
type Story = StoryObj<typeof ContactNetworkTab>;
...
//
// Nothing excludes the reader reading from the colleagues the server ranks, so
...
// This is the story that shows the second contact: the verdict, the chain, the
...
// introducer is the contact requesting it.
...
// through a third contact is a different favour from an introduction.
...
// cut: a picture that quietly drops contacts is one a reader trusts for a
...
// see quickly rather than after scrolling a picture of contacts who cannot help.
