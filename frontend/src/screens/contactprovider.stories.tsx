...
// What a contact's page shows about data BOUGHT from a provider: the values, how
...
// for arrangements of the same content: an expired credential and a contact the
// provider simply does not know are different sentences, and only one of them
// is something an operator can act on.
//
// The completed snapshot is `contactprovider.fixtures.ts`, shared with the
...
export default meta;
type Story = StoryObj<typeof ContactProviderSection>;
...
function section(value: Profile | undefined) {
  return () => {
    installFetchStub({
      "GET /contacts/p-1/enrichment-runs/run-1": () =>
        jsonResponse(completedProviderRun),
    });
    return (
      <StoryProviders>
        <ContactProviderSection contactId="p-1" profiles={value && [value]} />
      </StoryProviders>
    );
  };
}
 ...
 * — an absent card would read as "this contact has nothing to buy", which is a
 * claim about the contact rather than about the run.
 */
 ...
 * send somebody looking at the contact instead of at the connection.
 */
export const InvalidCredentials: Story = {
  render: section(profile({ state: "invalid_credentials" })),
};

/** Out of credits: the run did not run, and nothing about the contact changed.
...
/**
 * The section WITHHELD. `contact360` names it in `sections_omitted` and hands
