...
function renderPanel(contactId = "p-1") {
  ...
  const view = render(tree(contactId));
...
describe("ContactNetworkTab", () => {
  ...
  it("renders nothing rather than crashing the page on a response with no nodes", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText(en["contact.graph.loading"])).toBeNull(),
    );
  ...
  it("leads with the recommended route and its proof line", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(
          en["contact.intro.verdictDirect"].replace("{name}", "Direct Dana"),
        ),
      ).toBeTruthy(),
    );
  ...
  it("counts one interaction in the singular", async () => {
    stub({
      contact_id: "p-1",
      nodes: [
        { id: "contact:p-1", type: "contact", group: "anchor", label: "Anna" },
        {
          id: "user:u-1",
          type: "colleague",
          group: "direct",
  ...
  it("says a group was withheld rather than showing it as empty", async () => {
    ...
    expect(screen.queryByText(en["contact.graph.noDirect"])).toBeNull();
  });
  ...
  it("names the colleague at the other end of an account-arm edge, not the selected node", async () => {
    ...
    expect(screen.queryByText(en["contact.graph.withContact"])).toBeNull();
  });
  ...
  it("names the anchor as the page's subject when the edge reaches them", async () => {
    ...
    expect(
      await screen.findByText(en["contact.graph.withContact"]),
    ).toBeTruthy();
  });
  ...
  it("drops a selected node the newly loaded graph does not contain", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (request: Request) => {
        const forFirst = request.url.includes("/contacts/p-1/");
        const body = {
          contact_id: forFirst ? "p-1" : "p-2",
          nodes: [
            {
              id: forFirst ? "contact:p-1" : "contact:p-2",
              type: "contact",
              group: "anchor",
