 ...
 * read it to prove which contact was asked about, and a session probe landing
...
describe("the contact's files tab", () => {
  ...
  it("says the contact has no files rather than leaving the tab blank", async () => {
    ...
    expect(await screen.findByText(en["contact.documents.empty"])).toBeTruthy();
  });
  ...
  it("files an uploaded document against this contact, from this tab", async () => {
    ...
    const user = userEvent.setup();
    show(<ContactFilesTab contactId="p-7" />);
    ...
    expect(uploads[0].get("entity_type")).toBe("contact");
    expect(uploads[0].get("entity_id")).toBe("p-7");
  });

  it("scopes the request to the contact whose files these are", async () => {
    stub({ data: [], page: { has_more: false } });
    show(<ContactFilesTab contactId="p-42" />);

    await screen.findByText(en["contact.documents.empty"]);
    if (!lastRequest) {
      throw new Error("the tab asked for nothing at all");
    }
    const url = new URL(lastRequest.url);
    expect(url.searchParams.get("entity_type")).toBe("contact");
