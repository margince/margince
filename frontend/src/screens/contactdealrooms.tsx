...
// removing "Max, who left the buyer" knows the contact, not the deals — the
...
// list a reader reads, so identical-everywhere is the only property that
...
export function ContactDealRooms({
  emails,
  ...
  return (
    <Panel title={t("contactdealrooms.title")} sub={t("contactdealrooms.sub")}>
      <QueryStates
        query={rooms}
        pendingLines={2}
        pendingLabel={t("contactdealrooms.title")}
      >
...
// Revoking from the contact's side: find the seat by address in the room's
...
function RevokeSeat({
  ...
  return (
    <ConfirmModal
      open={open}
      onClose={onClose}
      title={t("contactdealrooms.revokeTitle", { room: room.title })}
