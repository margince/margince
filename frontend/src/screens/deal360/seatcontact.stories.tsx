...
// One stakeholder seat's contact, in both readings a deal record can meet.
//
...
// the name as flat text — so the only surface naming the contacts on a deal
...
const meta: Meta<typeof SeatContact> = {
  title: "Records/Deal seat contact",
  component: SeatContact,
...
type Story = StoryObj<typeof SeatContact>;
...
/** A seat this reader may read: the name is also the door to the contact. */
export const NamedContact: Story = {
  args: { seat: seat({ contact_name: "Dana Weiss" }) },
};

// The case the component exists to get right, and the one to look at.
//
// `contact_id` is always on the wire and only the NAME is withheld, so an href
...
export const WithheldContact: Story = {
  args: {
    seat: seat({
