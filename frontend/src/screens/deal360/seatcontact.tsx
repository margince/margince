...
/**
 * One stakeholder seat's contact: a link to them, or the sentence saying this
 ...
 * list — and all three printed `contact_name` as text, so the only surface
 * naming the contacts on a deal offered no way to open any of them. Three copies
 ...
 *
 * The withheld case is deliberately NOT a link. `contact_id` is always on the
 ...
 */
export function SeatContact({ seat }: Readonly<{ seat: Seat }>) {
  const t = useT();
  if (!seat.contact_name) {
