...
test("puts colleagues left, the account and deal centre, their contacts right", () => {
...
// A seat with a role the board has no lane for is still a contact the summary
...
// may not ask who can reach this contact, empty is the answer that nobody can.
// Drawing both as a contact with no line reports a withholding as an absence.
...
// A stakeholder can sit on one deal twice — the table's key is deal, contact
// AND role. Drawing them once per role produced two nodes with the same id,
// two identical edges and two React keys.
test("draws a contact once even when they hold two roles", () => {
...
// route edge runs colleague → contact. Reading the ends the other way round
...
test("refuses a node that is not a contact", () => {
  const model = mapModelFromCoverage(coverage(), "Brandt GmbH", COPY);
...
// contact reading is among them — so the warmest way in can be their own
