 ...
 * prints one reads it here, so the account and the contact agree on the word.
 */
      ...
      <StatCard
        ...
        value={t(
          withheld
            ...
            : "co.contacts.band.noOpenDeal",
        )}
        detail={t(
          withheld
            ? "co.contacts.band.committeeUnreadWhy"
            : "co.contacts.band.noOpenDealWhy",
        )}
      />
    );
  }
          ...
          : // No gaps means one of two things, and only one is good news. The
            ...
            t(
              unlisted > 0
                ...
                : "co.contacts.band.committeeComplete",
            )
      }
      detail={
        unlisted > 0
          ? t("co.contacts.band.someHidden", {
              count: formatNumber(unlisted, locale),
            })
          : t("co.contacts.band.seatsHeld", {
...
function CommitteeBoard({
  ...
  // dropped: a contact the summary counted and the board does not draw is a
  ...
  const columns = [
    ...ROLES.map((role) => ({
      ...
      deals: committee.seats
        ...
        .map((seat) => ({ id: seat.contact_id, name: seat.full_name, seat })),
    })),
    ...(other.length > 0
      ? [
          {
            stage: "other",
            label: t("co.contacts.board.otherRoles"),
  ...
  const switcher = (
    <SegmentedControl
      ...
      value={view}
      onChange={setView}
      label={t("co.contacts.view")}
      labels={{
        board: t("co.contacts.view.board"),
        map: t("co.contacts.view.map"),
      }}
    />
  );
  ...
  if (view === "map") {
    return (
      <>
        ...
        {asking && (
          <IntroRequestModal
            key={`${asking.contactId}:${asking.viaUserId}`}
            companyId={companyId}
            target={asking}
 ...
 *
 * Exported because the contact page draws the SAME picture and every label but
 * one reads identically there. `region` is the exception — it names what the
 * drawing is OF, and "at this account" is wrong on a contact's page — so it is
 * the one a caller passes in.
 */
export function mapLabels(
  t: ReturnType<typeof useT>,
  locale: Locale,
  region = t("co.contacts.map.region"),
) {
  return {
    region,
    band: {
    ...
    bestRoute: t("co.contacts.map.bestRoute"),
    alternatives: t("co.contacts.map.alternatives"),
    noRoute: t("co.contacts.map.noRoute"),
    laneMore: (hidden: number) =>
      t("co.contacts.map.more", { count: formatNumber(hidden, locale) }),
    clearFocus: t("co.contacts.map.clear"),
    emptyTitle: t("co.contacts.map.emptyTitle"),
    emptyBody: t("co.contacts.map.emptyBody"),
    nothingSelected: t("co.contacts.map.nothingSelected"),
  };
}

function mapCopy(t: ReturnType<typeof useT>) {
  return {
    routesWithheld: t("co.contacts.map.routesWithheld"),
    ourSide: t("co.contacts.map.ourSide"),
    account: t("co.contacts.map.account"),
    roles: {
    ...
    otherRoles: t("co.contacts.board.otherRoles"),
    missing: (role: string) => t("co.contacts.map.missing", { role }),
    assign: t("co.contacts.map.assignHint"),
    engagement: {
    ...
    awaitingReply: t("co.contacts.map.awaiting"),
    replyOwed: t("co.contacts.map.owed"),
    theyReplied: t("co.contacts.map.replied"),
    neverWritten: t("co.contacts.map.never"),
    onDeal: t("co.contacts.map.onDeal"),
    ...
    >
      {suggested && (
        <p className="cp-seat-mark">
          ...
          />{" "}
          {t("co.contacts.board.readFromMessages")}
        </p>
      )}
      <EntityRef kind="contact" id={seat.contact_id} name={seat.full_name} />
      {seat.engagement && (
...
function useCommitteeWrites(
  ...
  return {
    ...
    // same role written by a contact is what turns the machine's reading into
    // theirs.
    confirm: (seat: Seat) => {
...
function SuggestRoles({
  ...
  return (
    <Button
      ...
      busyLabel={t("co.contacts.board.suggesting")}
    >
      <Sparkles aria-hidden="true" />
      {t("co.contacts.board.suggest")}
    </Button>
  );
}
