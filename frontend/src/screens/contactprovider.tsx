...
// What a licensed data provider was PAID to tell us about this contact
 ...
 */
export function ContactProviderSection({
  contactId,
  profiles,
}: Readonly<{ contactId: string; profiles: Profile[] | undefined }>) {
  if (!profiles) {
    ...
    // rather than claiming this contact has nothing.
    return null;
  }
  return (
    <>
...
function ProviderPanel({
  contactId,
  profile,
}: Readonly<{ contactId: string; profile: Profile }>) {
  const t = useT();
    ...
    >
      <PanelBody>
        ...
        <BuyPriced
          ...
          running={running}
        />
        <RunWatch contactId={contactId} profile={profile} />
      </PanelBody>
    </Panel>
  );
}
...
function BuyPriced({
  ...
  const record = useContact360(contactId);
  const held = heldCategories(profile, record.data?.contact);
...
function heldCategories(
  ...
  if (
    profile.mobile_phones.length > 0 ||
    (contact?.phones ?? []).some((phone) => phone.phone_type === "mobile")
  ) {
    held.add("mobile");
  }
  return held;
 ...
 */
function useRunWatch(contactId: string, profile: Profile) {
  const queryClient = useQueryClient();
