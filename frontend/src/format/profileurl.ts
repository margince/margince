...
// What a human types when they mean a profile address, turned into one — and
...
// exists. A pasted address that is merely wrong stays wrong until a contact
// fixes it, which is the honest outcome when the only reader is the contact who
// pasted it.
//
// `webUrl` still owns the verdict on whether the result may become an href.
// This runs BEFORE it, turning the two things contacts actually paste into
...
export function normalizeProfileUrl(raw: string): string {
  ...
  if (!parsed) {
    // Not an address this product would ever link. Hand back what the contact
  ...
  // junk (tracking parameters, an originating-page marker). Two contacts copying
  ...
  // contact. The fragment goes for the same reason.
  parsed.search = "";
  parsed.hash = "";
...
// The address as a reader reads it: no scheme, no `www.`, no trailing slash.
//
