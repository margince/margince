...
Generated by `go test ./internal/compose/ -run TestTheAgentToolBudgetIsPublished -update-agent-tool-budget`; do not edit by hand. It reports what each SCHEDULED agent's tool listing costs the window it runs in — not the whole served catalog, which no agent is ever offered. Tokens are the ~4-bytes estimate the window itself estimates with, over the listing the runner's own renderer produces.
