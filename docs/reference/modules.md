...
| **agents** | The governed agent surface — the MCP tool registry, the admission gate (scope ∧ tier ∧ seat), the hosted HTTP transport and its JSON-RPC dispatcher, the approval flow, and the Surface-B reasoning loop. Reaches records only through the datasource seam. | Engine (MCP registry + `runner/`) | `agent_run, runner_job` (the loop's own run state; records belong to domain modules, staged actions to approvals) | the MCP tool surface (`/mcp`), `/agent-tools` |
...
| **ai** | The model runtime behind `ports/model` — provider adapters (Anthropic / OpenAI / Gemini / OpenAI-compatible BYOK, Ollama, local vLLM, the offline fake), the `SelectBrain` factory, the tiered `Router` (routing + budget + tracing), the outbound secret stripper — plus the effective-dated `ai_model_rate` sheet the read-side pricer prices calls against (`ai_call` stores tokens, never a price, so a corrected rate reprices history instead of contradicting it) and the Voice DNA HTTP slice. See [ai-runtime.md](../explanation/ai-runtime.md). | Runtime factory (`SelectBrain`) + `Router`; voice HTTP via Store | `ai_usage, ai_call, ai_call_payload, ai_call_config, ai_model_rate, voice_profile, voice_corpus_source` | `/voice-profiles` (+`/sources`), `/ai/usage`, `/ai-model-rates` |
...
| **finance** | The ingested finance mirror (ADR-0083/A128) — a bounded READ-ONLY import of an accounting source, so a company page can answer whether a customer actually pays us, and on time. A mirror, not an integration: there is no create or update action on a finance record, and that posture is the ABSENCE of a grant on the permission surface rather than a runtime check, so there is no code path to reach and no flag to set wrongly. A customer link maps two identifiers and never merges. Money is minor units in the issued currency, frozen on issue date through the effective-dated rate sheet; a missing rate refuses rather than converting silently. | Handlers→Store (`NewStore`) | `finance_connection, finance_external_customer, finance_customer_link, finance_invoice, finance_payment` | `/organizations/{id}/finance-summary` |
