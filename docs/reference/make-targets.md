...
| `rls-store-path` | No `internal/modules` statement addresses the superuser pool directly, which would run unconstrained by any grant; `// rls-exempt: <reason>` is the escape for a genuinely cross-workspace query |
