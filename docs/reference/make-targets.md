...
| `e2e-ai` | Certify AI tasks against the corpus (`-tags e2e_llm`, `TestE2ECertify`): paid, network, opt-in. Takes the two models it binds outright — `MODEL=<provider:model>` for the candidate and `JUDGE=<provider:model>` for the second model that grades it, both required and refused if equal, because one grading itself is certified by construction. `BASE_URL=` carries a broker host (`openai_compatible` fails closed without one) and `PROFILE=` the environment class a record is filed under. Narrow with `TASK=<task>`, repeat with `RUNS=<n>`. Dumps every candidate+judge request/response (the `ai_call_payload` shape, post-stripper) to a gitignored `.tmp/aicert/*.jsonl` and prints the path — on by default (`TRACE=<dir>` to relocate, `TRACE=` to disable). Fails loudly — never skips — without a binding or a corpus match |
...
| `bench-mobile` | MOBILE-AC-2: record open p95 against the 300 ms **perceived** budget on a throttled Fast-3G profile at 390px (MOBILE-PARAM-2). The by-hand frontend measurement, and the only place PERF-1's perceived budget is asserted: the `uat` lane keeps the structural claim without a number, because a single wall-clock sample there measures the runner. Switched on by `MARGINCE_BENCH_MOBILE=1`, which is what keeps the two runs from collecting each other's specs: `pnpm e2e` does not see `perf-mobile.spec.ts`, and this target sees nothing else |
