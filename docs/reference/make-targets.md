...
| `e2e-llm-guards` | Put the SCENARIOS' regex guards on trial instead of the product. A real model is asked for answers a good assistant would give to a scenario's prompt and answers carrying the specific defect it forbids, and every one is judged by `e2e/llm/probe.py` — which runs them through `check.check()` itself, so the probe agrees with the lane by construction. It reports FALSE REDS (a correct answer a guard caught) and FALSE GREENS (a defective answer every guard missed), naming the pattern and the text it matched. That is the failure mode nothing else sees: four review rounds of case 6 each found one by hand, and each round's own fix introduced the next round's. **Paid and opt-in** — refuses without `MARGINCE_E2E_LLM_GUARDS=1`, resolves a credential the same way `e2e-llm` does and pins its model for the same reason; needs no stack. `SCENARIO=<name>` audits one, `E2E_LLM_GUARDS_COUNT=<n>` asks for more answers of each kind, `E2E_LLM_GUARDS_OUT=<dir>` keeps the candidates. **The findings are candidates, not truth** — a model asked for a correct answer can write an incorrect one, so a human reads every finding and nothing is ever rewritten automatically. To judge sentences you wrote yourself, no model and no opt-in are needed: `python3 e2e/llm/probe.py <scenario.yaml> --expect correct "<answer>"` |
