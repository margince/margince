...
| `check-migration-versions` | Every migration version in the tree sorts ABOVE the highest on the base ref, and no version is claimed twice — in the tree or against the base. A version reused for a different migration applies in a different place on a fresh database than on one already past it, and the two schemas then differ with nothing to report it. A baseline consolidation is the one legitimate exception and must DECLARE itself with `MIGRATION_VERSIONS_BASELINE_RESET=1`; the declaration is honored only where the namespace both collapses (fewer migrations than the base) and shares no `(version, name)` pair with it, so it goes inert on its own once the consolidation merges. `MIGRATION_VERSIONS_REQUIRE_BASE=1` turns a missing base ref from a skip into a failure |
