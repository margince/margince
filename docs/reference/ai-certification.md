...
— and renders them for a reader rather than a terminal.

**A band is a claim about one deployment, not about the product.** Every record
names a (provider, model, env) binding, and green-lights no other: the same
model reached through a broker instead of an EU-hosted endpoint is a different
row and needs its own run. Nothing here is a merge gate — the certification
lane is paid, manual and BYOK-gated, so a prompt edit turns a record `stale`
rather than failing a build.

**RUNS/PASSED is how often a site did what its scenarios asked.** The four
outcome counts beside it are what the site's own validator REPORTED, never a
pass/fail column: a reply can be `accepted` and still fail the run, when the
...
|---|---|
| `current` | Every scenario this record measured still matches what this build sends, and there are none it has not seen. |
| `partial` | Right about every case it measured; the corpus has since grown cases it never saw. The `Scenarios` column says how many of each. |
| `stale` | A scenario it measured — or the prompt this build now builds from one — has changed since. The band is no longer a claim about what ships. |
| `absent` | Never certified on any binding. The honest state, not a failure: the columns are dashes because no run has measured them. |
...
the per-binding truth is in the tables below.

## Certification by provider and model

One line per (provider, model, env) binding, folded over every site it
measured. SITES is how many shipped invocation sites this binding has ever
been run against — not how many exist — and the three state columns split
...
| `openai_compatible` | `openai/gpt-oss-120b` | `eu_hosted` | 30 | 29 | 0 | 1 | 324 | 261 | 0.81 | 39483ms | 14 | 6 | 10 |
...
this build now builds from that scenario does — the certification stamp covers
both, so a prompt edit in the product invalidates a record that never mentioned
it. A scenario the record never saw does not make it stale; that is `partial`.

A record that *predates per-scenario stamps* carries one stamp for the whole
task and nothing finer, so it can only say that SOMETHING it measured has
moved — never which case. Re-certifying it costs the whole task; a record with
per-scenario stamps names the cases, and costs only those.
...
One section per task, one subsection per invocation site that task ships. A
task can ship several — `cold_start` ships four — and the site is the unit
here, because it is what the contract ships and what a scenario names.

In each record table, `Scenarios` is how many of the site's CURRENT scenarios
that record still describes, over how many the site ships today. A record
written before per-scenario stamps existed can only be judged whole, and shows
`-`.
...
| `openai_compatible · openai/gpt-oss-120b · eu_hosted` | `current` | 23/23 | `not_supported` | 69 | 39 | 0.57 | 7242ms | 39483ms | 39 | 19 | 11 | 0 |
