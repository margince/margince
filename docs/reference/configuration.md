...
| `MARGINCE_WEB_BIND` | `127.0.0.1` | desktop launcher | interface the web ui listener binds. Read from the launcher's **process environment**, not `margince.env`, because it describes the host the launcher runs on rather than the installation. A desktop install never sets it and keeps loopback; a container image that publishes the UI port sets it to `0.0.0.0` to bind every interface. The api, the bus and the database keep loopback whatever the value. |
