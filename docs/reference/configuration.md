...
| `MARGINCE_AICERT_MODEL`, `MARGINCE_AICERT_JUDGE_MODEL` | — (`make e2e-ai` defaults the judge to `gemini:gemini-3.5-flash`) | `make e2e-ai` | `provider:model` each. The run refuses without a judge, and without a candidate unless `MARGINCE_AICERT_ROUTING` names the bindings instead; `make e2e-ai` supplies the judge, so only the candidate is yours to name. The candidate is what the run certifies; the judge grades it and must be a DIFFERENT model, because one grading itself is certified by construction. An exported `MARGINCE_AICERT_JUDGE_MODEL` replaces the Makefile default, and `JUDGE=` overrides both. Surfaced as `MODEL=` and `JUDGE=`. |
| `MARGINCE_AICERT_JUDGE_FALLBACK_MODEL` | — (`make e2e-ai` defaults it to `gemini:gemini-3.1-flash-lite`) | `make e2e-ai` | optional `provider:model` that grades each task whose candidate IS the judge — a Gemini preset's `premium` rung is the pinned judge's own model. Unset, such a task is refused; a task whose candidate is both judges is refused either way, before a single paid call. `judge_served_model` on each record names which judge graded it. An exported value replaces the Makefile default, and `JUDGE_FALLBACK=` overrides both — empty to refuse every collision. |
...
| `MARGINCE_AICERT_BASE_URL`, `MARGINCE_AICERT_JUDGE_BASE_URL`, `MARGINCE_AICERT_JUDGE_FALLBACK_BASE_URL` | — | `make e2e-ai` | endpoint host root for a broker or OpenAI-wire host. Required for `openai_compatible`, which fails closed without one; empty for a native vendor, which uses its own default. An `openai_compatible` judge or fallback left without its own falls back to the candidate's. Surfaced as `BASE_URL=`, `JUDGE_BASE_URL=`, `JUDGE_FALLBACK_BASE_URL=`. |
