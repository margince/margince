...
| `MARGINCE_AICERT_MODEL`, `MARGINCE_AICERT_JUDGE_MODEL` | — (`make e2e-ai` pins the judge to `gemini:gemini-3.5-flash`) | `make e2e-ai` | **both required** — `provider:model` each. The candidate is what the run certifies; the judge grades it and must be a DIFFERENT model, because one grading itself is certified by construction. Surfaced as `MODEL=` and `JUDGE=`. |
| `MARGINCE_AICERT_JUDGE_FALLBACK_MODEL` | — (`make e2e-ai` sets `gemini:gemini-3.1-flash-lite`) | `make e2e-ai` | optional `provider:model` that grades each task whose candidate IS the judge — a Gemini preset's `premium` rung is the pinned judge's own model. Unset, such a task is refused; a task whose candidate is both judges is refused either way, before a single paid call. `judge_served_model` on each record names which judge graded it. Surfaced as `JUDGE_FALLBACK=` (empty to refuse every collision). |
