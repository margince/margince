...
| `--redis-tls` | `MARGINCE_REDIS_TLS` | `false` | `true` to connect to the event-bus Redis over TLS. Required when the instance refuses a plaintext connection (e.g. ElastiCache with `transit_encryption_mode = required`) — a deployment that enables that mode must set this too, or the client fails to connect at all rather than silently staying unencrypted |
...
| `MARGINCE_BLOBSTORE_KMS_KEY_ID` | — | KMS key id/ARN to request via explicit SSE-KMS headers on every write; unset writes with whatever the bucket's own default encryption applies (or none, for local MinIO) |
