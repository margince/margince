...
| Spacing roles | `frontend/scripts/check-ds-spacing-roles.sh` | a **newly added** screen rule that re-spaces a design-system primitive (the corpus is derived from `design-system/*.css` on every run), or that spells a rung where a role exists: `*-actions` gap, `*-cards` gap, `*-card`/`*-panel` padding. Same diff scope and the same `ds:ignore` waiver. Its verdict is tested directly, against fixture checkouts, in `check-ds-spacing-roles.test.sh` |
