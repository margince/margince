...
| `expect.bands` | yes | `certified_min` / `degraded_min` / `floor` — the 0–100 score gates the run's median and minimum are folded against. Omitting the block is refused rather than defaulted, since a missing gate would silently pass everything. |
