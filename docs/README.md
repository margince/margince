...
- [mcp-tool-coverage.md](reference/mcp-tool-coverage.md) — which served MCP tools any scenario actually exercises: per tool, the certification scenarios that expect it as the step to take, the use-case scenarios that name it, the scheduled agents that attach it, and what it costs. With `mcp-tool-coverage.json` beside it. Generated from the served surface and the two corpora, never hand-edited. It answers what the budget page and the certification page each answer half of — a tool can be paid for on every step of every run and be named by no case in either lane.
