...
- [data-enrichment-position.md](explanation/data-enrichment-position.md) — the whitepaper on data enrichment: why Margince ships no scraping plugin, and what EU and Vietnamese law actually permit.
- [data-enrichment-tiers.md](explanation/data-enrichment-tiers.md) — the tiered feature position that whitepaper concludes in: what we build freely, what we gate, what we refuse, each rated on GDPR posture, Vietnam compatibility, platform-contract exposure and works-council temperature.
