...
> **Single-company installation.** One installation serves one workspace; the
> server resolves its singleton workspace itself, so no request selects a tenant — there is no
...
> carries a `workspace_id` column or a row-level policy.)
