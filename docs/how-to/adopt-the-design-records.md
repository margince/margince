...
| Ask (prepared questions) | `AssistantPanel` as full-width rows, no free field; the three questions are the ones the server answers (`OrganizationQuestion`), so no new keys — a fourth is a server change first | | `co.ask.nothing`; disabled in overlay (`enabled={!overlay}`) |
