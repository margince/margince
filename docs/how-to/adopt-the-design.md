...
| `check-ds-spacing-roles.sh` | No new screen rule re-spaces a design-system primitive, and the three named contexts take their role token: `*-actions` gap → `--gapActions`, `*-cards` gap → `--gapCards`, `*-card`/`*-panel` padding → `--padCard`/`--padPanel` | Retune a role in `tokens.css`, where every screen moves with it; a screen that needs its own interval spaces its OWN element, not the primitive it sits in |
