...
- Shipped 2026-08-24 (batman, prompt language): **every prompt now says what language to answer in** ([#2490](https://github.com/margince/margince/pull/2490), [#2499](https://github.com/margince/margince/pull/2499)) — 26 prompt sites, roughly two dozen of which said nothing about language, so a Vietnamese conversation produced a Vietnamese claim a German colleague had to read. `compose/promptlang` is where the instruction is written; a fitness gate finds every `model.Request` literal and fails a new one that carries neither a rule nor a reasoned waiver. Shared writing (deal status card, offer lines, signal summaries, transcript proposals, triage reasons, agent summaries, dossier, growth fit) takes the installation's base language; the onboarding conversation takes the admin's locale, replacing its own weaker `Respond in <locale>.` sentence, which lacked the carve-outs that keep enum values and JSON keys untranslated. Ten data-extraction sites are waived in source with reasons — each is checked verbatim against a foreign-language source page, so a language instruction would drop every fact it extracted. Language is a component of the dossier, growth-fit and deal-status cache keys, or a language switch would serve old-language text until an unrelated fact moved. **Also fixed a red `main`** ([#2495](https://github.com/margince/margince/pull/2495)): #2486 and #2490 were each green alone and their merge did not compile. **Filed, fast-track-debt:** [#2491](https://github.com/margince/margince/issues/2491) (six certification records now describe prompts the build no longer sends; needs the paid lane), [#2493](https://github.com/margince/margince/issues/2493) (~34 Go-composed evidence sentences stay English and now wrap base-language model text), [#2496](https://github.com/margince/margince/issues/2496) (nothing checks the merge result when two PRs admin-merge; needs a process ruling), [#2497](https://github.com/margince/margince/issues/2497) (`orgbrief` is the one per-reader prompt still ungoverned — `Accept-Language` and `app_user.locale` are two sources of truth and one must be chosen).

- Shipped 2026-08-23 (batman, tag vocabulary): **an agent can now tag a project** ([#2453](https://github.com/margince/margince/pull/2453), closes [#2448](https://github.com/margince/margince/issues/2448)) — apply_tag/remove_tag's record_type enum was a hand-typed four-type copy; it now derives from `collections.TaggableEntityTypes()` through the tag seam, held by a compose gate plus the CHECK-anchored integration test. Along the way the review found `Store.ApplyTag`'s direct tag_id path skipped the target's object-read check that RemoveTag and EnsureTaggable already hold; closed in the same PR with a refusal test. Verified through the composed registry and the integration lane, not a live MCP client. No debt filed.

- Shipped 2026-08-23 (batman, next-move model lane): **the deal card's fallback task is written by the model, grounded in the deal's own timeline** ([#2417](https://github.com/margince/margince/pull/2417)) — the Next-move card no longer answers an empty or stalled deal with "Agree the next step on X"; the `deal_health` task ships its first site (`next_move`), following the meetingbrief shape: nonce fence, grounding filter, deterministic floor on any refusal, `generated_by` new on `DealNextBestAction` with the WrittenBy badge on the card. The rules keep the verb; the model writes only what the task says. A 15s lane deadline keeps the fallback inside the API's 30s write window. Verified against the live Anthropic lane: a call whose notes promised an offer produced "Send the revised offer including the two extra depots…", and the click created exactly that task. **Filed, fast-track-debt:** [#2418](https://github.com/margince/margince/issues/2418) (the filter accepts prose that only name-drops a real citation — extractive grounding is a design change shared with the other grounded-prose lanes).
...
- Open, needs a decision (2026-08-15): **the approval lifecycle's decision half**
  ...
  [#1335](https://github.com/margince/margince/issues/1335) an
...
- Open, in review (2026-08-15): **twelve configuration pages in the record page's
  voice** ([#1225](https://github.com/margince/margince/pull/1225)). Ready for
...
**The first slice shipped** ([#1293](https://github.com/margince/margince/pull/1293),
...
[#798]: https://github.com/margince/margince/issues/798
[#687]: https://github.com/margince/margince/issues/687
[#726]: https://github.com/margince/margince/issues/726
...
[#1130](https://github.com/margince/margince/issues/1130) carries the
...
- **[#589](https://github.com/margince/margince/issues/589) —
...
- **[#570](https://github.com/margince/margince/issues/570) —
