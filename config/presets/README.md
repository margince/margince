...
`frontier` and `premium` both land on `mistral-small-2603`; and the embeddings
lane cannot carry the pin at all, because `routing:` is not a field on an
embeddings binding. Both are stated in the file.
