...
It boots **cold**. You get the workspace and the admin seat that the
...
- **The write shape.** Every write commits three rows in one transaction:
  ...
  least once.
- **One installation, one workspace.** This is not multi-tenant
...
- **Hosted SaaS and multi-tenancy.** Not in this repository. One
  installation serves one workspace, and boot refuses a second.
