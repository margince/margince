#!/usr/bin/env bash
# The contact record page's demo account, seeded through the API.
#
...
command -v jq >/dev/null 2>&1 || { echo "seed-contact-page: jq is required" >&2; exit 1; }

fail() { echo "FAIL: $1" >&2; exit 1; }

workdir="$(mktemp -d -t seed-contact.XXXXXX)"
...
echo "== seed-contact-page: sign in =="
...
echo "== seed-contact-page: the account =="
company_id=""
...
# ---------------------------------------------------------------------------
# Contacts. contact_ref echoes the id whether we just made them or not, so a
# re-run threads the same records through everything below.
# ---------------------------------------------------------------------------

ensure_contact() { # ensure_contact <full_name> <title> <email> — prints the id
  local name="$1" title="$2" email="$3" id status
  status="$(api GET "/contacts?q=$(printf '%s' "$name" | jq -sRr @uri)&limit=10")"
  [[ "$status" = "200" ]] || fail "GET /v1/contacts returned HTTP $status"
  id="$(jq -r --arg n "$name" '.data[] | select(.full_name == $n) | .id' "$workdir/body" | head -1)"
  if [[ -n "$id" ]]; then
    echo "$id"
    return
  fi
  status="$(api POST /contacts "$(jq -n --arg n "$name" --arg t "$title" --arg e "$email" \
    '{full_name:$n,title:$t,emails:[{email:$e,is_primary:true}],source:"seed"}')")"
  [[ "$status" = "201" ]] || { cat "$workdir/body" >&2; fail "create contact $name returned HTTP $status"; }
  jq -r .id "$workdir/body"
}

echo "== seed-contact-page: the contacts =="
sarah_id="$(ensure_contact "Sarah Cole" "CFO" "sarah@glazedfrog.com")"
echo "  Sarah Cole: $sarah_id"
nadia_id="$(ensure_contact "Nadia Ferreira" "VP Operations" "nadia@glazedfrog.com")"
echo "  Nadia Ferreira: $nadia_id"
nick_id="$(ensure_contact "Nick Oettinger" "Head of Finance Systems" "nick@glazedfrog.com")"
echo "  Nick Oettinger: $nick_id"
mark_id="$(ensure_contact "Mark Hughes" "Director of Operations" "mark@glazedfrog.com")"
...
echo "== seed-contact-page: employment =="
employ() { # employ <contact-id> <role>
  local contact="$1" role="$2" status
  status="$(api POST /relationships "$(jq -n --arg p "$contact" --arg o "$company_id" --arg r "$role" \
    '{kind:"employment",contact_id:$p,company_id:$o,role:$r,is_current_primary:true,source:"seed"}')")"
  case "$status" in
...
echo "== seed-contact-page: the deal =="
...
echo "== seed-contact-page: the buying committee =="
seat() { # seat <contact-id> <role> <label>
  local contact="$1" role="$2" label="$3" status
  status="$(api POST /relationships "$(jq -n --arg p "$contact" --arg d "$deal_id" --arg r "$role" \
    '{kind:"deal_stakeholder",contact_id:$p,deal_id:$d,role:$r,source:"seed"}')")"
  case "$status" in
...
echo "== seed-contact-page: correspondence =="
...
log_activity() { # log_activity <contact-id> <kind> <direction|""> <subject> <body> <when> <label>
  local contact="$1" kind="$2" direction="$3" subject="$4" body="$5" when="$6" label="$7" status
  status="$(api GET "/activities?entity_type=contact&entity_id=$contact&limit=50")"
  ...
  status="$(api POST /activities "$(jq -n --arg k "$kind" --arg d "$direction" --arg s "$subject" \
    --arg b "$body" --arg o "$when" --arg p "$contact" \
    '{kind:$k,subject:$s,body:$b,occurred_at:$o,
      links:[{entity_type:"contact",entity_id:$p}],source:"seed"}
...
echo "== seed-contact-page: what was said =="
activity_id() { # activity_id <contact-id> <subject> — prints the id, empty if absent
  local contact="$1" subject="$2" status
  status="$(api GET "/activities?entity_type=contact&entity_id=$contact&limit=50")"
  [[ "$status" = "200" ]] || fail "GET /v1/activities returned HTTP $status"
  jq -r --arg s "$subject" '.data[] | select(.subject == $s) | .id' "$workdir/body" | head -1
}

claim() { # claim <contact-id> <kind> <body> <source-subject> <quote> [due-at]
  local contact="$1" kind="$2" body="$3" subject="$4" quote="$5" due="${6:-}" source status
  source="$(activity_id "$contact" "$subject")"
  [[ -n "$source" ]] || fail "no captured activity titled \"$subject\" to ground this claim on"

  status="$(api GET "/contacts/$contact/360")"
  [[ "$status" = "200" ]] || fail "GET /v1/contacts/$contact/360 returned HTTP $status"
  if jq -e --arg b "$body" '(.claims // [])[] | select(.body == $b)' "$workdir/body" >/dev/null; then
    echo "  OK: \"$body\" already recorded"
    return
  fi

  status="$(api POST "/contacts/$contact/claims" "$(jq -n --arg k "$kind" --arg b "$body" \
