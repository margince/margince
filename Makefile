...
ROOT_SCRIPT_GATES := check-craft-doc test-dev-isolation test-dev-dsn test-dev-env-local test-api-entrypoint \
  test-dev-cleanup \
  test-golangci-guard test-scheduled-report test-renovate-liveness test-ci-verdict test-merge-verdict \
...
## test-renovate-liveness — prove the liveness check still refuses to read a
## dead Renovate as a live one. Every case turns on that one direction: a bot
## that has stopped looks exactly like a healthy quiet week from the tree, so
## the check is only worth its lane with the counterfeits pinned — a human
## comment moving the dashboard's updated_at, an absent dashboard, a signal one
## day past the budget. Gated here rather than in the scheduled lane, because
## that lane exercises the check against the real tracker on a green day and
## would never reach the cases that matter.
test-renovate-liveness:
	@./scripts/check-renovate-liveness.test.sh
...
## test-fe-file-length — the length gate's own test, a separate target like
## every other gate-plus-test pair here. The RATCHET is what it holds: a waived
## file that grew, one that came back under the cap with its entry standing,
## and an entry for a file that is gone are three refusals nobody meets until
## they fire, and none can be exercised against the real tree, where each would
## be a real failure.
test-fe-file-length:
	@./scripts/check-fe-file-length.test.sh
