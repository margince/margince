...
var predicateLeadReads = gatekit.Waive(map[string]string{
	"internal/modules/activities/quietmailbox.go:quietRecordOwner":     "the owner arm of the cold-queue selector: the lead's owner_id — a USER id, not lead content — decides whether that seat's mailbox is visible enough to call the lead quiet. Nothing of the lead is selected; its only effect is to WITHHOLD a queue entry, never to surface one",
