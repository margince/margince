...
var calleeGatedLeadReads = gatekit.Waive(map[string]string{
	...
	"internal/modules/capture/sinklead.go:incumbentLeadFields":                 "the three foldable fields the collision card shows beside what an inbound message said, so the approver can see which captured values the accept would drop. Both halves are asked above it in the same transaction: captureLead opens with auth.Require(lead, create), and findLeadCollision takes auth.EnsureVisible on this exact row one frame earlier — a lead the granting human cannot see is skipped before this runs. What leaves is a staged card whose target_label already carries the same lead's name; the cost is that its company and job title travel with it",
