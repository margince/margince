...
var carriedElsewhere = gatekit.Waive(map[string]string{
	...
	"communication_override":    "carried by consent, through the same StopCarrier port the stop above travels on (contacts/stopcarry.go's CarryOverridesTx), inside the merge's own transaction and behind the same subject lock. Ratified separately rather than folded into the entry above because a reader checking one table must not have to infer the other: what holds this carry is consent's own CarryOverridesTx and the merge's refusal to proceed when the seam is unwired and the retiring subject holds a live vouch (OverrideCarrierNotWiredError)",
