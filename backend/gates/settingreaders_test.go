...
var rawSettingReads = gatekit.Waive(map[string]string{
	...
	"internal/modules/people/leadsla.go:unassignedEscalationTarget:SELECT value FROM setting WHERE key = $1":                                 "the intake seat the unassigned-lead escalation hands to, on the same ground loadLeadSLAPolicy states two entries above and in the same sweep: it runs under the SLA sweep's system principal, and the seat is an INPUT to a decision that is already the sweep's to make. Asking the settings object gate would ask it of a principal the sweep minted for itself. What reaches a caller is a lead escalated to a seat, never the setting value — and when the configured seat can no longer work the queue the read answers the same silence as no seat at all, so a stale value discloses nothing either",
