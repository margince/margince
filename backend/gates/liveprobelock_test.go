...
var unlockedLiveWrites = gatekit.Waive(map[string]string{
	...
	"internal/modules/consent:Store.StopForCredentialTx":      "two arms that cannot both run, and the reach that joins them is syntactic. The live probe belongs to the NAMED-PURPOSE arm, which delegates to the consent writer and returns before the suppression insert this gate names; that arm writes contact_consent, which Art. 17 anonymizes in place rather than deleting, so there is no residue to leave. The arm that DOES insert into communication_suppression takes no probe at all — a stop stays recordable against an archived subject on purpose, which is the state somebody who asked to be forgotten is in — and it holds stopSubjectKey under lockStopKey across the read and the write, so two presses on one subject queue rather than race. What rowProbeFor mentions is EnsureWritable for a withdrawal and EnsureWritableLive for a grant; only the second is the probe this gate is about, and this path never reaches it at runtime",
	"internal/modules/consent:Store.StopForCredential":        "the same two arms in their own transaction, for the public handler that has none; bounded exactly as the Tx form above is",
