...
// contacts.writeContactProfileField is the one writer of contact_profile_field, and the
...
// contactProfileFieldWrite matches a write of the table.
//
...
// is already there, and a second writer spelled `UPDATE contact_profile_field
...
var contactProfileFieldWrite = regexp.MustCompile(`(?i)(INSERT\s+INTO|UPDATE)\s+contact_profile_field`)

// contactProfileFieldOwner is where the writer this census requires lives. Its
// own statement is the definition rather than a copy of it.
const contactProfileFieldOwner = "internal/modules/contacts/contactprofilefieldwrite.go"

// bulkRelinkIsNotAFill ratifies the merge carry-over, which is the one write of
// this table that writeContactProfileField cannot serve.
//
...
var bulkRelinkIsNotAFill = gatekit.Waive(map[string]string{
	"internal/modules/contacts/mergerelink.go#carry-over": "re-homes existing rows whole at a merge, all of a contact's fields in one statement, rather than deciding one fill — it examines no value and writes none of its own",
})
...
var carryOverSource = regexp.MustCompile(`(?is)INSERT\s+INTO\s+contact_profile_field.*\bFROM\s+contact_profile_field\b`)
...
func TestEveryProfileFieldWriteUsesTheOneWriter(t *testing.T) {
	...
	for _, path := range handWrittenGoSources(t) {
		if filepath.ToSlash(path) == contactProfileFieldOwner {
			continue
		}
		// PRODUCT writes, and the boundary is architectural rather than a
		// shortcut: writeContactProfileField is unexported, so a fixture in another
		...
		for _, decl := range file.Decls {
			for _, sql := range contactProfileFieldStatements(decl) {
				judged++
				if !contactProfileFieldWrite.MatchString(sql) {
					continue
				}
				if isCarryOver(sql) && bulkRelinkIsNotAFill.Waived(t, filepath.ToSlash(path)+"#carry-over") {
					continue
				}
				findings = append(findings, fmt.Sprintf("%s: %s", path, firstContactProfileFieldLine(sql)))
			}
		}
	}
	...
	if judged < 5 {
		t.Fatalf("only %d statement(s) naming contact_profile_field were judged, so this census covered almost nothing", judged)
	}
	if len(findings) == 0 {
		return
	}
	t.Errorf("these statements write contact_profile_field by hand:\n  %s\n\n"+
		"contacts.writeContactProfileField is the one writer, and the ON CONFLICT clause is not the "+
...
// contactProfileFieldStatements returns the SQL statements in a declaration that name
// the table at all — the candidate set this census counts against.
func contactProfileFieldStatements(decl ast.Decl) []string {
	var out []string
	...
	ast.Inspect(decl, func(n ast.Node) bool {
		...
		text, ok := flattenSQL(n, seen, helperScope{names: map[string]bool{"writeContactProfileField": true}})
		...
		if !ok || !strings.Contains(strings.ToLower(text), "contact_profile_field") {
			return true
		}
		out = append(out, text)
		return true
...
// firstContactProfileFieldLine points the report at the offending line rather than
// dumping the statement.
func firstContactProfileFieldLine(sql string) string {
	lines := strings.Split(sql, "\n")
	for _, line := range lines {
		if strings.Contains(strings.ToLower(line), "contact_profile_field") {
...
var contactProfileFieldProbes = []struct {
	name  string
	fires bool
	src   string
}{
	{"the VALUES form three of the five copies used", true, "\nfunc write() string {\n\treturn `INSERT INTO contact_profile_field (contact_id, field, value) VALUES ($1, $2, $3) ON CONFLICT (contact_id, field) DO NOTHING`\n}"},
	// Two of the five wrapped after the table name, which is where a
	// line-keyed census goes blind.
	{"the column list wrapped onto the next line", true, "\nfunc write() string {\n\treturn `INSERT INTO contact_profile_field\n\t  (contact_id, field, value)\n\tSELECT $1, $2, $3`\n}"},
	{"the DO UPDATE form, which is the disagreement itself", true, "\nfunc write() string {\n\treturn `INSERT INTO contact_profile_field (contact_id, field, value) VALUES ($1, $2, $3) ON CONFLICT (contact_id, field) DO UPDATE SET value = EXCLUDED.value`\n}"},
	{"the statement split across a concatenation", true, "\nfunc write() string {\n\treturn `INSERT INTO ` + `contact_profile_field (contact_id, field) VALUES ($1, $2)`\n}"},
	{"an insert held inside a formatter's argument", true, "\nfunc write() string {\n\treturn fmt.Sprintf(`INSERT INTO contact_profile_field (contact_id, field) VALUES ($1, $2) %s`, tail)\n}"},
	...
	{"a read of the table", false, "\nfunc read() string {\n\treturn `SELECT value FROM contact_profile_field WHERE contact_id = $1`\n}"},
	{"the erasure delete", false, "\nfunc erase() string {\n\treturn `DELETE FROM contact_profile_field WHERE contact_id = $1`\n}"},
	{"a join naming the table", false, "\nfunc read() string {\n\treturn `SELECT 1 FROM contact p JOIN contact_profile_field f ON f.contact_id = p.id AND f.field = 'company_name'`\n}"},
	...
	{"a second writer spelled as an UPDATE", true, "\nfunc write() string {\n\treturn `UPDATE contact_profile_field SET value = $2 WHERE contact_id = $1 AND field = 'role'`\n}"},
	...
	// ever runs.
	{"the table identifier in upper case", true, "\nfunc write() string {\n\treturn `INSERT INTO CONTACT_PROFILE_FIELD (contact_id, field, value) VALUES ($1, $2, $3)`\n}"},
}
...
func TestOnlyACarryOverIsRatifiedInTheMergeRelink(t *testing.T) {
	t.Parallel()
	carryOver := "INSERT INTO contact_profile_field\n  (contact_id, field, value)\n  SELECT $2, field, value\n    FROM contact_profile_field WHERE contact_id = $1\n  ON CONFLICT (contact_id, field) DO NOTHING"
	...
	fill := "INSERT INTO contact_profile_field (contact_id, field, value)\n  VALUES ($1, $2, $3)\n  ON CONFLICT (contact_id, field) DO UPDATE SET value = EXCLUDED.value"
	if isCarryOver(fill) {
