...
var predicateCompanyReads = gatekit.Waive(map[string]string{
	...
	"internal/modules/finance/summary.go:companyStatus":                     "the existence probe behind the finance summary, and its own doc says why it is there: under row_scope=all auth.EnsureVisible skips the query, so an id naming NO company would be answered with a summary of the workspace's provider rather than a 404. The probe makes a made-up id answer the way a hidden one does, on an id the caller supplied and immediately after EnsureVisible has run on it. It reads `status` as well now, and that value reaches no reader: it decides whether the card WITHHOLDS its overdue figure on a relationship that has ended, so the column leaves this function as an absence rather than as a fact",
	...
	"internal/compose/signalproposals.go:readContradictions":                "the contradiction pass: it reads an account's status beside the open signals that disagree with it, so a proposal can be raised or withdrawn. It runs under the proposal executor's own principal and what leaves is a proposal row, gated where a human is shown it",
...
var lifecycleCompanyReads = gatekit.Waive(map[string]string{
	...
	"internal/modules/contacts/companystatus.go:SetCompanyStatusTx":              "the status stage move, reached from the signal-proposal executor under its own machine principal beside the acknowledgement it writes. It is a WRITE gated by the proposal that raised it; the read is the current stage, to refuse a move that has already happened",
...
var calleeGatedCompanyReads = gatekit.Waive(map[string]string{
	...
	"internal/modules/contacts/partnerlist.go:partnerListWhere":                   "the partner accounts list, behind ListPartners which asks the object gate at its entry",
