...
var predicateCompanyReads = gatekit.Waive(map[string]string{
	...
	"internal/compose/csvemployer.go:employerFor":                           "the per-name lookup a CSV import matches an imported contact's employer text against, row-scoped so a company the caller cannot see contributes neither a match nor an ambiguity. What the import reports is which row it linked; the answer itself is consumed inside the write. The cost is that an import can link to an account whose name the caller could not have listed — bounded by the row scope it does take",
