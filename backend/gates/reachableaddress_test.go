...
var (
	...
	// skipsArchived is the half deliberately left out of the shared constant.
	skipsArchived = regexp.MustCompile(`(?is)archived_at\s+IS\s+NULL`)
...
func TestOneAnswerToWhichAddressAContactIsKnownBy(t *testing.T) {
	...
	for _, path := range handWrittenGoSources(t) {
		...
		for _, decl := range file.Decls {
			for _, sql := range addressStatements(decl, scope) {
				judged++
				if ordersItself.MatchString(sql) {
					ordered = append(ordered, fmt.Sprintf("%s: %s", path, firstAddressLine(sql)))
				}
				if !skipsArchived.MatchString(sql) {
					unfiltered = append(unfiltered, fmt.Sprintf("%s: %s", path, firstAddressLine(sql)))
				}
			}
		}
	}
	...
	if len(ordered) > 0 {
		t.Errorf("these statements decide WHICH address a contact is known by themselves:\n  %s\n\n"+
			"contactaddress.ReachableOrder is the one answer — is_primary first, then the record's "+
			"own arrangement, then a stable tiebreak. A second spelling shows the same contact two "+
			"different addresses on two screens, each looking right on its own.",
...
func addressStatements(decl ast.Decl, owner helperScope) []string {
...
func TestWhatCountsAsPickingAnAddress(t *testing.T) {
	...
	// The two arms, each asked both ways: a gate that only ever answered "no
	// finding" would pass every census above for the wrong reason.
	ownOrder := `SELECT email FROM contact_email WHERE contact_id = $1 ORDER BY is_primary DESC LIMIT 1`
	if !ordersItself.MatchString(ownOrder) {
		t.Error("a hand-written ORDER BY on is_primary was not recognised as one")
	}
	shared := `SELECT email FROM contact_email WHERE contact_id = $1 AND archived_at IS NULL  LIMIT 1`
	if ordersItself.MatchString(shared) {
		t.Error("a statement using the shared constant was read as ordering itself — the constant is " +
			"a Go identifier and leaves no ORDER BY in the flattened text")
	}
	if !skipsArchived.MatchString(shared) {
		t.Error("an archived_at filter was not recognised")
	}
	if skipsArchived.MatchString(ownOrder) {
		t.Error("a statement with no archived_at filter was read as having one")
	}
}
