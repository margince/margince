...
//
//   - `company_id` in the PROJECTION — a read of `domain`/`is_primary` for
//     a company's own list is not asking who owns anything, and an existence probe
//     that selects 1 discloses no company at all.
//   - a filter ON THE DOMAIN — the bulk read that fetches every domain row for
//     a SET of companies projects `company_id` too, but it is answering "what
//     does this company own", the opposite direction, and it filters on the company.
//
...
var readsDomainClaim = regexp.MustCompile(
	`(?is)SELECT\s+[^;]*\bcompany_id\b[^;]*\bFROM\s+company_domain\b[^;]*\bdomain\s*=`)

// resolvers read the same table and project the same column while asking a
// DIFFERENT question: "which company does this domain resolve to", answered
...
var domainResolvers = gatekit.Waive(map[string]string{
	"internal/modules/people/dedupeorg.go": "exactCompanyByDomain is dedupe tier 1: it resolves which live company a candidate's domains already belong to so the record LANDS on it. The id goes to the merge, never to a caller, so there is no 409 and nothing to gate the disclosure of.",
...
func TestEveryDomainClaimAnswersThroughOneProbe(t *testing.T) {
	...
	if len(findings) > 0 {
		t.Errorf("%d statement(s) ask who owns a domain outside %s.\n\n"+
			"The answer carries a disclosure rule — the owning company's id is returned only when "+
...
func TestTheDomainClaimCensusSeesWhatItClaimsTo(t *testing.T) {
	t.Parallel()
	caught := []string{
		"SELECT company_id FROM company_domain WHERE domain = lower($1)",
		// Wrapped across lines, which is how it is actually written.
		"SELECT company_id\n\t\t\t  FROM company_domain\n\t\t\t WHERE domain = lower($1)",
		// Aliased and joined — still this question.
		"SELECT od.company_id, od.is_primary FROM company_domain od WHERE od.domain = $1",
		// Case is not the subject.
		"select COMPANY_ID from Company_Domain where domain = $1",
		// A set of candidate domains is the same question asked in bulk.
		"SELECT company_id FROM company_domain WHERE domain = ANY($1) AND archived_at IS NULL",
	}
...
var q = "SELECT company_id " +
	...
	missed := []string{
		// A company reading its OWN domains asks a different question, and every
		// domains surface does it — reporting these would bury the finding.
		"SELECT domain, is_primary FROM company_domain WHERE company_id = $1",
		// A bare existence probe discloses nothing about which company.
		"SELECT 1 FROM company_domain WHERE domain = lower($1)",
		// A WRITE is not a probe; the unique index answers those.
		"INSERT INTO company_domain (company_id, domain) VALUES ($1, $2)",
		// A different table whose name contains this one's.
		"SELECT company_id FROM company_domain_history WHERE domain = $1",
		// The opposite direction: what does this SET of companies own? It projects
		// company_id and reads the table, and it filters on the company.
		"SELECT company_id, id, domain, is_primary FROM company_domain WHERE company_id = ANY($1) AND archived_at IS NULL",
	}
