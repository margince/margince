...
var emailPassthroughs = gatekit.Waive(map[string]string{
	...
	"screens/brief.focus.tsx":            "draws no part of the message: the moment it arrived, for the card's label row, and its audience status as one more fact about whose row it is — the same access reading emailaccesseditor.tsx takes. No subject, no body, no party, no attachment. The message itself is handed to WorklistRow, which draws it through EmailEntry; the ranked column beside the card names its rows from `contact` and `itemTitle`",
