...
var readAuthorityOnAWritePath = gatekit.Waive(map[string]string{
	...
	"internal/modules/consent:ensureWithdrawalCredentialForSendTx": "the recipient of a marketing send, probed for the one thing this door is for: may this caller be given a link that STOPS that person's mail. It names nobody — bindWithdrawalSubject resolves the subject from the ADDRESS the message is going to — and the send is already authorized against that recipient by the consent gate and by the activity it creates, so the probe asserts no new authority over a record the mail is reaching anyway. Requiring write authority here refused the mint for every sender without person:update, an agent holding activity:create + person:read among them, and refusing it does not stop the message: it ships with no working List-Unsubscribe URL, which is worse for the subject than the share this admits. The authority is also strictly less than the SIBLING mint on this same path takes: PreferenceTokenForEmail asks person:read and this same visible probe for a credential that reads a consent state, withdraws AND grants, where this one can only stop mail. The door that lets a caller NAME a subject is EnsureWithdrawalCredentialTx beside it, and that one keeps person:update and the writable probe — a read share is not authority to mint somebody's opt-out. Unexported, so the weaker probe cannot be reached by naming a subject. The liveness half is load-bearing and tested: LockSubjectLive holds the row so a mint racing an Art. 17 erasure cannot write a fresh capability, carrying the plaintext address, for a subject just certified erased",
