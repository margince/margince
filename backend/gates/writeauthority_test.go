...
var readAuthorityOnAWritePath = gatekit.Waive(map[string]string{
	...
	"internal/modules/consent:EnsureWithdrawalCredentialTx":     "the recipient of a marketing send, probed for the one thing this mint is for: may this caller be given a link that STOPS that person's mail. Nothing is written to the person — the row inserted is a withdrawal_credential keyed on the ADDRESS the message is going to — and the only door that reaches here is a send the consent gate and the activity's own object grant have already authorized against this recipient. Requiring write authority refused the mint for every sender without person:update, an agent holding activity:create + person:read among them, and refusing it does not stop the message: it ships with no working List-Unsubscribe URL, which is worse for the subject than the share this waiver admits. The credential is also strictly WEAKER than the preference token minted beside it under the same probe (PreferenceTokenForEmail): that one reads a consent state, withdraws and grants, while this one can only stop mail. The liveness half is load-bearing and tested: LockSubjectLive holds the row so a mint racing an Art. 17 erasure cannot write a fresh capability, carrying the plaintext address, for a subject just certified erased",
