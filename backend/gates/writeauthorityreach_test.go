...
var writesWithoutARowProbe = gatekit.Waive(map[string]string{
	...
	"internal/modules/people:completePersonName":           "the fill above, with its result returned rather than recorded on a capture result — the same statement under the same predicate, extracted so the calendar path can reuse it instead of writing a second one. Everything said of fillMissingPersonName holds here and is the reason it is exempt: it completes columns that are null and never replaces what a human typed",
