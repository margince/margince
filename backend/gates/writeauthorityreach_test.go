...
var writesWithoutARowProbe = gatekit.Waive(map[string]string{
	...
	"internal/modules/deals:downgradeAndReview":          "the 🔻 tier of that same pass, split out of correct only so each tier's body reads on its own. Same authority and same reason as settleMember above: it runs under the workspace's system principal with no seat to narrow to, and the write it makes goes through apply, which locks the deal row and re-verifies the deal is still open. Note the UNDO of what this writes does take a row probe — RevertCorrection calls auth.EnsureWritable, because a person taking a machine's change back is acting as themselves and must own the record they are changing",
	...
	"internal/modules/people:RefreshDisplayNameTx":         "puts a name the record's own first_name and last_name already carry onto its display, for contacts that learned the name before the display followed it. Reached only from the scheduled repair, which runs as the system principal, so there is no caller whose authority a row probe could take. It writes one column to one value the row already holds elsewhere — no new fact enters the record — and only where displayNameSetByHumanTx says nobody typed that display name, which is the guard that matters here rather than who is asking",
	"internal/modules/people:touchPerson":                  "stamps updated_at on a person a LinkedIn match just resolved, so the row's own change is visible to anything reading it. Reached only through writeLinkedInHandle below, on the two decided paths that each hold the contact first — the human apply and the automatic confirm",
	"internal/modules/people:writeLinkedInHandle":          "puts a confirmed connection's profile URL on the contact, the one write a LinkedIn match makes to a person. Both decided paths reach it through confirmMatchWriteTail, and each HOLDS the contact through auth.HoldWritableLive before it does: the human apply at applyLinkedInMatchInTx's top, the automatic confirm at confirmOneMatch's. Extracting the shared tail put confirmMatchWriteTail between those holds and this write, so the walk's one-level suppression no longer sees the guard a frame further up — the same shape touchPerson above is exempt for",
	"internal/modules/people:completePersonName":           "the fill above, with its result returned rather than recorded on a capture result — the same statement under the same predicate, extracted so the calendar path can reuse it instead of writing a second one. Everything said of fillMissingPersonName holds here and is the reason it is exempt: it completes columns that are null, and replaces a display name only where no person has ever set one",
	"internal/modules/people:FillParticipantNamesTx":       "names the people one meeting's attendee rows resolved to, from the name the invitation gave them. Reached from the capture transaction through a compose-wired seam, where the authority is the connector's own grant taken at sync, and from the name-recovery pass, which runs as the system principal. It writes only through completePersonName above, so the completion-not-overwrite guarantee is the same one",
	"internal/modules/people:fillPersonNameFromAttendance": "the other ordering of that same fill: one person, named from the newest invitation that named them, run by the cohort repair after it has promoted their attendee rows. Same statement, same predicate, same reason",
	"internal/modules/people:PromotePersonCohortTx":        "the cohort repair, which files a person's already-captured mail and meetings under them and now also completes their name from the invitations they were on. It runs as the system principal on a schedule, so there is no caller whose authority a row probe could take; every write it reaches is either a link (nothing on the person itself) or the name completion above, under that function's own predicate",
	"internal/modules/people:RepairPersonCohort":           "the transaction wrapper around it, for the same reason",
