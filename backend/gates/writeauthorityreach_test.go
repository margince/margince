...
//
// The gap matters here more than it would elsewhere. Contact, company,
...
var writesWithoutARowProbe = gatekit.Waive(map[string]string{
	...
	"internal/modules/deals:HoldClosingOccurrenceTx": "a SELECT ... FOR SHARE, not a mutation. " +
		...
		"`deal`, and the row is never changed. A write probe would be the wrong test and a " +
		"stricter one: writing a review creates an ACTIVITY, gated on activity:create, and " +
		"requiring write authority over the DEAL would refuse a colleague who may perfectly well " +
		"say what they thought of an outcome they watched. The object grant is taken here through " +
		"auth.Require(deal, read), and the row scope by the reader itself",
	"internal/modules/deals:lockedAcquisitionSource":      "a SELECT ... FOR UPDATE, not a mutation. It takes the deal's row lock and re-reads one column so the acquisition-source decision is made on a value that cannot move before the write; the extractor reads the locking SELECT as a write to `deal`. A second probe would ask the same question its only caller has already asked: dealUpdatePatch runs inside updateDealInTx, which takes auth.EnsureWritable on this exact row, on this transaction, before the patch is built. Taking the lock HERE rather than leaving it to the patch is the point — the unlocked read the caller already did is fine for an audit before-image and not fine for deciding what may be assigned",
	"internal/modules/contacts:PromoteOwnCapturedContact": "the mailbox owner publishing a contact their own capture made. A row probe would be the WRONG test and a wider one: EnsureWritable answers \"may this caller edit this record\", which a write grant or a team scope satisfies — and neither is authority to publish somebody else's private correspondence. Capture privacy is the importing user's alone and does not yield to row scope (auth/rowscope.go), so the row test here is OWNERSHIP, and it is in the UPDATE's own predicate (owner_id = the authenticated principal) where nothing can route around it. A miss is ErrNotFound rather than a permission error, because existence is what the boundary hides. The object grant is still taken, through auth.Require",
	...
	"internal/modules/privacy:AnonymiseContacts":            "an owner destroying the contacts their own mailbox is the only reason the CRM knows. A row-writability probe would ask the wrong question: the record IS workspace-visible — that is the defect being undone — so the probe would pass for any seat and prove nothing. What confines this is the SELECTION, in capture.SelectPurgeableContactsTx, which admits a contact only when every live address matches the caller's own rule, no deal links them, and no other seat imported their mail. The caller is checked for being a contact at the entry point above",
	"internal/modules/privacy:anonymiseOneContact":          "the per-contact half of AnonymiseContacts, on its own transaction; the same reasoning and the same selection govern it",
	"internal/modules/privacy:anonymizeContact":             "the retention engine anonymizes a contact in place because their retention clock expired, under a policy that names the table. The subject is chosen by age; there is no caller asking for this row and no authority to test beyond the policy itself",
	...
	"internal/modules/privacy:archiveLead":                  "the lead half of the same retention pass, archiving instead of anonymizing: an unconverted lead past its window is archived because of its age, not because somebody asked",
	"internal/modules/contacts:shiftVisibilityTx":           "the one spelling of moving a contact between visibilities, extracted from promoteIfWorkspaceScoped so the promotion and the personal-verdict withdrawal cannot drift into auditing differently. A row probe fails here for exactly the reason both its callers' entries give: one direction starts from an owner-scoped row, which platform/auth reads as capture privacy and refuses to every principal but its owner, and the other ends on one. It decides nothing itself — the `from` visibility is pinned by the caller and the UPDATE matches no row when the record has already moved, so it cannot reverse a decision somebody else made. Authority is settled by the two callers, each ratified above on its own argument",
	"internal/modules/contacts:promoteIfWorkspaceScoped":    "a row probe here would refuse the write it exists to make. The row is owner-scoped, which platform/auth reads as capture privacy — invisible to every principal but its owner, an admin included — so EnsureWritable answers no for exactly the rows this promotes. What decides the promotion is not the caller's authority over the row but the verdict that judged its sender a business counterparty, and the surrounding ensureContact already takes auth.Require(contact, create) for the caller who may mint one at all. The write is one-directional and idempotent: it moves owner to workspace and never back",
	"internal/modules/contacts:archiveContactRows":          "the one spelling of archiving a contact inside a transaction, and its THREE callers take their authority differently rather than sharing one probe. ArchiveContact takes auth.EnsureWritable on the row at its entry point, which is the human path. The lead demotion took it on the LEAD, which is the act that authorizes unwinding the contact a promotion created. RetractCaptureOnlyContactTx cannot take one at all — see its own entry below — and states why. A probe here would be the wrong shape for the third and redundant for the first two, and moving it here would refuse the retraction outright",
	"internal/modules/contacts:RetractCaptureOnlyContactTx": "the mirror of promoteIfWorkspaceScoped above, and a row probe fails for the same reason: the row is owner-scoped, which platform/auth reads as capture privacy — invisible to every principal but its owner — so EnsureWritable answers no for exactly the rows this withdraws. It runs under the confidentiality verdict's system principal with no human behind it. What confines it is the SELECTION and its own predicate: capture.ContactsOrphanedByPrivacyTx admits only a contact whose every thread with this seat is held personal, and the statement here then refuses anything a human created, a human has edited or published, or another seat owns. It deliberately does NOT refuse a record the SENDER verdict promoted: that promotion is another machine's guess with no human behind it, and treating it as protection let the earlier of two machine answers outrank the later one — a private medical correspondent stayed visible to a whole workspace on nothing but the order two passes ran in. A promoted record is narrowed back to its owner before the archive, because an archived workspace row is still listable by anyone asking for archived ones. It archives rather than destroys, landing the same audit row a human's archive would",
	...
	"internal/modules/deals:settleMember":                   "the per-deal half of that same pass, and it carries the exception for the same reason: it assesses ONE member of the frozen eligible set and hands the write to correct/apply, which locks the row and re-verifies the deal is still open before patching. The pass is the unit of authority here, ratified above; a seat probe on the individual member would narrow the sweep to whoever happened to own that deal and silently leave the rest of the pipeline uncorrected",
	"internal/modules/deals:downgradeAndReview":             "the 🔻 tier of that same pass, split out of correct only so each tier's body reads on its own. Same authority and same reason as settleMember above: it runs under the workspace's system principal with no seat to narrow to, and the write it makes goes through apply, which locks the deal row and re-verifies the deal is still open. Note the UNDO of what this writes does take a row probe — RevertCorrection calls auth.EnsureWritable, because a contact taking a machine's change back is acting as themselves and must own the record they are changing",

	// The shared fill, probed by BOTH of its callers rather than by itself.
	// ApplySiteContactFields takes EnsureWritableLive and SKIPS an out-of-scope
	...
	// record — a second contact or company for the same human — which is worse
	// for Rep A than the write it would have prevented.
	"internal/modules/contacts:EnsureCounterparty":            "capture resolving the contact and company a captured message is about, and attaching it. The row it lands on is frequently a colleague's, by design: refusing would not protect that record, it would create a duplicate of it alongside. What this may write to an incumbent is bounded instead — a name fills only where the column is still empty, and a header carrying an impersonation tell teaches an existing record nothing",
	"internal/modules/contacts:writeCompanyColumn":            "the cold-start writer's per-column half, reached only from applyColdStartTx and bounded by the same accepted proposal",
	"internal/modules/contacts:applyUnclaimedCompanyColumn":   "the fill arm of writeCompanyColumn, reached only through it, so the proposal that bounds one bounds the other. Newly visible to this census rather than newly unprobed: its statements sit in a package-level table, which the index could not read until it folded them",
	"internal/modules/contacts:setCompanyColumn":              "the company form's per-column half, reached only from writeCompanyFields. Its authority is taken two frames up in resolveOrCreateAnchor — auth.EnsureWritable on the anchor, after anchorCompany has resolved it live and locked it FOR UPDATE for the rest of the transaction. Newly visible for the same reason applyUnclaimedCompanyColumn is",
	"internal/modules/contacts:EnsureCounterpartyTx":          "capture's resolution running inside the caller's transaction, reached only from EnsureCounterparty",
	"internal/modules/contacts:ensureContact":                 "the contact half of that same capture resolution",
	"internal/modules/contacts:fillMissingContactName":        "capture's name fill on an incumbent, reached only from ensureContact. The write predicate is first_name IS NULL AND last_name IS NULL, so it only ever completes a record nobody has split into parts; full_name moves with them unless a contact set it, which displayNameSetByHumanTx answers from who minted the row (contact.captured_by) and whether a human has edited the name since (an audit update naming full_name). A name a human typed is left alone at both moments. It completes what a machine guessed rather than overwriting what somebody decided, which is why the row it lands on being a colleague's is not the disclosure a probe would prevent",
	"internal/modules/contacts:RefreshDisplayNameTx":          "puts a name the record's own first_name and last_name already carry onto its display, for contacts that learned the name before the display followed it. Reached only from the scheduled repair, which runs as the system principal, so there is no caller whose authority a row probe could take. It writes one column to one value the row already holds elsewhere — no new fact enters the record — and only where displayNameSetByHumanTx says nobody typed that display name, which is the guard that matters here rather than who is asking",
	"internal/modules/contacts:touchContact":                  "stamps updated_at on a contact a LinkedIn match just resolved, so the row's own change is visible to anything reading it. Reached only through writeLinkedInHandle below, on the two decided paths that each hold the contact first — the human apply and the automatic confirm",
	"internal/modules/contacts:writeLinkedInHandle":           "puts a confirmed connection's profile URL on the contact, the one write a LinkedIn match makes to a contact. Both decided paths reach it through confirmMatchWriteTail, and each HOLDS the contact through auth.HoldWritableLive before it does: the human apply at applyLinkedInMatchInTx's top, the automatic confirm at confirmOneMatch's. Extracting the shared tail put confirmMatchWriteTail between those holds and this write, so the walk's one-level suppression no longer sees the guard a frame further up — the same shape touchContact above is exempt for",
	"internal/modules/contacts:completeContactName":           "the fill above, with its result returned rather than recorded on a capture result — the same statement under the same predicate, extracted so the calendar path can reuse it instead of writing a second one. Everything said of fillMissingContactName holds here and is the reason it is exempt: it completes columns that are null, and replaces a display name only where no contact has ever set one",
	"internal/modules/contacts:FillParticipantNamesTx":        "names the contacts one meeting's attendee rows resolved to, from the name the invitation gave them. Reached from the capture transaction through a compose-wired seam, where the authority is the connector's own grant taken at sync, and from the name-recovery pass, which runs as the system principal. It writes only through completeContactName above, so the completion-not-overwrite guarantee is the same one",
	"internal/modules/contacts:fillContactNameFromAttendance": "the other ordering of that same fill: one contact, named from the newest invitation that named them, run by the cohort repair after it has promoted their attendee rows. Same statement, same predicate, same reason",
	"internal/modules/contacts:PromoteContactCohortTx":        "the cohort repair, which files a contact's already-captured mail and meetings under them and now also completes their name from the invitations they were on. It runs as the system principal on a schedule, so there is no caller whose authority a row probe could take; every write it reaches is either a link (nothing on the contact itself) or the name completion above, under that function's own predicate",
	"internal/modules/contacts:RepairContactCohort":           "the transaction wrapper around it, for the same reason",
	...
	"internal/modules/contacts:applyEvidenceFieldsWithOverwrite": "the company evidence writer, reached from three paths that each take the probe before calling it: the cold-start accept (gateResolvedColdStartTarget, on the company the URL resolved to), Enrich, and the site-read confirmation",

	"internal/modules/contacts:recomputeUnderOverrideTx": "the sticky-override branch of recomputeLeadScoreTx, reached from nowhere else. Its four callers each take the row probe before they get here — RecomputeLeadScore, both manual-signal paths and UpdateLead — and the probe belongs at those entry points rather than in a branch, because an archived lead is a silent no-op to the workflow lane and a 404 to a human",

	"internal/modules/contacts:absorbCompanyReferences": "the company merge's cascade: it re-points the deals, projects and child companies that NAMED the absorbed company at the survivor. The merge itself takes write authority on BOTH companies through mergePair before anything moves, and the rows re-pointed here are not the ones being decided about — refusing to re-point a deal the caller cannot write would leave it pointing at a company that no longer exists",
	"internal/modules/contacts:RouteLead":               "routing assigns an ownerless lead to a chosen rep, and an ownerless row is nobody's to write by construction (the write arm renders no owner_id IS NULL branch) — auth.EnsureWritable here could only ever refuse. It self-guards instead: a lead that already has an owner returns already_owned before anything is written, so routing can never overwrite a human's assignment. The claim-then-write primitive this shape wants is storekit.ClaimOwnership, which takes the claimant as `me` and so does not fit an assignment to a third party",
	...
	"internal/modules/privacy:anonymizeContactRecord":   "the retention engine's contact write, reached only from the pass that selected the row by its expired clock",
...
func TestEveryWriteToAShareableRecordReachesAWriteAuthorityProbe(t *testing.T) {
	...
	for dir, byReceiver := range pkgs {
		...
		// outright would be a hole: `UpdateContact` on the store shares its name
