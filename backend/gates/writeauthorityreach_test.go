...
var writesWithoutARowProbe = gatekit.Waive(map[string]string{
	...
	"internal/modules/people:RetractMisattributedSignatureFields": "the enrichment pass taking back fields it wrote off a message the person never sent. A row probe would ask the wrong question and answer it wrongly in both directions: the pass runs as the installation's own system principal with no human behind it, so there is no caller scope to test, and narrowing the sweep to one seat's rows would leave every other contact wearing another sender's title while the pass reported success. What confines it is the SELECTION — SenderPredicate, the same expression that decides what may be written in the first place — plus the two protections the per-message retraction already spells out: it deletes only rows this pass itself wrote (source = capture_enrich) and never one a human has corrected. It restores the mirrored column to the value it replaced rather than clearing it, so a title somebody typed survives",
