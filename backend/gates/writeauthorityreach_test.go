...
var writesWithoutARowProbe = gatekit.Waive(map[string]string{
	...
	"internal/modules/people:writeFormColumn":              "the frame between writeCompanyFields and setCompanyColumn, holding the branch that decides whether a field HAS a column to write. Same authority, one frame nearer: auth.EnsureWritable on the anchor in resolveOrCreateAnchor, taken before the form is written at all",
