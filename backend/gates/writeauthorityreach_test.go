...
var writesWithoutARowProbe = gatekit.Waive(map[string]string{
	...
	"internal/modules/contacts:touchContact":                  "stamps updated_at on a contact a LinkedIn match just resolved, so the row's own change is visible to anything reading it. Reached only from the apply path, which is a system principal driving an accepted match",
