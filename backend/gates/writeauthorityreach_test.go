...
var writesWithoutARowProbe = gatekit.Waive(map[string]string{
	"internal/modules/people:placePhones":              "the retained-row half of a whole-list phone replace, reached only from replacePersonPhones — which is itself reached only from UpdatePerson, one frame after auth.EnsureWritable(ctx, tx, \"person\", id.UUID) on the SAME person row in the SAME transaction. The probe is real; it is one hop further than this witness's per-function AST walk follows from a guarded caller, the same gap activities:finalizeRelinkedActivity is ratified for in updateguard_test.go",
