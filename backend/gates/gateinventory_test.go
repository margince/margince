...
const (
	...
	// gateDir is where the census looks, relative to the module root TestMain
	// puts every gate at. The page names a gate by its path RELATIVE to this
	// directory, which is what keeps a row reading `foo_test.go` rather than
	// `gates/foo_test.go` — the rows say which gate, and where the directory
	// sits is this constant's business, not 163 rows'.
	gateDir = "gates"
...
// gate is one declared gate: its file, the name the page calls it by, its
// shape, and the first sentence of its own doc comment.
//
// file and name differ only in the gateDir prefix, and both are carried because
// they are read by different audiences: a failure message names `file`, so the
// reader can open it from the module root, while the page prints `name`.
type gate struct {
	file, name, kind, hardness, holds string
}

// TestNoGoSourceHidesWhereThisCensusCannotSeeIt closes the drop point the move
// opened.
//
// This census globs gates/*_test.go — flat, one directory. Two places a Go file
// can now sit where it is a test to `go test ./...` and invisible to everything
// here:
//
//   - The module root. Every doc in this tree said `backend/foo_test.go` until
//     the gates moved, so recreating one there is the natural next mistake. It
//     would compile as its own package, run CACHED (it is not in
//     UNCACHED_TEST_PKGS — the stale-green replay that variable exists to
//     prevent), owe no //gate:kind, and appear on no page.
//   - A subdirectory of gates/. Same three, and the non-recursive glob is why.
//
...
func TestNoGoSourceHidesWhereThisCensusCannotSeeIt(t *testing.T) {
	...
	for _, path := range atRoot {
		t.Errorf("%s sits at the backend module root, where this census does not look and the "+
			"`test` target does not run it uncached. Gates belong in %s/; production code belongs "+
			"in a package under internal/, cmd/ or pkg/", path, gateDir)
	}

	err = filepath.WalkDir(gateDir, func(path string, entry fs.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		...
		if filepath.Dir(path) != gateDir {
			t.Errorf("%s is nested inside %s/, and this census globs that directory flat — the file "+
				"is a package of its own that no gate here judges. Move it up beside the others",
				path, gateDir)
		}
		return nil
	})
	if err != nil {
		t.Fatalf("walking %s: %v", gateDir, err)
	}
}
...
func TestEveryGateDeclaresItsShape(t *testing.T) {
	...
	if len(gates) < gateFloor {
		t.Fatalf("this census found %d gate file(s) and expects at least %d, so the walk over "+
			"backend/gates/*_test.go has stopped finding them rather than the tree having shrunk",
			len(gates), gateFloor)
	}
}
...
func TestTheGateInventoryPageIsCurrent(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("reading %s: %v\nRegenerate it from the backend directory with:\n"+
			"  go test ./gates -run GateInventory -update-gate-inventory", gateInventoryPage, err)
	}
	if string(got) == want {
		return
	}
...
func readGateDeclarations(t *testing.T) (gates []gate, untagged []string) {
	...
	for _, path := range paths {
		...
		// Rel and not TrimPrefix: a path that is not under gateDir must be an
		// error and not a silently un-trimmed row. Today the glob cannot
		// return one, but the next author to widen this into a walk gets a
		// failure that names the file instead of a page that quietly grew a
		// differently-shaped row.
		name, relErr := filepath.Rel(gateDir, path)
		if relErr != nil {
			t.Fatalf("naming %s relative to %s: %v", path, gateDir, relErr)
		}
		gates = append(gates, gate{path, filepath.ToSlash(name), kind, hardness, firstSentenceOf(parsed)})
	}
...
func TestGatePatternsCitesGatesThatExist(t *testing.T) {
	...
	for _, citation := range cited {
		// The page cites a gate by the same bare name the inventory rows use,
		// so the file it names is resolved under gateDir — the one place a
		// gate lives.
		if _, statErr := os.Stat(filepath.Join(gateDir, citation[1])); statErr != nil {
