...
//
// contact360's readProfileFields overlays the verdict and its comment says it is
...
var profileFieldRead = gatekit.TableReadPattern("contact_profile_field")
...
// `DELETE`/`UPDATE … SET contact_id` touch the table without ever putting a
...
// `SELECT ppf.contact_id, ppf.value FROM contact_profile_field` serves the value
...
// column of a projection, so `SELECT ppf.contact_id, ppf.value` escaped it —
// and a star projection escaped it too, naming none of the value words at all.
// Each was found by a contact or a bot, not by the gate, which is the argument
// for this test.
func TestTheProfileFieldCensusSeesWhatItClaimsTo(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"a value column first in the projection": {
			source: `func read() string { return "SELECT value, field FROM contact_profile_field" }`,
			want:   []string{"read"},
		},
		// The shape that escaped the first widening.
		"a value column later in the projection": {
			source: `func read() string { return "SELECT ppf.contact_id, ppf.value FROM contact_profile_field ppf" }`,
			want:   []string{"read"},
		},
		// And the shape that named none of the words.
		"a qualified star": {
			source: `func read() string { return "SELECT ppf.* FROM contact_profile_field ppf" }`,
			want:   []string{"read"},
		},
		"a bare star": {
			source: `func read() string { return "SELECT * FROM contact_profile_field" }`,
			want:   []string{"read"},
		},
		...
		"an existence probe is not a serve": {
			source: `func probe() string { return "SELECT 1 FROM contact_profile_field f WHERE f.field = 'company_name'" }`,
			want:   nil,
		},
		"a delete is not a serve": {
			source: `func retire() string { return "DELETE FROM contact_profile_field WHERE contact_id = $1" }`,
			want:   nil,
		},
		...
		"a reader that consults the ledger passes": {
			source: `func read() string {
				applyFieldVerdicts()
				return "SELECT value FROM contact_profile_field"
			}`,
			want: nil,
		},
		"an overlay declared elsewhere does not answer for the reader": {
			source: `func read() string { return "SELECT value FROM contact_profile_field" }
func applyFieldVerdicts() {}`,
	...
	for _, column := range expectedValueColumns {
		t.Run("serving "+column+" is a serve", func(t *testing.T) {
			source := `func read() string { return "SELECT ` + column + ` FROM contact_profile_field" }`
