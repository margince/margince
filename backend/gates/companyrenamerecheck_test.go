...
// statement setting `company.display_name` or `company.legal_name`
...
// `DedupeCompanyForCreate`, `resolveOrCreateAnchor` and
// `manualDedupeCompany` are the three that feed it. A row that reaches the
...
// `UPDATE company SET ` + column + ` = $2`, and drives it with
...
//	UPDATE company SET description = 'x' -- don't
//	SET legal_name = $1                        an apostrophe inside a comment closes nothing
//	UPDATE company /* ; */ SET legal_name = $1
...
func TestEveryCompanyRenameReachesTheDuplicateRecheck(t *testing.T) {
	t.Parallel()
	...
	if len(unreadable) > 0 {
		t.Errorf("these functions declare a local with the same name as a package-level "+
			"statement that renames a company, so this census could not tell which one "+
			"they read:\n    %s\n\n"+
	...
	if len(findings) > 0 {
		t.Errorf("these functions rename a company, and no route to them calls %s:\n    %s\n\n"+
...
func TestTheRenameDetectorReadsSQLAndNotProse(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			"a semicolon inside a block comment", "UPDATE company /* ; */ SET legal_name = $1", true,
			"the scan stopped at a semicolon that ends nothing, so a real rename went unseen",
		},
		{
			"a semicolon inside a line comment", "UPDATE company -- ;\n SET legal_name = $1", true,
			"same, with the comment spelled the other way",
		},
		{
			"a semicolon inside a dollar-quoted body", "UPDATE company SET legal_name = $tag$ ; $tag$", true,
			"a dollar-quoted literal is a body, not the end of a statement",
		},
		{
			"a column named only inside a comment", "UPDATE company SET description = $1 -- legal_name =", false,
			"a rename mentioned in prose is not a rename, and reporting it teaches readers to skip this gate",
		},
		{
			"a column named only inside a string", "UPDATE company SET description = 'legal_name = x'", false,
			"same, inside a literal the database stores rather than executes",
		},
		{
			"a nested block comment", "UPDATE company /* outer /* inner */ still comment */ SET legal_name = $1", true,
			"Postgres nests block comments and a non-greedy regex does not; the tail was read as SQL",
		},
		{
			"a nested block comment hiding a column", "UPDATE company SET description = $1 /* a /* b */ legal_name = */", false,
			"everything between the outer pair is comment, however many pairs are inside it",
		},
		{
			"the column assembled by the caller", "UPDATE company SET ", true,
			"the fragment names no column because the caller supplies it; the gate asks anyway",
		},
		{
			"a block comment opened inside a line comment", "UPDATE company -- /*\n SET legal_name = $1", true,
			"a `/*` the database reads as prose opened a comment that ran on and swallowed the rename after it",
		},
		{
			"an apostrophe inside a line comment", "UPDATE company -- don't\n SET legal_name = $1", true,
			"an apostrophe in prose pairs with the next real quote and takes the SQL between them with it",
		},
		{
			"an apostrophe inside a block comment", "UPDATE company /* don't */ SET legal_name = $1", true,
			"same, spelled the other way",
		},
		{
			"placeholders are not a dollar-quoted body", "UPDATE company SET description = $1, legal_name = $2", true,
			"a tag is an identifier, so `$1 … $2` is two parameters and not a body hiding what is between them",
		},
		{
			"a body that does not close on a different tag", "UPDATE company SET description = $q$ $$ legal_name = $q$", false,
