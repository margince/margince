...
var ungatedEntryPoints = gatekit.Waive(map[string]string{
	"internal/modules/capture:ReaderAddressesTx": "answers one named seat's OWN email addresses \u2014 what they declared about themselves, plus the account labels of the mailboxes they connected. There is no other seat's data to withhold: the statement is keyed on the user id the caller passes, and every row it can return is that person's own identity. The waiting queue asks it to tell a message written to the reader from one written to a colleague, and it runs on the queue's transaction \u2014 under a background principal for the overnight assembly and a human one for a page read, so a human-only gate would refuse exactly the pass that needs it. What the ANSWER may then influence is bounded elsewhere: the queue reports the addressing fact and never excludes on it, and the rows themselves are already narrowed by the reader's own row scope",
