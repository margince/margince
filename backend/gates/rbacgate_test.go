...
var ungatedEntryPoints = gatekit.Waive(map[string]string{
	...
	"internal/modules/identity:ListSessions":                   "reads the CALLER's own rows and nobody else's, on the same terms as MyWorkingHours above: the user id comes off the bound principal via selfScoped rather than from a parameter, so there is no other account it could reach and no object for a grant to narrow. It lists the sessions open under that member's account — a fact about their own sign-ins — and the opaque token behind each is never read back. A principal with no human behind it is refused",
	"internal/modules/identity:RevokeSession":                  "acts on the CALLER's own rows and nobody else's: the user id comes off the bound principal via selfScoped, not a parameter, and every statement is scoped to it, so a session id belonging to anyone else is a row-scope miss answered as not-found rather than a reachable object. Ending one's own session is session self-management, the same self-scoping ChangePassword above relies on, and a principal with no human behind it is refused outright",
	"internal/modules/identity:StartTOTPEnrolment":             "acts on the CALLER's own enrolment and nobody else's: selfScoped takes the user id off the bound principal, never a parameter, so it can enrol no other account. Setting up a second factor is a member's own choice about their own login, the same self-scoping SaveMyWorkingHours relies on; a principal with no human behind it is refused",
	"internal/modules/identity:ConfirmTOTP":                    "confirms the CALLER's own pending enrolment and nobody else's: selfScoped takes the user id off the bound principal, not a parameter, so it can confirm no other account's factor. Authorized by possession of a valid code against the caller's own sealed secret, the second-factor analogue of ChangePassword verifying the current password; a non-human principal is refused",
	"internal/modules/identity:MFAEnrolment":                   "reads the CALLER's own second-factor state and nobody else's: the user id comes off the bound principal via selfScoped, not a parameter, so there is no other account it could report on. It answers whether this member has a factor and how many recovery codes remain, a fact about their own login, and refuses a principal with no human behind it",
	"internal/modules/identity:DisableMFA":                     "removes the CALLER's own factor and nobody else's: selfScoped takes the user id off the bound principal, not a parameter, so it can disable no other account's second factor. Turning off one's own MFA is self-management like ChangePassword beside it, and removing a CONFIRMED factor additionally demands a current authenticator or recovery code in the same transaction — a session alone must not strip the factor guarding it; a principal with no human behind it is refused",
	...
	"internal/modules/identity:SSOEnforced":                    "pre-authentication by construction: it reads the installation-wide enforced-SSO policy on an anonymous login request to decide whether that request may use the password path at all — gating it would demand the very credential the login is about to establish. It returns a single boolean about the installation, never anything about an account, and reads as the installation (system actor), the same anonymous posture the sign-in provider policy read already takes for the login screen",
	"internal/modules/identity:MFARequired":                    "pre-authentication by construction, the sibling of SSOEnforced: it reads the installation-wide require-MFA policy so admission can decide whether a member with no factor is confined to the enrolment routes — a single boolean about the installation, nothing about an account, read as the installation (system actor). Gating it would demand a grant to answer a question that gates access itself",
	"internal/modules/identity:GroupRoleMap":                   "pre-authentication by construction, the third sibling of SSOEnforced and MFARequired: it reads the installation-wide group→role grant map while a federated login is deciding what the arriving member holds — gating it would demand the very session the login is about to mint. It returns installation policy, never anything about an account, read as the installation (system actor); what a caller can do with it is bounded by resolveFederatedUser, which has already refused everyone the installation never invited",
	"internal/modules/identity:CompleteMFAChallenge":           "pre-authentication by construction, the second half of a password login: the signed 202 challenge the handler verified is the authorization, so there is no session principal to gate on — gating it would demand the very session it is about to mint. What authorizes it is possession of the second factor, verified against the member's own sealed secret or an unused recovery code inside the transaction, the same shape checkCredentials has on the password path; a wrong code is a neutral refusal and no session is minted",
