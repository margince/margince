...
var ungatedEntryPoints = gatekit.Waive(map[string]string{
	...
	"internal/modules/automation:Confirm":                     "Claim's other half, and ungated for Claim's reasons: it stamps applied_at on the row that same firing just took, from inside the same dispatched firing under the same system principal. It takes no argument a caller chose — the three values addressing the row are the ones Claim was given — and it writes one timestamp, returning nothing. It cannot reach a row another firing holds: the statement matches only a claim still unapplied, so a late Confirm from a firing whose claim was reclaimed changes nothing. Without it a create that landed would look abandoned once its lease expired, and the record would be written a second time — which is the corruption the claim exists to stop",
	...
	"internal/modules/activities:ResolveMeetingToken":  "pre-authentication capability resolver; validates the hashed token and expiry, returns only the bound invitation and host, and writes nothing",
	"internal/modules/activities:ResolveProposalToken": "pre-authentication capability resolver; validates the hashed token and expiry, returns only the bound proposal and host, and writes nothing",
	...
	"internal/modules/activities:MarkCaptureLabelDeclined":   "the classify sweep's own bookkeeping, driven by the worker under the workspace GUC with no human principal, same posture as SetCaptureLabel: it stamps when every rung declined to label the message and which classify prompt they declined, which removes it from that prompt's backlog; it states no verdict, discloses nothing and writes no other column",
