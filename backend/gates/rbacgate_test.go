...
var ungatedEntryPoints = gatekit.Waive(map[string]string{
	"internal/modules/capture:ReaderAddressesTx": "answers one seat's OWN email addresses \u2014 their login, what they declared about themselves, and the labels of the mailboxes they connected. It takes no object grant because there is no object: these are private aliases rather than a record, and the RBAC vocabulary has no verb for them. What bounds it is selfOnly, in the function: a human principal may ask only about themselves, and a system principal is admitted because the overnight assembly reads for a named seat with no acting human to compare against \u2014 bounded there by the caller, which resolves the id from the reader it is assembling for and never from a request. A read for another seat is refused with ErrPermissionDenied",

	"internal/modules/contacts:AnchorProfileStanding": "answers whether this installation has described ITSELF yet, and whether the description carries the minimum a reader may rely on \u2014 two booleans about us, and no field of the profile. The administered read beside it (GetAnchorCompany) is where the legal name, the website and every typed field live, and that one is an administrator's. This one is not, and the growth fit is the case that decides it: GrowthFitService.Get is a reading aid any human seat opens on any account, and it caps the band it reports when this installation has not said what it offers \u2014 so a seat that could not resolve this would either be refused a page they may read or be handed a stronger band than the evidence supports. The onboarding conversation asks the same question to decide whether it is speaking about a company that exists. Widening it to return the company would make it the administered read under another name, which is the borrowing #470 exists to undo",

	"internal/modules/contacts:DecidableForMerge": "answers WHICH of the given rows this caller could merge — a permission map, not data. It deliberately requires no object grant, because a reader whose role lacks the update verb is a legitimate caller whose answer is \"no verb for you\", and refusing the read would cost them the merge card they are entitled to see. WritableSubset omits every row they cannot see, so the map never names a record the caller could not have listed",

	"internal/modules/consent:Disclosures": "the same answer as DisclosuresFor below, for a caller that holds no transaction \u2014 the send path composes a body outside one, so this opens its own and delegates. Ungated for exactly the reason that one is: the disclosures a message carries must apply to whoever composes it, and a rep holds contact grants and no settings grant, so a gate here would REFUSE the send outright for a rep who may write to the contact perfectly well, turning an operator permission into a precondition for correspondence. It reads nothing about any subject, taking only a category, and answers the installation's own public identity \u2014 the words it prints at the bottom of its mail, which any recipient already has in their inbox",

	"internal/modules/consent:DisclosuresFor": "answers which legally required disclosures a message must carry and what words meet them \u2014 the controller's own name, address, privacy contact and objection route. It is ungated because it must apply to WHOEVER composes the message: a rep writing a reply, a worker dispatching a scheduled send under the system principal, and the controller lane mailing a privacy notice all need it, and a gate here would mean a message went out WITHOUT its legally required disclosure because the sender lacked an operator permission. There is nothing to withhold: the value is the installation's own public identity, the words it prints at the bottom of its mail, so any recipient already has them in their inbox \u2014 and it reads nothing about any subject, taking only a category. WRITING the particulars stays gated on installation_settings at update, through the settings store that owns the table, which is where the trust actually sits",

	"internal/modules/consent:PrivacyInformationFor": "assembles the Art. 14 disclosure behind a mailed notice link: how this contact was obtained, what their data is used for, and the rights they hold. It is gated by the LINK rather than by an object grant, the same authority every other public confirm page runs on \u2014 whoever holds the plaintext proved the mailbox, and the reader is a member of the public with no seat to grant anything to. Requiring a grant would make the page unreachable by the one reader entitled to it. What bounds it is the KIND check in the function plus the token resolution above it: the one HTTP path that reaches this resolves the token first (handlers_confirm.go), and a reference of any other link kind is refused here so a record or consent link cannot be turned into a disclosure. It discloses less than the record page it sits beside \u2014 no name, no employer, no address, no phone, no provenance \u2014 because the message that carried the link promised to tell somebody what is held, not to show them their file",

	"internal/modules/consent:RecordInquiry": "records that a contact asked for the meeting they have just booked. Its one caller is the PUBLIC booking door, which has no session and no seat to grant anything to — a gate here would make the row unwritable by the only caller entitled to write it, and the basis would then be missing for exactly the contacts who initiated contact. What bounds it is the fact it records rather than a principal: the door has committed a meeting this contact booked, in the same request, and the row cites that meeting as its evidence. It asserts nothing a seat could not read off the record, and the hand-recorded verbs beside it — a named human claiming an exchange with no other evidence — keep their RequireHuman",

	"internal/modules/consent:LockStopsTx": "takes an advisory lock on the subjects it is handed and does nothing else \u2014 no statement reads a row, writes one, or returns one, so there is no data for a gate to withhold and no state a caller can change through it. It exists because the ORDER of two locks matters: recording a stop takes this lock and then reads the contact row, while a merge locks the contact rows and then reaches for this lock inside CarryStopsTx, which is a deadlock. Splitting the lock out lets the merge take it first. The write it precedes, CarryStopsTx, IS gated and stays gated; gating the lock as well would refuse the merge paths that legitimately hold lead:update or contact:create rather than contact:update, which is the refusal that made this seam wrong in the first place. The worst a caller with no grant achieves is to make their own transaction wait",
	...
	"internal/modules/capture:SlimBatch":                      "the raw-capture part sweep, run by the worker under the system principal on a clock rather than a request. Same posture as SweepOlderThan above and bounded the same way: no caller names a row, no id or value comes from a request, and its subject is derived entirely from the database — the unstamped rows, and the attachment rows already joined to them. It returns COUNTS (rows considered, rows slimmed, parts it could not prove, payload bytes freed) and never a row, an id or any message text, so there is nothing for a row gate to withhold. An object gate would refuse the only caller: the system principal holds no object grants, and a human cannot invoke this at all. What it writes is narrower than what a gate would protect — it removes an attachment's octets from raw_capture only after proving the object store holds them, and TestTheSweepSlimsOnlyWhatItCanProve holds that a message whose bytes were never staged keeps them", // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	"internal/modules/knowledge:ReconcileHandbook":            "brings the shipped handbook corpus in line with the pages THIS BINARY carries, reached from nothing but the api boot step, which runs under a system principal holding no object grants at all \u2014 so there is no human grant a gate here could consult, and adding one would refuse the only caller rather than protect anything. Nothing about its subject is caller-chosen: the pages come from the embedded filesystem compiled into the binary, and every row it touches is found by `managed_source = 'handbook'`, so a corpus or document a contact created is unreachable from here by construction \u2014 TestTheReconciliationLeavesAWorkspacesOwnCorpusAlone holds that. The tenant is bound the way every boot write is: bootLedgerScope resolves the installation's workspace and binds the transaction to it, and the boot cannot pick another. It returns a COUNT of pages written, never a row and never any prose",
	...
	"internal/modules/contacts:LookupPlace":                   "the place cache holds no customer data: a place NAME and the coordinates a public geocoder gave for it — 'stuttgart' → 48.77, 9.18. Nothing in it is about a company, a contact or a workspace, and it is installation-global on purpose because a place is a place whoever asks. There is no subject for a grant to be about. The caller that reaches it (the geocode worker) is already gated on company.read for the company whose address it is resolving",
	"internal/modules/contacts:RememberPlace":                 "the write half of the same cache, with the same rationale: what it stores is a public coordinate for a public place name. The lookup that produced it was already authorized as a company read",
	"internal/modules/contacts:LookupTechnical":               "the technical lookup cache holds no customer data: a DOMAIN and what the public internet answered about it — the MX host that receives its mail, the service labels its certificates reveal. Nothing in it is about a contact, and it is installation-global on purpose because a domain's DNS records are the same for every tenant that holds that domain. There is no subject for a grant to be about. The caller that reaches it (the technical lookup worker) is already gated on company.read for the company whose domain it is reading",
	"internal/modules/contacts:RememberTechnical":             "the write half of the same cache, with the same rationale, plus one of its own: what it stores has already been through the classifier, so the raw certificate hostnames — the only part that could ever carry a personal name — never reach it. The lookup that produced it was already authorized as a company read",
	...
	"internal/modules/activities:ClearNotMineOnHandOff":       "the hand-off reconcile (compose/workflows.go, registered as system workflows on the four ownership events). Exempt from OBJECT RBAC because no human's grants can narrow a fact about who owns a record now, and a row-scope gate would be the wrong shape besides: the judgements it ends belong to OTHER readers, and narrowing to the caller's own would clear the one row a hand-off has no business clearing. NOT ungated: onlyTheOwnershipReconcile admits the system principal and refuses everyone else, because an open bulk write here would let any authenticated user put a record's whole thread back on every colleague's Worklist at once, each row indistinguishable from one a hand-off produced (TestAHumanMayNotReArmAColleaguesSetAside). Nothing else is caller-chosen: the record id comes off the event envelope the bus delivered, the owner is read from the record itself rather than supplied, and it returns a COUNT rather than any row",
	...
	"internal/modules/introductions:ExpireDue":                "worker sweep (compose/jobs_introexpiry.go): it records the answer a PASSED DEADLINE already gave — a colleague's silence — which is why the audit row names the clock and not a human. Exempt from OBJECT RBAC because no human's grants can narrow a close no human chose, and a row-scope gate would be the wrong shape besides: an ask is between two colleagues and the sweep is party to neither. NOT ungated: onlyTheExpirySweep admits the system principal presenting ExpiryActor and refuses everyone else, because an open bulk-close would let any authenticated user cancel every open introduction in the installation at once, each one audited as though the clock had done it (TestOnlyTheClockMayExpireIntroductions). Nothing is caller-chosen: it takes no id, its subject is derived entirely from the database's own clock against due_at, and it returns a COUNT rather than any row",
	...
	"internal/modules/capture:StrandedContacts":               "the link-reconcile sweep's selector, run under the system principal with no request and no human actor. It reads contacts and the ledger to find captured contacts nobody was ever asked about; it writes nothing. What confines the WRITE that follows is not RBAC but provenance: the owner it carries is the contact row's own owner_id, set at capture from the authenticated connector principal, and a row without one is skipped rather than assigned to somebody",
	...
	"internal/modules/capture:ListDueCompanies":               "auto-enrich sweep (compose/captureautoenrich.go) under the system principal",
	"internal/modules/capture:MarkQueued":                     "auto-enrich sweep bookkeeping for a company it just queued",
	...
	"internal/modules/contacts:ContactsOwedACohortRepair":     "the nightly repair's selector: it lists contacts whose captured mail is not on their record yet, reads nothing but ids, and is reached only from the link_reconcile sweep under the system principal — there is no human in a sweep for object-RBAC to admit",
	"internal/modules/contacts:RepairContactCohort":           "PromoteContactCohortTx on its own transaction for the sweep that owns no other write; the repair it runs is ratified beside it, and attaching mail the workspace already holds to a record it already has creates nothing and admits nobody",
	"internal/modules/contacts:DomainsOwedTheirContacts":      "the same sweep's other selector: companies whose domain has contacts with no employer. It runs as the system BECAUSE no human may do it — attaching a contact to a company is a write about the CONTACT, and a rep naming a company holds no authority over contacts they cannot see",
	"internal/modules/contacts:AttachDomainBacklog":           "the plant that answers that selector, the same one the domain-triage verdict runs: it never reassigns anybody a human already placed, takes its own row locks, and is reached only from the sweep",
	"internal/modules/contacts:PromoteContactCohortTx":        "the repair that makes a contact's captured cohort independent of the order it arrived in: it attaches mail the workspace already holds to a record it already has, and creates nothing. Reached from the verdict sweep on its transaction and from the contact-event consumer, neither of which has a human principal for object-RBAC to admit; the rows it can touch are bounded by the contact's own live addresses",
	"internal/modules/contacts:SuppressBulkSenderDomainTx":    "the verdict engine's own effect, inside the transaction that concluded the sender is bulk mail; it runs under the system principal with no human actor, exactly like the ClaimDue sweep that reaches it",
	...
	"internal/modules/identity:MintSetupToken":                 "pre-authentication by construction (ADR-0105): it issues the credential a claim is made WITH, and it REFUSES on an installation that holds a company — checked in its own transaction under the installation advisory lock, so the sentence is enforced rather than assumed of its callers. Before that point there are no roles, no users and nobody who could hold a grant. What bounds abuse is the single-outstanding partial index: a second call while one is live is refused rather than issuing a rival credential",
	...
	"internal/modules/identity:MyDelivery":                     "reads the CALLER's own row and nobody else's: the user id comes off the bound principal rather than from a parameter, so there is no other account it could reach and no object for a grant to narrow — the same shape as SaveMyLocale below. It refuses every principal that is not a human seat, agents included, because what lands in a contact's inbox is theirs and an agent acting under their authority must not read it",
	"internal/modules/identity:SaveMyDelivery":                 "writes the CALLER's own row and nobody else's, on the same terms as SaveMyLocale below: the id comes off the principal, no parameter names an account, and the values are confined to the vocabulary the column's CHECK admits. An agent acting under someone's authority is refused, because deciding how often the product may interrupt a contact is that contact's own call",
	...
	"internal/modules/identity:MyWorkingHours":                 "reads the CALLER's own row and nobody else's, on the same terms as MyDelivery above: the user id comes off the bound principal rather than from a parameter, so there is no other account it could reach. It answers when this contact is bookable and whether they chose it, which is a fact about their own week",
	"internal/modules/identity:SaveMyWorkingHours":             "writes the CALLER's own row and nobody else's, on the same terms as SaveMyLocale below: the id comes off the principal and no parameter names an account. An agent acting under someone's authority is refused, because when a contact is bookable is that contact's call — and an installation-wide version of this setting, which an admin WOULD gate, is the shape the design refuses (docs/explanation/scheduling.md)",
	"internal/modules/identity:SaveMyLocale":                   "writes the CALLER's own row and nobody else's: the user id comes off the bound principal rather than from a parameter, so there is no other account it could reach and no object for a grant to narrow — the same shape as ChangePassword above. It refuses every principal that is not a human seat, agents included, because a display language is a preference about a contact's own screen and an agent acting under someone's authority must not change what they read. The value is confined to the languages the product ships a catalog for; nothing else is written, and nothing is read back that the caller did not already have",
	"internal/modules/identity:ClaimInstallation":              "pre-authentication by construction (ADR-0105): it CREATES the first admin, so there is no principal to gate on — gating it would require the grant it is about to mint. The setup token is the authorization, checked inside the same transaction as the create, and an installation that already holds a company is refused before the token is even read",
	"internal/modules/identity:RotateSetupToken":               "operator-only recovery, the same posture as reset-password beside it (ADR-0061 §4): reachable only from cmd/migrate with the OWNER DSN, never over HTTP, because rotating invalidates a live claim credential and that is precisely what an attacker wants while the operator still holds one. Like MintSetupToken it refuses on an installation that holds a company, under the installation advisory lock — before that point there is no principal to gate on, and after it there is nothing left to claim",
	...
	"internal/modules/integrations:ExecuteSubmit":              "worker execution (compose/jobs_providerruns.go) under the system principal: advances a run that QueueRun already object-gated (integrations grant) AND row-gated (EnsureVisible on the contact) when it was queued; the run id comes from the durable job the same transaction committed, no request path reaches it, and the egress lease inside it bounds what the call may touch",
	...
	"internal/modules/contacts:ExhaustedDomains":               "triage sweep (compose/capturedomaintriage.go) under the system principal: reads domains whose crawl attempts are spent so the sweep can settle them rather than strand them; no record, no human actor",
	"internal/modules/contacts:GetMyEmailSignature":            "self-only: reads the CALLER's own email_signature row, keyed on the authenticated principal's user id, and there is no path here to another member's; a signature is the words a contact signs their name with and no seat including admin has a reason to read one",
	"internal/modules/contacts:SaveMyEmailSignature":           "self-only, same row and same key as GetMyEmailSignature — a member writing the sign-off that goes out under their own name, which nobody may write on their behalf",
	"internal/modules/contacts:SignatureFor":                   "self-only, enforced in the method: it refuses any user id that is not the authenticated caller's, so the send path signs with the sender's own sign-off and can ask for no other; the argument exists because the send path holds the sender id explicitly rather than implying it",
	"internal/modules/contacts:GetMyLinkedInAccount":           "self-only: reads the CALLER's own linkedin_account row, keyed on the authenticated principal's user id, and there is no path here to another member's; an object grant would be the wrong question because a member needs no permission to see their own profile",
	"internal/modules/contacts:RetireStaleTriageRead":          "triage sweep bookkeeping under the system principal: finishes a dossier that stopped reporting so its domain can be asked again; touches no record and has no human actor",
	"internal/modules/contacts:ListDueDomains":                 "triage sweep (compose/capturedomaintriage.go) under the system principal: reads domains still owed a company verdict, no record and no human actor — the twin of capture:ListDueCompanies",
	"internal/modules/contacts:MarkTriageQueued":               "triage sweep bookkeeping for a domain it just enqueued a crawl for; arms the retry cursor, touches no record",
	"internal/modules/contacts:MyLinkedInMatchTotals":          "self-only: counts the CALLER's own ghosts, keyed on the authenticated principal's user id, and returns two integers rather than a record; a member needs no permission to be told where their own import stands",
	"internal/modules/contacts:RenormalizeLinkedInCompanyKeys": "worker maintenance under the system principal: rewrites a DERIVED column (normalized_company) and collapses the duplicates an older normalizer left; no human actor exists to gate, and it returns counts rather than any record",
	"internal/modules/contacts:SaveMyLinkedInAccount":          "self-only, same row and same key as GetMyLinkedInAccount — a member editing their own LinkedIn profile URL, which no seat including admin may do on their behalf",
	...
	"internal/modules/identity:InstallationWorkspace":     "singleton-company resolution (ADR-0061), bound by the middleware before any principal exists",
	"internal/modules/identity:BootstrapInstallation":     "boot-time provisioning under the system principal (ADR-0061); no human principal can exist before it",
	...
	"internal/modules/consent:StopForCredentialTx":    "records the stop a withdrawal link presses when its subject holds no per-purpose consent to withdraw — a lead, or an address no record holds. It WRITES, unlike the three resolves beside it, so the ratification has to say what bounds it rather than that it discloses nothing. Three things do. The SUBJECT IS NOT CALLER-CHOSEN: it takes a WithdrawalRef, which only ResolveWithdrawalToken produces, from a credential whose hash the caller had to present — so the rows it can touch are the ones whose link the caller holds. The EFFECT IS BOUNDED to one row of one kind: a marketing objection at MACHINE authority, which any seat may lift, deliberately not the subject level a hand-recorded objection takes, because possession of a mailed link is good evidence of a press and not proof of who made it. That row is as NARROW as the link that pressed it — an all-marketing link writes the broad row, a single-subscription link one bound to that purpose alone — so the effect never exceeds the authority the recipient was handed. And it is IDEMPOTENT: a replay writes nothing. A gate here would refuse the only caller — the anonymous one-click POST, which RFC 8058 requires to work from a mail client with no session — and protect nobody, since the worst a holder achieves is stopping mail to the address their own link names",
	"internal/modules/consent:StopForCredential":      "the same write in its own transaction, for the public handler that has none; bounded exactly as the Tx form above is",

	"internal/modules/consent:ResolveWithdrawalToken": "public withdrawal resolve: possession of the emailed link IS the authority, on the same session-less edge as the two resolves below. Its bounds are DIFFERENT from theirs and deliberately so, because this credential is meant to outlive them: 256-bit crypto/rand, sha256 at rest (the preference token beside it is plaintext, which is tolerable only because that one is short-lived), a 24-month ceiling matching what the mail itself is retained for, and revocation by erasure, merge and the admin compromise route. What makes the long life safe is the NARROWNESS: the ref it returns carries an address and a scope and nothing else, so a holder cannot read a consent state, cannot grant, and cannot reach business correspondence. Unknown, revoked and expired all read as absent, so the surface is not an oracle for which it was",
	...
	"internal/modules/contacts:BeginSiteRead":                "worker-loop status transition (queued→running), not a human principal; the human's authority was checked at StartSiteRead, and what bounds the write is the CAS itself — one dossier id, carried by the admitted job, updated only from the statuses a claim may take",
	"internal/modules/contacts:DeferSiteRead":                "worker-loop scheduling transition (running→deferred); the admitted durable job supplies the retry boundary, and the write reaches one dossier id under the same claim-guarded CAS as BeginSiteRead",
	"internal/modules/contacts:FinishSiteRead":               "worker-loop status transition (running→terminal), not a human principal; the human's authority was checked at StartSiteRead, and the write reaches one dossier id under the same claim-guarded CAS as BeginSiteRead",
	"internal/modules/contacts:UpdateSiteReadProgress":       "worker-loop progress hint on a still-running dossier, same seam as Begin/FinishSiteRead: no human principal, StartSiteRead held the gate, and the write is keyed on the claimed dossier's own id and its live lease",
	"internal/modules/contacts:UpdateSiteReadDraft":          "worker-loop grounded-draft update on a still-running dossier, same seam as progress: admission happened at start, and the versioned write is keyed on the claimed dossier's own id and its live lease",
	"internal/modules/contacts:RecordSiteReadLogo":           "worker-loop object reference parked on an UNBOUND dossier, same seam as UpdateSiteReadDraft: no human principal, StartOnboardingSiteRead held the gate, the write is keyed on the claimed dossier's own id — and it touches no record, because the record it is for does not exist until a confirmation binds it under the company gate",
	"internal/modules/contacts:DiscardSiteReadLogo":          "worker-loop clearing of RecordSiteReadLogo's own parked reference on a dossier that ended without a company; same seam and same admission as the write it undoes, it touches no record, and it names the orphaned object only while no company does",
	...
	"internal/modules/approvals:StageUnlessDeclinedTx":       "StageUnlessDeclined in a transaction the CALLER owns, admitted on exactly the same ground: it is the same body, reached by the same gated mutation, and the transaction is the only difference. The caller that needs it is the transcript reader, whose quotations and the record that it finished are one fact — staging them in transactions of their own is what let an erasure land between the two",
	...
	"internal/modules/deals:SeedPipelineTx":                  "the configured variant of the same boot seed (ADR-0061): deployment-file pipeline, system principal, compose-injected edge",
	...
	"internal/modules/deals:ActiveDealColumns":               "the deal twin of contacts:ActiveContactColumns, on the same reasoning: it answers which cf_* columns the workspace has active and of what type — schema rather than a record — and exists because that catalog read cannot run inside the transaction UpdateDealTx is handed. The write it feeds takes deal:update before any value lands in those columns",
	...
	"internal/modules/contacts:SetChannelIdentityBlocked":    "reachability bookkeeping driven by the telegram ingest worker under the workspace-channel connector principal (compose/telegramingest.go builds that principal onto the context before it classifies the update, so this write has a named actor), never a human caller: the trigger is Telegram's own my_chat_member delivery, which the poller received through a resolved status='connected' connection. It writes no record data — only blocked_at on the one identity the delivery itself names, which is what bounds the write — and the flip is not silent: the audit row and contact.updated event it commits alongside are stamped from that same principal, so a reachability change is traceable to the delivery that caused it",
	"internal/modules/contacts:EnqueueIdentityConflict":      "the sink capture's ensure path hands routeExact's rival pair to (compose/capture.go's raiseIdentityConflict), under the same connector principal with no human in the call: recording that two independently-established keys name two DIFFERENT contacts writes no key onto either of them, so it can neither merge nor disclose — there is nothing for object-RBAC to admit, and the human authority sits on the queue's own disposition, not on recording that a disagreement exists. Reachability is honestly stated: a conflict needs a candidate carrying two different KINDS of exact key, and no shipped ensure path builds one — Telegram supplies a channel identity and nothing else, and the API create's address lane is refused before the ladder reads (contacts/creatededupe.go) — so today only the phone and channel lanes can speak, one at a time, and this entry point waits for a second key kind rather than serving traffic",
	"internal/modules/contacts:EnqueueDomainSplit":           "the company twin of EnqueueIdentityConflict above, admitted for the same reason and with the same reach. Capture's ensure asks the domain lane about a sender's own domain and its base; when those two are registered to two different companies it hands the pair to compose's raiseDomainSplit, under the connector principal with no human in the call. Recording that two domains name two DIFFERENT companies writes no domain onto either of them, so it can neither merge nor disclose — there is nothing for object-RBAC to admit, and the human authority sits on the queue's own disposition. It answers a boolean and never record data",
	"internal/modules/contacts:SignatureCandidates":          "enrich-backlog read driven by the worker sweep under the workspace GUC, no human principal (ADR-0063 §2.9); reads only connector-created rows still missing both fields",
	"internal/modules/contacts:MarkSignatureRead":            "the same sweep's read cursor: it records WHICH mail the model was already shown for a contact, so the pass stops paying for the same empty signature nightly — a bookkeeping row with no record data, written under the workspace GUC with no human principal to admit",
	"internal/modules/contacts:CompanyNameCandidates":        "promotion-backlog read driven by the nightly sweep under the workspace GUC, no human principal (PO-F-2a); reads only provisionally-named companies and the signature evidence naming them",
	...
	"internal/modules/comms:RecordDirectedExecutionTx":    "stamps a delivery with the decision it goes out under, on the caller's transaction, immediately after that decision was checked and spent. The gate that matters already ran and could not be moved here: directing a send needs auth.RequireHuman AND communication_exception:create, and both are consent's to check because consent owns the instruction row — this module cannot see it. What this writes is a marker naming an instruction the same transaction just consumed, and the database refuses a marker naming anything else (the FK) or a second delivery naming one instruction (the unique index). A caller reaching this without an instruction to name writes nothing, because there is no id to write",
	"internal/modules/comms:ClearPayloadRef":              "retires spent one-time link material, called by the dispatcher under the system principal after the relay has taken the message or the delivery reached a terminal state. It only ever NULLs a reference — there is no read, no other column and nothing a caller could name — and gating it on a human would leave live credentials standing whenever a job, rather than a human, closed the send",
	"internal/modules/comms:ExpiredControllerPayloads":    "the read half of ClearPayloadRef's write, and the same posture: the payload sweep runs under the system principal on a cadence, so there is no human seat to check, and it returns delivery IDs only — no address, no body, no material. What it names is one-time link material past its own expiry, which is a fact about the clock rather than about any subject, and gating it on a human would leave live credentials standing exactly when a job rather than a human is the only thing looking",
	"internal/modules/comms:PayloadRefFor":                "reads the vault reference the sweep must destroy BEFORE clearing the column that names it — clearing first and failing would orphan the material with nothing left pointing at it. System principal on the same cadence as ExpiredControllerPayloads, one column by primary key, and the reference is an opaque vault handle rather than the secret: a caller holding it still needs the workspace-scoped vault to read anything",
	...
	"internal/modules/notices:Create":                     "the notice transport's writer for a producer holding no transaction of its own, reached from the automation engine's notify action and the vCard staging review under the system principal — there is no human request to gate, the recipient comes from the producing flow's own decoded configuration, and captured_by records the engine identity. The row's FK refuses a recipient who is not a live seat, and the read side is where a seat's scope binds",
	"internal/modules/notices:CreateTx":                   "the same writer as Create, on a transaction the producing flow already holds so the notice commits with the change it announces — the approval fan-out, which writes it beside the mail job it stages, and the lead-SLA escalation, whose task and notice are one write or neither. Same posture and the same absent human: the recipient comes from the producing flow's own decoded configuration, captured_by records the engine identity, and the row's FK refuses a recipient who is not a live seat. It narrows delivery rather than widening it — the recipient's own notification preference is read first, and a class they switched off writes nothing at all",
	"internal/modules/notices:ClaimEmailAttempt":          "taking the one mail attempt a notice is allowed, under the SYSTEM principal the sending job binds — there is no human request to gate and no seat whose authority could bound it. The notice id is not a caller's to choose: the job that names it was staged in the transaction that wrote the notice, and the claim reads the recipient OUT of the row rather than taking one, so no argument can reach another colleague's line. It narrows delivery rather than widening it — a notice already attempted, already read on screen, or gone yields nothing at all — and what comes back is handed to no reader: it is rendered into a message addressed to the seat the row itself names, whose liveness the sender checks again before sending. captured_by records the sender's own identity",
	"internal/modules/notices:EmailFailed":                "recording why that claimed attempt produced no message, under the same system principal and on the same ground: no human asks for it, the statement is predicated on the claim this sender already took, and it writes one column no seat's authority bounds. It returns no record — the answer is whether the claimed row was still there — so there is nothing for a row scope to withhold",
	"internal/modules/notices:DigestCandidates":           "the morning digest's enumeration, under the SYSTEM principal the hourly pass binds — there is no human request to gate and no seat whose authority could bound a question about who the installation owes a message to. It reads no record value: the claim table, the routing choices, and which KINDS of notice are waiting, which is routing metadata rather than anything a record says. It takes no caller input at all beyond the local day, so no argument can widen it, and what it answers is a list of recipient ids the pass narrows against identity's own seat roster and then serves one by one — each under that recipient's own principal",
	"internal/modules/notices:DigestStillDue":             "the same enumeration re-asked of ONE recipient in the instant before their claim is spent, on the same ground and under the same system principal: no human asks for it, and the recipient is not a caller's to choose — it is an id this pass just read out of the sweep above. It answers a single boolean, never a record, and it only ever NARROWS: a colleague who moved the class off the batch, or who has answered everything on screen since, is skipped",
	"internal/modules/notices:ClaimDigestRun":             "taking the one morning a seat is allowed per local day, under the system principal the pass binds. There is no human request to gate: an hourly tick is not somebody's act, and the row it writes carries no record data — a recipient and a date. It cannot reach a colleague this pass did not already enumerate, it writes nothing a seat's authority could bound, and it returns no record at all, only whether this tick won the day. captured_by records the sender's own identity",
	"internal/modules/notices:DigestFailed":               "recording why that claimed morning produced no message, under the same system principal and on the same ground: no human asks for it, the statement is predicated on the claim this pass already took, and it writes one column — an operator-facing transport diagnosis, not tenant data. It returns no record; the answer is whether the claimed row was still there, so there is nothing for a row scope to withhold",
	"internal/modules/notices:DigestBody":                 "the CONTENT read, and the one entry point here that is NOT a system act: it refuses any principal but the recipient's own. The pass binds that seat's effective authority — grants, teams and seat type in one snapshot — before calling, and the store compares the bound human against the recipient parameter and answers the permission sentinel on any mismatch, so a digest cannot be read against the wrong colleague's scope by any argument. recipient_user_id = that same bound seat bounds the rows. It takes no object grant because a notice is not a record type a role grants: it is a line addressed to one contact, and holding that seat is the whole authority for reading what reached you. What the subjects may NAME is a further question the sending lane asks of auth.VisibleSubset under this same principal at render time — the stored reference is re-scoped rather than trusted",
	"internal/modules/notices:ListFor":                    "the personal notification centre read — the same read UnreadFor is, over the reader's whole history rather than the unread lane. It writes nothing; recipient_user_id = the authenticated caller, never a parameter, so another colleague's history cannot be expressed, and the same self-made stage-change filter only narrows that set. A caller with no contact behind it — an agent carrying its grantor's id included — is refused with the permission sentinel before any query, because reading what reached a seat is that human's own act",
	"internal/modules/notices:MarkAllRead":                "the recipient settling every notice they hold: one UPDATE scoped recipient_user_id = the authenticated caller, never a parameter, so no colleague's row is reachable by any argument and a stranger's lane is not merely hidden but unaddressable. A caller with no contact behind it gets the permission sentinel before any statement, the same rule MarkRead applies to settling one",
	"internal/modules/notices:UnreadFor":                  "the personal notices read. It writes nothing; recipient_user_id = the authenticated caller, never a parameter, bounds access, and the self-made stage-change filter only narrows that set — and a caller with no human behind it is refused with the permission sentinel before any query, which the Worklist renders as a withheld lane",
	"internal/modules/notices:MarkRead":                   "the recipient settling their own notice: the statement is scoped recipient_user_id = the authenticated caller, so another colleague's notice reads as absent (404) and existence stays hidden; a caller with no contact behind it gets the permission sentinel before any query",
	"internal/modules/notices:MyNotificationPreferences":  "the seat's own notification settings read. user_id = the authenticated caller, never a parameter, so another colleague's settings cannot be expressed at all; a caller with no human behind it — an agent carrying its grantor's id included — is refused with the permission sentinel before any query, because an agent acting under a grantor's authority must not read what reaches that grantor",
	"internal/modules/notices:SaveNotificationPreference": "the seat deciding how one class of notification reaches THEM. user_id = the authenticated caller, never a parameter; the same permission sentinel precedes any query for a caller with no contact behind it. It takes no object grant because there is no object: holding a seat is the whole authority needed to decide how often the product may interrupt you, the way their display language and their delivery settings are theirs",
...
// (ADR-0090), and the `setting` table carries no RLS beneath it — so the
...
var entryPointsOutsideModules = gatekit.Waive(map[string]string{
	...
	"internal/compose/company360/assemble.go":        "company360.Service.Assemble — a compose read service assembling the record-360 view across domain tables; this package does reference the platform auth gate (auth.Require, EnsureVisible, the scope clauses), but whether the gate's transitive resolution proves that for each entry point is a judgement this change has not made",
	"internal/compose/company360/graph.go":           "company360.Service.Graph — the same read service's relationship graph, in a package that reaches auth through its section helpers; enrolling compose in this gate is a separate decision from proving where the entry points are",
	"internal/compose/company360/coverage.go":        "company360.Service.Coverage — the account's coverage reading, ratified on the same terms as its siblings: it opens with the contacts store's own GetCompanyTx (auth.Require + EnsureVisible), the roster it folds carries the contact row scope as a predicate, and the deals it lists go through scopeClause; enrolling compose in this gate is the separate decision the other company360 entry points are ratified under",
	"internal/compose/company360/contactlist.go":     "company360.Service.ContactPage — the account's contact list, the paging surface behind the 360's contacts section; it opens with the same contacts-store GetCompanyTx (auth.Require + EnsureVisible) that Graph and Assemble open with, and the roster read it pages carries the contact row scope as a predicate, but enrolling compose in this gate is the same separate decision its siblings are ratified under",
	"internal/compose/company360/introdraft.go":      "company360.Service.IntroRequestDraft — the ask a rep sends a colleague. It opens with auth.RequireHuman, then the contacts store's own GetCompanyTx (auth.Require + EnsureVisible) refuses an account this caller cannot open; the contact is read through contactIdentity under the contact row scope, and the route through mayReadRoutes + contactRoutes, so a caller without activity:read learns nothing about who can reach whom. It WRITES nothing at all — no draft row, no activity, no audit entry beyond the model call's own — which is why there is no mutation for this gate to be the last line in front of. Ratified here only as a subject the roots do not cover",
	"internal/compose/company360/roleproposals.go":   "company360.Service.ProposeRoles — the buying-role reading, and the one WRITE in this package's ratified set. It opens with auth.RequireHuman, then asks relationship.create and deal.update as OBJECT grants, then auth.EnsureWritableLive on the deal itself — that last one matters because the seats are written under a substituted system principal, which is unbounded, so the caller's row-level write authority has to be established while they are still the actor. Every read that feeds the prompt is the coverage card's own (contacts.StrengthForCompanyContacts, contactIdentity, deals.Stakeholders) and the message bodies carry auth.ActivityContentClause. The seats themselves go through contacts.Store.CreateRelationshipTx, which this gate DOES judge directly; enrolling compose in this gate is the separate decision its siblings are ratified under",
	"internal/compose/company360/dismissal.go":       "company360.Service.DismissSuggestion — the suggestion-dismissal write; it opens with auth.RequireHuman and auth.Require, so it is not a suspected gap, but this gate has not been the thing that checked it",
	"internal/compose/company360/viewbaseline.go":    "company360.Service.Acknowledge — the record-view acknowledgement write, likewise opening with auth.RequireHuman and auth.Require; ratified here only as a subject the roots do not cover",
	"internal/compose/project360/assemble.go":        "project360.Service.Assemble — the project record-360 read, the company and contact views' sibling and ratified on the same terms as contact360/assemble.go: its mandatory anchor read opens with the deals store's own auth.Require + EnsureVisible (GetProjectTx) and every section is a module store's gated transaction-taking read, but whether this gate's transitive resolution proves that per entry point is the judgement enrolling compose would have to make",
	"internal/compose/contact360/assemble.go":        "contact360.Service.Assemble — the contact record-360 read, the company view's sibling and ratified on the same terms as company360/assemble.go: its mandatory root read opens with the contacts store's own auth.Require + EnsureVisible and every section carries its object grant, but whether this gate's transitive resolution proves that per entry point is the judgement enrolling compose would have to make",
	"internal/compose/contact360/handlers.go":        "contact360.Handlers.AcknowledgeContactView — the contact view's visit acknowledgement, the twin of company360/viewbaseline.go and likewise opening with auth.RequireHuman; ratified here only as a subject the roots do not cover",
	"internal/compose/contact360/momentdismissal.go": "contact360.Service.DismissMoment — the contact page's moment dismissal, the twin of company360/dismissal.go: it opens with auth.RequireHuman, auth.Require and auth.EnsureVisibleLive, so it is not a suspected gap, but this gate has not been the thing that checked it",
	...
	"internal/compose/magic/service.go":              "magic.Service.Read — the machinery's receipt. It reads and writes nothing, and it is NOT ungated: every audit row it serves is placed by joining the table its entity_type names, under that table's own gate, because audit_log carries no workspace_id (migration 1787320004 dropped the tenant column from the append-only ledgers) and so cannot be scoped by itself. done.go asks auth.Require for the entity's object grant AND renders auth.ScopeClauseFor's row predicate for the five owner-scoped records; activities go through auth.Require plus auth.ActivityContentClause, whose audience arm a row-scope clause would miss. Both halves are needed and neither implies the other: auth.UnboundedFor answers true for a rep on `deal`, so the clause renders EMPTY and a read leaning on it alone would serve every row to a seat holding no grant — the trap of taking a scope clause for a gate, held by TestASeatThatCannotReadDealsSeesNoMachineActionOnOne. An entity type this build cannot place is counted in not_shown rather than served, because a row it cannot scope is a row it cannot prove the reader may see",
	...
	"internal/compose/attention/handled.go":          "attention.Service.HandledForYou — the reader's own receipt of what was done for them. It opens no transaction and reads no table: every row comes from the Receipts seam, whose implementation is the approvals module's own gated read, and this file only bounds and shapes what that returned. So there is no record it could be the last line in front of. The admission is auth.RequireHuman plus that seam's own scope — a receipt is the acting reader's, and a principal with no human behind it has nobody the acts were taken for. Ratified here only as a subject the roots do not cover", "internal/compose/meetingbrief/service.go": "meetingbrief.Service.Get — the pre-meeting brief read. UNLIKE its contactbrief sibling it carries no auth.RequireHuman: agents read it through prep_for_meeting, so that one tool and the contact page cannot answer the same question differently. What admits the read instead is auth.Require on activity and contact, then auth.EnsureActivityContentVisibleLive on the meeting itself, and every record it is written from arrives through the caller's own gated 360 and the contacts store's own claim read, so the row-scope gates run in those reads rather than here. An agent is capped the same way a contact is: a passport's authority is re-derived from the granting human's live seat on every call, never read off the principal. It also writes nothing — there is no cache — so there is no mutation for this gate to be the last line in front of",
	"internal/compose/companydossier/cachedread.go":       "companydossier.Service.CachedSections — the cache-READ half of the company dossier, for a drafter that must never trigger an assembly. It opens with auth.RequireHuman and resolves the acting user, and the cache it reads is keyed per reader, so a row assembled for somebody else is not reachable through it; the dossier itself was assembled through the caller's own gated reads. Ratified here only as a subject the roots do not cover",
	"internal/compose/contactbrief/service.go":            "contactbrief.Service.Get — the relationship brief read, ratified on the same terms as its companybrief sibling: it opens with auth.RequireHuman and every record it is written from arrives through the caller's own gated 360, so the row-scope gates run in that read rather than here",
	"internal/compose/contactresearch/service.go":         "contactresearch.Service.Run and .Save — the deep-research surface. Run opens with auth.RequireHuman and reads only through the caller's own gated contact360, so it can research nobody they cannot open; Save carries its write into contacts.Store.SaveResearchClaims, which this gate DOES judge directly. Ratified here only as a subject the roots do not cover",
	"internal/compose/companydossier/service.go":          "companydossier.Service.Get — the company dossier read, opening with auth.RequireHuman and assembling only from reads the caller makes themselves, so the row-scope gates run in the contacts store this calls rather than here",
	"internal/compose/companydossier/growthfitservice.go": "companydossier.GrowthFitService.Get — the growth-fit read, ratified on the same terms as its dossier sibling: it opens with auth.RequireHuman and every record it counts arrives through a read the caller makes themselves, so the row-scope gates run in the contacts store rather than here",
	"internal/compose/companybrief/service.go":            "companybrief.Service.Get and .Ask — the company brief read and its question surface, both opening with auth.RequireHuman; whether RequireHuman alone is the right admission for them is a question for the tier's own review, not for this sweep",
	"internal/compose/companyscan/service.go":             "companyscan.Service.Get and .Ensure — the account scan, ratified on companybrief's terms: both open with auth.RequireHuman and resolve the acting user, the row they read is keyed per reader behind auth.EnsureVisible on the account, and everything the scan is written from arrives through the caller's own gated 360 and the content-gated words read, so the row-scope gates run in those reads rather than here. Run is the worker's entry and binds the reader's own principal before any read",
	"internal/compose/company360/advice.go":               "company360.Service.UndismissedAdvice and .KeepUndismissed — the advice seam the account scan merges with. Both resolve the acting user, both run behind auth.EnsureVisible on the account, and the rules they run are the same grant-gated reads the composite's own suggestions section runs, so the object and row gates are those reads' rather than a second spelling here",
	"internal/compose/contactdraft/service.go":            "contactdraft.Service.Draft — the contact-side email draft, ratified on the same terms as its accountdraft mirror: it opens with auth.RequireHuman and every record it grounds in arrives through the caller's own gated contact 360, so the row-scope gates run in that read rather than here. It also writes nothing, so there is no mutation for this gate to be the last line in front of",
	"internal/compose/leaddraft/service.go":               "leaddraft.Service.Draft — the lead-side email draft, ratified on the same terms as its contactdraft mirror: it opens with auth.RequireHuman, the lead arrives through the contacts store's own gated GetLead and the correspondence through the activities store's own list, so the row-scope gates run in those reads rather than here. It also writes nothing, so there is no mutation for this gate to be the last line in front of",
	"internal/compose/accountdraft/service.go":            "accountdraft.Service.Draft — the account-started email draft, ratified on the same terms as its companybrief sibling: it opens with auth.RequireHuman and every record it grounds in arrives through the caller's own gated 360, so the row-scope gates run in that read rather than here. It also writes nothing, so there is no mutation for this gate to be the last line in front of",
