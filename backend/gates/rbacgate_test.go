...
var ungatedEntryPoints = gatekit.Waive(map[string]string{
	...
	"internal/modules/identity:MintSetupToken":               "pre-authentication by construction (ADR-0105): it issues the credential a claim is made WITH, and it REFUSES on an installation that holds a workspace — checked in its own transaction under the installation advisory lock, so the sentence is enforced rather than assumed of its callers. Before that point there are no roles, no users and nobody who could hold a grant. What bounds abuse is the single-outstanding partial index: a second call while one is live is refused rather than issuing a rival credential",
	...
	"internal/modules/identity:InstallationWorkspace":     "singleton-workspace resolution (ADR-0061), bound by the middleware before any principal exists",
