...
var ungatedEntryPoints = gatekit.Waive(map[string]string{
	...
	"internal/modules/activities:MarkCaptureLabelDeclined":   "the classify sweep's own bookkeeping, driven by the worker under the workspace GUC with no human principal, same posture as SetCaptureLabel: it stamps one timestamp saying every rung declined to label the message, which removes it from the classify backlog and states no verdict, discloses nothing and touches no other column",
