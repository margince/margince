...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/integrations:BackfillSweep":            "worker sweep (compose/jobs_providerlookup.go) under the system principal: the catch-up pass that queues a free lookup for contacts no run has covered. There is no human on it and no record returned — the runs it queues are each gated by QueueRun's own person:update check, which it reaches through admit and queueOne like every other queuer. What bounds it is the installation's automatic_lookup posture, read at the top of the pass, and its per-tick budget",
