...
var ungatedEntryPoints = gatekit.Waive(map[string]string{
	...
	"internal/modules/introductions:ExpireDue":                "worker sweep (compose/jobs_introexpiry.go): it records the answer a PASSED DEADLINE already gave — a colleague's silence — which is why the audit row names the clock and not a contact. Exempt from OBJECT RBAC because no human's grants can narrow a close no human chose, and a row-scope gate would be the wrong shape besides: an ask is between two colleagues and the sweep is party to neither. NOT ungated: onlyTheExpirySweep admits the system principal presenting ExpiryActor and refuses everyone else, because an open bulk-close would let any authenticated user cancel every open introduction in the installation at once, each one audited as though the clock had done it (TestOnlyTheClockMayExpireIntroductions). Nothing is caller-chosen: it takes no id, its subject is derived entirely from the database's own clock against due_at, and it returns a COUNT rather than any row",
	...
	"internal/modules/notices:UnreadFor":               "the personal notices read. It writes nothing, its one predicate is the attribution itself — recipient_user_id = the authenticated caller, never a parameter — and a caller with no contact behind it is refused with the permission sentinel before any query, which the Worklist renders as a withheld lane",
	"internal/modules/notices:MarkRead":                "the recipient settling their own notice: the statement is scoped recipient_user_id = the authenticated caller, so another contact's notice reads as absent (404) and existence stays hidden; a caller with no contact behind it gets the permission sentinel before any query",
