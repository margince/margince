...
var ungatedEntryPoints = gatekit.Waive(map[string]string{
	...
	"internal/modules/people:RememberPlace":                   "the write half of the same cache, with the same rationale: what it stores is a public coordinate for a public place name. The lookup that produced it was already authorized as a company read",
	...
	"internal/modules/people:RememberTechnical":               "the write half of the same cache, with the same rationale, plus one of its own: what it stores has already been through the classifier, so the raw certificate hostnames — the only part that could ever carry a personal name — never reach it. The lookup that produced it was already authorized as a company read",
	...
	"internal/modules/people:ListDueDomains":                 "triage sweep (compose/capturedomaintriage.go) under the system principal: reads domains still owed a company verdict, no record and no human actor — the twin of capture:ListDueCompanies",
	...
	"internal/modules/people:DiscardSiteReadLogo":            "worker-loop clearing of RecordSiteReadLogo's own parked reference on a dossier that ended without a company; same seam and same admission as the write it undoes, it touches no record, and it names the orphaned object only while no company does",
	...
	"internal/modules/people:CompanyNameCandidates":          "promotion-backlog read driven by the nightly sweep under the workspace GUC, no human principal (PO-F-2a); reads only provisionally-named companies and the signature evidence naming them",
...
var entryPointsOutsideModules = gatekit.Waive(map[string]string{
	...
	"internal/compose/company360/coverage.go":       "company360.Service.Coverage — the account's coverage reading, ratified on the same terms as its siblings: it opens with the people store's own GetCompanyTx (auth.Require + EnsureVisible), the roster it folds carries the person row scope as a predicate, and the deals it lists go through scopeClause; enrolling compose in this gate is the separate decision the other company360 entry points are ratified under",
	"internal/compose/company360/contactlist.go":    "company360.Service.ContactPage — the account's contact list, the paging surface behind the 360's people section; it opens with the same people-store GetCompanyTx (auth.Require + EnsureVisible) that Graph and Assemble open with, and the roster read it pages carries the person row scope as a predicate, but enrolling compose in this gate is the same separate decision its siblings are ratified under",
	"internal/compose/company360/introdraft.go":     "company360.Service.IntroRequestDraft — the ask a rep sends a colleague. It opens with auth.RequireHuman, then the people store's own GetCompanyTx (auth.Require + EnsureVisible) refuses an account this caller cannot open; the contact is read through contactIdentity under the person row scope, and the route through mayReadRoutes + contactRoutes, so a caller without activity:read learns nothing about who can reach whom. It WRITES nothing at all — no draft row, no activity, no audit entry beyond the model call's own — which is why there is no mutation for this gate to be the last line in front of. Ratified here only as a subject the roots do not cover",
	"internal/compose/company360/roleproposals.go":  "company360.Service.ProposeRoles — the buying-role reading, and the one WRITE in this package's ratified set. It opens with auth.RequireHuman, then asks relationship.create and deal.update as OBJECT grants, then auth.EnsureWritableLive on the deal itself — that last one matters because the seats are written under a substituted system principal, which is unbounded, so the caller's row-level write authority has to be established while they are still the actor. Every read that feeds the prompt is the coverage card's own (people.StrengthForCompanyContacts, contactIdentity, deals.Stakeholders) and the message bodies carry auth.ActivityContentClause. The seats themselves go through people.Store.CreateRelationshipTx, which this gate DOES judge directly; enrolling compose in this gate is the separate decision its siblings are ratified under",
	...
	"internal/compose/person360/assemble.go":        "person360.Service.Assemble — the person record-360 read, the company view's sibling and ratified on the same terms as company360/assemble.go: its mandatory root read opens with the people store's own auth.Require + EnsureVisible and every section carries its object grant, but whether this gate's transitive resolution proves that per entry point is the judgement enrolling compose would have to make",
	"internal/compose/person360/handlers.go":        "person360.Handlers.AcknowledgePersonView — the person view's visit acknowledgement, the twin of company360/viewbaseline.go and likewise opening with auth.RequireHuman; ratified here only as a subject the roots do not cover",
	"internal/compose/person360/momentdismissal.go": "person360.Service.DismissMoment — the person page's moment dismissal, the twin of company360/dismissal.go: it opens with auth.RequireHuman, auth.Require and auth.EnsureVisibleLive, so it is not a suspected gap, but this gate has not been the thing that checked it",
	...
	"internal/compose/companydossier/service.go":          "companydossier.Service.Get — the company dossier read, opening with auth.RequireHuman and assembling only from reads the caller makes themselves, so the row-scope gates run in the people store this calls rather than here",
