...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/capture:NoiseJudgedStandsTx":          "the re-read the retraction sweep takes on its OWN transaction, and the reason the sweep is safe rather than a gap in it: the scan and the archive are separate transactions, so a keep_out withdrawn or a verdict corrected in between must cost the contact nothing. Run under the same system principal as NoiseJudgedContacts above, with no request and no human actor \u2014 there is no human grant for a gate to consult and one would refuse the only caller. It WRITES NOTHING and returns a bool: not a row, not an address, not anything about the person. Its subject is not caller-chosen either, being the address and owner the scan read off capture's own tables a moment earlier. The waivers on both sides of it already name it as the mechanism that confines them; this is its own record of the same fact",
