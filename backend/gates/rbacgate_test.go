...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/comms:HardBouncesFor":  "the personal bounce-lane read. It writes nothing, its one predicate is the attribution itself — user_id = the authenticated caller, never a parameter — and a caller with no person behind it is refused with the permission sentinel before any query (TestHardBouncesForRefusesACallerWithNoPersonBehindIt); the joined activity_link row only names the person a send this caller already owns is filed under",
