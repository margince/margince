...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/people:RetractCaptureOnlyPersonTx": "withdraws a contact CAPTURE created, once the confidentiality classifier judged the conversation it came from to be the mailbox owner's private life. Reached only from the confidentiality verdict engine's apply, inside the same transaction as the verdict, under a system principal holding no object grants — so there is no human grant a gate here could consult, and adding one would refuse the only caller. Nothing about its subject is caller-chosen in the sense a gate protects: capture answers WHICH records a private thread orphaned from its own tables, and this refuses every record that is not narrowly its own to withdraw — a human-created one, one a human has edited, one already promoted to the workspace, or one belonging to another seat. It archives, which is reversible and audited: archivePersonRows lands the write shape, so a retraction leaves the same audit row a human's archive would. It returns a bool, never a row and never any of the person's data",
	...
	"internal/modules/capture:RecordOutcomeOnThreadTx":        "the same verdict write as RecordOutcomeTx, applied to the messages of the thread this seat had ALREADY imported when the answer came back. Same transaction, same claim, same absence of a principal — and the same confinement: user_id comes from the ledger row and the stamp is scoped to it, so one seat's answer never reaches a colleague's contribution. What bounds WHICH messages it may open is not RBAC but the admission rule it shares with inheritedVerdictTx — a sibling takes an opening verdict only from a sender the verdict actually read",
	...
	"internal/modules/identity:RevokePassportTx":             "reads the passport row and refuses one whose on_behalf_of is not the caller (ErrNotFound, existence-hiding) unless they hold admin — the same ownership check every other revoke path runs, inside the caller's transaction",
