...
var ungatedEntryPoints = gatekit.Waive(map[string]string{
	...
	"internal/modules/approvals:AutonomyChoiceFor":    "AutoApplyMode with one more field, ungated for the identical reason and on the identical subject: it takes no user id, reads the row of the principal on the context, and there is no row a caller can ask for but not be. What it adds is whether that row EXISTS — a caller whose kind does not default to manual cannot otherwise tell a rep who asked to be asked from one who has never been asked, and reading silence as refusal would leave every deal in the pipeline on a date nobody maintains. Still one enum value and one boolean about the caller's own preference: no record, no count, nothing about anybody else. A principal with no contact behind it is refused outright rather than reading a zero-uuid row",
