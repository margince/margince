...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/comms:HardBouncesFor":  "the personal bounce-lane read. It writes nothing, its send predicate is the attribution itself — user_id = the authenticated caller, never a parameter — and a caller with no person behind it is refused with the permission sentinel before any query (TestHardBouncesForRefusesACallerWithNoPersonBehindIt). The joined activity_link row is bounded by auth.LinkTargetVisibleClause, the same clause every link projection asks, so a person this caller may not read never reaches the wire even as a bare id — owning the send licenses nothing about who its activity touches",
