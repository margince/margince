...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/capture:NoiseJudgedStandsTx":          "the retraction's own recheck, and the reason the selector beside it is ratified: it re-asks, on the transaction that is about to archive, the very predicate the sweep selected on, so a keep_out withdrawn or a verdict corrected between the scan and the write still saves the contact. Reached only from the link-reconcile retraction sweep, under the system principal with no human actor — there is no grant to consult, and a gate would refuse the only caller. It takes an address and an owner id the sweep read from its own selection rather than anything a caller chose, returns a bool, and never a row: the person's data does not pass through it. The write it guards is people.RetractCaptureOnlyPersonTx, which re-checks the full eligibility under the person row's lock",
