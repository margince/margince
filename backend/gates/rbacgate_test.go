...
var entryPointsOutsideModules = gatekit.Waive(map[string]string{
	...
	"internal/compose/dealstatus/cards.go":          "dealstatus.Service.CachedCards — the same card's VERDICT, read for a page of deals at once so the worklist can carry a standing it does not decide. It stands exactly where CachedMoves beside it stands: it READS ONLY THE CACHE and assembles nothing, so it makes no record read this gate could be the last line in front of, and what it returns was written by Service.Get under that caller's own gated reads. The admission is the cache key itself — the WHERE names user_id, and the workspace binding is WithWorkspaceTx's — so a reader reaches only cards written from records they were already admitted to, and a deal they have no card for is simply absent. It differs from CachedMoves in one way that REDUCES its reach rather than widening it: a move names an activity id, so that one re-asks the audience on every read; a standing carries only the sentence and the verdict word, names no record, and therefore has no id to serve past a gate. It writes nothing at all",
