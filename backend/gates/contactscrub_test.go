...
var clearedOnlyByTheEraser = gatekit.Waive(map[string]string{
	...
	"activity_identity":           "an external identity is retired with the content it names and never before it. The anonymize redacts no timeline, so the message is still there and still answers to its Message-ID — which is the right answer, because retiring the key while the message stands would send the next arrival of that same message to a second row beside the first. The eraser retires only the identities of rows it actually emptied, and leaves the ones the statutory floor shielded standing for that same reason.",
