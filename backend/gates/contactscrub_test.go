...
// Erasing a contact and anonymizing one are the same act with one difference:
...
const (
	...
	eraseRoot      = "Eraser.EraseContact"
	anonymizeRoot  = "anonymizeContactRecord"
)
...
var clearedOnlyByTheEraser = gatekit.Waive(map[string]string{
	...
	"activity_identity":           "the external identities a message answered to, keyed by activity id. The same absence the `activity` entry above records, one table further out: the anonymize never reaches an activity at all — it strips the contact row and the satellites naming the subject — so it meets no identity row either. An act that never asked the question, rather than a subject's data surviving one that did. If the anonymize is ever given a timeline pass, this is one of the tables it has to reach.",
	"activity_mail_reference":     "the Message-IDs a message said it replied to, keyed by activity id and retired beside activity_identity. The same absence the `activity_identity` entry above records: the anonymize never reaches an activity, so it meets no reply link either. If the anonymize is ever given a timeline pass, this is one of the tables it has to reach.",
	...
	"lead":                        "the lead row is anonymized by the eraser's own lead pass; the contact/anonymize action does not reach it, so a subject who exists as both a contact and a lead is anonymized as one and not the other. That split is the finding, not a decision.",
...
func TestErasingAndAnonymizingClearTheSameTables(t *testing.T) {
	...
	if len(eraserOnly) > 0 {
		t.Errorf("%d table(s) are cleared when a contact is ERASED and not when they are ANONYMIZED.\n\n"+
	...
	if len(anonymizeOnly) > 0 {
		t.Errorf("%d table(s) are cleared when a contact is ANONYMIZED and not when they are ERASED.\n\n"+
