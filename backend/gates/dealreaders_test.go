...
var predicateDealReads = gatekit.Waive(map[string]string{
	...
	"internal/compose/network/accountcoverage.go:accountStakeholderEdge":      "the stakeholder-edge predicate inside the account coverage read: whether a contact holds a seat on a live deal at this account. The deal appears only inside the join condition, and what the coverage answers is about CONTACTS, under the grants that read owns",
...
var calleeGatedDealReads = gatekit.Waive(map[string]string{
	...
	"internal/compose/network/coveragefacts.go:readDealFacts":              "the deal facts an account's coverage rests on. AccountCoverageFor asks the deal object gate before the facts are read",
