...
var audienceReadersAdmitted = gatekit.Waive(map[string]string{
	"internal/compose/rawcapturelinkbackfill.go:selectStoredOriginalLinks":           "the pass that names the original behind every activity captured before the reference column existed, as the system principal. It reads an activity's id and natural key and no content of any kind, and what survives the transaction is raw_capture_id. It composes no audience because the audience decides who may READ a message, and this pass shows nobody anything: a limited message left unnamed is one whose stored original no purge and no retention sweep can reach, so gating it here would keep that payload rather than protect it. The cost is nil; no column it reads reaches a caller",
	"internal/compose/rawcapturelinkbackfill.go:backfillRawCaptureLinksBatch":        "the same pass's count of the originals nothing names yet. The activity table enters it as a NOT EXISTS on the reference and nothing else — no column projected, no text read — and the whole output is one integer for an operator's log",
