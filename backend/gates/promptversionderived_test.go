...
var uncachedPrompts = gatekit.Waive(map[string]string{
	"internal/compose:classifySystem":           "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:companyReadMessageSystem": "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:confidentialitySystem":    "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:corpusAskSystem":          "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:documentSystem":           "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:extractSystem":            "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:firstDraftSystem":         "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:fxExtractSystem":          "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:judgeSystemPrompt":        "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:offerDraftSystem":         "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:rateExtractSystem":        "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:replyDraftSystem":         "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:settleSystem":             "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:signatureEnrichSystem":    "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:stageEvidenceSystem":      "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:transcriptSystem":         "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:triageSystem":             "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:verdictSystem":            "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:voiceEvalDraftSystem":     "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
	"internal/compose:voiceEvalJudgeSystem":     "compose is one large package holding every one-shot site's prompt, and owed_verdict's stamp made it a deriving package. This prompt caches nothing keyed on a digest: its site answers one call and writes the answer to a column or a draft, so there is no stored output a reworded prompt could leave stale. The six surfaces that DO derive a version each live in their own directory with one prompt in it; owed_verdict's stamp is not a cache key either, it is the ruleset recorded on activity.owed_verdict_ruleset so a prompt change can re-judge what older rules decided.",
