...
// output is cached.
//
// Empty today, and the way it got there is worth the line. It held twenty
// entries, one per prompt in internal/compose, every one carrying the same
// paragraph — because a single digest call added there for owed_verdict's
// ruleset stamp made twenty unrelated one-shot prompts this census's business.
// That waived a package boundary rather than a false positive, and the next
// prompt added anywhere in compose would have wanted a twenty-first copy.
//
// The answer was to give the digesting surface its own package
// (internal/compose/owedverdict), which is what the six older ones already do.
// Scoping this census by FILE was the other candidate and is NOT safe: a
// package's `promptVersion` is package-scoped, so a prompt declared in a
// sibling file can be keyed by it — companybrief's askSystem is exactly that,
// and file scoping would have stopped asking about it without saying so.
