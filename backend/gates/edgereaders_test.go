...
var predicateEdgeReads = gatekit.Waive(map[string]string{
	...
	"internal/modules/people/demote.go":                                        "the guard that refuses demoting a company still carrying live edges — an existence test whose answer is the refusal itself, on a write the caller is already gated for. The cost is that a refused demote confirms edges exist, which the write grant already implies",
...
var lifecycleEdgeReads = gatekit.Waive(map[string]string{
	...
	"internal/modules/people/merge_company.go":                             "the company merge, same shape and same write grant. The cost is the same",
	...
	"internal/modules/people/enrichment.go":                                "the enrichment writer reads the employer edge to decide which company a fetched fact belongs to, on a path that runs as a system principal from the enrichment job. The cost is that enrichment resolves an employer with no per-caller gate",
	...
	"internal/modules/people/companynamepromotion.go":                      "company-name promotion reads the employment edges it is about to re-point when a captured company name is promoted to a record — a write path running as a system principal. The cost is the same as enrichment's",
