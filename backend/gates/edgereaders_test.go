...
var lifecycleEdgeReads = gatekit.Waive(map[string]string{
	...
	"internal/modules/contacts/mergecompanyedges.go:relinkCompanyEdges":            "the company merge's extracted edge writer, under the same company merge grant and transaction; all loser edges must move together rather than depending on the caller's list scope",
	"internal/modules/contacts/relationshippatchsql.go:demoteForRelationshipPatch": "the guarded relationship patch clears competing primary flags under the shared employment lock, after the patch's anchor-write check; the index invariant must include all of the contact's employments",
