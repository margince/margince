...
var restrictedReadersAdmitted = gatekit.Waive(map[string]string{
	...
	"internal/modules/privacy/erasure_graph.go:subjectNamedOnAParticipantRow": "the Art. 17 participant scrub is a WRITER, and the one thing it reads from activity is channel_provider — a registry key naming a transport, never a subject's content, never projected anywhere. It reads it because a chat roster names the third human in a group by an account id alone, and an account id is only that human's against the provider that issued it; matching one across every transport is the untyped over-deletion this module refuses elsewhere. Excluding a held activity here would do the opposite of what the hold protects: it would leave the erased subject's account standing on that roster row forever, readable and matchable back to them by the next roster naming it, while every other arm of the same statement erased them. The cost is that the transport of a held activity decides whether a participant row on it is scrubbed — a fact about the row's own erasability, and the direction of the effect is always toward removing the subject",
