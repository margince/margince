...
var restrictedReadersAdmitted = gatekit.Waive(map[string]string{
	"internal/compose/rawcapturelinkbackfill.go:selectStoredOriginalLinks":     "the one-off pass that names the original behind every activity captured before activity.raw_capture_id existed, as the system principal. Three columns reach it — id, source_system, source_id — and none survives the transaction: what it writes is the reference itself. Excluding a held row would leave exactly that row unnamed, and an unnamed original is one the purge and the retention sweep cannot reach: the hold would end up PRESERVING a held conversation's verbatim provider payload past the erasure of the message it holds, which is the opposite of what the hold protects. The cost is that a held activity's natural key is matched against raw_capture by a system pass",
	"internal/compose/rawcapturelinkbackfill.go:backfillRawCaptureLinksBatch":  "the same pass's tally of what it could not place, and the activity table enters it only as a NOT EXISTS on the reference column. No activity column is projected, compared against anything a caller supplies, or read at all: the whole output is one integer, logged so an operator can tell a drained backlog from a stuck one. A held row changes that integer by one and discloses nothing",
	"internal/modules/capture/pendingsweeps.go:PendingStore.PurgeRawCaptureTx": "the count the redaction purge takes of its own batch — how many of these records name a provider original — asked one statement before it destroys them, so a delete that reaches fewer can refuse rather than report success. It reads one column, raw_capture_id, of rows the caller already named; nothing reaches a caller but the comparison. Excluding a held row would exempt a held message from destruction and leave its original standing, the same inversion RedactCapturedNoiseTx is admitted against",
