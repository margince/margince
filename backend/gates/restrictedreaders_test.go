...
var restrictedReadersAdmitted = gatekit.Waive(map[string]string{
	"internal/modules/activities/audience.go:readAudienceImage":               "reads audience metadata for the transactional audit, including a held row before SetAudience reaches the retention trigger that returns 423. No content is selected or returned to a caller; the refused transaction rolls back. The creation caller has just inserted and guarded the new row. Cost: existing audience and member metadata can be read inside a refused mutation; held-row integration tests require that refusal to retain its public contract",
