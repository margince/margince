...
var entryPointsOutsideModules = gatekit.Waive(map[string]string{
	...
	"internal/compose/dealbrief/service.go":           "dealbrief.Service.Get — the deal's standing brief. It gathers its facts through gated reads and nothing else: deals.Store.GetDeal and DealHealth (deal.read plus the deal row scope), activities.Store.ListActivities and ListOpenTasks (activity.read plus the link-walk scope, narrowed to the deal), dealrooms.Store.ListRooms and ListThreads (deal_room.read plus the deal scope; a caller without the grant reads no room and gets no room section). The fold over them (write) is a pure function of what those reads returned, and it writes nothing",
