...
var ungatedEntryPoints = gatekit.Waive(map[string]string{ // #nosec G101 -- waiver rationales for the fitness gate, not credentials
	...
	"internal/modules/knowledge:MarkTuningIdentity": "records which embed binding a corpus's grounding floor was tuned against, so a later swap can report the number as a leftover rather than a threshold. It is written by the same system-principal paths that move the vectors, because the stamp is only true at the moment they do. It carries no caller-chosen value \u2014 the identity is the live binding's, read from the embed lane, not from a request \u2014 and the column is administrative metadata no read surface returns on its own. Changing the floor ITSELF is a gated entry point (EditCorpus, knowledge_corpus:update); this only records what it was tuned under",
