...
var rowScopedFKDecisions = gatekit.Waive(map[string]string{
	...
	"assurance_task_item.task_activity_id":    "gated: caller-supplied, and Store.BundleException puts it through auth.EnsureActivityVisibleLive in the transaction that files the finding — the ACTIVITY probe, because an activity's scope is inherited from what it is linked to rather than carried on the row. rbacBundle answers whether a principal may bundle at all, not whether it may file under THIS task: without the probe a caller could name an activity it cannot open and learn from the outcome that it exists, and the cycle's task would point at one its own filer could not read",
	"privacy_notice_case.person_id":           "server-derived: the duty is opened by the NoticeCaseOpen consumer off a PersonCreated envelope, under the system principal — env.Entity.ID names the contact the event is about and nothing on a request reaches this column. There is no caller whose scope could differ from it: a notice duty is owed because the record was created, not because somebody asked for one",
