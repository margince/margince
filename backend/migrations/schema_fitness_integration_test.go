...
var rowScopedFKDecisions = gatekit.Waive(map[string]string{
	...
	// The append-only record of how a reply came to be judged. BOTH writers
	// funnel through appendReplyVerdictHistory, so the history row can only land
	// after whichever gate applied to the write above it, and the two gates
	// differ because the two acts do.
	"activity_reply_verdict_history.activity_id": "gated on both writers, each for its own act: a human correction is client-supplied, so CorrectReplyVerdict puts the id through auth.EnsureActivityWritable inside the same transaction and its UPDATE re-tests inbound/archived/audience/restricted, answering apperrors.ErrNotFound on no rows so a refusal cannot disclose that the message is there; the classifier's own judgement is server-derived, naming a row its backlog already selected under the system principal rather than reading an id off a request body, and SetReplyVerdict writes history only when that same guarded UPDATE applied",
