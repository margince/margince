...
var rowScopedFKDecisions = gatekit.Waive(map[string]string{
	...
	"activity_review_response.deal_id":        "gated: caller-supplied — it is the deal whose page the review was filed from — and put through auth.EnsureLinkTarget inside the same transaction BEFORE this row lands. The probe is the one insertActivityLinks takes on the note's own deal link, written by the LogActivityTx call above the INSERT, so a deal the caller's row scope cannot reach answers apperrors.ErrNotFound and the review never commits. The composite FK beside it (deal_id, closing_occurrence_id) then refuses a closing belonging to a different deal, so a gated deal cannot carry a review of somebody else's outcome",
