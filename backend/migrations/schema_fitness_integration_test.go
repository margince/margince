...
var rowScopedFKDecisions = gatekit.Waive(map[string]string{
	...
	"withdrawal_credential.person_id":        "gated on the door that mints one by hand and server-derived on the door that mints one per send. EnsureWithdrawalCredentialTx takes auth.Require(person, update), then auth.EnsureWritableLive on the named person and auth.LockSubjectLive before the insert — WRITABLE rather than visible, because a person is shareable and a read-only share would otherwise mint an opt-out link over somebody else's mail. The send path never takes an id at all: WithdrawalTokenForEmail resolves the subject from the ADDRESS the message is going to, through bindWithdrawalSubject, and leaves the credential unbound when two live records hold that address rather than picking one. ON DELETE SET NULL, so erasing the person leaves the link dead rather than dangling",
