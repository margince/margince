...
var rowScopedFKDecisions = gatekit.Waive(map[string]string{
	...
	"withdrawal_credential.person_id":        "server-derived, on the one door that mints one: a marketing send. WithdrawalTokenForEmail never takes an id — it resolves the subject from the ADDRESS the message is going to, through bindWithdrawalSubject, and leaves the credential unbound when two live records hold that address rather than picking one. EnsureWithdrawalCredentialTx beneath it takes auth.Require(person, read), then auth.EnsureVisibleLive on whatever that resolve named and auth.LockSubjectLive before the insert. VISIBLE rather than writable, matching the sibling mint PreferenceTokenForEmail: a caller entitled to send this person marketing mail owes them a link that stops it, and requiring write authority refused the mint while the message went out regardless — stripping the opt-out instead of protecting anybody. The credential is also the weaker of the two capabilities, since it can stop mail and do nothing else where a preference token reads state, withdraws and grants. LIVE is the load-bearing half: it refuses an anonymized tombstone, which is the row an erasure race leaves behind. ON DELETE SET NULL, so erasing the person leaves the link dead rather than dangling",
