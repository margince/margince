...
var rowScopedFKDecisions = gatekit.Waive(map[string]string{
	...
	"stage_progression_outcome.deal_id":      "server-derived: the ledger row is opened in the same transaction that stages the proposal, naming the deal the progression pass is ALREADY reading — facts.DealID comes off that read, never from a request body. There is nothing here a caller could point elsewhere, and the write is gated on deal:update besides, because opening a ledger row for a deal is a write about that deal",
