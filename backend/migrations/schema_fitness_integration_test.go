...
var rowScopedFKDecisions = gatekit.Waive(map[string]string{
	...
	"activity_reply_verdict_history.activity_id": "gated: both writers reach this row only inside the transaction whose UPDATE on activity already matched — the classifier's under the backlog predicate it selected on, a human correction under auth.EnsureActivityWritable, and both re-test archived/audience/restricted at write time. A history row therefore cannot name an activity its writer had not just written, and a correction on a row out of scope answers apperrors.ErrNotFound rather than disclosing that the message is there",
