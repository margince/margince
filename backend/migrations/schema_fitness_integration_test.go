...
var rowScopedFKDecisions = gatekit.Waive(map[string]string{
	...
	"org_scan.organization_id":                 "gated: the scan row is queued only after orgscan.Service.Ensure runs the caller's own org360 Assemble, the same gated read the brief takes — an account the caller cannot read has no scan queued for it, the row is keyed on that same caller, and the worker assembles again under that caller's own principal before it writes a finding",
