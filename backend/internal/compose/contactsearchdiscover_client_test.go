...
func TestDiscoverProfileURLRefusesAStrangerAtTheSameCompany(t *testing.T) {
	g := &ContactAutoEnrich{search: &fakeSearch{results: []websearch.Result{
	...
	if _, found, err := g.discoverProfileURL(context.Background(), "Anna Weber", "ScaleCommerce"); err != nil || found {
		t.Errorf("found=%v err=%v — a different contact at the same employer was accepted", found, err)
	}
}
...
func TestDiscoverProfileURLAcceptsANameWithInternalPunctuation(t *testing.T) {
	for name, res := range map[string]websearch.Result{
		...
		g := &ContactAutoEnrich{search: &fakeSearch{results: []websearch.Result{res}}}
		...
		if !found {
			t.Errorf("%s: the result names this contact and was refused", name)
		}
	}
}

// A company page and a job posting live on the same hosts as a profile and are
// not contacts. Filing one onto a contact would be a claim about something that
// is not a human at all.
func TestDiscoverProfileURLRefusesWhatIsNotAContact(t *testing.T) {
	g := &ContactAutoEnrich{search: &fakeSearch{results: []websearch.Result{
	...
	if _, found, _ := g.discoverProfileURL(context.Background(), "Anna Weber", "ScaleCommerce"); found {
		t.Error("a company page or a job posting was filed as a contact's profile")
	}
}
...
func TestDiscoverProfileURLKeepsTheAddressAndItsReceipt(t *testing.T) {
	...
	g := &ContactAutoEnrich{search: fake}
...
// A search that failed is not a contact that failed: the contact is already
...
func TestDiscoverProfileURLReportsAProviderFailure(t *testing.T) {
	g := &ContactAutoEnrich{search: &fakeSearch{err: errors.New("provider unavailable")}}
	if _, found, err := g.discoverProfileURL(context.Background(), "Anna Weber", "ScaleCommerce"); err == nil || found {
