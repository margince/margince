...
// deterministic fallback and the wire mapping are all contactdraft's, and this
...
// one — contactdraft and accountdraft — and the difference between them is where
// the recipient comes from, which is not a reason to write a third.
//
// The shape is the one contactdraft's own contract describes: the record IS the
...
// guarantee is structural, the same way contactdraft states it. The two writes
...
// contactdraft's fence with the rest.
package leaddraft
