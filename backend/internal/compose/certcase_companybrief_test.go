...
// The summarize/company_brief certification case.
//
...
// prepareCompanyBrief prepares the one fixture these tests vary the EXPECTATION
// against; the refusal tests below pass their own fixtures directly.
func prepareCompanyBrief(t *testing.T, expected string) aitasks.PreparedCase {
	t.Helper()
	prepared, err := companyBriefCases{}.Prepare(
		json.RawMessage(companyBriefFixtureJSON), json.RawMessage(expected))
	if err != nil {
...
func TestCompanyBriefCaseSiteMatchesTheCensusedSite(t *testing.T) {
	site := companyBriefCases{}.Site()
	if site.Variant != "company_brief" || string(site.Task) != "summarize" {
		t.Errorf("Site() = %+v, want the summarize/company_brief one-shot", site)
	}
}
...
func TestCompanyBriefCaseMintsTheIdsTheModelSees(t *testing.T) {
	first := prepareCompanyBrief(t, `["stalled_retrofit"]`)
	second := prepareCompanyBrief(t, `["stalled_retrofit"]`)
...
func TestCompanyBriefCaseRefusesAnExpectationNoReplyCouldFail(t *testing.T) {
	for name, expected := range map[string]string{
		"nothing expected":                    `[]`,
		"a label the fixture does not supply": `["a_deal_that_is_not_there"]`,
	} {
		t.Run(name, func(t *testing.T) {
			if _, err := (companyBriefCases{}).Prepare(
				json.RawMessage(companyBriefFixtureJSON), json.RawMessage(expected)); err == nil {
...
func TestCompanyBriefCaseRefusesAFixtureNoExpectationCouldName(t *testing.T) {
...
func TestCompanyBriefCaseEvaluatesEachOutcome(t *testing.T) {
...
func TestCompanyBriefCaseReportsAMissedRecordAsAWrongAnswer(t *testing.T) {
	prepared := prepareCompanyBrief(t, `["stalled_retrofit","last_email"]`)
	sent := requestIDs(t, prepared)
	dealID := firstRecordID(t, sent)
