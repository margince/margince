...
// laundering a hand-typed string past the type. The contact brief renders the
...
// briefSystem is the contact_brief site's prompt.
//
...
A recent message may carry a "speaker": "them" means the contact wrote it, "you" means the READER wrote it. Attribute every quoted or paraphrased line to its speaker and never to the other one. A view, a complaint or an assessment in a message whose speaker is "you" is the READER'S OWN and must never be reported as the contact's. Where a message carries NO speaker, the record does not say who wrote it: quote it if you must, attributed to nobody, and never guess. Answer state is the point too: distinguish an unanswered message from a settled one. The summary says which.
...
// quotes behind each claim — text written by contacts outside this workspace. It
...
func Write(
	...
	written, err := writeWithModel(ctx, lane, contactID, in, lang)
	if err != nil {
...
func writeWithModel(
	...
	resp, err := ai.Ask(ctx, lane, req, func(text string) error {
		_, err := ParseBrief(text, contactID, in)
		return err
	})
	if err != nil {
		return nil, err
	}
	kept, err := ParseBrief(resp.Text, contactID, in)
	if err != nil {
		return nil, err
	}
	if len(kept) == 0 {
		return nil, errors.New("the brief reply cited nothing about this contact")
	}
	return kept, nil
}
...
//
// contactID pins the one contact this brief is about, so a contact citation
// cannot name a different one.
func ParseBrief(reply, contactID string, in Input) ([]Sentence, error) {
	var parsed struct {
	...
	return keepGroundedSentences(parsed.Sentences, contactID, in), nil
}
...
// about why.
func keepGroundedSentences(sentences []Sentence, contactID string, in Input) []Sentence {
	known := knownRecords(contactID, in)
...
//
// Keying on the id alone would accept a real deal id cited as a contact: the id
...
func knownRecords(contactID string, in Input) map[Evidence]string {
	known := map[Evidence]string{{EntityType: citeContact, EntityID: contactID}: contactThemselves(in)}
	if in.OpenDeal != nil {
...
// contactThemselves is the contact's OWN facts: the input with its lists emptied.
//
// The whole input would make a citation of the contact a skeleton key — most
// sentences cite them, and every figure anywhere in the payload would answer
// for it.
func contactThemselves(in Input) string {
