...
const (
	briefContactID = "11111111-1111-4111-8111-111111111111"
...
// about a contact with nothing to check it against.
...
// The pair is the reference, not the id: a real deal id cited as a contact
...
func TestParseBriefDropsARealIDCitedAsTheWrongKind(t *testing.T) {
	...
	kept, err := ParseBrief(`{"sentences":[
		{"text":"They own the renewal.","evidence":[
			{"entity_type":"contact","entity_id":"`+briefDealID+`"}]}
	]}`, briefContactID, inputFixture())
	if err != nil {
		t.Fatalf("ParseBrief: %v", err)
	}
	if len(kept) != 0 {
		t.Errorf("kept %+v, want the deal-cited-as-a-contact sentence dropped", kept)
	}
}

// The brief is about ONE contact. Accepting any contact citation would let a
// reply hand back an id this reader never saw, rendered as a link they could
// click into a record their scope may hide.
func TestParseBriefRefusesAContactCitationThatIsNotThisContact(t *testing.T) {
	t.Parallel()
	kept, err := ParseBrief(`{"sentences":[
		{"text":"They are the economic buyer.","evidence":[
			{"entity_type":"contact","entity_id":"`+strangerID+`"}]}
	]}`, briefContactID, inputFixture())
	if err != nil {
		t.Fatalf("ParseBrief: %v", err)
	}
	if len(kept) != 0 {
		t.Errorf("kept %+v, want a sentence about another contact dropped", kept)
	}
}
...
func TestAnUnknownNatureIsReadAsAFact(t *testing.T) {
	...
	kept, err := ParseBrief(`{"sentences":[
		{"text":"They are waiting on us.","nature":"prophecy","evidence":[
			{"entity_type":"contact","entity_id":"`+briefContactID+`"}]}
	]}`, briefContactID, inputFixture())
	if err != nil {
		t.Fatalf("ParseBrief: %v", err)
	}
...
func TestWriteFallsBackRatherThanFailing(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// answer.
		{"a reply that cites nothing of this contact", &scriptedLane{reply: `{"sentences":[
			{"text":"They spoke to your CFO.","evidence":[
				{"entity_type":"activity","entity_id":"` + strangerID + `"}]}]}`}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			written, by, err := Write(t.Context(), tc.lane, briefContactID, inputFixture(),
...
func TestWriteKeepsWhatItGroundsAndNamesTheModel(t *testing.T) {
	...
	written, by, err := Write(t.Context(), lane, briefContactID, inputFixture(), string(textlang.English))
	if err != nil {
		t.Fatalf("Write: %v", err)
	}
