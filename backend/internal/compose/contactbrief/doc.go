...
// Package contactbrief writes the standing relationship brief on the contact
// record page.
//
// Tables owned: contact_brief (the per-user brief cache).
//
// It is the company brief applied to a contact, and the mirroring is the ruling
...
//
// PER VIEWER. The input is assembled by running the contact's own reads AS THE
...
// claims and deals move without touching the contact row, so a key derived from
...
//
// There is no free-text ask on a contact (ADR-0096 D6). Prepared, citable
