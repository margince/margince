...
func machineFact(name, value string, retrieved time.Time) crmcontracts.CompanyFact {
	id := rowID()
	return crmcontracts.CompanyFact{
		Id:          id,
		Field:       crmcontracts.CompanyFactField(name),
		Value:       value,
		Source:      crmcontracts.CompanyFactSource(crmcontracts.CompanyFactSourceSiteRead),
		RetrievedAt: &retrieved,
		UpdatedAt:   retrieved,
	}
}
