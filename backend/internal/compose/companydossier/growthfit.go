...
var (
	...
	requiredFactInputs = []requiredFactInput{
		{field: crmcontracts.CompanyFactFieldEmployeeRange, label: "how big they are"},
		{field: crmcontracts.CompanyFactFieldTechnology, label: "the technology they run"},
	}
)
...
func freshFact(in Input, field crmcontracts.CompanyFactField, now time.Time) (bool, time.Time) {
	for _, have := range in.Facts {
		if have.Field != field {
			continue
		}
		human := have.Source == crmcontracts.CompanyFactSourceHuman
