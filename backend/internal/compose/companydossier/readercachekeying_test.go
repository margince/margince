...
// different rows. A statement keyed on the company alone breaks that two
...
func TestEveryReaderCacheIsKeyedOnTheReaderAndNotTheCompanyAlone(t *testing.T) {
	...
	for name, statements := range caches {
		...
		if !strings.Contains(statements["upsertOne"], "ON CONFLICT (user_id, company_id)") {
			t.Errorf("%s.upsertOne does not resolve conflicts on (user_id, company_id), so the cache "+
