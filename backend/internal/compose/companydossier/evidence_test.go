...
func TestARecordTheReaderCannotSeeIsIndistinguishableFromOneThatDoesNotExist(t *testing.T) {
	in := Input{CompanyID: "o-1", ProfileFields: []crmcontracts.CompanyProfileField{siteReadField()}}
...
func TestAFactReceiptCarriesItsOwnProvenance(t *testing.T) {
	id := rowID()
	in := Input{CompanyID: "o-1", Facts: []crmcontracts.CompanyFact{{
		Id:              id,
...
func TestAKindWithNoReceiptAnswersNotFound(t *testing.T) {
	facts := stubFacts{in: Input{CompanyID: "o-1"}}
...
func (s stubFacts) ListCompanyProfileFields(
	context.Context, ids.CompanyID,
) ([]crmcontracts.CompanyProfileField, error) {
	return s.in.ProfileFields, nil
}

func (s stubFacts) ListCompanyFacts(
	context.Context, ids.CompanyID,
) ([]crmcontracts.CompanyFact, error) {
	return s.in.Facts, nil
}

func (s stubFacts) GetCompany(
	context.Context, ids.CompanyID, storekit.ArchivedFilter,
) (crmcontracts.Company, error) {
	return crmcontracts.Company{DisplayName: s.in.Name}, nil
}
