...
var waivedEnvelopeFields = gatekit.Waive(map[string]string{
	...
	"RawCaptureID": "the stored original this record was read from, and it is core-settled rather than unpublished: the sink stores a record's Raw and keeps the id it settles on, and the only other writer is a core poll that persisted the original before the parse was enqueued. A unit able to state one could point its own activity at ANOTHER lane's stored original — and the Art. 15 disclosure gate hands back a raw_capture payload when an open activity names the row, so the forged reference would read out a message the unit never saw. A unit supplies Raw and the core names what it stored",
