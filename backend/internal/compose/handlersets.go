...
// Its own function so the composition root reads as a list of what is wired
...
func (s *Server) wireContact360(pool *pgxpool.Pool) {
	...
	s.contact360Handlers = contact360.NewHandlers(s.contact360Svc)
	...
	s.meetingBriefHandlers = meetingbrief.NewHandlers(s.meetingBriefSvc)
...
// the per-record endpoints read the same columns under the same gates.
func (s *Server) wireProject360(pool *pgxpool.Pool) {
	...
	s.project360Handlers = project360.NewHandlers(svc)
}
