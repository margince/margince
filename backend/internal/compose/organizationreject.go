...
//
// Rejecting a company is two native writes in one commit: the organization row
...
// organization table holds none of the workspace's records — so the unguarded
...
// RejectOrganization shadows the people transport so the mode guard runs
// before the native store sees the request.
func (s Server) RejectOrganization(w http.ResponseWriter, r *http.Request, id crmcontracts.Id, params crmcontracts.RejectOrganizationParams) {
	if refuseInOverlayMode(w, r, s.sorDispatch) {
		return
	}
	s.peopleHandlers.RejectOrganization(w, r, id, params)
}
