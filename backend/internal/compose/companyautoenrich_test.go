...
// companyEvent builds the envelope the trigger reads, occurring now.
func companyEvent(eventType string, id ids.UUID) events.Envelope {
	return events.Envelope{
		Type:       eventType,
		OccurredAt: time.Now().UTC(),
		Entity:     events.EntityRef{Type: "company", ID: id},
	}
}
...
func TestCompanyAutoEnrichTriggerIgnoresWhatItCannotActOn(t *testing.T) {
	g := &CompanyAutoEnrichTrigger{}
	ctx := context.Background()

	stale := companyEvent("company.created", ids.NewV7())
	stale.OccurredAt = stale.OccurredAt.Add(-2 * companyAutoEnrichFreshWindow)
