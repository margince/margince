...
func renderAgentToolBudget(t *testing.T) agentToolBudget {
	...
	for _, spec := range mustScheduledAgents() {
		attached := make([]mcp.ToolSpec, 0, len(spec.Tools))
		for _, name := range spec.Tools {
			attached = append(attached, byName[name])
		}
...
const agentToolBudgetNote = "Generated by `" + agentToolBudgetCommand + "`; do not edit by hand. " +
	...
	"whole served catalog, which no agent is ever offered. Tokens are the ~4-bytes estimate the " +
	"window itself estimates with, over the listing the runner's own renderer produces."
