...
// The certification case for summarize/company_dossier.
//
...
type companyDossierCases struct{}

func (companyDossierCases) Site() aitasks.Site {
	return aitasks.Site{Task: ai.TaskSummarize, Variant: "company_dossier", Kind: ai.SiteKindOneShot}
}
...
func (companyDossierCases) Prepare(fixture, expected json.RawMessage) (aitasks.PreparedCase, error) {
	var f growthFitFixture
	if err := json.Unmarshal(fixture, &f); err != nil {
		return nil, fmt.Errorf("summarize/company_dossier: the fixture is not the shape this site takes: %w", err)
	}
	var want []string
	if err := json.Unmarshal(expected, &want); err != nil {
		return nil, fmt.Errorf(
			"summarize/company_dossier: the expected answer is not a list of record labels the dossier must cite: %w", err)
	}
	in, label, err := growthFitInput(f)
	if err != nil {
		return nil, fmt.Errorf("summarize/company_dossier: %w", err)
	}
	if err := refuseUngroundableDossier(want, label); err != nil {
		return nil, fmt.Errorf("summarize/company_dossier: %w", err)
	}
	return &companyDossierCase{in: in, label: label, expected: want}, nil
}
...
type companyDossierCase struct {
...
// own request builder.
func (c *companyDossierCase) Run(ctx context.Context, completer aitasks.Completer) (aitasks.Trace, error) {
	...
	req := companydossier.DossierRequest(c.in, string(textlang.English))
	trace := aitasks.Trace{Requests: []model.Request{req}}
	resp, err := completer.Complete(ctx, req)
	if err != nil {
		return trace, fmt.Errorf("summarize/company_dossier: %w", err)
	}
...
func (c *companyDossierCase) Evaluate(trace aitasks.Trace) aitasks.Outcome {
	sections, err := companydossier.ParseDossier(trace.Output, c.in)
	if err != nil {
