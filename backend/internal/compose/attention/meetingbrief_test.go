...
// it opens as `?prep=<activity>` on a CONTACT's record — so naming it needs two
...
func TestAMeetingWithSomebodyOnItReachesTheBrief(t *testing.T) {
	...
	row := classifyMeeting(meetingItem(Meeting{
		...
		StartsAt: fixedClock().Add(2 * time.Hour), ContactID: contact,
	}), fixedClock())
	...
	if row.item.WithContact == nil || ids.UUID(*row.item.WithContact) != contact {
		t.Errorf("the row names contact %v, want %v", row.item.WithContact, contact)
	}
}
...
// contacts the caller cannot read — the lane cannot tell those apart and does not
...
func TestAMeetingNamingNobodyOffersNoBrief(t *testing.T) {
	...
	if row.item.WithContact != nil {
		t.Errorf("the row names contact %v on a meeting that named nobody", row.item.WithContact)
	}
}
