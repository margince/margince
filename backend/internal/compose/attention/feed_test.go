...
func TestADuplicateOutranksAnApprovalBecauseAMergeCannotBeUndone(t *testing.T) {
	svc := NewService(
		...
		&stubTasks{}, stubReceipts{}, stubBriefing{}, nil, nil, nil, nil, nil, nil, fixedClock)
...
func TestAWithheldLaneIsNamedRatherThanReportedEmpty(t *testing.T) {
	svc := NewService(
		...
		stubReceipts{}, stubBriefing{}, nil, nil, nil, nil, nil, nil, fixedClock)
	out, err := svc.Assemble(context.Background())
	if err != nil {
...
func TestABrokenLaneFailsTheReadRatherThanReadingAsQuiet(t *testing.T) {
	svc := NewService(
		...
		stubDuplicates{}, &stubTasks{}, stubReceipts{}, stubBriefing{}, nil, nil, nil, nil, nil, nil, fixedClock)
...
func TestAReceiptOffersNoDecision(t *testing.T) {
	svc := NewService(
		...
		stubBriefing{}, nil, nil, nil, nil, nil, nil, fixedClock)
	out, err := svc.Assemble(context.Background())
	if err != nil {
		t.Fatalf("assembling: %v", err)
	}
...
func TestABriefingLaneRefusedIsNamedRatherThanReportedQuiet(t *testing.T) {
	svc := NewService(
		stubApprovals{}, stubDuplicates{}, &stubTasks{}, stubReceipts{},
		stubBriefing{err: apperrors.ErrPermissionDenied}, nil, nil, nil, nil, nil, nil, fixedClock)
	out, err := svc.Assemble(context.Background())
	if err != nil {
		t.Fatalf("assembling: %v", err)
	}
...
func TestABrokenBriefingLaneFailsTheReadRatherThanReadingAsQuiet(t *testing.T) {
	svc := NewService(
		stubApprovals{}, stubDuplicates{}, &stubTasks{}, stubReceipts{},
		stubBriefing{err: errors.New("the brief read fell over")}, nil, nil, nil, nil, nil, nil, fixedClock)
	if _, err := svc.Assemble(context.Background()); err == nil {
...
func TestEveryLaneIsReadOncePerFeed(t *testing.T) {
	...
	svc := NewService(
		...
		commitments, stubAtRisk{}, decay, meetings, nil, nil, fixedClock,
	)
	if _, err := svc.Assemble(context.Background()); err != nil {
		t.Fatalf("assembling: %v", err)
	}
...
func TestAWithheldLaneAppearsInLanesOmittedExactlyOneTime(t *testing.T) {
	svc := NewService(
		stubApprovals{}, stubDuplicates{}, &stubTasks{}, stubReceipts{}, stubBriefing{},
		&stubCommitments{err: apperrors.ErrPermissionDenied}, stubAtRisk{}, nil, nil, nil, nil, fixedClock,
	)
	out, err := svc.Assemble(context.Background())
	if err != nil {
		t.Fatalf("assembling: %v", err)
