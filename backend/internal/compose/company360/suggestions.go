...
var (
	suggestNoReply     = crmcontracts.Company360SuggestionKindNoReply
	suggestStalledDeal = crmcontracts.Company360SuggestionKindStalledDeal
	suggestNoNextStep  = crmcontracts.Company360SuggestionKindNoNextStep
	suggestConflict    = crmcontracts.Company360SuggestionKindLifecycleConflict
)
...
func (a *assembly) readSuggestions() error {
	...
	found, dropped, err := a.svc.suggestionsFor(a.ctx, a.tx, a.companyID, a.now, in)
	if err != nil {
		return err
	}
...
func (s *Service) suggestionsFor(
	ctx context.Context, tx pgx.Tx, companyID ids.CompanyID, now time.Time, in suggestionInputs,
) ([]crmcontracts.Company360Suggestion, int, error) {
...
func candidateSuggestions(
	companyID ids.CompanyID, now time.Time, in suggestionInputs,
) []crmcontracts.Company360Suggestion {
	found := make([]crmcontracts.Company360Suggestion, 0, maxSuggestions)
	...
	found = appendIf(found, lifecycleConflict(companyID, in))
	if in.timeline && in.hasNewest {
		found = appendIf(found, staleThread(companyID, now, in.newest))
	}
	if in.pipeline {
		...
		if in.timeline {
			...
			found = appendIf(found, noNextStepSuggestion(companyID, in))
		}
	}
	return found
}
...
func (s *Service) withoutDismissed(
	...
	found []crmcontracts.Company360Suggestion,
	...
	dismissed, err := s.dismissedFingerprints(ctx, tx, companyID, candidates)
	if err != nil {
		return nil, err
	}
	kept := make([]crmcontracts.Company360Suggestion, 0, len(found))
	for _, suggestion := range found {
...
func appendIf(
	into []crmcontracts.Company360Suggestion, one *crmcontracts.Company360Suggestion,
) []crmcontracts.Company360Suggestion {
	if one == nil {
		return into
	}
...
func setDraftReply(out *crmcontracts.Company360Suggestion, activityID ids.UUID) {
	id := openapi_types.UUID(activityID)
	out.Action = NewSuggestionAction(crmcontracts.Company360SuggestionActionKindDraftReply)
	out.Action.ActivityId = &id
}

// setOpenDeal points at the deal that stalled.
func setOpenDeal(out *crmcontracts.Company360Suggestion, dealID ids.UUID) {
	id := openapi_types.UUID(dealID)
	out.Action = NewSuggestionAction(crmcontracts.Company360SuggestionActionKindOpenDeal)
...
func setAddTask(out *crmcontracts.Company360Suggestion, step nextStep) {
	out.Action = NewSuggestionAction(crmcontracts.Company360SuggestionActionKindAddTask)
	body := step.body
...
func staleThread(
	companyID ids.CompanyID, now time.Time, newest lastMessage,
) *crmcontracts.Company360Suggestion {
	...
	out := &crmcontracts.Company360Suggestion{
		...
		Fingerprint: fingerprint(string(suggestNoReply), companyID.String(), evidence),
		Evidence:    evidence,
	}
...
// here from a date.
func stalledDealSuggestions(stalled []stalledDeal) []crmcontracts.Company360Suggestion {
	out := make([]crmcontracts.Company360Suggestion, 0, len(stalled))
	for _, deal := range stalled {
		...
		out = append(out, crmcontracts.Company360Suggestion{
