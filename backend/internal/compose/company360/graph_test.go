...
// compose/integration/company360_graph_integration_test.go.
...
func newGraph(t *testing.T) (*graphAssembly, ids.CompanyID) {
	t.Helper()
...
func TestContactsAreOrderedStrongestFirst(t *testing.T) {
	...
	want := []ids.UUID{companyID.UUID, strong.UUID, middling.UUID, weak.UUID}
	got := nodeIDs(g)
	if len(got) != len(want) {
...
func TestDroppedCountReportsPastTheBoundedRead(t *testing.T) {
	...
	g.placeContacts()
	g.placeDeals()
	g.placeRelated([]graphRelatedCompany{
		{companyID: ids.NewV7(), displayName: "Holding", relation: graphRelationParent},
	})

	// 900 − 15 contacts, 40 − 1 deals, 25 − 1 companies.
...
func TestAStakeholderWhoAlsoWorksHereIsOneNode(t *testing.T) {
	...
	if edges[crmcontracts.CompanyGraphEdgeKindEmployment] != 1 ||
		edges[crmcontracts.CompanyGraphEdgeKindDealStakeholder] != 1 {
...
func TestTheHierarchyEdgeAlwaysPointsParentToChild(t *testing.T) {
	g, companyID := newGraph(t)
	parent := graphRelatedCompany{companyID: ids.NewV7(), displayName: "Holding", relation: graphRelationParent}
	child := graphRelatedCompany{companyID: ids.NewV7(), displayName: "Subsidiary", relation: graphRelationChild}

	g.relatedTotal = 2
	g.placeRelated([]graphRelatedCompany{parent, child})

	want := map[ids.UUID]ids.UUID{parent.companyID: companyID.UUID, companyID.UUID: child.companyID}
...
// TestAPartnerEdgePointsFromTheCompanyThatRecordsIt: `referred_by` on the
...
// invert half of them.
func TestAPartnerEdgePointsFromTheCompanyThatRecordsIt(t *testing.T) {
	g, companyID := newGraph(t)
	kind := "referred_by"
	counterparty := ids.NewV7()
	theirs := graphRelatedCompany{
		companyID: counterparty, displayName: "Referrer", relation: graphRelationPartner,
		partnerKind: &kind, edgeOwner: &counterparty,
	}
	ours := ids.NewV7()
	oursOwner := companyID.UUID
	mine := graphRelatedCompany{
		companyID: ours, displayName: "Reseller", relation: graphRelationPartner,
		partnerKind: &kind, edgeOwner: &oursOwner,
	}

	g.relatedTotal = 2
	g.placeRelated([]graphRelatedCompany{theirs, mine})

	oriented := map[ids.UUID]ids.UUID{}
	for _, edge := range g.out.Edges {
		if edge.Kind != crmcontracts.CompanyGraphEdgeKind(kind) {
			t.Errorf("partner edge kind is %q, want %q", edge.Kind, kind)
		}
		oriented[ids.UUID(edge.From)] = ids.UUID(edge.To)
	}
	if oriented[counterparty] != companyID.UUID {
		t.Errorf("their edge runs %s -> %s, want it to start at the counterparty", counterparty, oriented[counterparty])
	}
	if oriented[companyID.UUID] != ours {
		t.Errorf("our edge runs %s -> %s, want it to start at this account", companyID, oriented[companyID.UUID])
	}
}
...
// (readRelatedCompanies), so it is not provable here; the integration suite
...
func TestACompanyAttachedTwiceIsOneNodeAndOneDrop(t *testing.T) {
	...
	both := ids.NewV7()
	owner := both
	related := []graphRelatedCompany{
		{companyID: both, displayName: "Holding", relation: graphRelationParent},
		{companyID: both, displayName: "Holding", relation: graphRelationPartner, partnerKind: &kind, edgeOwner: &owner},
	}
	...
	if companies := len(g.out.Nodes) - 1; companies != 1 {
		t.Errorf("placed %d companies, want 1 for a company attached two ways", companies)
	}
...
func TestTheAccountOwnerWhoAlsoWroteIsOneNode(t *testing.T) {
	...
	users := 0
	for _, node := range g.out.Nodes {
		if node.Kind == crmcontracts.CompanyGraphNodeKindUser {
			users++
	...
	edges := map[crmcontracts.CompanyGraphEdgeKind][2]ids.UUID{}
	for _, edge := range g.out.Edges {
		edges[edge.Kind] = [2]ids.UUID{ids.UUID(edge.From), ids.UUID(edge.To)}
	}
	if got := edges[crmcontracts.CompanyGraphEdgeKindOwns]; got != [2]ids.UUID{owner.userID, companyID.UUID} {
		t.Errorf("owns edge is %v, want %v -> %v", got, owner.userID, companyID)
	}
	if got := edges[crmcontracts.CompanyGraphEdgeKindInContactWith]; got != [2]ids.UUID{owner.userID, contact.UUID} {
...
func TestOurSideVocabularyIsInTheContract(t *testing.T) {
	if !crmcontracts.CompanyGraphNodeKindUser.Valid() {
		t.Error("the user node kind is not a value the contract declares")
	}
	for _, kind := range []crmcontracts.CompanyGraphEdgeKind{
		crmcontracts.CompanyGraphEdgeKindOwns,
		crmcontracts.CompanyGraphEdgeKindInContactWith,
	} {
		if !kind.Valid() {
