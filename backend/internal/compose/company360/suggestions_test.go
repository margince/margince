...
// compose/integration/company360_suggestions_integration_test.go — including the
...
func testCompanyID(t *testing.T) ids.CompanyID {
	t.Helper()
	return ids.From[ids.CompanyKind](ids.NewV7())
}
...
func TestStaleThreadFiresOnOurUnansweredMessage(t *testing.T) {
	...
	got := staleThread(companyID, suggestNow, newest)
	if got == nil {
...
func TestStaleThreadFiresAtTheReplyWindowAndNotBefore(t *testing.T) {
	...
	if got := staleThread(companyID, suggestNow, sentAgo(noReplyDays-1, crmcontracts.ActivityDirectionOutbound)); got != nil {
		t.Errorf("suggestion %+v one day inside the reply window", got)
	}
	got := staleThread(companyID, suggestNow, sentAgo(noReplyDays, crmcontracts.ActivityDirectionOutbound))
	if got == nil {
...
func TestStaleThreadStaysSilentWithoutADirection(t *testing.T) {
	...
	if got := staleThread(companyID, suggestNow, newest); got != nil {
...
func TestNoNextStepReportsTheAccountsOwnDealCount(t *testing.T) {
	got := noNextStepSuggestion(testCompanyID(t), liveAccount(42, "digest-a"))
	if got == nil {
...
func TestNoNextStepFiresOnlyOnAnActiveAccount(t *testing.T) {
	...
	if got := noNextStepSuggestion(companyID, liveAccount(1, "digest-a")); got == nil {
		t.Error("no suggestion for an open deal with nothing scheduled")
	}
	if got := noNextStepSuggestion(companyID, liveAccount(0, "")); got != nil {
...
func TestNoNextStepStaysSilentWhenSomethingIsScheduled(t *testing.T) {
	...
	in.scheduled = true
	if got := noNextStepSuggestion(testCompanyID(t), in); got != nil {
...
func TestNoNextStepNeedsBothGrants(t *testing.T) {
	...
	found := candidateSuggestions(companyID, suggestNow, dealsOnly)
...
func TestCandidateSuggestionsRunOnlyTheRulesTheGrantsSupport(t *testing.T) {
	...
	found := candidateSuggestions(companyID, suggestNow, timelineOnly)
...
func TestNoNextStepRidesEveryOpenDeal(t *testing.T) {
	companyID := testCompanyID(t)
	first := noNextStepSuggestion(companyID, liveAccount(1, "digest-a"))
	second := noNextStepSuggestion(companyID, liveAccount(1, "digest-b"))
...
func TestFingerprintSeparatesKindAndEvidence(t *testing.T) {
	...
	cited := []crmcontracts.CompanyBriefEvidence{{
	...
	moved := []crmcontracts.CompanyBriefEvidence{{
...
func TestFingerprintShapeIsWhatTheDismissalAccepts(t *testing.T) {
	produced := fingerprint(string(suggestNoReply), testCompanyID(t).String(), nil)
