...
func TestTheFingerprintIgnoresTheTitleAndTheDate(t *testing.T) {
	...
	before := fingerprint("no_reply", "company-1", evidence)

	// Whatever a title or a date says, the situation is the same situation.
	after := fingerprint("no_reply", "company-1", evidence)
	if before != after {
	...
	companyID := ids.CompanyID{UUID: ids.NewV7()}
	msg := lastMessage{
		ID:        ids.NewV7(),
	...
	first := staleThread(companyID, now, msg)
	...
	later := staleThread(companyID, now.Add(24*time.Hour), msg)
	if later == nil {
...
func TestATitleSaysWhatTheRuleKnows(t *testing.T) {
	now := time.Date(2026, 8, 10, 12, 0, 0, 0, time.UTC)
	companyID := ids.CompanyID{UUID: ids.NewV7()}
	out := staleThread(companyID, now, lastMessage{
		ID:        ids.NewV7(),
...
func TestARuleThatFiresOnAnAbsenceCarriesNoDate(t *testing.T) {
	companyID := ids.CompanyID{UUID: ids.NewV7()}
	out := noNextStepSuggestion(companyID, suggestionInputs{
