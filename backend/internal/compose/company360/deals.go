...
func openDealsWhere(companyPos int, dealScope string) string {
	return fmt.Sprintf(
		`WHERE d.company_id = $%d AND d.status = 'open' AND d.archived_at IS NULL AND (%s)`,
		companyPos, dealScope)
}
...
func dealsSection(ctx context.Context, tx pgx.Tx, companyID ids.CompanyID, now time.Time,
	baseCurrency string,
) (crmcontracts.Company360Deals, error) {
	var args []any
	...
	open, err := pgx.CollectRows(rows, func(row pgx.CollectableRow) (crmcontracts.Company360Deal, error) {
		var d crmcontracts.Company360Deal
		...
		d.Status = crmcontracts.Company360DealStatus(status)
		if stageIDPtr != nil {
			stageID = *stageIDPtr
	...
	lifetime, lost, err := closedTotals(ctx, tx, companyID, baseCurrency)
	if err != nil {
		return crmcontracts.Company360Deals{}, err
	}
	return crmcontracts.Company360Deals{
		Data:        open,
		Page:        page,
...
func closedTotals(ctx context.Context, tx pgx.Tx, companyID ids.CompanyID,
	baseCurrency string,
