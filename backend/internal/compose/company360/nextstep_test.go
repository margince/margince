...
func TestTheAskIsTheStepTheButtonWrites(t *testing.T) {
	...
	out := noNextStepSuggestion(company, suggestionInputs{
...
func TestAnAccountStepCarriesTheAccountsName(t *testing.T) {
	...
	nameless := noNextStepSuggestion(company, suggestionInputs{
...
func TestTheStepHangsOnTheDealOnlyWhenThereIsOne(t *testing.T) {
	...
	sole := noNextStepSuggestion(company, suggestionInputs{open: pipeline{
		OpenCount: 1, OpenDigest: "d",
	...
	several := noNextStepSuggestion(company, suggestionInputs{open: pipeline{
		OpenCount: 2, OpenDigest: "d",
	...
	if kind, id := soleLink(t, *several.Action.Task); kind != "company" || ids.UUID(id) != company.UUID {
		t.Errorf("the step links %s %v, want the account %v", kind, id, company.UUID)
	}
	...
	// hang it on.
	short := noNextStepSuggestion(company, suggestionInputs{open: pipeline{
		OpenCount: 1, OpenDigest: "d",
	}})
	if short == nil || short.Action == nil || short.Action.Task == nil {
		t.Fatalf("the rule prepared no step: %+v", short)
	}
	if kind, _ := soleLink(t, *short.Action.Task); kind != "company" {
