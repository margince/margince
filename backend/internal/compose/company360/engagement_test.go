...
func TestEachSuggestionNamesWhatPerformingItMeans(t *testing.T) {
	now := time.Date(2026, 8, 3, 12, 0, 0, 0, time.UTC)
	company := ids.From[ids.CompanyKind](ids.NewV7())
	unanswered := ids.NewV7()

	stale := staleThread(company, now, lastMessage{
	...
	next := noNextStepSuggestion(company, suggestionInputs{open: pipeline{OpenCount: 2, OpenDigest: "d"}})
...
func TestNoNextStepNamesTheDealsItWasReadFrom(t *testing.T) {
	...
	one := noNextStepSuggestion(company, suggestionInputs{open: pipeline{
	...
	two := noNextStepSuggestion(company, suggestionInputs{open: pipeline{
	...
	bare := noNextStepSuggestion(company, suggestionInputs{open: pipeline{OpenCount: 4, OpenDigest: "d"}})
	if bare == nil || len(bare.Evidence) != 1 ||
		bare.Evidence[0].EntityType != "company" {
