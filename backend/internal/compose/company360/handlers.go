...
// DraftIntroRequest implements POST /companies/{id}/intro-request-draft.
...
func (h Handlers) GetCompany360(w http.ResponseWriter, r *http.Request, id crmcontracts.Id, params crmcontracts.GetCompany360Params) {
	...
	view, err := h.svc.AssembleScoped(r.Context(), ids.From[ids.CompanyKind](ids.UUID(id)), opts)
	if err != nil {
		httperr.Write(w, r, err)
		return
	}
...
// ListCompanyContacts implements GET /companies/{id}/contacts.
func (h Handlers) ListCompanyContacts(w http.ResponseWriter, r *http.Request, id crmcontracts.Id,
	...
	page, err := h.svc.ContactPage(r.Context(), ids.From[ids.CompanyKind](ids.UUID(id)), q)
	if err != nil {
		httperr.Write(w, r, err)
		return
	}
