...
func TestGetCompany360RefusesAnOverlayWorkspace(t *testing.T) {
	h := NewHandlers(nil, overlayModeReturning(true, nil))
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/v1/companies/x/360", nil)

	h.GetCompany360(rec, req, crmcontracts.Id(ids.NewV7()), crmcontracts.GetCompany360Params{})
...
func TestAcknowledgeCompanyViewRefusesAnOverlayWorkspace(t *testing.T) {
	h := NewHandlers(nil, overlayModeReturning(true, nil))
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/v1/companies/x/view-ack", nil)

	h.AcknowledgeCompanyView(rec, req, crmcontracts.Id(ids.NewV7()))
...
func TestDismissCompanySuggestionRefusesAnOverlayWorkspace(t *testing.T) {
	h := NewHandlers(nil, overlayModeReturning(true, nil))
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/v1/companies/x/suggestions/dismiss",
		strings.NewReader(`{"fingerprint":"`+strings.Repeat("a", 64)+`"}`))
	req.Header.Set("Content-Type", "application/json")

	h.DismissCompanySuggestion(rec, req, crmcontracts.Id(ids.NewV7()))
...
func TestGetCompany360RefusesWhenTheModeCannotBeResolved(t *testing.T) {
