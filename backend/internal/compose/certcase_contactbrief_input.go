...
// The fixture summarize/contact_brief is prepared from, and the refusals that
...
// contactBriefFixture is one relationship as the brief reads it.
type contactBriefFixture struct {
	...
	Deal            *contactBriefDeal      `json:"deal"`
	Changes         []contactBriefChange   `json:"changes"`
	Messages        []contactBriefMessage  `json:"messages"`
	Claims          []contactBriefClaimRow `json:"claims"`
}

// contactBriefDeal is the commercial stake this contact sits on.
type contactBriefDeal struct {
	Name        string `json:"name"`
...
// contactBriefChange is one thing that moved about the relationship.
type contactBriefChange struct {
	Kind string `json:"kind"`
	Days int    `json:"days"`
}

// contactBriefMessage is one captured message, labelled so the expectation can
...
type contactBriefMessage struct {
...
// contactBriefClaimRow is one thing this contact said, as the extractor read it.
type contactBriefClaimRow struct {
	Kind      string `json:"kind"`
...
// contactBriefExpectation is what a right brief looks like.
type contactBriefExpectation struct {
...
func contactBriefInput(f contactBriefFixture) (contactbrief.Input, map[string]string) {
	...
	if f.Deal != nil {
		in.OpenDeal = &contactbrief.DealIn{
	...
	for _, change := range f.Changes {
		in.Changes = append(in.Changes, contactbrief.ChangeIn{
...
// `Recent[0]` as the newest message; a scenario written the way contacts write
...
func foldFixtureMessages(
	in *contactbrief.Input, f contactBriefFixture, now time.Time,
) map[string]string {
	...
	slices.SortStableFunc(ordered, func(a, b contactBriefMessage) int {
...
func refuseUnpreparableBrief(f contactBriefFixture, want contactBriefExpectation) error {
	if len(f.Messages) < 2 {
		return fmt.Errorf(
			"summarize/contact_brief: the fixture supplies %d message(s); with fewer than two there is no wrong one to cite",
			len(f.Messages))
	}
	if strings.TrimSpace(want.NamesToken) == "" {
		return errors.New(
			"summarize/contact_brief: the expectation names no relationship-specific token, so generic prose would satisfy it")
	}
	...
	if !strings.Contains(strings.ToLower(fixtureWords(f)), token) {
		return fmt.Errorf(
			"summarize/contact_brief: the expectation's token %q appears in nothing the summary carries, so only an invented brief could name it",
			want.NamesToken)
	}
	...
	for _, avoided := range want.Avoids {
		if strings.TrimSpace(avoided) == "" {
			return errors.New("summarize/contact_brief: the expectation forbids a blank phrase, which every reply contains")
		}
	}
	return nil
}
...
func refuseUnnameableMessages(f contactBriefFixture, want contactBriefExpectation) error {
	seen := map[string]bool{}
	for i, message := range f.Messages {
		if strings.TrimSpace(message.Label) == "" {
			return fmt.Errorf("summarize/contact_brief: the message at position %d carries no label", i+1)
		}
		if seen[message.Label] {
			return fmt.Errorf(
				"summarize/contact_brief: two messages are labelled %q, so an expectation naming it means neither",
				message.Label)
		}
	...
	if !seen[want.CitesLabel] {
		return fmt.Errorf(
			"summarize/contact_brief: the expectation names %q, which the fixture does not carry — no reply could satisfy it",
			want.CitesLabel)
	}
	return nil
}
...
func fixtureWords(f contactBriefFixture) string {
