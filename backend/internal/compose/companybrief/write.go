...
func Write(ctx context.Context, lane Completer, companyID string, in Input, lang string) ([]Section, crmcontracts.WrittenBy, error) {
	...
	written, err := writeWithModel(ctx, lane, companyID, in, lang)
	if err != nil {
...
func accountEvidence(companyID string) []Evidence {
	return []Evidence{{EntityType: citeCompany, EntityID: companyID}}
}
...
// as instruction.
//
// The company profile RIDES this request, unlike the one AskRequest sends: the
// brief is allowed to assess, and a fit assessment cannot be written without
// knowing what we sell (see Input.withoutProfile). What keeps that honest is
// the nature label on every claim, not withholding the text.
...
//
// companyID pins the one account this brief is about, so a company
// citation cannot name a different one.
func ParseBrief(text, companyID string, in Input) ([]Sentence, error) {
	var reply struct {
	...
	return keepGroundedSentences(reply.Sentences, companyID, in), nil
}

func writeWithModel(ctx context.Context, lane Completer, companyID string, in Input, lang string) ([]Section, error) {
	req := BriefRequest(in, lang)
	resp, err := ai.Ask(ctx, lane, req, func(text string) error {
		_, err := ParseBriefSections(text, companyID, in)
		return err
	})
	if err != nil {
		return nil, err
	}
	kept, err := ParseBriefSections(resp.Text, companyID, in)
	if err != nil {
		return nil, err
	}
	if len(kept) == 0 {
...
func ParseBriefSections(reply, companyID string, in Input) ([]Section, error) {
	var parsed struct {
	...
	known := knownRecords(companyID, in)
...
// company citation would let a reply hand back an id this reader never
// saw — rendered as a link they could click into a record their scope may
// hide. The one company a brief may cite is the one it is about.
...
func keepGroundedSentences(sentences []Sentence, companyID string, in Input) []Sentence {
	return claims.Keep(sentences, knownRecords(companyID, in), knownNature, natureFact)
}
...
func knownRecords(companyID string, in Input) map[Evidence]string {
	known := map[Evidence]string{{EntityType: citeCompany, EntityID: companyID}: accountItself(in)}
...
//
// The whole input would make a citation of the company a skeleton key —
...
const (
	sectionSnapshot = string(crmcontracts.CompanyBriefSectionKindSnapshot)
	sectionFit      = string(crmcontracts.CompanyBriefSectionKindFit)
	sectionHealth   = string(crmcontracts.CompanyBriefSectionKindHealth)
	sectionActivity = string(crmcontracts.CompanyBriefSectionKindActivity)
	sectionNextStep = string(crmcontracts.CompanyBriefSectionKindNextStep)
)
