...
const groundedBriefReply = `{"sections":[
	{"kind":"snapshot","sentences":[
		{"text":"They sell managed hosting.","evidence":[{"entity_type":"company","entity_id":"` +
	briefCompanyID + `"}]}]}]}`
...
func TestTheBriefWriterReAsksThroughItsOwnParse(t *testing.T) {
	...
	sections, err := writeWithModel(t.Context(), lane, briefCompanyID, inputFixture(), string(textlang.English))
	if err != nil {
...
func TestTheQuestionAnswererReAsksThroughItsOwnParse(t *testing.T) {
	...
	lane := &aitest.ReAsking{
		...
		Second: `{"sentences":[{"text":"They sell managed hosting.","evidence":[` +
			`{"entity_type":"company","entity_id":"` + askCompanyID + `"}]}]}`,
	}
	answered, err := answerWithModel(t.Context(), lane, declaredQuestions(t)[0], askCompanyID,
