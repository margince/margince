...
const floorVersion = "company-brief-floor-v6"
...
// Digested at ONE fixed language, the same way dealstatus and companydossier do it.
...
func FromView(view crmcontracts.Company360) Input {
	...
	if view.Company.SizeBand != nil {
		in.SizeBand = string(*view.Company.SizeBand)
	}
...
func foldDeals(view crmcontracts.Company360, in *Input) {
	if view.Deals == nil {
		return
	}
...
func foldTasks(view crmcontracts.Company360, in *Input) {
...
func foldRecent(view crmcontracts.Company360, in *Input) {
...
//
// It hashes the INPUT rather than the company's row version, because
