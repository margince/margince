...
// that setting nothing about the account has moved — every other component of
// the key is identical — so a key that did not carry the language would report
// a hit and serve the old-language brief indefinitely, until some unrelated
...
func TestTheBriefFingerprintMovesWhenTheInstallationChangesLanguage(t *testing.T) {
	...
	if english == german {
		t.Error("the same account hashes identically in English and German, so an installation that " +
			"switched language would keep serving its old-language briefs until an unrelated fact moved")
	}
	...
	if again != german {
		t.Error("the same account and the same language hashed differently twice: no brief would ever be reused")
	}
}
...
// produce exactly that for some pair without failing the two-language case above.
func TestEveryShippedLanguageKeysTheBriefDistinctly(t *testing.T) {
	...
	for _, lang := range textlang.Shipped {
		...
		if other, clash := seen[key]; clash {
			t.Errorf("languages %q and %q share a cache key, so one's brief would be served as the other's", other, lang)
		}
