...
// Pure mechanics behind GET /companies/{id}/hierarchy-rollup
// (RD-T04): the RBAC-aware BFS prune over the parent→children company
...
// gated tree walk and measures are in companyrollupread.go, the HTTP
...
// companyTreeNode is one row of the flattened company hierarchy:
...
// pruneUnreadable walks the company tree breadth-first from rootID over the
...
func pruneUnreadable(rootID ids.UUID, nodes []companyTreeNode, readable func(ids.UUID) bool) (included []ids.UUID, restricted []restrictedNode, rootReadable bool) {
	included = []ids.UUID{}
	...
	childrenByParent := make(map[ids.UUID][]companyTreeNode, len(nodes))
	for _, n := range nodes {
		if n.parentID == nil {
			continue
		}
...
// on any surface and not only where companyrollup_handlers runs.
//
