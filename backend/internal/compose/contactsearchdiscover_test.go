...
// it may write into a contact's record is asserted rather than assumed.
...
// contact, so they are dropped rather than filed onto the record.
...
// are not contacts. Filing one onto a contact would be a confident claim about
...
func TestIsProfileURLAcceptsOnlyPersonalProfiles(t *testing.T) {
	if isProfileURL("https://www.linkedin.com/company/scalecommerce") {
		t.Error("a company page was accepted as a contact's profile")
	}
	if isProfileURL("https://www.linkedin.com/jobs/view/123") {
		t.Error("a job posting was accepted as a contact's profile")
	}
...
func TestMentionsNameRequiresEveryPartOfTheName(t *testing.T) {
	...
	if mentionsName(result, "Anna Weber") {
		t.Error("a different contact at the same company was accepted as a match")
	}
	if !mentionsName(result, "Markus Weber") {
		t.Error("the named contact was not recognised in their own result")
	}
}
