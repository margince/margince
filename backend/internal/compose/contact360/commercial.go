...
// What is commercially at stake with this contact, and who else is in the room.
//
// The deal-roles section beside this one lists every seat this contact holds;
...
func (s *Service) commercialSection(ctx context.Context, tx pgx.Tx, contactID ids.ContactID, out *crmcontracts.Contact360) error {
	...
	commercial := crmcontracts.Contact360Commercial{Committee: []crmcontracts.Contact360CommitteeMember{}}
	seat, found, err := s.leadingDealSeat(ctx, tx, contactID)
	if err != nil {
		return err
	}
	if !found {
...
// refuse to show.
func (s *Service) leadingDealSeat(ctx context.Context, tx pgx.Tx, contactID ids.ContactID) (dealSeat, bool, error) {
	var args []any
	...
	// seat held by a contact this caller may not read.
...
//
// Each row carries the contact row-scope predicate: a colleague the caller may
...
func (s *Service) committeeFor(ctx context.Context, tx pgx.Tx, contactID ids.ContactID, dealID ids.UUID) ([]crmcontracts.Contact360CommitteeMember, error) {
	var args []any
	arg := func(v any) int { args = append(args, v); return len(args) }
	dealPos := arg(dealID)
	contactPos := arg(contactID)
	...
	contactScope, err := auth.ScopeClauseFor(ctx, "contact", "p", arg)
	if err != nil {
		return nil, err
	}
	if contactScope == "" {
		contactScope = scopeAll
	}
	rows, err := tx.Query(ctx, fmt.Sprintf(`
		...
		WHERE r.kind = 'deal_stakeholder' AND r.deal_id = $%d
		  AND r.contact_id <> $%d AND r.archived_at IS NULL AND (%s) AND (%s)
		ORDER BY p.full_name, p.id
		LIMIT %d`, dealPos, contactPos, edgeBound, contactScope, committeeCap), args...)
	if err != nil {
		return nil, fmt.Errorf("read the buying committee: %w", err)
	}
	defer rows.Close()

	committee := make([]crmcontracts.Contact360CommitteeMember, 0, committeeCap)
	for rows.Next() {
...
// contactPhotoURL points at the stream endpoint when a photo was uploaded, and
...
func contactPhotoURL(contactID ids.UUID, objectKey *string) *string {
	if objectKey == nil || *objectKey == "" {
		return nil
	}
	url := fmt.Sprintf("/v1/contacts/%s/photo", contactID)
	return &url
}
