...
// answer "what happened with this contact"; these answer "what does the product
...
func (s *Service) profileFieldsSection(ctx context.Context, tx pgx.Tx, contactID ids.ContactID, out *crmcontracts.Contact360) error {
...
// readProfileFields is every read of contact_profile_field that RENDERS it to a
...
func (s *Service) readProfileFields(ctx context.Context, tx pgx.Tx, contactID ids.ContactID) ([]crmcontracts.ContactProfileField, error) {
	...
	return s.applyFieldVerdicts(ctx, tx, contactID, out)
}
...
func (s *Service) applyFieldVerdicts(
	...
	fields []crmcontracts.ContactProfileField,
) ([]crmcontracts.ContactProfileField, error) {
	verdicts, err := s.feedback.VerdictsForTx(ctx, tx, "contact", contactID.UUID)
	if err != nil {
		return nil, err
	}
	for i := range fields {
		...
		verdict := crmcontracts.ContactProfileFieldVerdict(decision.Verdict)
		f.Verdict = &verdict
