...
// The deliverability section: which of the contact's addresses last refused a
...
// deadAddressesSection derives which of the contact's addresses last refused a
...
func (s *Service) deadAddressesSection(ctx context.Context, tx pgx.Tx, out *crmcontracts.Contact360) error {
	// The store is asked even for a contact with no addresses: its grant
	...
	if out.Contact.Emails != nil {
		addresses = make([]string, 0, len(*out.Contact.Emails))
		for _, email := range *out.Contact.Emails {
