...
func TestAFailedRunOnAnUnlookupableContactIsNotAProviderFault(t *testing.T) {
	...
	if got != crmcontracts.ContactProviderProfileStateNothingToLookUp {
...
func TestAFailedRunOnAFindableContactStillReadsAsAProviderFault(t *testing.T) {
	...
	if got != crmcontracts.ContactProviderProfileStateProviderError {
...
func TestTheConnectionsOwnFaultOutranksAnUnlookupableRecord(t *testing.T) {
	...
	for status, want := range map[string]crmcontracts.ContactProviderProfileState{
		"invalid_credentials":  crmcontracts.ContactProviderProfileStateInvalidCredentials,
		"insufficient_credits": crmcontracts.ContactProviderProfileStateInsufficientCredits,
		"rate_limited":         crmcontracts.ContactProviderProfileStateRateLimited,
	} {
...
func TestADisconnectedProviderStillSaysStaleOverAnUnlookupableRecord(t *testing.T) {
	...
	if got != crmcontracts.ContactProviderProfileStateStale {
		t.Errorf("state = %q, want stale: this contact has runs on record, and what was bought is still "+
...
func TestAPaidRunIsNotHiddenBehindAMissingIdentifier(t *testing.T) {
	...
	for _, c := range []struct {
		name string
		run  providerRunRow
		want crmcontracts.ContactProviderProfileState
	}{
		{
			...
			want: crmcontracts.ContactProviderProfileStateCompletedClaimsUnwritten,
		},
		{
			name: "outcome never learned",
			run:  providerRunRow{state: string(provider.RunSubmissionUnknown)},
			want: crmcontracts.ContactProviderProfileStateSubmissionUnknown,
		},
		{
			...
			want: crmcontracts.ContactProviderProfileStateCompleted,
		},
	} {
