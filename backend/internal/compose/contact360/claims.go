...
func (s *Service) claimsSection(ctx context.Context, tx pgx.Tx, contactID ids.ContactID, opts AssembleOptions, out *crmcontracts.Contact360) error {
	if err := requireRead(ctx, "activity"); err != nil {
		return err
	}
	claims, err := s.contacts.ClaimsForContact(ctx, tx, contactID, opts.ProjectID, sectionCap)
	if err != nil {
		return err
	}
	owed, err := s.openCommitments(ctx, tx, contactID, opts)
	if err != nil {
		return err
	}
...
func (s *Service) openCommitments(
	ctx context.Context, tx pgx.Tx, contactID ids.ContactID, opts AssembleOptions,
) ([]crmcontracts.ConversationClaim, error) {
	if err := requireRead(ctx, "activity"); err != nil {
		return nil, err
	}
	return s.contacts.OpenCommitmentsForContact(ctx, tx, contactID, opts.ProjectID, commitmentScanCap)
}
