...
// The company_scan row: one per (reader, account), the carrier of the read in
...
const rowColumns = `id, user_id, company_id, status, attempt, requested_at, started_at, finished_at,
...
func scanRow(rows pgx.Row) (row, error) {
	var r row
	var findings []byte
	err := rows.Scan(&r.ID, &r.UserID, &r.CompanyID, &r.Status, &r.Attempt, &r.RequestedAt, &r.StartedAt,
...
func load(ctx context.Context, tx pgx.Tx, userID ids.UserID, companyID ids.CompanyID) (row, bool, error) {
	r, err := scanRow(tx.QueryRow(ctx,
		`SELECT `+rowColumns+` FROM company_scan WHERE user_id = $1 AND company_id = $2`, userID, companyID))
	if errors.Is(err, pgx.ErrNoRows) {
		return row{}, false, nil
	}
	if err != nil {
		return row{}, false, fmt.Errorf("companyscan: read the scan: %w", err)
	}
	return r, true, nil
}
...
func queue(ctx context.Context, tx pgx.Tx, userID ids.UserID, companyID ids.CompanyID) (row, bool, error) {
	r, err := scanRow(tx.QueryRow(ctx, `
		INSERT INTO company_scan (user_id, company_id, status)
		VALUES ($1, $2, 'queued')
		ON CONFLICT (user_id, company_id) DO UPDATE
		SET status = 'queued', attempt = company_scan.attempt + 1, requested_at = now(),
		    started_at = NULL, finished_at = NULL, next_attempt_at = NULL
		WHERE company_scan.status NOT IN ('queued','running')
		   OR COALESCE(company_scan.started_at, company_scan.next_attempt_at, company_scan.requested_at)
		        < now() - ($3 * interval '1 microsecond')
		RETURNING `+rowColumns, userID, companyID, ScanLease.Microseconds()))
	if errors.Is(err, pgx.ErrNoRows) {
		return row{}, false, nil
	}
	if err != nil {
		return row{}, false, fmt.Errorf("companyscan: queue the scan: %w", err)
	}
...
func begin(ctx context.Context, tx pgx.Tx, scanID ids.UUID) (row, bool, error) {
	...
	if err != nil {
		return row{}, false, fmt.Errorf("companyscan: claim the scan: %w", err)
	}
	return r, true, announce(ctx, tx, r)
}
...
func deferBudget(ctx context.Context, tx pgx.Tx, h held, next time.Time) error {
	...
	if err != nil {
		return fmt.Errorf("companyscan: defer the scan: %w", err)
	}
...
func settle(ctx context.Context, tx pgx.Tx, scanID ids.UUID, claimedAt *time.Time, out outcome) error {
	...
	if err != nil {
		return fmt.Errorf("companyscan: encode the findings: %w", err)
	}
	r, err := scanRow(tx.QueryRow(ctx, `
		UPDATE company_scan
	...
	if err != nil {
		return fmt.Errorf("companyscan: settle the scan: %w", err)
	}
	return announce(ctx, tx, r)
}
...
func fail(ctx context.Context, tx pgx.Tx, h held, reason string) error {
	...
	if err != nil {
		return fmt.Errorf("companyscan: fail the scan: %w", err)
	}
	return announce(ctx, tx, r)
}
...
func announce(ctx context.Context, tx pgx.Tx, r row) error {
	...
	if err != nil {
		return fmt.Errorf("companyscan: log the scan's state change: %w", err)
	}
	var label *string
	if err := tx.QueryRow(ctx,
		`SELECT left(display_name, $2) FROM company WHERE id = $1`, r.CompanyID, subjectLabelBound,
	).Scan(&label); err != nil && !errors.Is(err, pgx.ErrNoRows) {
		return fmt.Errorf("companyscan: read the account's name for the rail: %w", err)
	}
	task := ActivityAITask
	subjectType := "company"
	subjectID := openapi_types.UUID(r.CompanyID.UUID)
	...
	if err := storekit.EmitPipelinePayload(ctx, tx, ledgerID, payload); err != nil {
		return fmt.Errorf("companyscan: publish the scan's state change: %w", err)
	}
	return nil
}
