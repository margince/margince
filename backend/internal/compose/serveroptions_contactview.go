...
// The contact view's model-backed lanes.
//
...
// other, and folding them into one option would tie a contact-page decision to a
// company-page one.
...
// WithContactBrief binds the summarize lane that writes the contact page's
...
//
// It rebuilds the service over the SAME composite read wireContact360 built, so
// the brief's scope stays the caller's own gated read rather than a second one
// assembled here.
func WithContactBrief(brain completer, routingVersion string) Option {
...
// WithContactDraft binds the lane that writes an email to one contact — the
// contact-side mirror of WithAccountDraft, whose sending half is likewise
// POST /emails.
//
...
// identity behind the envelope. Drafting still writes nothing; contactdraft is
...
func WithContactDraft(brain completer) Option {
	return func(s *Server, pool *pgxpool.Pool) {
		svc := contactdraft.NewService(s.contact360Svc, brain).
...
//
// A separate option from WithContactDraft, and deliberately so: they are two
...
// contactdraft.Write, prompt, voice floor and deterministic fallback behind
