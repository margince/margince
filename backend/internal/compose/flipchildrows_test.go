...
var twoEmailRowsMapping = overlay.ObjectMapping{
	...
	Fields: []overlay.FieldMapping{
		{
			From: []string{"email"}, To: "contact_email.email",
		...
		{
			From: []string{"hs_additional_emails"}, To: "contact_email.email",
			Kind: overlay.TargetChild, Transform: "lowercase",
...
func TestFlipCarriesEveryRowOfAChildCollection(t *testing.T) {
	...
	wantEmails := []contacts.ContactEmailInput{
	...
	wantDomains := []contacts.CompanyDomainInput{
...
func TestFlipSkipsAValuelessChildRowAndKeepsTheRest(t *testing.T) {
	emails := flipContactEmails(map[string]any{"contact_email": []any{
	...
	want := []contacts.ContactEmailInput{{Email: "ada@home.test", EmailType: "personal", Position: 2}}
	...
	want2 := []contacts.CompanyDomainInput{{Domain: "acme.test"}}
...
// contact_email.email_type is CHECK-constrained, so a type the contract does not
...
func TestFlipHoldsEveryEmailRowsTypeToTheContractEnum(t *testing.T) {
	...
	want := []contacts.ContactEmailInput{
		{Email: "ada@example.test", EmailType: "work", IsPrimary: true, Position: 0},
