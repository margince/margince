...
func TestTheRecommendationIsTheHeadOfTheList(t *testing.T) {
	...
	graph := graphWith(
		[]crmcontracts.ContactGraphNode{
			colleague(1, "Thin but direct", crmcontracts.ContactGraphNodeGroupDirect),
			colleague(2, "Busy Ben", crmcontracts.ContactGraphNodeGroupAccount),
...
// Two of their contacts talking to each other is a fact about the account, not a
...
func TestAContactToContactEdgeIsNeverARoute(t *testing.T) {
	...
	graph := graphWith(
		[]crmcontracts.ContactGraphNode{
			...
			colleague(1, "Our quiet rep", crmcontracts.ContactGraphNodeGroupDirect),
		},
		[]crmcontracts.ContactGraphEdge{
	...
	if routes[0].ViaDisplayName != "Our quiet rep" {
		t.Errorf("route goes via %q; a pair of their own contacts is not a door",
...
func TestARouteIdSaysWhichKindItIs(t *testing.T) {
	...
	if direct.RouteType != crmcontracts.ContactGraphRouteTypeContactGraphRouteTypeDirect {
		t.Errorf("the direct route is typed %q", direct.RouteType)
	}
	if direct.ThroughContactId != nil {
		t.Error("a direct route named an intermediary")
	}
	if indirect.RouteType != crmcontracts.ContactGraphRouteTypeContactGraphRouteTypeThroughContact {
		t.Errorf("the indirect route is typed %q", indirect.RouteType)
	}
	if indirect.ThroughContactId == nil {
...
func TestAnExchangeOutranksAOneSidedRelationshipInTheList(t *testing.T) {
	...
	graph := graphWith(
		[]crmcontracts.ContactGraphNode{
			colleague(1, "Talks at them", crmcontracts.ContactGraphNodeGroupDirect),
			colleague(2, "Actually converses", crmcontracts.ContactGraphNodeGroupDirect),
		},
		[]crmcontracts.ContactGraphEdge{
			{
				From: userNodeID(uuidFor(1)), To: anchor,
...
func TestEveryRouteIsOfferableBeforeAnythingHasBeenAsked(t *testing.T) {
	...
	routes := chooseRoutes(graph, graphNow)
	if routes[0].Availability != crmcontracts.ContactGraphRouteAvailabilityAvailable {
