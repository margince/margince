...
// (backend/internal/compose/network/contactroutes_test.go)
func chooseRoutes(out *crmcontracts.ContactGraph, now time.Time) []crmcontracts.ContactGraphRouteCandidate {
	idx := indexNodes(out)
	if idx.anchor == "" {
		return nil
	}

	candidates := []crmcontracts.ContactGraphRouteCandidate{}
...
type nodeIndex struct {
	...
	contacts map[string]*openapi_types.UUID
}

func indexNodes(out *crmcontracts.ContactGraph) nodeIndex {
	idx := nodeIndex{
		labels:   map[string]string{},
		users:    map[string]*openapi_types.UUID{},
		contacts: map[string]*openapi_types.UUID{},
	...
	for i := range out.Nodes {
		...
		idx.contacts[n.Id] = n.ContactId
		if n.Group == crmcontracts.ContactGraphNodeGroupAnchor {
			idx.anchor = n.Id
		}
	}
	return idx
...
// colleague describes two of their contacts talking to each other. It belongs in
// the picture and never in this list, because the list's only verb is "ask
// this contact", and there is nobody here to ask.
func candidateFor(
	e *crmcontracts.ContactGraphEdge, idx nodeIndex, now time.Time,
) (crmcontracts.ContactGraphRouteCandidate, bool) {
	via := idx.users[e.From]
	if via == nil {
		return crmcontracts.ContactGraphRouteCandidate{}, false
	}
	candidate := crmcontracts.ContactGraphRouteCandidate{
		RouteType:      crmcontracts.ContactGraphRouteTypeContactGraphRouteTypeDirect,
		ViaUserId:      *via,
	...
	candidate.RouteType = crmcontracts.ContactGraphRouteTypeContactGraphRouteTypeThroughContact
	candidate.RouteId = fmt.Sprintf("through:%s:%s", *via, *through)
	candidate.ThroughContactId = through
...
func sortCandidates(candidates []crmcontracts.ContactGraphRouteCandidate) {
...
func beatsCandidate(a, b crmcontracts.ContactGraphRouteCandidate) bool {
	aDirect := a.RouteType == crmcontracts.ContactGraphRouteTypeContactGraphRouteTypeDirect
	bDirect := b.RouteType == crmcontracts.ContactGraphRouteTypeContactGraphRouteTypeDirect
	if aDirect != bDirect {
		return aDirect
	}
...
func evidenceOf(e *crmcontracts.ContactGraphEdge, now time.Time) crmcontracts.ContactGraphRouteEvidence {
	ev := crmcontracts.ContactGraphRouteEvidence{
...
func twoWay(e *crmcontracts.ContactGraphEdge) bool {
	return e.Inbound90d != nil && *e.Inbound90d > 0 && e.Outbound90d != nil && *e.Outbound90d > 0
}

func bucketOf(b crmcontracts.ContactGraphEdgeStrengthBucket) *crmcontracts.ContactGraphRouteCandidateStrengthBucket {
	bucket := crmcontracts.ContactGraphRouteCandidateStrengthBucket(b)
	return &bucket
}

func receiptsOf(e *crmcontracts.ContactGraphEdge) *[]crmcontracts.ContactGraphReceipt {
	if e.Receipts == nil {
		return nil
	}
	receipts := *e.Receipts
