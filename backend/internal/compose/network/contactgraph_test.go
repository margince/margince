...
func graphWith(nodes []crmcontracts.ContactGraphNode, edges []crmcontracts.ContactGraphEdge) *crmcontracts.ContactGraph {
	...
	all := append([]crmcontracts.ContactGraphNode{{
		Id:        contactNodeID(anchorID),
		Type:      crmcontracts.ContactGraphNodeTypeContact,
		Group:     crmcontracts.ContactGraphNodeGroupAnchor,
		Label:     "Anna Weber",
		ContactId: &pid,
	}}, nodes...)
	return &crmcontracts.ContactGraph{Nodes: all, Edges: edges}
}

func colleague(b byte, name string, group crmcontracts.ContactGraphNodeGroup) crmcontracts.ContactGraphNode {
	return colleagueNode(uuidFor(b), name, group)
}

func contactNode(b byte, name string) crmcontracts.ContactGraphNode {
	pid := openapi_types.UUID(uuidFor(b))
	return crmcontracts.ContactGraphNode{
		Id:        contactNodeID(uuidFor(b)),
		Type:      crmcontracts.ContactGraphNodeTypeContact,
...
// A user and a contact are different kinds of node. Bare uuids as ids would be
...
func TestNodeIdsKeepUsersAndContactsApart(t *testing.T) {
	same := uuidFor(7)
	if userNodeID(same) == contactNodeID(same) {
