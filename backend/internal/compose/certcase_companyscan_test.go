...
// The account_scan/company_scan certification case. As with the brief's: a case
...
func preparedScan(t *testing.T) *companyScanCase {
	t.Helper()
	prepared, err := companyScanCases{}.Prepare(json.RawMessage(companyScanFixtureJSON), json.RawMessage(`["ask"]`))
	if err != nil {
		t.Fatalf("prepare: %v", err)
	}
	scan, ok := prepared.(*companyScanCase)
	if !ok {
...
func scanReply(t *testing.T, scan *companyScanCase, label, quote string) string {
	t.Helper()
...
func TestCompanyScanCaseSiteIsTheAccountScansOneShot(t *testing.T) {
	site := companyScanCases{}.Site()
	if site.Variant != "company_scan" || string(site.Task) != "account_scan" {
		t.Errorf("Site() = %+v, want the account_scan/company_scan one-shot", site)
	}
}

func TestCompanyScanCaseMintsTheIdsTheModelSeesAndSendsTheProductionRequest(t *testing.T) {
...
func TestCompanyScanCaseCarriesTheLanesFailure(t *testing.T) {
	scan := preparedScan(t)
...
func TestCompanyScanCaseRefusesWhatNoReplyCouldFail(t *testing.T) {
	for name, tc := range map[string]struct{ fixture, expected string }{
		"nothing expected":                  {companyScanFixtureJSON, `[]`},
		"a message the fixture lacks":       {companyScanFixtureJSON, `["a_message_that_is_not_there"]`},
		"a contact rather than an exchange": {companyScanFixtureJSON, `["jonas"]`},
		...
		"an expectation of the wrong shape": {companyScanFixtureJSON, `{"ask":true}`},
	} {
		t.Run(name, func(t *testing.T) {
			if _, err := (companyScanCases{}).Prepare(json.RawMessage(tc.fixture), json.RawMessage(tc.expected)); err == nil {
...
func TestCompanyScanCaseEvaluatesEachOutcome(t *testing.T) {
	scan := preparedScan(t)
