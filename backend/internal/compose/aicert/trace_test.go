...
func TestCertifyTaskWithTraceWritesCandidateAndJudgePayloads(t *testing.T) {
	...
	if _, err := certifyTask(wsContext(t), ai.TaskSummarize, []Scenario{sc}, testCensus(t), ai.ProviderConfig{Provider: ai.ProviderFake, Model: "candidate"}, ai.ProviderConfig{Provider: ai.ProviderFake, Model: "judge"}, ai.ProfileCloudHosted, 1, quietLogger(), &certifyHooks{
...
func TestCertifyTaskWithTraceWritesEveryCallARunMade(t *testing.T) {
	...
	if _, err := certifyTask(wsContext(t), ai.TaskSummarize, []Scenario{sc}, retryCensus(t),
		ai.ProviderConfig{Provider: ai.ProviderFake, Model: "candidate"}, ai.ProviderConfig{Provider: ai.ProviderFake, Model: "judge"}, ai.ProfileCloudHosted, 1, quietLogger(), &certifyHooks{
			candidateOpts: []ai.LocalOption{ai.WithFakeClient(candidateFake)},
			judgeOpts:     []ai.LocalOption{ai.WithFakeClient(judgeFake)},
			trace:         trace,
		}); err != nil {
