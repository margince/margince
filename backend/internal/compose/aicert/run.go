...
func runOnce(ctx context.Context, candidate *ai.Router, candidateRec *traceRecorder, judge *ai.Router, judgeRec *traceRecorder, sc Scenario, task ai.Task, census *aitasks.Registry, log *slog.Logger, trace *payloadTrace, run, attempt int) (runOutcome, error) {
	...
	judged, err := judgeScore(ctx, judge, judgeRec, sc, caseTrace, output, log)
	if err != nil {
	...
	graded.Score = judged.score
	graded.JudgeScores = judged.scores
	graded.Ungraded = judged.ungraded
	graded.JudgeServedModel = judged.servedModel
	graded.JudgeDegraded = judged.degraded
	return graded, nil
}
