...
func TestAICertificationPage(t *testing.T) {
	...
	assertAICertProfilesAreNamed(t, doc.Presets)
...
func renderAICertPage(doc aiCertDoc, verdictRule string, bars []aiCertQualityBar) []byte {
	...
	writeAICertFolded(&page, "How to read the tables below", writeAICertGlossary)
	writeAICertFolded(&page, "Every site, and the best model tested on it", func(page *strings.Builder) {
		writeAICertIndex(page, doc.Sites)
	})
	writeAICertFolded(&page, "Certification by provider and model", func(page *strings.Builder) {
...
// writeAICertFolded wraps a section in a collapsed block. The blank line after
// the summary is what lets GitHub render the tables inside it as tables.
func writeAICertFolded(page *strings.Builder, summary string, section func(*strings.Builder)) {
...
func writeAICertIndex(page *strings.Builder, sites []aiCertSite) {
	fmt.Fprintf(page, "### Index of sites (%d)\n\n", len(sites))
...
func writeAICertStale(page *strings.Builder, sites []aiCertSite) {
	...
	page.WriteString("A record goes stale when a scenario it measured changes, when the prompt\n")
	page.WriteString("this build now builds from that scenario changes, or when how a run is graded\n")
	page.WriteString("changes. The stamp covers all three, so a prompt edit invalidates a record that\n")
	page.WriteString("never mentioned it. A scenario the\n")
...
func writeAICertSites(page *strings.Builder, sites []aiCertSite) {
	...
	page.WriteString("One section per task, and one folded block per site it ships. The scenario table\n")
