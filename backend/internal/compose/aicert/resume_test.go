...
func withJournal(t *testing.T, dir string, now time.Time, fn func(*runJournal)) {
	t.Helper()
	j, err := openRunJournal(context.Background(), dir, testJudgeBinding, ai.ProfileCloudHosted, now, quietLogger())
	if err != nil {
...
func TestCompactionKeepsAnotherJudgesLiveRuns(t *testing.T) {
	...
	otherJudge, err := openRunJournal(context.Background(), dir,
		ai.ProviderConfig{Provider: ai.ProviderFake, Model: "a-different-judge"}, ai.ProfileCloudHosted, fixedResumeNow, quietLogger())
	if err != nil {
...
func TestASecondRunIsRefusedTheResumeDirectoryRatherThanDestroyingIt(t *testing.T) {
	dir := t.TempDir()
	first, err := openRunJournal(context.Background(), dir, testJudgeBinding, ai.ProfileCloudHosted, fixedResumeNow, quietLogger())
	if err != nil {
		t.Fatalf("openRunJournal: %v", err)
	}
	defer func() {
	...
	_, second := openRunJournal(context.Background(), dir, testJudgeBinding, ai.ProfileCloudHosted, fixedResumeNow, quietLogger())
	if second == nil {
...
func TestTheResumeDirectoryIsReleasedForTheNextRun(t *testing.T) {
	...
	second, err := openRunJournal(context.Background(), dir, testJudgeBinding, ai.ProfileCloudHosted, fixedResumeNow, quietLogger())
	if err != nil {
