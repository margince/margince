...
// The page-fact lane's CONTACTS gate, split from the fact and entity gates it
// shares a call with (sitepagefacts.go). A contact is the one claim whose
...
// routinely two different contacts's. So this gate works from a quote the model
...
// attributionQuote checks the model's claim that the page gives THIS contact
...
// bridge two contacts is caught by the verbatim check; one that trims the
// contacts in between is caught by the same check, because the page does not
// read that way either.
//
// The role is held to the same bar as the name. A model that retypes it
// rather than copying it loses the contact — billiger.de's reader produced
...
func attributionQuote(claim pageFactsContact, name, role, pageText string) (string, bool) {
...
//	Geschäftsführer Anna Muster … Prokurist Bernd     (two contacts, two titles)
//
...
// layout for a law.
//
// So the model declares it: alongside the quote it lists every other contact
// printed inside, and an undeclared companion refuses the claim.
//
// The declaration says one specific thing — these contacts share the label the
...
func reachesPastAnother(claim pageFactsContact, quote, name, role string, claims []pageFactsContact) bool {
	quoteNorm := normalizeEvidence(quote)
	...
	for _, other := range claims {
		...
		// contact under a longer spelling ("Anna Muster-Schmidt").
...
func gatePageContacts(parsed pageFactsReply, page crawlPage, idx snippetIndex, drop func(lane, field, value, reason string)) []siteContact {
	var out []siteContact
	contactIndex := map[string]int{}
	for _, p := range parsed.Contacts {
		name := strings.TrimSpace(p.N)
		role := strings.TrimSpace(p.R)
		if name == "" || role == "" {
			drop(laneContacts, p.N, p.R, dropEmptyValue)
			continue
		}
		...
		// the span that shows the role belongs to this contact.
		if _, namedOK := idx.nameInCited(p.E, name); !namedOK {
			drop(laneContacts, name, role, dropValueNotInSnippet)
			continue
		}
		...
		if !quoted || reachesPastAnother(p, evidence, name, role, parsed.Contacts) {
			drop(laneContacts, name, role, dropNameRoleUnlinked)
			continue
		}
		...
		if publishedEmail == "" {
			drop(laneContacts, name, role, dropNoPublishedEmail)
			continue
		}
		// And it has to be an address on THIS site's own domain, because the
		// proposal files the contact at the company whose site was read.
		//
		// A printed address proves contactability, not affiliation. A "what our
		// clients say" wall that prints the quoted contact's own address yields a
		...
		if !addressOnSiteDomain(publishedEmail, page.URL) {
			drop(laneContacts, name, role, dropEmailOffSiteDomain)
			continue
		}
		contact := siteContact{
			Name:            name,
		...
		key := siteContactIdentity(name, publishedEmail)
		if prior, dup := contactIndex[key]; dup {
			// Two claims for one contact, and the reply's ORDER must not decide
			...
			if len(contact.EvidenceSnippet) < len(out[prior].EvidenceSnippet) {
				drop(laneContacts, out[prior].Name, out[prior].Role, dropDuplicate)
				out[prior] = contact
				continue
			}
			drop(laneContacts, name, role, dropDuplicate)
			continue
		}
		contactIndex[key] = len(out)
