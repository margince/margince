...
// The cg:company-auto-enrich trigger against the real queue: a company
...
// (companyautoenrich_test.go), where a nil pool proves the refusal precedes the
// query.
...
func TestAnCompanyEventQueuesTheEnrichPassNow(t *testing.T) {
	...
	trigger := NewCompanyAutoEnrichTrigger(e.Pool, inserter, slog.Default())
	ctx := context.Background()
	kind := CaptureAutoEnrichSweepArgs{}.Kind()

	// A create queues exactly one pass.
	if err := trigger.HandleEvent(ctx, envelopeFor(e.WS, "company.created", "company", ids.NewV7())); err != nil {
		t.Fatalf("company.created: %v", err)
	}
	if n := countJobsOfKind(ctx, t, e.Pool, kind); n != 1 {
		t.Fatalf("company.created queued %d enrich pass(es), want exactly 1", n)
	}
	...
	// company.updated in a loop and every event landing its own row
	// would bury the shared default queue.
	if err := trigger.HandleEvent(ctx, envelopeFor(e.WS, "company.updated", "company", ids.NewV7())); err != nil {
		t.Fatalf("company.updated: %v", err)
	}
	if n := countJobsOfKind(ctx, t, e.Pool, kind); n != 1 {
