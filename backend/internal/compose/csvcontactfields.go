...
// How a contact row becomes the store's inputs, and back again.
//
// Split from csvfields.go because a contact is the one import object whose
// identity lives in a CHILD TABLE. Every other target is a column on the record:
// map it, write it, compare it. An email is a row in contact_email with its own
...
// contactCreateFrom builds the create input for one mapped row.
//
// FullName is composed rather than merely read, because a contact row must carry
...
// names no contact at all.
func contactCreateFrom(fields map[string]string, source string) contacts.CreateContactInput {
	...
	in.Emails = contactEmailsFrom(fields)
	return in
}

// contactUpdateFrom builds the patch for the fields that actually differ.
// Emails stays nil unless the file carried the column, so a row that never
// mentioned an address does not read as "this contact now has none".
func contactUpdateFrom(changed map[string]string) contacts.UpdateContactInput {
	...
	in.Emails = contactEmailsFrom(changed)
	return in
}

// contactFullName is the name ladder contactCreateFrom documents.
func contactFullName(fields map[string]string) string {
...
// contactEmailsFrom reads the single email column a spreadsheet carries into the
// child-row shape the store takes. A file has one Email column, so the address
// it names is the primary work address; a contact's other addresses are a flip
// concern and are preserved on update rather than expressed here.
func contactEmailsFrom(fields map[string]string) []contacts.ContactEmailInput {
	email := strings.TrimSpace(fields[fieldEmail])
	if email == "" {
		return nil
	}
	return []contacts.ContactEmailInput{{
		Email: email, EmailType: contactEmailTypeWork, IsPrimary: true, Position: 1,
...
// contactEmailTypeWork is the kind a spreadsheet's Email column names. A file
// carries one address per contact and no column saying what sort it is, and a
// work address is what a CRM import means.
const contactEmailTypeWork = "work"

// emailsMergedOnto folds the file's single email column onto the addresses the
// contact already holds, so an import that names one address does not archive the
// others.
//
// It is addressMergedOnto's argument applied to child rows: replaceContactEmails
...
//
// Only same-type rows are demoted. uq_contact_email_primary allows one primary
// per (contact_id, email_type), not one per contact — so clearing the flag on
// every retained row would strip a contact's primary PERSONAL address because
...
// the contact's addresses alone rather than send an empty set, which
// replaceContactEmails would read as "archive them all".
func emailsMergedOnto(current []byte, mapped []contacts.ContactEmailInput) ([]contacts.ContactEmailInput, bool, error) {
	if len(mapped) == 0 {
	...
	merged := append([]contacts.ContactEmailInput(nil), mapped...)
	merged[0].Position = 1
	...
	for _, held := range record.Emails {
		...
		kind := held.EmailType
		if kind == "" {
			kind = contactEmailTypeWork
		}
		merged = append(merged, contacts.ContactEmailInput{
...
// that advertises it.
//
// A lead encodes its address as a top-level `email` string. A CONTACT does not:
...
// one to compare against; a contact's other addresses are not expressible in the
...
func storedPrimaryEmail(current map[string]json.RawMessage) json.RawMessage {
	...
	// The primary WORK address, because that is what contactEmailsFrom writes and
	// so the only row the file's column can be compared against. A contact may
	...
	for _, row := range rows {
		if row.IsPrimary && row.EmailType == contactEmailTypeWork {
			chosen = row.Email
			break
		}
	}
	if chosen == "" {
		for _, row := range rows {
			if row.EmailType == contactEmailTypeWork {
				chosen = row.Email
				break
			}
		}
