...
func TestAGermanCorrespondenceGetsAGermanFloorDraft(t *testing.T) {
	draft := contactdraft.Deterministic(contactdraft.Input{
		...
		Recipient: contactdraft.RecipientIn{
...
func TestALongSilenceIsAcknowledged(t *testing.T) {
	fresh := contactdraft.Deterministic(contactdraft.Input{
		Envelope:  envelopeFor(textlang.German, convstate.BandFresh),
		Recipient: contactdraft.RecipientIn{ID: "p1", FirstName: "Marek"},
	})
	stale := contactdraft.Deterministic(contactdraft.Input{
...
func TestOnlyARealThreadSubjectEarnsTheReplyPrefix(t *testing.T) {
	...
	// to ourselves.
	outbound := contactdraft.Deterministic(contactdraft.Input{
		Envelope:  envelopeFor(textlang.English, convstate.BandFresh),
		Recipient: contactdraft.RecipientIn{ID: "p1", FirstName: "Marek"},
		Recent: []contactdraft.ActIn{
