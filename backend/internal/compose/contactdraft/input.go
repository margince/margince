...
// What one draft is written from, in the contact page's own grounding order: the
...
// caller typed outranks a record, and a claim the contact made themselves
...
type Input struct {
	...
	// Deal is the open opportunity this contact sits on, when the caller can see
	...
	// Claims are the things this contact actually said — what they asked for,
	...
	// Meeting is the next scheduled meeting THIS CONTACT is on, when there is
...
type RecipientIn struct {
	...
	// BuyingRole is the seat this contact holds on the deal, as recorded. Never
...
// MeetingIn is the next meeting this contact is on.
//
...
type ActIn struct {
	...
	// Snippet is the opening of the newest INBOUND message on this contact's
	...
	// reaches a contact through activity_link, which records what a message
...
func (in Input) String() string {
	return fmt.Sprintf("contactdraft{to:%q deal:%v claims:%d}",
