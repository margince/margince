...
// whose write runs on its own goroutine (company logo write-back,
...
func TestCompanyLogoWritesBackATrimmedLegacyLogoSoTheNextReadNeedsNoCrop(t *testing.T) {
	...
	// The write-back runs on its own goroutine (handlers_companylogo.go)
	...
	handlers.GetCompanyLogo(second, httptest.NewRequest(http.MethodGet, url, nil).WithContext(ctx), crmcontracts.Id(companyID.UUID))
...
// seedLoggedCompany's own write to the SAME key (the object write-back later
...
// drainAttempted discards signals a setup write already queued (seedLoggedCompany's
...
// ONE write-back for it: streamLogo coalesces by key (handlers_companylogo.go)
...
// identical key.
func TestCompanyLogoWriteBackCoalescesConcurrentReadersOfTheSameUntrimmedKey(t *testing.T) {
	e := Setup(t)
...
// prevent (handlers_companylogo.go). This proves the after-check: a
// replace that lands mid-Put gets its own resurrected object collected, not
// left for nothing to ever reference again.
func TestCompanyLogoWriteBackCollectsItsOwnObjectWhenAReplaceRacesTheWrite(t *testing.T) {
	e := Setup(t)
	inner := newCountingBlobstore()
	blob := newGatedBlobstore(inner)
	handlers := contacts.NewHandlers(e.DB()).WithBlobstore(blob)
	...
	// The anchor company, not a fresh one: a REPLACE that can actually
	...
	staleKey := blobstore.WorkspaceKey(ids.From[ids.WorkspaceKind](e.WS), "company_logo", companyID.String()+"/"+ids.NewV7().String())
	...
	blob.hold(staleKey)
	rec := httptest.NewRecorder()
	handlers.GetCompanyLogo(rec, httptest.NewRequest(http.MethodGet, url, nil).WithContext(ctx), crmcontracts.Id(companyID.UUID))
	if rec.Code != http.StatusOK {
		t.Fatalf("GET logo = %d, want 200: %s", rec.Code, rec.Body.String())
	}
	select {
	...
	replacement := "company_logo/" + companyID.String() + "/" + ids.NewV7().String()
...
func TestCompanyLogoWriteBackLogsRatherThanFailsWhenThePutErrors(t *testing.T) {
	e := Setup(t)
	inner := newCountingBlobstore()
	blob := newGatedBlobstore(inner)
	handlers := contacts.NewHandlers(e.DB()).WithBlobstore(blob)
	...
	// Discards seedLoggedCompany's own "put key" signal above: waitAttempted below
...
// than retried inline.
func TestCompanyLogoWriteBackLogsRatherThanFailsWhenTheCollectingDeleteErrors(t *testing.T) {
	e := Setup(t)
	inner := newCountingBlobstore()
	blob := newGatedBlobstore(inner)
	handlers := contacts.NewHandlers(e.DB()).WithBlobstore(blob)
