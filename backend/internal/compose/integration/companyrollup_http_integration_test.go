...
// HTTP-level coverage for GET /companies/{id}/hierarchy-rollup: the
// handler (compose.companyRollupHandlers.GetCompanyHierarchyRollup) and
// its wire mapping (companyRollupToWire) that companyrollup_integration_test.go
// never drives — that suite calls compose.CompanyHierarchyRollup directly, so
...
//
// The tree/self fixture creates its company tree and its one open deal
// through the real HTTP write paths (companies accept parent_company_id
// on create), never raw SQL — unlike companyrollup_integration_test.go, this
...
// companyRollupMoneyWire mirrors the contract's Money schema.
type companyRollupMoneyWire struct {
	AmountMinor int64  `json:"amount_minor"`
	Currency    string `json:"currency"`
}

// companyRollupRestrictedWire mirrors one restricted_excluded item's wire shape.
type companyRollupRestrictedWire struct {
	ID          string `json:"id"`
	DisplayName string `json:"display_name"`
}

// companyRollupResponseWire mirrors the contract's CompanyHierarchyRollup
...
type companyRollupFxProblem struct {
...
// createCompanyRollupCompany creates one company through the real write path,
...
func createCompanyRollupCompany(t *testing.T, e *apptest.AppEnv, name, parentID string) string {
	...
	var company AnyMap
	if status := e.Call(t, "POST", "/v1/companies", body, nil, &company); status != http.StatusCreated {
		t.Fatalf("create company %q = %d %v", name, status, company)
	}
	return company["id"].(string)
}

// companyRollupOpenStage resolves the workspace-seeded default pipeline's
...
func companyRollupOpenStage(t *testing.T, e *apptest.AppEnv) (pipelineID, stageID string, winProbability int) {
	t.Helper()
...
// createCompanyRollupOpenDeal opens one deal on company through the real write
...
func createCompanyRollupOpenDeal(t *testing.T, e *apptest.AppEnv, pipelineID, stageID, companyID string, amountMinor int64, currency string) {
	t.Helper()
	var deal AnyMap
...
// companyRollupHTTPDealAmountMinor is 10,000.00 in minor units: chosen so
...
const companyRollupHTTPDealAmountMinor = 1_000_000

// assertCompanyRollupTreeHappyPath drives the tree-scope GET on root and checks
...
func assertCompanyRollupTreeHappyPath(t *testing.T, e *apptest.AppEnv, root string, winProbability int) {
	t.Helper()
	var rollup companyRollupResponseWire
	status := e.Call(t, "GET", "/v1/companies/"+root+"/hierarchy-rollup", nil, nil, &rollup)
	...
	wantWeighted := companyRollupHTTPDealAmountMinor / 100 * int64(winProbability)
...
// assertCompanyRollupSelfScope drives scope=self on child and checks it
// reports the child alone — the parent's deal never rolls down.
func assertCompanyRollupSelfScope(t *testing.T, e *apptest.AppEnv, child string) {
	t.Helper()
	var rollup companyRollupResponseWire
	status := e.Call(t, "GET", "/v1/companies/"+child+"/hierarchy-rollup?scope=self", nil, nil, &rollup)
	if status != http.StatusOK {
...
// assertCompanyRollupFXUnavailable seeds a fresh root with an open USD deal and
// no stored USD->EUR rate, and checks the wire shape of the resulting
// fx_rate_unavailable refusal.
func assertCompanyRollupFXUnavailable(t *testing.T, e *apptest.AppEnv, pipelineID, stageID string) {
	t.Helper()
	fxRoot := createCompanyRollupCompany(t, e, "Rollup HTTP FX Root", "")
	createCompanyRollupOpenDeal(t, e, pipelineID, stageID, fxRoot, 10_000, "USD") // no USD->EUR rate on file

	var problem companyRollupFxProblem
	status := e.Call(t, "GET", "/v1/companies/"+fxRoot+"/hierarchy-rollup", nil, nil, &problem)
...
func TestCompanyRollupHTTP(t *testing.T) {
	e := apptest.SetupApp(t)
	e.BootstrapWorkspace(t)
	pipelineID, stageID, winProbability := companyRollupOpenStage(t, e)

	root := createCompanyRollupCompany(t, e, "Rollup HTTP Root", "")
	child := createCompanyRollupCompany(t, e, "Rollup HTTP Child", root)
	createCompanyRollupOpenDeal(t, e, pipelineID, stageID, root, companyRollupHTTPDealAmountMinor, "EUR")

	t.Run("200 tree happy path", func(t *testing.T) {
		assertCompanyRollupTreeHappyPath(t, e, root, winProbability)
	})

	t.Run("200 self scope", func(t *testing.T) {
		assertCompanyRollupSelfScope(t, e, child)
	})
	...
	t.Run("422 fx_rate_unavailable", func(t *testing.T) {
		assertCompanyRollupFXUnavailable(t, e, pipelineID, stageID)
	})

	t.Run("404 nonexistent company", func(t *testing.T) {
		status := e.Call(t, "GET", "/v1/companies/"+ids.NewV7().String()+"/hierarchy-rollup", nil, nil, nil)
		if status != http.StatusNotFound {
			t.Fatalf("nonexistent company rollup status = %d, want 404", status)
		}
	})
}
