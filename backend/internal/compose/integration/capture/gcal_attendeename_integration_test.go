...
//
// A contact minted from a calendar notification addressed to a bare address is
...
// name — and the whole chain from decode to contact row dropped it.
//
...
// that records it, the seam that carries it to the contacts module, and the fill
...
// contactName reads what a contact record is called: the split columns and the
// displayed name, which move together or not at all.
func contactName(t *testing.T, e *integration.SearchEnv, id ids.UUID) (first, last, full string) {
	t.Helper()
	if err := database.WithWorkspaceTx(e.Admin(), e.Pool, func(tx pgx.Tx) error {
		return tx.QueryRow(context.Background(), `
			SELECT coalesce(first_name, ''), coalesce(last_name, ''), full_name
			  FROM contact WHERE id = $1`, id).Scan(&first, &last, &full)
	}); err != nil {
...
func TestACalendarInvitationNamesAContactItsAddressCouldNot(t *testing.T) {
	...
	// Through the real contacts store, with only the local part for a name AND
	// under a CONNECTOR principal — which together are what capture writes for
	// a contact minted from a bare address.
	//
	// The principal is not incidental. captured_by records who minted the row,
	// and completeContactName reads it: a contact a human created is one whose
	...
	buyer, err := store.EnsureContactByEmail(connectorCreator(e), "Buyer", "buyer@acme.com", "manual")
	if err != nil {
		t.Fatalf("seeding the attendee: %v", err)
	}
	if first, last, full := contactName(t, e, buyer); first != "" || last != "" || full != "Buyer" {
		t.Fatalf("seeded contact is %q/%q/%q, want the one-word name a bare address produces", first, last, full)
	}

	syncOneGcalMeeting(t, e)

	first, last, full := contactName(t, e, buyer)
	if first != "Robin" || last != "Buyer" {
		t.Errorf("contact is %q %q, want Robin Buyer — the invitation named them and the fill had both columns empty",
			first, last)
	}
	if full != "Robin Buyer" {
...
// guard is completeContactName's, shared with the mail ladder, and this is the
...
func TestAnInvitationNeverRenamesAContactAHumanNamed(t *testing.T) {
	...
	store := contacts.NewStore(e.DB())
	buyer, err := store.EnsureContactByEmail(contactCreator(e), "Robin Q. Buyer-Smith", "buyer@acme.com", "manual")
	if err != nil {
		t.Fatalf("seeding the attendee: %v", err)
	}
	...
	if _, _, full := contactName(t, e, buyer); full != "Robin Q. Buyer-Smith" {
