...
// Importing a file of CONTACTS, end to end.
//
// Split from csvimport_integration_test.go on the object rather than the line
// count: a contact is the one import object whose identity lives in a child table
...
// knows imports as CONTACTS, through the same dry-run-then-approve flow.
//
...
// contacts rather than leads, and re-running the identical file converges instead
// of reporting an update forever. That last one is the defect a contact object
...
func TestCSVImportOfKnownContactsCreatesContacts(t *testing.T) {
	e := setupImportApp(t)
	contactsBefore := importedContactCount(t, e)
	leadsBefore := leadCount(t, e)

	profile, status := uploadCSV(t, e, "contact", prospectCSV)
	...
	run, runStatus := createRunWithMapping(t, e, "contact", profile.SourceRef,
	...
	if got := importedContactCount(t, e); got != contactsBefore {
		t.Fatalf("the dry run created %d contacts; a validation pass writes nothing", got-contactsBefore)
	}
	...
	if got := importedContactCount(t, e); got != contactsBefore+3 {
		t.Fatalf("contacts = %d, was %d — a `contact` run creates contacts", got, contactsBefore)
	}
	if got := leadCount(t, e); got != leadsBefore {
		t.Fatalf("leads = %d, was %d — a `contact` run does not reach the lead table", got, leadsBefore)
	}
	...
	second, secondStatus := uploadCSV(t, e, "contact", prospectCSV)
	if secondStatus != http.StatusOK {
		t.Fatalf("second upload → %d, want 200", secondStatus)
	}
	rerun, rerunStatus := createRunWithMapping(t, e, "contact", second.SourceRef,
		map[string]string{"Email": "email", "Full Name": "full_name", "Title": "title"})
...
// contact is corrected rather than duplicated — which is the whole point of
// keying a contact file on its addresses. What must never happen is the preview
...
func TestCSVImportOfAClaimedEmailPreviewsWhatTheCommitDoes(t *testing.T) {
	...
	first, status := uploadCSV(t, e, "contact", prospectCSV)
	if status != http.StatusOK {
		t.Fatalf("upload → %d, want 200", status)
	}
	...
	second, secondStatus := uploadCSV(t, e, "contact", clash)
	if secondStatus != http.StatusOK {
		t.Fatalf("second upload → %d, want 200", secondStatus)
	}
...
// A contact file naming employers links each contact to their company, and says
...
func TestCSVImportLinksContactsToTheirEmployers(t *testing.T) {
	e := setupImportApp(t)
	...
	// Three contacts: two naming a company that exists, one naming a company
	// nobody has imported.
	...
	run, runStatus := createRunWithMapping(t, e, "contact", profile.SourceRef, mapping)
	if runStatus != http.StatusAccepted {
		t.Fatalf("create run → %d, want 202", runStatus)
	}
	...
	if report.Links == nil {
		t.Fatal("the report carries no links section, so a contact approving it is told nothing about employers")
	}
	...
	// matter: "3 asked for, 1 we cannot make" is the decision a contact is
	...
	// Every contact lands, including the one whose employer was not found: an
	// unresolvable company is a missing link, never a missing contact.
	if got := importedContactCount(t, e); got != 3 {
		t.Fatalf("contacts = %d, want all 3 — a row's employer not resolving must not lose the contact", got)
	}
}
...
//
// The contact is never created, so there is nobody to link. A preview counting
