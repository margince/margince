...
func companyIDsIn(ctx context.Context, t *testing.T, e *Env, spec string) []ids.UUID {
	t.Helper()
	rows := companiesIn(ctx, t, e, spec)
...
func TestTheAccountsListSortsByTheWebsiteItDraws(t *testing.T) {
	...
	nameless := e.SeedCompany(t, "Mercator", &e.Rep1)

	assertIDOrder(t, companyIDsIn(ctx, t, e, "website_url"), []ids.UUID{zeta, alma, nameless},
	...
	rows := companiesIn(ctx, t, e, "website_url")
...
func TestOrderingAccountsByContactsAgreesWithTheCountTheyShow(t *testing.T) {
	...
	assertIDOrder(t, companyIDsIn(ctx, t, e, "contact_count"), []ids.UUID{quiet, busy},
		"contacts ascending")

	rows := companiesIn(ctx, t, e, "-contact_count")
	if len(rows) != 2 {
...
func TestAWithheldCountOrdersTheAccountsListByNothing(t *testing.T) {
	...
	big := e.SeedCompany(t, "Alma Werke", &e.Rep1)
	small := e.SeedCompany(t, "Zeta Holding", &e.Rep1)
	for i := range 3 {
	...
	assertIDOrder(t, companyIDsIn(e.Admin(), t, e, "-open_deal_count"), []ids.UUID{big, small},
		"open deals descending, for a reader who may see them")

	blind := e.As(e.Rep1, []ids.UUID{e.Team1}, AccountRepPerms)
	rows := companiesIn(blind, t, e, "-open_deal_count")
	if len(rows) != 2 {
		t.Fatalf("listed %d accounts, want 2", len(rows))
	}
	for _, o := range rows {
