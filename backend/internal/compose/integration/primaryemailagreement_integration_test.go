...
//
// "The contact's email" is a decision, not a field: a person carries several
...
// seedAddresses gives one person their addresses in the order named, then
// retires the ones asked for — through the real writer, so the row under test
// is one the product really makes.
func seedAddresses(t *testing.T, e *Env, name string, in []people.PersonEmailInput, retire []string) ids.UUID {
	t.Helper()
	p, err := e.People.CreatePerson(e.Admin(), people.CreatePersonInput{
	...
	for _, addr := range retire {
		e.WsExec(t, `UPDATE person_email SET archived_at = now()
		             WHERE person_id = $1 AND email = $2`, ids.UUID(p.Id), addr)
	}
...
func TestTheServedAddressIsTheOneTheSharedRulePicks(t *testing.T) {
	...
	for _, tc := range []struct {
		name    string
		emails  []people.PersonEmailInput
		retired []string
		want    string
	}{
		{
		...
		},
		{
			...
			// different: mail to a retired address bounces, or reaches a person
...
// servedPerson reads one contact off the list — the surface whose cell prints
// the address and whose header orders by it.
func servedPerson(ctx context.Context, t *testing.T, e *Env, id ids.UUID) crmcontracts.Person {
	t.Helper()
	rows, _, err := e.People.ListPeople(ctx, people.ListPeopleInput{})
	if err != nil {
...
// happens to sit first, which is what a sort over `person_email` without the
...
func TestTheContactsListSortsByTheAddressItPrints(t *testing.T) {
	...
	zeta := seedAddresses(t, e, "Zeta Person", []people.PersonEmailInput{
		{Email: "zzz@buyer.test", EmailType: "work"},
		{Email: "alma@buyer.test", EmailType: "work", IsPrimary: true},
	}, nil)
	alma := seedAddresses(t, e, "Alma Person", []people.PersonEmailInput{
	...
	silent := seedAddresses(t, e, "Silent Person", []people.PersonEmailInput{
