...
// margince#3231: every activity write that resolved the row storekit.LiveOnly
// answered 404 for a row a statutory retention hold had archived, even though
...
func TestARetentionHeldActivityAnswers423NotNotFoundOnWrite(t *testing.T) {
	...
	assert423 := func(name string, err error) {
		...
		if errors.Is(err, apperrors.ErrNotFound) {
			t.Fatalf("%s: answered not-found — the row IS there and held, the same lie margince#3231 filed", name)
		}
...
// arm) must not skip the OWNERSHIP gate too. A caller with no relationship
// to the row — not its author, not linked to anything they may write —
// still answers ErrPermissionDenied, exactly as it would for an ordinary
// live row it does not own; a 423 here would be a NEW disclosure the fix
// was never asked to make, telling an unrelated caller a row exists and is
// held. Constructed with a direct INSERT of an already-restricted row
// rather than a live erasure: activity_refuse_restricted_mutation is a
// BEFORE UPDATE/DELETE trigger, so it never fires on this insert, and the
// only thing this test needs is a row that already carries restricted_at
// and archived_at, linked to a person the caller cannot write.
func TestARetentionHeldRowStaysHiddenFromAnUnrelatedWriter(t *testing.T) {
	...
	subject := "rewritten"
	if _, err := e.Activities.UpdateActivity(rep, held, activities.UpdateActivityInput{Subject: &subject}); !errors.Is(err, apperrors.ErrPermissionDenied) {
		t.Fatalf("an unrelated writer on a held row answered %v, want ErrPermissionDenied — "+
			"a 423 here discloses the row exists and is held to a caller who cannot otherwise see it", err)
	}
}
