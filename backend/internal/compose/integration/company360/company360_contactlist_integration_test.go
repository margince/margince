...
func TestContactPageNarrowsByEngagement(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		state := tc.state
		got, err := svc.ContactPage(ctx, ids.CompanyID{UUID: company},
			company360svc.ContactListQuery{Status: &state})
		if err != nil {
...
func TestContactPageSearchesNameAndTitle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		needle := tc.needle
		got, err := svc.ContactPage(ctx, ids.CompanyID{UUID: company},
			company360svc.ContactListQuery{Query: &needle})
		if err != nil {
...
func TestContactPageRefusesACursorFromAnotherOrder(t *testing.T) {
	...
	_, err = svc.ContactPage(ctx, ids.CompanyID{UUID: company}, company360svc.ContactListQuery{
...
//
// The gate is GetCompanyTx, the same call the 360 opens with — so this
...
func TestContactPageHidesAnAccountItCannotRead(t *testing.T) {
	...
	_, err := svc.ContactPage(e.Admin(), ids.CompanyID{UUID: ids.NewV7()},
		company360svc.ContactListQuery{})
	if err == nil {
...
func TestContactPageTreatsAnOmittedSortAsRecommended(t *testing.T) {
	...
	if _, err := svc.ContactPage(ctx, ids.CompanyID{UUID: company}, company360svc.ContactListQuery{
...
func TestContactPageSortsEachColumnBothWays(t *testing.T) {
	...
	ascending, err := svc.ContactPage(ctx, ids.CompanyID{UUID: company},
		company360svc.ContactListQuery{Sort: "name"})
	if err != nil {
		t.Fatalf("sorting by name: %v", err)
	}
	descending, err := svc.ContactPage(ctx, ids.CompanyID{UUID: company},
		company360svc.ContactListQuery{Sort: "-name"})
	if err != nil {
