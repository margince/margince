...
func accountTimeline(ctx context.Context, t *testing.T, e *integration.Env, company ids.UUID, limit int, cursor string) ([]ids.UUID, string) {
	t.Helper()
	entityType := "company"
	in := activities.ListActivitiesInput{EntityType: &entityType, EntityID: &company, Limit: &limit}
	if cursor != "" {
		in.Cursor = &cursor
	}
...
func TestAccountTimelineReachesAMeetingThroughSomebodyInTheRoom(t *testing.T) {
	...
	withLeaver := accountMeetingAt(t, owner, "a call with somebody who has left", company360Clock.Add(-2*time.Hour))
	attend(t, owner, withLeaver, leaver)
	withStranger := accountMeetingAt(t, owner, "another account's meeting", company360Clock.Add(-3*time.Hour))
...
func TestAccountTimelinePagesWithoutDuplicatesOrOmissions(t *testing.T) {
	...
	for i := range 5 {
		mail := accountMailAt(t, owner, e.WS, "thread", company360Clock.Add(-time.Duration(i+1)*time.Hour))
		if i%2 == 0 {
	...
	cursor := ""
	for range 5 {
		got, next := accountTimeline(rep, t, e, company, 2, cursor)
		for _, id := range got {
			seen[id]++
...
func TestSinceLastVisitCountsMailRolledUpThroughAContact(t *testing.T) {
	...
	if view.SinceLastVisit.BaselineAt == nil || !view.SinceLastVisit.BaselineAt.Equal(company360Clock) {
		t.Errorf("baseline_at = %v, want the acknowledged instant %v",
			view.SinceLastVisit.BaselineAt, company360Clock)
	}
}
...
func TestLastTouchSeparatesWhoWroteLastAndWalksTheSameThreeLinks(t *testing.T) {
	...
	if view.LastInboundAt == nil || !view.LastInboundAt.Equal(company360Clock.Add(-30*time.Hour)) {
		t.Errorf("last inbound = %v, want the contact-linked reply at -30h", view.LastInboundAt)
	}
	if view.LastOutboundAt == nil || !view.LastOutboundAt.Equal(company360Clock.Add(-2*time.Hour)) {
...
func TestLastTouchIsNullRatherThanZeroWhenNothingWasCaptured(t *testing.T) {
	...
	rep := e.As(e.Rep1, []ids.UUID{e.Team1}, integration.AccountRepPerms)
	outbound := integration.AccountMailDirectedAt(t, owner, e.WS, "our first mail", "outbound", company360Clock.Add(-time.Hour))
	integration.LinkToCompany(t, e, outbound, company)
