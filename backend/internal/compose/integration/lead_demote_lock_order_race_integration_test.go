...
//
// Both writers touch a lead and the contact it was promoted into. The merge
// locks the two contacts (LockPair) and then repoints lead.promoted_contact_id,
// so it goes contact -> lead. The demote used to go lead -> contact, and two
...
//     the demote would sit on the contact's lock while holding the lead, which
//     is exactly the state the merge deadlocks against;
//   - contact-held (round two) fails if the contact lock is dropped entirely.
...
const (
	holdContactRow = `SELECT 1 FROM contact WHERE id = $1 FOR UPDATE`
	holdLeadRow    = `SELECT 1 FROM lead WHERE id = $1 FOR UPDATE`

	probeLeadForUpdate    = `SELECT 1 FROM lead WHERE id = $1 FOR UPDATE NOWAIT`
	probeContactForUpdate = `SELECT 1 FROM contact WHERE id = $1 FOR UPDATE NOWAIT`
)

func TestTheDemoteTakesTheContactBeforeTheLead(t *testing.T) {
	e := Setup(t)

	t.Run("parked on the contact, it is not holding the lead", func(t *testing.T) {
		theDemoteWaitsForTheContactEmptyHanded(t, e)
	})
	t.Run("parked on the lead, it is already holding the contact", func(t *testing.T) {
		theDemoteReachesTheLeadHoldingTheContact(t, e)
	})
}

// theDemoteWaitsForTheContactEmptyHanded pins the contact — the row the merge
...
// holding the lead is one half of the cycle; a merge that has the contact and
...
func theDemoteWaitsForTheContactEmptyHanded(t *testing.T, e *Env) {
	lead, contact := promotedPair(t, e, "Ada Parked", "ada.parked@prospect.test")

	holder, holderPID := pinRow(t, holdContactRow, contact.UUID)

	demoted := make(chan error, 1)
	demoteReturned := make(chan struct{})
	go func() {
		defer close(demoteReturned)
		_, err := e.Contacts.DemoteLead(e.Admin(), lead, "not a real opportunity")
		demoted <- err
	}()

	waitUntilSomethingQueuesBehind(t, holderPID, demoteReturned,
		"the demote returned without ever waiting on the promoted contact — it does not lock the row "+
			"the merge path locks first, so nothing here orders the two against each other",
		"the demote never queued behind the contact lock: it either reads that row without holding it, "+
	...
	// holding the lead while it waits for the contact — lead-then-contact, the
	...
	if held, err := lockIsRefused(t, probeLeadForUpdate, lead.UUID); err != nil {
		...
		t.Fatal("the demote is waiting on the promoted contact while already holding the lead. " +
			"MergeContact locks the contact first and then writes the lead, so those two orders " +
...
// theDemoteReachesTheLeadHoldingTheContact pins the LEAD and asserts the contact
// is already locked by the time the demote gets there.
//
// Without this half, deleting the contact lock outright passes round one: a
// demote that never touches the contact is trivially "not holding the lead
// while waiting for it", because it never waits. An order is only an order if
// both locks are taken.
func theDemoteReachesTheLeadHoldingTheContact(t *testing.T, e *Env) {
	lead, contact := promotedPair(t, e, "Bo Parked", "bo.parked@prospect.test")
	...
	if held, err := lockIsRefused(t, probeContactForUpdate, contact.UUID); err != nil {
		t.Fatalf("asking whether the parked demote holds the contact: %v", err)
	} else if !held {
		t.Fatal("the demote reached the lead without holding the promoted contact. It takes no lock " +
			"on the row it is about to unwind, so a merge can repoint that contact underneath it — " +
...
func promotedPair(t *testing.T, e *Env, name, email string) (ids.LeadID, ids.ContactID) {
	t.Helper()
	...
	if merged {
		t.Fatalf("%s promoted into an existing contact, so this round would contend over a row "+
			"another round is also using", name)
	}
	return lead, ids.From[ids.ContactKind](ids.UUID(contact.Id))
}
