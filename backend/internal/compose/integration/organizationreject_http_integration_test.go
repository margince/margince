...
func TestRejectingACompanyOverHTTPAnswersBothHalves(t *testing.T) {
	...
	if status := e.Call(t, "POST", "/v1/organizations", map[string]any{
	...
	if status := e.Call(t, "POST", "/v1/organizations/"+created.ID+"/reject", map[string]any{
...
func TestRejectingACompanyOverHTTPRefusesAReasonItCannotStore(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if status := e.Call(t, "POST", "/v1/organizations/"+created.ID+"/reject",
