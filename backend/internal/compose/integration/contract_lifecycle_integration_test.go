...
// it did not.
// The title is the fixture's, not a caller's: every contract this helper
// makes is the same agreement under test.
const activeContractTitle = "MSA 2026"
...
func anActiveContract(t *testing.T, e *Env, company ids.UUID) ids.ContractID {
	...
	first, err := e.Contracts.CreateContract(e.Admin(), contracts.CreateContractInput{
		CompanyID: ids.From[ids.CompanyKind](company),
		Title:     activeContractTitle, ValueBasis: contracts.BasisTotal, Source: "manual",
	})
	if err != nil {
		t.Fatal(err)
	}
