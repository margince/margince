...
// linked contact) refuses BEFORE any write, leaving the slot free.
...
// linked to the contact; extra fields merge over the base.
func bookingAt(start time.Time, contactID string, extra AnyMap) AnyMap {
	body := AnyMap{
		"start":   start,
...
func TestBookingConsentPassthrough(t *testing.T) {
	...
	assertConsentRefusalsLeaveNothingBehind(t, e, contact.ID, tuesday, purposeID)
	assertConsentlessBookingStaysPlain(t, e, contact.ID, tuesday)
	assertConsentedBookingLandsGrantWithProof(t, e, contact.ID, tuesday, purposeID, consent)
}
...
func assertConsentRefusalsLeaveNothingBehind(t *testing.T, e *apptest.AppEnv, contactID string, tuesday time.Time, purposeID string) {
	t.Helper()
	slot := tuesday // 10:00 — reused by every refusal, then proven free

	// An unknown purpose cannot be recorded.
	unknownPurpose := bookingAt(slot, contactID, AnyMap{
		"consent": AnyMap{
	...
	noPolicy := bookingAt(slot, contactID, AnyMap{
	...
	// because this door creates the contact before it records the grant: a
	// refusal further in would leave that row behind, and this door is
	// anonymous.
	noWording := bookingAt(slot, contactID, AnyMap{
	...
	// Consent is contact-keyed: no linked contact, nothing to attach to.
	subjectless := bookingAt(slot, contactID, AnyMap{
		"links": []AnyMap{},
		"consent": AnyMap{
	...
	if status := e.Call(t, "POST", "/v1/bookings", subjectless, nil, nil); status != 422 {
		t.Fatalf("booking consent without a linked contact → %d, want 422", status)
	}

	var meetings, consents int
	if err := e.Owner.QueryRow(context.Background(), `
		SELECT (SELECT count(*) FROM activity WHERE kind = 'meeting'),
		       (SELECT count(*) FROM contact_consent)`).Scan(&meetings, &consents); err != nil {
		t.Fatal(err)
	}
	...
	if status := e.Call(t, "POST", "/v1/bookings", bookingAt(slot, contactID, nil), nil, nil); status != http.StatusCreated {
...
func assertConsentlessBookingStaysPlain(t *testing.T, e *apptest.AppEnv, contactID string, tuesday time.Time) {
	t.Helper()
	var booked struct {
		Kind string `json:"kind"`
	}
	if status := e.Call(t, "POST", "/v1/bookings", bookingAt(tuesday.Add(time.Hour), contactID, nil), nil, &booked); status != http.StatusCreated || booked.Kind != "meeting" {
		t.Fatalf("consentless booking → %d %+v, want a plain 201 meeting", status, booked)
	}
	var consents int
	if err := e.Owner.QueryRow(context.Background(), `SELECT count(*) FROM contact_consent`).Scan(&consents); err != nil {
		t.Fatal(err)
	}
	if consents != 0 {
...
// contact_consent grant carrying the passthrough proof verbatim.
func assertConsentedBookingLandsGrantWithProof(t *testing.T, e *apptest.AppEnv, contactID string, tuesday time.Time, purposeID string, consent AnyMap) {
	t.Helper()
	if status := e.Call(t, "POST", "/v1/bookings",
		bookingAt(tuesday.Add(2*time.Hour), contactID, AnyMap{"consent": consent}), nil, nil); status != http.StatusCreated {
	...
	if err := e.Owner.QueryRow(context.Background(),
		...
		contactID, purposeID).Scan(&state); err != nil {
	...
	if err := e.Owner.QueryRow(context.Background(), `
		...
		contactID, purposeID).Scan(&policyVersion, &policyText); err != nil {
