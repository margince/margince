...
// filed under whoever the attendee resolution reached first. Reading the contact
...
func TestTheContactPageShowsAThreadTheContactWasOnlyCopiedOn(t *testing.T) {
	e := Setup(t)
	author := e.As(e.Rep1, []ids.UUID{e.Team1}, activityLifecyclePerms)
	filedUnder := e.SeedContact(t, "Primary Counterparty", &e.Rep1)
	copied := e.SeedContact(t, "Copied Colleague", &e.Rep1)
	...
	e.WsExec(t, `INSERT INTO activity_participant (id, activity_id, contact_id, role)
	...
	page, err := svc.Assemble(author, ids.From[ids.ContactKind](copied))
	if err != nil {
	...
	filedPage, err := svc.Assemble(author, ids.From[ids.ContactKind](filedUnder))
	if err != nil {
