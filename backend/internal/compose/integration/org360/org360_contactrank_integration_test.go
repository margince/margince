...
//
// The fixture puts the ONLY contact who has written in — waiting on our reply
// — at the end of the id order, which is where the old cut dropped them. A
// reader opening the account then saw twenty-five untried strangers, no way
// in, and a has_more flag that truthfully reported more contacts while saying
// nothing about the one that mattered. Person ids are UUIDv7 and therefore
// ascend in creation order, so seeding that contact last is what places them
// outside the old cut.
func TestOrganization360RanksContactsBeforeTruncatingTheSection(t *testing.T) {
	...
	// The one waiting on a reply leads, because answering them is the move
	// already owed.
	if got := ids.UUID(rows[0].PersonId); got != waiting {
		t.Fatalf("the section opens on %s; the contact waiting on a reply is %s and must lead",
			got, waiting)
	}
}
