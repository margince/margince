...
// What the contact-brief case owes the certification lane: it separates the
...
// contactBriefStub answers from the ids the REQUEST carries, which is the only
...
type contactBriefStub struct{ answer func(prompt string) string }

func (s *contactBriefStub) Complete(_ context.Context, req model.Request) (model.Response, error) {
...
// consequential one.
func contactBriefScenario(t *testing.T) (json.RawMessage, json.RawMessage) {
	t.Helper()
	fixture, err := json.Marshal(contactBriefFixture{
		Name: "Anna Weber", Title: "Head of Operations", Employer: "Acme Logistik GmbH",
		Messages: []contactBriefMessage{
			{
	...
	expected, err := json.Marshal(contactBriefExpectation{
...
func TestTheContactBriefCaseSeparatesTheThreeThingsAReplyCanBe(t *testing.T) {
	t.Parallel()
	fixture, expected := contactBriefScenario(t)
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			prepared, err := contactBriefCases{}.Prepare(fixture, expected)
			if err != nil {
				t.Fatalf("Prepare: %v", err)
			}
			stub := &contactBriefStub{answer: func(prompt string) string { return tc.answer(t, prompt) }}
...
func TestTheContactBriefCaseCatchesAPhraseTheReaderWasNotShown(t *testing.T) {
	t.Parallel()
	fixture, expected := contactBriefScenario(t)
	prepared, err := contactBriefCases{}.Prepare(fixture, expected)
	if err != nil {
	...
	stub := &contactBriefStub{answer: func(prompt string) string {
...
func TestTheContactBriefCaseOrdersFixtureMessagesNewestFirst(t *testing.T) {
	t.Parallel()
	// Written oldest first, which is the order the same two messages carry in
	// contact_brief_reads_what_was_said_01.yaml reversed.
	fixture, err := json.Marshal(contactBriefFixture{
		Name: "Anna Weber",
		Messages: []contactBriefMessage{
			{
...
func TestTheContactBriefCaseRefusesAScenarioThatCouldNotFail(t *testing.T) {
	t.Parallel()
	base, _ := contactBriefScenario(t)
	for _, tc := range []struct {
		name   string
		want   contactBriefExpectation
		detail string
	}{
		{
			name:   "a token the floor already prints",
			want:   contactBriefExpectation{CitesLabel: "objection", NamesToken: "Thursday at ten"},
			detail: "deterministic floor",
		},
		{
			name:   "a token nothing in the summary carries",
			want:   contactBriefExpectation{CitesLabel: "objection", NamesToken: "quantum"},
			detail: "appears in nothing the summary carries",
		},
		{
			name:   "a message the fixture does not carry",
			want:   contactBriefExpectation{CitesLabel: "invoice", NamesToken: "analytics"},
			detail: "which the fixture does not carry",
		},
		{
			name:   "no token at all",
			want:   contactBriefExpectation{CitesLabel: "objection"},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err = contactBriefCases{}.Prepare(base, expected)
			if err == nil {
