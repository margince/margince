...
// The prompt half of captured-company auto-enrich: a company
...
//
// It queues the WORKSPACE PASS, not a read for the event's company.
...
// sweepWorkspace) — and re-derives which companies are due, so this
...
// companyAutoEnrichFreshWindow is how old a company event may be and still
...
// (events.Subscriber), so the first boot replays the company stream's
...
const companyAutoEnrichFreshWindow = time.Hour

// CompanyAutoEnrichTrigger queues one auto-enrich workspace pass per
// company event.
type CompanyAutoEnrichTrigger struct {
	pool    *pgxpool.Pool
	enqueue *jobs.Runner
	log     *slog.Logger
}

// NewCompanyAutoEnrichTrigger builds the trigger over an insert-only jobs runner.
func NewCompanyAutoEnrichTrigger(pool *pgxpool.Pool, enqueue *jobs.Runner, log *slog.Logger) *CompanyAutoEnrichTrigger {
	return &CompanyAutoEnrichTrigger{pool: pool, enqueue: enqueue, log: log}
}
...
// through.
func (g *CompanyAutoEnrichTrigger) HandleEvent(ctx context.Context, env events.Envelope) error {
	if env.Entity.Type != string(recordTypeCompany) {
		return nil
	}
	switch env.Type {
	// Every event that can make a company newly due: appearing,
	// gaining a domain (a company save's domain change arrives as
	// company.updated), or inheriting one in a merge. An archive needs
	...
	if time.Since(env.OccurredAt) > companyAutoEnrichFreshWindow {
		return nil
	}
	// The uniqueness is the flood bound: any authenticated writer can emit
	// company.updated in a loop (the store's emit has no value-changed
	...
	// listed the due companies before the new row landed, and then
