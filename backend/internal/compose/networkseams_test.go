...
func TestAStakeholderSeatCarriesTheirName(t *testing.T) {
	...
	answer := toAgentCoverage(network.DealCoverage{
		...
		Stakeholders: []deals.DealStakeholder{
			{ContactID: athina, Role: "economic_buyer", Engaged: false},
			{ContactID: jim, Role: "champion", Engaged: true},
		},
	...
	if answer.Stakeholders[0].ContactName != "Athina Kanioura" {
		t.Errorf("the economic buyer is unnamed: %+v", answer.Stakeholders[0])
	}
	if answer.Stakeholders[0].ContactID != athina {
...
// TestCoverageWithoutContactReadIsRefusedRatherThanUnnamed proves a caller who
// may not read contacts gets no seats at all, refused upstream. What this pins
// is that if a name is ever missing for an ordinary reason — a contact row with
...
func TestASeatWithNoResolvedNameKeepsItsRole(t *testing.T) {
	...
	answer := toAgentCoverage(network.DealCoverage{
		DealID:       ids.NewV7(),
		Stakeholders: []deals.DealStakeholder{{ContactID: unnamed, Role: "economic_buyer"}},
	...
	if answer.Stakeholders[0].ContactName != "" {
		t.Errorf("a name was invented: %q", answer.Stakeholders[0].ContactName)
	}
...
// A finding names the contacts it is about, so the sentence a model writes has a
// contact in it rather than a uuid.
func TestAFindingNamesTheContactsItIsAbout(t *testing.T) {
	jim := ids.NewV7()
	risks := toAgentRisks([]network.Risk{{
		Kind: "single_threaded_theirs", Summary: "the deal rests on one relationship",
		ContactIDs: []ids.UUID{jim},
	}}, map[ids.UUID]string{jim: "Jim Roth"})

	if len(risks) != 1 {
		t.Fatalf("mapped %d risks, want 1", len(risks))
	}
	if len(risks[0].Contacts) != 1 || risks[0].Contacts[0].Name != "Jim Roth" {
		t.Errorf("the finding names nobody: %+v", risks[0])
	}
	if risks[0].Contacts[0].ContactID != jim {
		t.Error("the name did not travel with its own id")
	}
	if len(risks[0].ContactIDs) != 1 {
		t.Error("naming lost the ids, which stay the handle")
	}
}

// A name that did not resolve cannot shift another name onto the wrong contact.
//
// This is why the finding carries {contact_id, name} objects rather than a
// second array beside contact_ids. Two arrays can diverge — a caller with
// deal:read and no contact:read has ids and no names, and the transaction is
// Read Committed, so a contact archived mid-read leaves one list shorter. A
...
func TestAnUnresolvedNameCannotShiftOntoAnotherContact(t *testing.T) {
	hidden, jim := ids.NewV7(), ids.NewV7()
	risks := toAgentRisks([]network.Risk{{
		Kind: "single_threaded_theirs", Summary: "the deal rests on one relationship",
		ContactIDs: []ids.UUID{hidden, jim},
	}}, map[ids.UUID]string{jim: "Jim Roth"})

	if len(risks[0].ContactIDs) != 2 {
		t.Fatalf("an id was dropped: %+v", risks[0].ContactIDs)
	}
	if len(risks[0].Contacts) != 1 {
		t.Fatalf("contacts %+v — the readable contact is named and the unreadable one skipped",
			risks[0].Contacts)
	}
	// The whole point: the surviving name is attached to ITS OWN id, not to
	// the first id in the list.
	if risks[0].Contacts[0].ContactID != jim {
		t.Errorf("Jim Roth's name landed on %v, which is somebody else", risks[0].Contacts[0].ContactID)
	}
}
...
func TestAFindingWithNoReadableNamesOmitsThem(t *testing.T) {
	risks := toAgentRisks([]network.Risk{{
		Kind: "going_cold", Summary: "nobody is talking", ContactIDs: []ids.UUID{ids.NewV7()},
	}}, map[ids.UUID]string{})

	if risks[0].Contacts != nil {
		t.Errorf("empty names shipped as %+v rather than being omitted", risks[0].Contacts)
	}
}
...
func TestTheSweepNamesEveryFindingInOneRead(t *testing.T) {
	...
	flagged := []agents.AtRiskDeal{
		{DealID: ids.NewV7(), Name: "one", Risks: []agents.CoverageRisk{
			{Kind: "single_threaded_theirs", ContactIDs: []ids.UUID{jim}},
		}},
		{DealID: ids.NewV7(), Name: "two", Risks: []agents.CoverageRisk{
			{Kind: "going_cold", ContactIDs: []ids.UUID{athina, jim}},
		}},
	}
	...
	if len(flagged[0].Risks[0].Contacts) != 1 {
		t.Fatalf("the first deal's finding is unnamed: %+v", flagged[0].Risks[0])
	}
	if len(flagged[1].Risks[0].Contacts) != 2 {
		t.Fatalf("the second deal's finding named %d of 2 contacts — a later deal was skipped",
			len(flagged[1].Risks[0].Contacts))
	}
	if flagged[1].Risks[0].Contacts[0].Name != "Athina Kanioura" {
		t.Errorf("names are attached in the wrong order: %+v", flagged[1].Risks[0].Contacts)
	}
}
