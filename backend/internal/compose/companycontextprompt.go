...
type companyContextReader interface {
	GetAnchorCompanyContext(context.Context, []people.CompanyContextScope) (people.CompanyContext, error)
}
...
func (p *companyContextProvider) Prepare(ctx context.Context, task ai.Task, req model.Request) (model.Request, error) {
	...
	companyContext, err := p.reader.GetAnchorCompanyContext(ctx, scopes)
	if err != nil {
