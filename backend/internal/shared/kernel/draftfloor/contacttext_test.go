...
//
// These helpers stand between a human's typed text and a drafted message a rep
...
// use for one inside a contact's name in a drafted message.
...
// NamesContact matches a WORD, in either case.
//
...
// nobody passes.
func TestNamesContactMatchesAWordAndNotASubstring(t *testing.T) {
	t.Parallel()
	if !NamesContact("Hi SOFIA, could you help?", "Sofia") {
		t.Error("a name in another case was not recognised")
	}
	if NamesContact("Our annual review is due.", "Ann") {
		t.Error(`"Ann" was satisfied by "annual"`)
	}
	if !NamesContact("Ann asked about it.", "Ann") {
	...
	// answer for a contact whose display name is not on file.
	if !NamesContact("anything at all", "") {
...
func TestACombiningMarkExtendsTheWord(t *testing.T) {
	...
	if !NamesContact("Grüße Lena, kurz zum Angebot", "Lena") {
