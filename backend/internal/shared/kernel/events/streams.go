...
// Which STREAMS exist, and how a type's name maps onto one.
//
// Split from the catalog beside it because the two answer different questions
// and change for different reasons: this file changes when the bus gains a
// stream, catalog.go changes every time a verb is added to one that already
// exists. Keeping them together put a §4.1 stream decision in the same file as
// three hundred routing rows, and grew past the file ceiling on the ordinary
// motion of adding an event.
