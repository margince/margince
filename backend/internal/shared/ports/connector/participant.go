...
// contact_email stores one and a case difference would otherwise read as a
...
type MessageParticipant struct {
	...
	// every attendee in full, and a contact minted from a bare address is
	...
	//
	// It is the WEAKEST identity of the four. Capture resolves it to a contact
	// record through contact_channel_identity and never to a colleague's seat:
...
const (
	...
	// misstate who was openly addressed, which is a fact contacts rely on.
...
// hundred contacts is a distribution list, and every name on it is evidence of a
