...
type Pass struct {
	...
	Every time.Duration
	// Running says a pass of this kind is in flight right now. Queued and
	// running are different states to somebody watching a counter that has not
	// moved, and the first is the one that ends by itself.
	Running bool
	...
	const q = `
		SELECT EXISTS (SELECT 1 FROM river_job WHERE kind = $1 AND state::text = 'running'),
		       (SELECT min(scheduled_at) FROM river_job
		         WHERE kind = $1 AND state::text IN ('available','scheduled')
		           AND scheduled_at > now()),
		       (SELECT max(finalized_at) FROM river_job
		         WHERE kind = $1 AND state::text = 'completed')`
	var scheduled, completed *time.Time
	if err := pool.QueryRow(ctx, q, kind).Scan(&out.Running, &scheduled, &completed); err != nil {
