...
// wrote may hold a connection: a saved filter's tree, an agent's query plan,
// a builder's preview.
//
// One number, here, because it answers one question and it was answered twice.
// The filter preview and the search plan each declared five seconds with its
// own paragraph arguing for it — two writers of one invariant, each reasoning
// from the same fact and neither able to see the other move. The reason is not
// a property of either surface: somebody is WAITING on the answer, and a glance
