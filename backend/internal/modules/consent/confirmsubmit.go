...
// token without the answer it carried loses the answer and gives the contact no
...
type ConfirmSubmission struct {
	...
	// the ordinary case — most contacts confirm without changing anything.
	...
	// contact who corrects their address without answering has not consented.
...
func (s *Store) SubmitConfirmation(ctx context.Context, token string, in ConfirmSubmission) ([]RightsCaseReceipt, error) {
	...
	err := s.db.Tx(ctx, func(tx pgx.Tx) error {
		...
		// point. Art. 17 erasure takes the contact and then deletes this
		// subject's confirm_token rows; a transaction taking the token first
		// and the contact second closes a cycle, and nothing in this tree
		...
		// ordering anything.
		contactID, kind, err := s.subjectOfConfirmTokenTx(ctx, tx, token)
		if err != nil {
			return err
		}
		...
		// single-use: a concurrent submit blocks on the contact row first, then
...
// touches the contact record: the subject holds a bearer token and sits outside
...
func stageSubmission(ctx context.Context, tx pgx.Tx, ref ConfirmRef, kind string, field, value *string) (ids.UUID, error) {
	...
	if err := tx.QueryRow(ctx, `
		...
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id`,
		ref.ContactID, ref.TokenID, kind, field, value).Scan(&submissionID); err != nil {
		return ids.UUID{}, err
	}
	// Audited against the CONTACT, because that is the record a later reader
	...
	// field that moved. Nothing about the contact changed — the field a
	// correction proposes still holds its old value, deliberately — so there is
	// no prior state for a before-image to name.
	if _, err := storekit.AuditEvent(ctx, tx, "update", "contact", ref.ContactID.UUID, map[string]any{
...
func stageOneProposal(ctx context.Context, tx pgx.Tx, ref ConfirmRef, kind string,
	...
	reference, err := openRightsCaseTx(ctx, tx, ref.ContactID, submissionID, kind, receivedAt)
	if err != nil {
		return RightsCaseReceipt{}, err
	}
