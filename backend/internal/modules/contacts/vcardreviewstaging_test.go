...
func TestOnlyRefusedCardsReachTheReviewStager(t *testing.T) {
	...
	results := []VCardResult{
		...
		{Index: 1, Outcome: VCardNeedsReview, ContactID: &candidate},
		{Index: 2, Outcome: VCardCreated},
	}
	var staged []VCardEntry
	var candidates []*ids.ContactID
	h := Handlers{}.WithVCardReviewStager(func(_ context.Context, entry VCardEntry, c *ids.ContactID) error {
...
func TestAStagingFaultDoesNotStopTheOtherCards(t *testing.T) {
	...
	var staged int
	h := Handlers{}.WithVCardReviewStager(func(context.Context, VCardEntry, *ids.ContactID) error {
		staged++
		if staged == 1 {
