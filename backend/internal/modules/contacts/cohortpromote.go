...
// Repairing a contact's whole captured cohort, not just the message that
// happened to create them.
//
// linkActivityToContact settles ONE activity — the message the ensure ran for.
...
// and creates the contact from a message ten minutes into the run, a human types
...
// depend on the ORDER in which the activity and the contact arrived.
//
// Ownership: activity_link and activity_participant belong to activities, and
// contacts writes both here under the waivers that already ratify
// linkActivityToContact and nameContactAmongParticipants. The reason generalizes
// from those — contacts is the only module that can settle the merge redirect and
// read contact_email, which is what the cohort is DEFINED by, and activities can
...
// matches its own selection, so the next contact event repairs the next batch,
...
type CohortPromotion struct {
	// Linked is activities that gained a contact link they did not have.
	Linked int64
	// Promoted is address-only participant rows that now name the contact.
	Promoted int64
}

// PromoteContactCohortTx repairs every captured activity this contact's live
// addresses reach: the missing links, and the participant rows that named them
// only by address.
//
// The contact is settled against a merge FIRST, for the reason
// linkActivityToContact settles it: no reader of activity_link walks
// merged_into_id, so a link written to a retired id leaves the message on a
// record nobody opens.
func (s *Store) PromoteContactCohortTx(
	...
	if err := tx.QueryRow(ctx,
		...
		return CohortPromotion{}, fmt.Errorf("contacts: resolving the contact a cohort belongs to: %w", err)
	}
	...
	promoted, err := promoteParticipantsToContact(ctx, tx, nil, canonical, &batch)
	if err != nil {
		return CohortPromotion{}, err
	}
	// After the promotion, never before it: this reads the contact-resolved
	...
	// contact — and without it a contact minted from a bare invitation address
	// keeps the local part of their own email as their name forever.
	if err := fillContactNameFromAttendance(ctx, tx, canonical); err != nil {
		return CohortPromotion{}, err
	}
	...
	// but the contacts on the invitation while the invitation emails beside it
	...
	if len(linked) == 0 && len(promoted) == 0 {
		...
		// contact's history, and auditing every replayed contact.updated would
	...
	// before-image to judge — the subject is the contact whose cohort moved, not
	// any one of the messages.
	auditID, err := storekit.AuditEvent(ctx, tx, "update", entityContact, canonical.UUID,
		map[string]any{"cohort_linked": len(linked), "cohort_promoted": len(promoted)})
	if err != nil {
		return CohortPromotion{}, fmt.Errorf("contacts: auditing a cohort repair: %w", err)
	}
	...
	for _, activity := range changedActivities(linked, promoted) {
		if err := storekit.EmitEvent(ctx, tx, auditID, activity, crmcontracts.PublicEventActivityUpdated{
			...
			return CohortPromotion{}, fmt.Errorf("contacts: publishing a cohort repair: %w", err)
		}
	}
...
// participant row just learned the contact is precisely a message whose edges
...
// linkCapturedCohort attaches every captured message from any of the contact's
...
// is cleared so it cannot match anyway, but linking one to a live contact would
// put it back in a reader's reach through that contact's timeline.
//
// Only mail linked to NOBODY. A message already attached to a contact belongs to
// that contact's record, and a cohort inference about an address must not
// relabel it. This is deliberately wider than linkActivityToContact's guard,
...
// contact-first ordering would have produced — and asks nothing of a kind that
// never carries one.
func linkCapturedCohort(ctx context.Context, tx pgx.Tx, contactID ids.ContactID) ([]ids.UUID, error) {
	rows, err := tx.Query(ctx, `
		INSERT INTO activity_link (activity_id, entity_type, contact_id)
		SELECT a.id, 'contact', $1
		  FROM activity a
		 WHERE a.captured_by LIKE 'connector:%'
		   AND a.restricted_at IS NULL
		   AND a.counterparty_email IN (
		       SELECT lower(pe.email) FROM contact_email pe
		        WHERE pe.contact_id = $1 AND pe.archived_at IS NULL)
		   AND NOT EXISTS (
		   ...
		   -- This arm guarded only on "no contact link exists", so a message
		...
		RETURNING activity_id`, contactID, cohortRepairBatch, maxMeetingLinksPerActivity)
	if err != nil {
		return nil, fmt.Errorf("contacts: linking a contact's captured cohort: %w", err)
	}
	defer rows.Close()
	var linked []ids.UUID
	for rows.Next() {
...
// ContactsOwedACohortRepair lists contacts whose captured mail is not all on their
...
func (s *Store) ContactsOwedACohortRepair(ctx context.Context, limit int) ([]ids.ContactID, error) {
		...
		rows, err := tx.Query(ctx, `
			WITH owed AS (
				SELECT pe.contact_id
				  ...
				  JOIN contact_email pe ON lower(pe.email) = ap.address AND pe.archived_at IS NULL
				 WHERE ap.contact_id IS NULL AND ap.user_id IS NULL AND ap.address IS NOT NULL
				   AND NOT EXISTS (
				       SELECT 1 FROM activity_participant other
				        WHERE other.activity_id = ap.activity_id
				          AND other.role = ap.role
				          AND other.contact_id = pe.contact_id)
				UNION
				SELECT pe.contact_id
				  FROM activity a
				 ...
				 WHERE a.captured_by LIKE 'connector:%' AND a.restricted_at IS NULL
				   ...
				   -- the scan keeps offering is a contact the sweep returns to
				...
				-- row. Without this arm a contact owed nothing BUT meeting links
				...
				SELECT coalesce(att.merged_into_id, att.id)
				  ...
				  JOIN contact att ON att.id = ap.contact_id
				   ...
				   AND NOT EXISTS (
				       SELECT 1 FROM activity_link l
				        WHERE l.activity_id = a.id
				          AND l.contact_id = coalesce(att.merged_into_id, att.id))
				   AND (SELECT count(*) FROM activity_link cap
				         WHERE cap.activity_id = a.id) < $2
			)
			SELECT owed.contact_id FROM owed
			  JOIN contact live ON live.id = owed.contact_id
			 WHERE live.archived_at IS NULL AND live.merged_into_id IS NULL
			 ORDER BY owed.contact_id
			 LIMIT $1`, limit, maxMeetingLinksPerActivity)
		if err != nil {
			return fmt.Errorf("contacts: listing the contacts owed a cohort repair: %w", err)
		}
		defer rows.Close()
		for rows.Next() {
...
// RepairContactCohort is PromoteContactCohortTx on its own transaction, for the
// sweep that owns no other write.
func (s *Store) RepairContactCohort(ctx context.Context, contactID ids.ContactID) (CohortPromotion, error) {
	var done CohortPromotion
	err := s.db.Tx(ctx, func(tx pgx.Tx) error {
		var err error
		done, err = s.PromoteContactCohortTx(ctx, tx, contactID)
		return err
	})
	if err != nil {
		return CohortPromotion{}, err
...
// DomainsOwedTheirContacts lists companies whose domain has live contacts with no
...
// Attaching a contact to a company is a write about the CONTACT, and the human
...
func (s *Store) DomainsOwedTheirContacts(ctx context.Context, limit int) ([]DomainBacklog, error) {
		...
		rows, err := tx.Query(ctx, `
			SELECT DISTINCT od.company_id, od.domain
			 ...
			 WHERE od.archived_at IS NULL
			   ...
			   -- And not a contact the index will refuse anyway. uq_rel_employment
			   -- admits ONE live employment per (contact, company), so
			   ...
			   AND NOT EXISTS (
			       SELECT 1 FROM relationship held
			        WHERE held.contact_id = p.id AND held.company_id = od.company_id
			          AND `+employment.LiveSlotSQL("held")+`)
			 ORDER BY od.company_id, od.domain
			 LIMIT $1`, limit)
		if err != nil {
			return fmt.Errorf("contacts: listing the domains owed their contacts: %w", err)
		}
		defer rows.Close()
		for rows.Next() {
...
// DomainBacklog names one company domain whose contacts are not attached yet.
type DomainBacklog struct {
	CompanyID ids.CompanyID
	Domain    string
}

// AttachDomainBacklog gives the live contacts on one company domain their
...
func (s *Store) AttachDomainBacklog(ctx context.Context, owed DomainBacklog) (int, error) {
	...
	err := s.db.Tx(ctx, func(tx pgx.Tx) error {
		...
		// merge in between would otherwise attach a domain's contacts to a record
