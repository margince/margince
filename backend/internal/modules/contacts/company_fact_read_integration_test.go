...
// The company-360 evidence reads: facts and profile fields surface exactly
// what a human confirmed, row-scoped and ordered for a stable render, and
// a company outside the caller's scope is existence-hidden (404), never
...
func seedCompanyWithEvidence(ctx context.Context, t *testing.T, e *dedupeEnv) ids.CompanyID {
	t.Helper()
...
func TestListCompanyProfileFieldsReturnsConfirmedFieldsRowScoped(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	...
	// A company outside the caller's scope is existence-hidden, never leaked.
	if _, err := e.store.ListCompanyProfileFields(ctx, ids.From[ids.CompanyKind](ids.NewV7())); !errors.Is(err, apperrors.ErrNotFound) {
		t.Fatalf("foreign company must 404, got %v", err)
	}
}

func TestListCompanyFactsReturnsConfirmedFactsGroupedOrder(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	companyID := seedCompanyWithEvidence(ctx, t, e)
	...
	if _, err := e.store.ListCompanyFacts(ctx, ids.From[ids.CompanyKind](ids.NewV7())); !errors.Is(err, apperrors.ErrNotFound) {
		t.Fatalf("foreign company must 404, got %v", err)
	}
}
