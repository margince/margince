...
const (
	...
	contactFK = "contact_id"
	companyFK = "company_id"
)

// SenderPredicate renders "this contact WROTE this message", as opposed to the
// message merely reaching them.
//
// A thread is linked to everybody it concerns, so activity_link membership
// answers "does this message reach this contact" and nothing more. Reading it as
...
// takes its contact_id from the FIRST activity_link and its address from
// activity.counterparty_email, so one row can name one contact while carrying
// another's address. Trusting contact_id there would re-admit the confusion this
...
//
// contact and activity are SQL expressions the caller supplies: a bind
...
// holding this SQL would count as a read of `contact` that reaches no row scope
// — which is true of the fragment and false of every statement it appears in.
func SenderPredicate(contact, activity string) string {
	return fmt.Sprintf(senderPredicateSQL, contact, activity)
}
		...
		SELECT 1 FROM activity_participant ap
		 WHERE ap.activity_id = %[2]s.id AND ap.role = 'from'
		   AND CASE WHEN ap.address IS NOT NULL
		            THEN EXISTS (SELECT 1 FROM contact_email pe
		                          WHERE pe.contact_id = %[1]s AND pe.archived_at IS NULL
		                            AND pe.email = ap.address)
		            ELSE ap.contact_id = %[1]s END)`
