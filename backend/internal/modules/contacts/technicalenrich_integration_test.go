...
func TestAHumanCorrectionSurvivesEveryLaterLookup(t *testing.T) {
	...
	// A contact corrects the VALUE of the row the lookup wrote, keeping its
	...
	if value != "Eigener Mailserver" {
		t.Errorf("the lookup overwrote a contact's correction with %q", value)
	}
...
func TestAHumanHeldRowIsNeverReconciledAway(t *testing.T) {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		...
		t.Fatalf("mark the row as a contact's: %v", err)
	}
	...
	held := technicalFactsOf(ctx, t, e, companyID)
	if len(held[FactOperatedService]) != 1 {
		t.Error("the reconciliation removed a row a contact had claimed — the upsert's precedence " +
...
//
// A contact who corrects the mail provider holds THAT row. A later reading of a
...
// two mail systems, one of them the one a contact had just rejected.
func TestAHumanAnswerSettlesTheWholeSingleValuedField(t *testing.T) {
	...
	held := technicalFactsOf(ctx, t, e, companyID)
	if len(held[FactMailProvider]) != 1 {
		t.Fatalf("the record claims %v mail providers — a company has one, and a contact had already "+
	...
	if value != "Eigener Mailserver" {
		t.Errorf("the surviving mail provider is %q, want the contact's answer", value)
	}
}
