...
// The cohort repair over a real Postgres: a contact who arrives AFTER their mail
...
// walks newest-first, so the contact is created partway through the run and every
...
// it before any contact exists: a counterparty_email on the activity, an
...
// cohortStateOf answers, for one activity, whether it carries a contact link and
// whether its participant row names that contact — the two halves that must move
// together.
func (e *dedupeEnv) cohortStateOf(ctx context.Context, t *testing.T, activity ids.ActivityID, contact ids.ContactID) (linked, named bool) {
	t.Helper()
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		if err := tx.QueryRow(ctx, `
			SELECT EXISTS (SELECT 1 FROM activity_link
			                WHERE activity_id = $1 AND entity_type = 'contact' AND contact_id = $2)`,
			activity, contact).Scan(&linked); err != nil {
			return err
		}
		return tx.QueryRow(ctx, `
			SELECT EXISTS (SELECT 1 FROM activity_participant
			                WHERE activity_id = $1 AND contact_id = $2)`,
			activity, contact).Scan(&named)
	}); err != nil {
		t.Fatal(err)
	}
	return linked, named
...
// The ordinary late-contact case: three messages are captured while nobody knows
// who the sender is, the contact is then created, and all three are on their
// record — not just the one the ensure ran for.
func TestTheCohortRepairFindsMailCapturedBeforeTheContactExisted(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()

	const email = "late@cohort.test"
	// Captured while the sender was an open question: from-lines and a CC,
	// because a contact CC'd on a thread is as stranded as its sender.
	...
	// Now the contact arrives — by the ensure, on a message of its own.
	res, err := e.store.EnsureCounterparty(ctx, e.ensureInput(ctx, t, email, "Late Sender", "cohort.test"))
	if err != nil {
		t.Fatalf("ensure: %v", err)
	}
	if !res.ContactCreated {
		t.Fatalf("ensure = %+v, want the contact created", res)
	}

	// The ensure alone settles only its own message; the repair is what reaches
	// the rest, and it is what the contact event will run.
	var done CohortPromotion
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		var err error
		done, err = e.store.PromoteContactCohortTx(ctx, tx, res.ContactID)
		return err
	}); err != nil {
		t.Fatalf("cohort repair: %v", err)
	}
	if done.Linked != int64(len(earlier)) {
		t.Errorf("repair linked %d activities, want %d — a message captured before the contact is still that contact's mail",
			done.Linked, len(earlier))
	}
	for i, activity := range earlier {
		linked, named := e.cohortStateOf(ctx, t, activity, res.ContactID)
		if !linked {
			t.Errorf("earlier message %d has no contact link: it is invisible to every reader that reaches an account through activity_link", i)
		}
		if !named {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		if err := tx.QueryRow(ctx, `
			...
			res.ContactID).Scan(&audits); err != nil {
			return err
		}
		return tx.QueryRow(ctx, `
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		var err error
		second, err = e.store.PromoteContactCohortTx(ctx, tx, res.ContactID)
		return err
	}); err != nil {
...
// is exactly as stranded as a new contact's would be.
func TestTheCohortRepairReachesMailUnderAnAliasAddedLater(t *testing.T) {
	...
	if linked, _ := e.cohortStateOf(ctx, t, stranded, res.ContactID); linked {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		...
		return insertContactEmails(ctx, tx, ws, res.ContactID, "manual", "human:test",
			[]ContactEmailInput{{Email: alias, EmailType: "work", Position: 1}})
	}); err != nil {
		t.Fatal(err)
	}
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		_, err := e.store.PromoteContactCohortTx(ctx, tx, res.ContactID)
		return err
	}); err != nil {
		t.Fatalf("cohort repair: %v", err)
	}

	linked, named := e.cohortStateOf(ctx, t, stranded, res.ContactID)
	if !linked || !named {
		t.Errorf("mail under the alias: linked=%v named=%v, want both — an address the contact owns is the contact's mail whenever it was learned",
			linked, named)
	}
}
...
func TestTheCohortRepairLeavesMailThatAlreadyBelongsToSomebody(t *testing.T) {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `
			...
			VALUES ($1, $2, 'contact', $3)`, ids.NewV7(), claimed, incumbent.ContactID)
		return err
	}); err != nil {
		t.Fatal(err)
	}

	// A second contact now takes that address — the shared-mailbox shape.
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		_, err := e.store.PromoteContactCohortTx(ctx, tx, other.ContactID)
		return err
	}); err != nil {
		t.Fatalf("cohort repair: %v", err)
	}

	if linked, _ := e.cohortStateOf(ctx, t, claimed, other.ContactID); linked {
	...
	if linked, _ := e.cohortStateOf(ctx, t, claimed, incumbent.ContactID); !linked {
		t.Error("the original owner's link is gone")
	}
}

// A merged-away contact's cohort belongs to the survivor, because no reader of
...
func TestTheCohortRepairWritesUnderTheSurvivorOfAMerge(t *testing.T) {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		if _, err := tx.Exec(ctx,
			`UPDATE contact SET merged_into_id = $2 WHERE id = $1`, retired.ContactID, survivor.ContactID); err != nil {
			return err
		}
		_, err := tx.Exec(ctx,
			`UPDATE contact_email SET contact_id = $2, is_primary = false WHERE contact_id = $1`,
			retired.ContactID, survivor.ContactID)
		return err
	}); err != nil {
		t.Fatal(err)
	}
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		_, err := e.store.PromoteContactCohortTx(ctx, tx, retired.ContactID)
		return err
	}); err != nil {
		t.Fatalf("cohort repair: %v", err)
	}

	if linked, named := e.cohortStateOf(ctx, t, stranded, survivor.ContactID); !linked || !named {
		t.Errorf("survivor: linked=%v named=%v, want both — a merge redirects the record, and the mail follows it", linked, named)
	}
	if linked, _ := e.cohortStateOf(ctx, t, stranded, retired.ContactID); linked {
...
func TestTheCohortRepairLeavesHandLoggedActivitiesAlone(t *testing.T) {
	...
	if linked, _ := e.cohortStateOf(ctx, t, manual, res.ContactID); linked {
