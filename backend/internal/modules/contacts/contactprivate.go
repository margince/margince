...
// RetractCaptureOnlyContactTx archives a contact capture created, once the
...
//     (`agent:`) both mint contacts nobody asked for; a record a human typed
...
//     a system principal and records the contact only in `on_behalf_of`. Reading
...
//
//   - It has no other correspondence. A contact who also writes about business is
...
func (s *Store) RetractCaptureOnlyContactTx(
	ctx context.Context, tx pgx.Tx, id ids.ContactID, ownerID ids.UUID,
) (bool, error) {
	...
	if _, err := tx.Exec(ctx, `
		SELECT 1 FROM contact WHERE id = $1 FOR UPDATE`, id.UUID); err != nil {
		return false, fmt.Errorf("contacts: locking a captured contact for retraction: %w", err)
	}
	...
	// override's lock alone and never needs the contact's, so under READ
	...
	if err := tx.QueryRow(ctx, `
		...
		return false, fmt.Errorf("contacts: reading whether a captured contact may be retracted: %w", err)
	}
	if !eligible {
		return false, nil
	}
	// NARROW BEFORE ARCHIVING, and it is not cosmetic. The contacts list honours
	...
	if err := archiveContactRows(ctx, tx, id, time.Now().UTC(), nil); err != nil {
		return false, fmt.Errorf("contacts: retracting a captured contact: %w", err)
	}
	return true, nil
}

// CaptureOnlyHolder names one capture-created record and the mailbox owner it
// was minted for — the pair RetractCaptureOnlyContactTx is called with.
...
// entitled to retract: machine-created contacts holding the address, whatever
// their visibility.
//
// Both machine prefixes, and no visibility filter, for the reason
// RetractCaptureOnlyContactTx gives: the sender verdict mints contacts under
...
// RetractCaptureOnlyContactTx, which re-reads it on the same transaction; a
...
func (s *Store) CaptureOnlyHoldersOfAddressTx(ctx context.Context, tx pgx.Tx, email string) ([]CaptureOnlyHolder, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("contacts: listing the captured holders of an address: %w", err)
	}
	defer rows.Close()
	var out []CaptureOnlyHolder
	for rows.Next() {
		var h CaptureOnlyHolder
		if err := rows.Scan(&h.ContactID, &h.OwnerID); err != nil {
			return nil, fmt.Errorf("contacts: reading a captured holder of an address: %w", err)
		}
		out = append(out, h)
	}
...
// lockSenderOverridesTx takes the write lock on every address this contact
...
func lockSenderOverridesTx(ctx context.Context, tx pgx.Tx, id ids.ContactID, ownerID ids.UUID) error {
	rows, err := tx.Query(ctx, `
		SELECT email FROM contact_email
		 WHERE contact_id = $1 AND archived_at IS NULL
		 ORDER BY email`, id.UUID)
	if err != nil {
		return fmt.Errorf("contacts: reading a contact's addresses before locking their overrides: %w", err)
	}
	defer rows.Close()
