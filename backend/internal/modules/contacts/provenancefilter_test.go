...
// (backend/internal/modules/contacts/provenancefilter_test.go)
...
// place. A second place turning CapturedByKind into SQL is how the contact list
// and the lead list come to disagree about which prefix counts as an AI — the
// two are read side by side, so the disagreement reaches a contact before it
// reaches a test.
//
