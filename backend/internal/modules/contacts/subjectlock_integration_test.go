...
//
// In `contacts` rather than beside the primitive: locking needs a real row, and
...
func TestALockedSubjectMakesTheEraserWait(t *testing.T) {
	...
	go func() {
		holding <- e.store.tx(ctx, func(tx pgx.Tx) error {
			if err := auth.LockSubjectLive(ctx, tx, entityContact, contactID.UUID); err != nil {
				return err
			}
...
func TestLockSubjectLiveRefusesWhatItCannotHold(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			name: "a subject archived before the lock", table: entityContact, id: contactID.UUID, archived: true,
		}, {
			name: "a subject that never existed", table: entityContact, id: ids.NewV7(),
		}, {
			...
			name: "a table outside the row-scoped set", table: "audit_log", id: contactID.UUID,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if tc.archived {
				if err := e.store.tx(ctx, func(tx pgx.Tx) error {
					_, err := tx.Exec(ctx, `UPDATE contact SET archived_at = now() WHERE id = $1`, tc.id)
					return err
				}); err != nil {
