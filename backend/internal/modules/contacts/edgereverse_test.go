...
// value would reverse more than the contact asked to reverse.
...
func TestAnEdgeWithNoAnchorIsAFaultRatherThanAnEmptyAnswer(t *testing.T) {
	...
	if _, _, err := anchorIDOf(relationshipRow{ID: ids.NewV7(), Kind: "employment"}); err == nil {
		t.Error("an employment with no contact was given an anchor")
	}
	contact := ids.From[ids.ContactKind](ids.NewV7())
	object, got, err := anchorIDOf(relationshipRow{Kind: "employment", ContactID: &contact})
	if err != nil {
		t.Fatalf("anchoring an employment on its contact: %v", err)
	}
	if object != anchorContact {
		t.Errorf("anchor object = %q, want %q — an employment annotates its contact", object, anchorContact)
	}
	if got != contact.UUID {
		t.Errorf("anchor = %s, want the contact the employment names", got)
	}
}
