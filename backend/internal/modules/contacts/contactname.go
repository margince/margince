...
// and do we know the contact's first and last name. The second is the one that
...
// Abstaining costs a null column; guessing wrong writes a contact's name wrongly
...
type ParsedName struct {
	...
	// NOT contact.title: that column holds a JOB title, and "Dr." is not one.
	Honorific string
	First     string
	Last      string
	// Full is always usable — contact.full_name is NOT NULL. At worst it is the
...
// ParseContactName reads the best name available from a header display name and
// an address, in that order of authority: what the sender calls themselves
// beats what their mailbox is called.
func ParseContactName(displayName, email string) ParsedName {
	...
	if parsed.Full == "" {
		// Neither header said anything usable. contact.full_name is NOT NULL and
		...
		if addr := strings.TrimSpace(email); addr != "" {
			...
			parsed.Full = unknownContactName
		}
	}
	return parsed
}

// unknownContactName is what a counterparty with no display name and no address
...
const unknownContactName = "Unknown sender"
...
func parseLocalPart(email string) ParsedName {
	...
	if len(tokens) == 0 || !complete {
		...
		// decomposed "josé" is a different contact), so the whole reading is
		// refused rather than silently shortened.
		return ParsedName{Full: local}
	}
	// One token is a surname, a handle, or a first name — the local part does
	// not say which, so it names the contact without splitting them.
	if len(tokens) == 1 {
...
// must then abstain: dropping a field silently would rename the contact.
...
// and reading them as "Support Eu" invents two contacts who do not exist.
//
...
func isWordLike(token string) bool {
	...
	// contact whose surname is a hyphen.
	return letters > 0
}
...
func uncommaName(name string) string {
	...
	// that is a title or a department the reversal would file as a contact's
	// first name.
...
// affiliationSeparators mark where a display name stops naming the contact and
