...
func TestDecideCompanyNameWeighsCorroboration(t *testing.T) {
	alice := ids.New[ids.ContactKind]()
	bob := ids.New[ids.ContactKind]()
	...
	tests := []struct {
	...
	}{
		{
			...
			candidate: CompanyNameCandidate{
				DisplayName: "Gitex",
				Signatures:  []SignatureCompanyName{{ContactID: alice, Value: "Gitex Global GmbH"}},
			},
			wantVerdict:       true,
		...
		{
			// Two contacts at one company are two mailboxes on ONE mail
			...
			candidate: CompanyNameCandidate{
				...
				Signatures: []SignatureCompanyName{
					{ContactID: alice, Value: "Gitex Global GmbH"},
					{ContactID: bob, Value: "Gitex Global"},
				},
			},
			wantVerdict: true,
		...
		{
			name: "the same contact signing twice is one claim, not two",
			candidate: CompanyNameCandidate{
				DisplayName: "Gitex",
				Signatures: []SignatureCompanyName{
					{ContactID: alice, Value: "Gitex Global GmbH"},
		...
		},
		{
			...
			candidate: CompanyNameCandidate{
				DisplayName: "Gitex",
				Signatures: []SignatureCompanyName{
					{ContactID: alice, Value: "Gitex"},
					{ContactID: bob, Value: "GITEX GmbH"},
				},
			},
			wantVerdict: false,
		},
		{
			...
			candidate: CompanyNameCandidate{
				DisplayName: "Gitex",
				Signatures:  []SignatureCompanyName{{ContactID: alice, Value: "   "}},
			},
			wantVerdict: false,
		},
		{
			...
			candidate: CompanyNameCandidate{
				DisplayName: "Gitex",
				Signatures: []SignatureCompanyName{
					{ContactID: alice, Value: "Gitex Events"},
					{ContactID: bob, Value: "Gitex Global"},
				},
				DossierNames: []string{"Gitex Global"},
			},
...
func TestDecideCompanyNameKeyIsTheNormalizedClaim(t *testing.T) {
	...
	restyled, ok := DecideCompanyName(CompanyNameCandidate{
		DisplayName: "Gitex",
		Signatures:  []SignatureCompanyName{{ContactID: bob, Value: "  GITEX   GLOBAL  GMBH "}},
	})
	if !ok {
...
// The contacts list is the evidence a reviewer is shown, so it must name every
// contact behind the winning claim — and only them.
func TestDecideCompanyNameReportsTheSigningContacts(t *testing.T) {
	alice := ids.New[ids.ContactKind]()
	bob := ids.New[ids.ContactKind]()
	carol := ids.New[ids.ContactKind]()
	got, ok := DecideCompanyName(CompanyNameCandidate{
		DisplayName: "Gitex",
		Signatures: []SignatureCompanyName{
			{ContactID: alice, Value: "Gitex Global GmbH"},
			{ContactID: bob, Value: "Gitex Global"},
			{ContactID: carol, Value: "Something Else"},
		},
	})
	if !ok {
		t.Fatal("expected a verdict")
	...
	if len(got.Contacts) != 2 {
		t.Fatalf("contacts = %v, want the two who signed the winning name", got.Contacts)
	}
	for _, id := range got.Contacts {
		if id == carol {
