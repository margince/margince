...
func (s *Store) WorksWithPeers(ctx context.Context, tx pgx.Tx, anchor ids.ContactID) (map[ids.UUID]bool, error) {
	var args []any
	...
	rows, err := tx.Query(ctx, fmt.Sprintf(`
		SELECT CASE WHEN r.contact_id = $%d THEN r.counterparty_contact_id ELSE r.contact_id END
		  FROM relationship r
		 WHERE r.kind = 'works_with' AND r.archived_at IS NULL
		   AND (r.contact_id = $%d OR r.counterparty_contact_id = $%d)
		   AND (%s)`, anchorPos, anchorPos, anchorPos, scope), args...)
	if err != nil {
		return nil, fmt.Errorf("contacts: reading who is already recorded as working with a contact: %w", err)
	}
	defer rows.Close()
