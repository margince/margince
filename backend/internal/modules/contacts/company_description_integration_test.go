...
func TestCompanyDescriptionRoundTripsThroughCreateAndUpdate(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	...
	if company.Description == nil || *company.Description != written {
		t.Fatalf("create returned description %v, want %q", company.Description, written)
	}
	...
	reread, err := e.store.GetCompany(ctx, companyID, storekit.LiveOnly)
	if err != nil {
		t.Fatal(err)
	}
	...
	empty := ""
	cleared, err := e.store.UpdateCompany(ctx, companyID, UpdateCompanyInput{
		Description: &empty,
	})
	if err != nil {
...
func TestCompanyDescriptionSurvivesAnUnrelatedEdit(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	...
	updated, err := e.store.UpdateCompany(ctx,
		ids.From[ids.CompanyKind](ids.UUID(company.Id)),
		UpdateCompanyInput{Industry: &industry})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompanyDescriptionFillsOnlyABlankMergeSurvivor(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	retiredLine := "Fabricates aluminium window systems."

	blankSurvivor, err := e.store.CreateCompany(ctx, CreateCompanyInput{
		DisplayName: "Blank Survivor GmbH", Source: "manual",
	})
	if err != nil {
		t.Fatal(err)
	}
	retired, err := e.store.CreateCompany(ctx, CreateCompanyInput{
	...
	heldSurvivor, err := e.store.CreateCompany(ctx, CreateCompanyInput{
		DisplayName: "Held Survivor GmbH", Source: "manual", Description: &ownLine,
	})
	if err != nil {
		t.Fatal(err)
	}
	secondRetired, err := e.store.CreateCompany(ctx, CreateCompanyInput{
...
// A site read REPLACES a description no contact authored. An agent creating the
...
func TestASiteReadReplacesAnAgentsDescription(t *testing.T) {
	...
	company, err := e.store.CreateCompany(e.asAgent(), CreateCompanyInput{
	...
	applySiteReadDescription(e.as(), t, e, companyID, fromSite)

	reread, err := e.store.GetCompany(e.as(), companyID, storekit.LiveOnly)
	if err != nil {
		t.Fatal(err)
	}
...
// The same read leaves a contact's sentence alone. This is the half that makes
...
func TestASiteReadLeavesAHumansDescriptionAlone(t *testing.T) {
	...
	if _, err := e.store.UpdateCompany(ctx, companyID, UpdateCompanyInput{Description: &typed}); err != nil {
		t.Fatal(err)
	}

	applySiteReadDescription(ctx, t, e, companyID, "Whatever the marketing page happens to claim.")
...
// A description a contact supplies AT CREATE is theirs too. The edit path is
...
func TestASiteReadLeavesAHumansCreatedDescriptionAlone(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	typed := "What we tell contacts we do, written when the record was made."
	company, err := e.store.CreateCompany(ctx, CreateCompanyInput{
...
// survivorship fill moves a contact's sentence onto a record that never had one,
// and if the claim did not travel with it the next site read of the survivor
// would replace words a contact wrote.
func TestAMergedInDescriptionKeepsItsHumanAuthor(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	typed := "The sentence a contact wrote on the record that lost the merge."
	retired, err := e.store.CreateCompany(ctx, CreateCompanyInput{
	...
	reread, err := e.store.GetCompany(ctx, survivorID, storekit.LiveOnly)
	if err != nil {
		t.Fatal(err)
	}
	if reread.Description == nil || *reread.Description != typed {
...
func TestAnAgentsEditDoesNotClaimTheDescription(t *testing.T) {
	e := setupDedupe(t)
	company, err := e.store.CreateCompany(e.as(), CreateCompanyInput{
	...
	if _, err := e.store.UpdateCompany(e.asAgent(), companyID,
		UpdateCompanyInput{Description: &fromAgent}); err != nil {
		t.Fatal(err)
	}
...
func TestAnAgentEchoingAHumansDescriptionDoesNotTakeIt(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	typed := "The sentence a contact wrote and an agent merely repeats."
	company, err := e.store.CreateCompany(ctx, CreateCompanyInput{
	...
	same := typed
	if _, err := e.store.UpdateCompany(e.asAgent(), companyID,
		UpdateCompanyInput{Description: &same}); err != nil {
		t.Fatal(err)
	}
...
// field_provenance for every company that already had one.
//
...
func TestABackfilledLegacyDescriptionIsHeldByItsRowAuthor(t *testing.T) {
	...
	companyID := ids.From[ids.CompanyKind](ids.UUID(company.Id))
	stripDescriptionStamp(ctx, t, e, companyID)
...
func stripDescriptionStamp(ctx context.Context, t *testing.T, e *dedupeEnv, companyID ids.CompanyID) {
	t.Helper()
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `DELETE FROM field_provenance
			WHERE object_type = 'company' AND object_id = $1 AND field_name = 'description'`, companyID)
		return err
	}); err != nil {
...
func backfillDescriptionAuthors(ctx context.Context, t *testing.T, e *dedupeEnv) {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `
			...
			SELECT 'company', o.id, 'description', o.source, o.captured_by, o.created_at
			FROM company o
			WHERE o.description IS NOT NULL
			  AND NOT EXISTS (
			    SELECT 1 FROM field_provenance fp
			    WHERE fp.object_type = 'company'
...
func TestCompanyDescriptionRefusesAPastedParagraph(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
