...
// out-of-scope company is existence-hidden like every other read.
...
func seedLogoCompany(ctx context.Context, t *testing.T, e *dedupeEnv, name, domain string) ids.CompanyID {
	t.Helper()
...
func TestSetCompanyLogoRecordsTheMarkItsProvenanceAndItsURL(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	companyID := seedLogoCompany(ctx, t, e, "Voltaq Systems GmbH", "voltaq.test")

	before, err := e.store.GetCompany(ctx, companyID, storekit.LiveOnly)
	if err != nil {
		t.Fatalf("read before: %v", err)
	}
	if before.LogoUrl != nil {
		t.Fatalf("a fresh company has no logo, got %q", *before.LogoUrl)
	}
	if _, err := e.store.CompanyLogoKey(ctx, companyID, LogoWide); !errors.Is(err, apperrors.ErrNotFound) {
		t.Fatalf("a company with no logo must answer not-found, got %v", err)
	}

	key := e.ws.String() + "/company_logo/" + companyID.String()
	written, superseded, err := e.store.SetCompanyLogo(ctx, companyID, key, "https://voltaq.test/touch.png")
	if err != nil {
		t.Fatalf("SetCompanyLogo: %v", err)
	}
	if !written {
		t.Fatal("the write reported no change on a company with no logo")
	}
	if superseded != nil {
		t.Fatalf("the first logo supersedes nothing, got %q", *superseded)
	}

	after, err := e.store.GetCompany(ctx, companyID, storekit.LiveOnly)
	if err != nil {
		t.Fatalf("read after: %v", err)
	}
	wantURL := *LogoURL(companyID.UUID, &key, LogoWide)
	...
	gotKey, err := e.store.CompanyLogoKey(ctx, companyID, LogoWide)
	if err != nil {
		t.Fatalf("CompanyLogoKey: %v", err)
	}
	if gotKey != key {
	...
	err = e.store.tx(ctx, func(tx pgx.Tx) error {
		if err := tx.QueryRow(ctx, `
			...
			companyID).Scan(&source, &capturedBy, &evidence); err != nil {
			return err
		}
...
func TestSetCompanyLogoNeverReplacesTheOneAContactSet(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	companyID := seedLogoCompany(ctx, t, e, "Nordwind Energie AG", "nordwind.test")
	...
	err := e.store.tx(ctx, func(tx pgx.Tx) error {
		...
		return storekit.StampFields(ctx, tx, "company", companyID.UUID, "human", "human:"+e.rep.String(),
	...
	held, err := e.store.LogoHeldByHuman(ctx, companyID)
	if err != nil {
	...
	written, _, err := e.store.SetCompanyLogo(ctx, companyID,
		e.ws.String()+"/company_logo/"+companyID.String()+"/resolved", "https://nordwind.test/favicon.ico")
	if err != nil {
		t.Fatalf("SetCompanyLogo: %v", err)
	}
	if written {
...
func TestSetCompanyLogoHandsBackTheObjectItSuperseded(t *testing.T) {
	...
	e := setupDedupe(t)
	ctx := e.asAgent()
	companyID := seedLogoCompany(e.as(), t, e, "Erneut GmbH", "erneut.test")

	first := e.ws.String() + "/company_logo/" + companyID.String() + "/" + ids.NewV7().String()
	if _, _, err := e.store.SetCompanyLogo(ctx, companyID, first, "https://erneut.test/a.png"); err != nil {
		t.Fatalf("first logo: %v", err)
	}

	second := e.ws.String() + "/company_logo/" + companyID.String() + "/" + ids.NewV7().String()
	written, superseded, err := e.store.SetCompanyLogo(ctx, companyID, second, "https://erneut.test/b.png")
	if err != nil {
...
func TestCompanyLogoIsRowScopedLikeEveryOtherRead(t *testing.T) {
	e := setupDedupe(t)
	owner := e.as()
	companyID := seedLogoCompany(owner, t, e, "Fremdfirma GmbH", "fremd.test")
	key := e.ws.String() + "/company_logo/" + companyID.String()
	if _, _, err := e.store.SetCompanyLogo(owner, companyID, key, "https://fremd.test/touch.png"); err != nil {
		t.Fatalf("seed the logo: %v", err)
	}
	// A company is workspace-readable identity, so ownership alone hides
	// nothing: make it this rep's capture-private record (visibility='owner'),
	// the one state that still keeps a company out of another seat's reach.
	if err := e.store.tx(owner, func(tx pgx.Tx) error {
		_, err := tx.Exec(owner, `UPDATE company SET owner_id = $2, visibility = 'owner' WHERE id = $1`, companyID, e.rep)
		return err
	}); err != nil {
		t.Fatalf("bind the company's owner: %v", err)
	}

	// A caller scoped to their own records only: the company is another
	...
	if _, err := e.store.CompanyLogoKey(stranger, companyID, LogoWide); !errors.Is(err, apperrors.ErrNotFound) {
		t.Fatalf("an out-of-scope logo read must be existence-hidden, got %v", err)
	}
	if _, err := e.store.LogoHeldByHuman(stranger, companyID); !errors.Is(err, apperrors.ErrNotFound) {
		t.Fatalf("an out-of-scope provenance read must be existence-hidden, got %v", err)
	}
	if _, _, err := e.store.SetCompanyLogo(stranger, companyID, key, "https://fremd.test/other.png"); !errors.Is(err, apperrors.ErrNotFound) {
