...
type heldFact struct {
	...
	ValueKey string
	Value    string
	// HumanHeld marks a row a contact has claimed. It is read from captured_by
...
func readTechnicalFacts(
	ctx context.Context, tx pgx.Tx, companyID ids.CompanyID, fields []string,
) ([]heldFact, error) {
	rows, err := tx.Query(ctx, `
		SELECT field, value_key, value, captured_by LIKE 'human:%'
		  FROM company_fact
		 WHERE company_id = $1 AND category = 'signal' AND field = ANY($2)`,
		companyID, fields)
	if err != nil {
	...
	for _, observation := range in.Observations {
		tag, err := tx.Exec(ctx, `
			INSERT INTO company_fact
			  (company_id, category, field, value, value_key, evidence_snippet,
...
//
// Only single-valued fields: a human confirming one operated service says
...
// record already says what a human decided it says, and announcing a change
