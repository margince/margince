...
// lockContactForAttach takes the contact's row lock before an edge is hung off
...
// LIVE relationship pointing at an ARCHIVED contact — which is the orphan the
...
// settles."
//
// Attaching an edge to a contact is precisely that question. This lock does not
// change the contact — it holds the row so an archive cannot slip past — and
...
// level down.
func lockContactForAttach(ctx context.Context, tx pgx.Tx, contactID ids.ContactID) error {
	var got ids.UUID
	err := tx.QueryRow(ctx,
		`SELECT id FROM contact WHERE id = $1 AND archived_at IS NULL FOR UPDATE`,
		contactID.UUID).Scan(&got)
	if errors.Is(err, pgx.ErrNoRows) {
		// The same refusal storekit.LockRow gives for a row its filter cannot
		// resolve, so an attach onto an archived contact reads the way an attach
