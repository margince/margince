...
// matcher, split from the contact half so each file holds one concept.
//
// It answers the weaker of the two questions the import exists for: not "who
// is this contact" but "does this connection work somewhere we sell to". A
// wrong answer here misattributes a reach count; a wrong answer on the contact
...
func matchGhostCompanies(ctx context.Context, tx pgx.Tx) error {
	...
	if err != nil {
		return fmt.Errorf("contacts: reading LinkedIn connections to place: %w", err)
	}
	...
	if _, err := tx.Exec(ctx, `
		...
		return fmt.Errorf("contacts: attaching LinkedIn connections to accounts: %w", err)
	}
	return nil
}
...
// account.
//
// The capture-privacy arm, matching ghostOwnerCapturePrivacy on the contact
...
func companyKeys(ctx context.Context, tx pgx.Tx) (map[string]companyCandidate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("contacts: reading accounts for LinkedIn placement: %w", err)
	}
	defer rows.Close()
