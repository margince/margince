...
func (s *Store) EmailAlreadyOnFileTx(ctx context.Context, tx pgx.Tx, email string) (bool, error) {
	...
	contactScope, err := scopeOrAllRows(ctx, "contact", "p", arg)
	if err != nil {
		return false, err
	}
	...
	// uq_contact_email_dedupe and uq_lead_email_dedupe and seq-scan both tables
	// once per proposed contact. Every sibling probe of these columns spells it
	// this way.
	var found bool
