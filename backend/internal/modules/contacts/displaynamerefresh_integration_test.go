...
// the contact, a display name that does not, and a machine on the row.
//
...
// nothing.
func (e *dedupeEnv) staleDisplayName(ctx context.Context, t *testing.T, display, first, last, capturedBy string) ids.ContactID {
	t.Helper()
	id := ids.From[ids.ContactKind](ids.NewV7())
...
// A contact a contact added by hand keeps the name they typed, whatever the split
// columns say. captured_by is what says a human made this record.
func TestTheRepairLeavesAContactAContactCreatedAlone(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	...
	if moved {
		t.Error("the repair rewrote a contact a contact created")
	}
	if full, _, _ := e.storedName(ctx, t, id); full != "Bobby" {
		t.Errorf("full_name = %q, want the name the human typed — they added this contact, "+
...
// would put one audit row and one contact.updated event on the bus per pass, for
