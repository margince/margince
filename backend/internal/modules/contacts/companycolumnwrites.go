...
// clumsy. The censuses that hold the company writers together — the rename
...
const (
	...
	fillUnclaimed companyWriteAuthority = iota + 1
	// replaceStanding writes over a value already there.
	replaceStanding
)

// companyColumnWrite is one column's pair of statements, keyed below by the
// column name the read-back and the form both use.
type companyColumnWrite struct {
	fill    string
	replace string
}
...
func (w companyColumnWrite) statementFor(authority companyWriteAuthority) string {
...
// companyColumnWrites is the one place a company's editable columns are
// written. The identifier is never a bind parameter: the statement is fixed
// here and only values bind.
var companyColumnWrites = map[string]companyColumnWrite{
	columnLegalName: {
		fill:    `UPDATE company SET legal_name = $2 WHERE id = $1 AND archived_at IS NULL AND legal_name IS NULL`,
		replace: `UPDATE company SET legal_name = $2 WHERE id = $1 AND archived_at IS NULL AND legal_name IS DISTINCT FROM $2`,
	},
	columnIndustry: {
		fill:    `UPDATE company SET industry = $2 WHERE id = $1 AND archived_at IS NULL AND industry IS NULL`,
		replace: `UPDATE company SET industry = $2 WHERE id = $1 AND archived_at IS NULL AND industry IS DISTINCT FROM $2`,
	},
	...
	// address column that changes (the company_geocode migration), so no
	...
	columnAddress: {
		fill: `UPDATE company SET address_line1 = $2 WHERE id = $1 AND archived_at IS NULL
		       AND address_line1 IS NULL AND address_city IS NULL AND address_postal_code IS NULL`,
		replace: `UPDATE company SET address_line1 = $2 WHERE id = $1 AND archived_at IS NULL
	...
	columnDescription: {
		fill: `UPDATE company SET description = $2 WHERE id = $1 AND archived_at IS NULL
		       AND description IS NULL AND $2::text IS NOT NULL`,
		replace: `UPDATE company SET description = $2 WHERE id = $1 AND archived_at IS NULL
		          AND description IS DISTINCT FROM $2`,
	},
}

// companyDescriptionMax is what company_description_length admits — the CHECK
...
const companyDescriptionMax = 500
