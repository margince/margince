...
func readGhostKeys(ctx context.Context, tx pgx.Tx, onlyOwner ids.UUID) ([]ghostKeyRow, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("contacts: reading LinkedIn keys to re-normalize: %w", err)
	}
	defer rows.Close()
	var all []ghostKeyRow
	for rows.Next() {
...
func collapseGroup(ctx context.Context, tx pgx.Tx, group []ghostKeyRow, wanted map[ids.UUID]string) (merged, rekeyed int, err error) {
	...
	for _, r := range group {
		...
		if _, err := tx.Exec(ctx, `DELETE FROM linkedin_connection WHERE id = $1`, r.id); err != nil {
			return 0, 0, fmt.Errorf("contacts: collapsing a duplicate LinkedIn connection: %w", err)
		}
		merged++
	}
	if stored(keep.stored) != wanted[keep.id] {
		if _, err := tx.Exec(ctx, `
			...
			return 0, 0, fmt.Errorf("contacts: re-keying a LinkedIn connection: %w", err)
		}
		rekeyed++
	}
...
func foldGhostInto(ctx context.Context, tx pgx.Tx, keep, doomed ids.UUID) error {
	_, err := tx.Exec(ctx, `
		UPDATE linkedin_connection k
		   SET email               = coalesce(k.email, d.email),
		       ...
		       -- The contact travels WITH the winning decision. Coalescing it
		       ...
		       matched_contact_id = CASE
		         WHEN array_position($3::text[], d.match_status) > array_position($3::text[], k.match_status)
		           THEN d.matched_contact_id ELSE coalesce(k.matched_contact_id, d.matched_contact_id) END,
	...
	if err != nil {
		return fmt.Errorf("contacts: folding a duplicate LinkedIn connection into its survivor: %w", err)
	}
	return nil
}
...
// so the row that is kept is the row whose matched_contact_id the merged record
