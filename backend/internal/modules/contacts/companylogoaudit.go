...
// The write shape of a company's logo. It lives apart from the resolve
...
// logoWrite is one change to a company's mark, as the record of it needs
...
// the audit row, and the company.updated event that links both into the
// trace. It runs inside the caller's transaction, so the mark and the record
// of who set it commit together or not at all.
func recordLogoWrite(ctx context.Context, tx pgx.Tx, id ids.CompanyID, slot LogoSlot, write logoWrite) error {
	...
	field := slot.field()
	if err := storekit.StampFields(ctx, tx, "company", id.UUID, write.source, write.by,
	...
	auditID, err := storekit.AuditWithEvidence(ctx, tx, "update", "company", id.UUID,
