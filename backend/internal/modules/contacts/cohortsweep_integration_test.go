...
func TestTheSweepFindsTheContactsOwedARepairAndThenStops(t *testing.T) {
	...
	owed, err := e.store.ContactsOwedACohortRepair(ctx, 50)
	if err != nil {
		t.Fatalf("listing the owed: %v", err)
	}
	if !containsContact(owed, res.ContactID) {
		t.Fatalf("the sweep does not see %s, whose captured mail is not on their record", res.ContactID)
	}

	if _, err := e.store.RepairContactCohort(ctx, res.ContactID); err != nil {
		t.Fatalf("repair: %v", err)
	}
	if linked, named := e.cohortStateOf(ctx, t, stranded, res.ContactID); !linked || !named {
	...
	after, err := e.store.ContactsOwedACohortRepair(ctx, 50)
	if err != nil {
		t.Fatalf("listing the owed again: %v", err)
	}
	if containsContact(after, res.ContactID) {
...
// A participant row still naming a merged-away contact is the shape that used to
...
func TestTheSweepDrainsAContactWhoseParticipantRowPredatesAMerge(t *testing.T) {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		if _, err := tx.Exec(ctx,
			`UPDATE contact SET merged_into_id = $2 WHERE id = $1`,
			retired.ContactID, survivor.ContactID); err != nil {
			return err
		}
		// The address follows the survivor, as a merge moves it.
		if _, err := tx.Exec(ctx,
			`UPDATE contact_email SET contact_id = $2, is_primary = false WHERE contact_id = $1`,
			retired.ContactID, survivor.ContactID); err != nil {
			return err
		}
		...
		_, err := tx.Exec(ctx, `
			...
			VALUES ($1, $2, $3, 'cc')`, ids.NewV7(), stranded, retired.ContactID)
		return err
	}); err != nil {
		t.Fatal(err)
	}
	...
	if containsContact(owed, retired.ContactID) {
		t.Error("the sweep offers a merged-away record; a repair under it lands on a page nobody can open")
	}
	if !containsContact(owed, survivor.ContactID) {
		t.Fatalf("the sweep does not offer the survivor %s, who now owns the stranded mail", survivor.ContactID)
	}

	if _, err := e.store.RepairContactCohort(ctx, survivor.ContactID); err != nil {
		t.Fatalf("repair: %v", err)
	}
	if linked, _ := e.cohortStateOf(ctx, t, stranded, survivor.ContactID); !linked {
	...
	if containsContact(after, survivor.ContactID) {
