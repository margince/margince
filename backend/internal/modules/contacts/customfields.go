...
// ActiveContactColumns is the caller-side half of GetContactTx: a caller that
...
// this package.
//
// It takes contact:read, the grant the read it feeds takes, so the refusal
...
func (s *Store) ActiveContactColumns(ctx context.Context) (CustomColumns, error) {
	return s.activeCustomColumns(ctx, "contact", principal.ActionRead)
}

// ActiveCompanyColumns is ActiveContactColumns for GetCompanyTx, and
...
// ActiveLeadColumns is ActiveContactColumns for FillEmptyLeadFieldsTx. It takes
...
var ErrCustomFieldsNeedTheStoresOwnTransaction = errors.New(
	"contacts: a caller-opened create cannot carry custom fields — the store-opened entry point reads the catalog before it opens its transaction")
