...
// queue is the near-match a key cannot catch: the same contact keyed in twice
// under two spellings, or once with an address and once without. Detection
// reads the lead's own columns and NEVER the contact table; a lead is proposed
...
// contact weight, and the "same employer" term is answered from what a lead
...
// PO-F-1 does for a contact, so a lead pair and a contact pair at the same
