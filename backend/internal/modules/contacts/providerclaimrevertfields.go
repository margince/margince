...
func revertOne(ctx context.Context, tx pgx.Tx, f appliedField) (bool, error) {
	...
	case tableContactSocial:
		return revertSocialHandle(ctx, tx, f)
	case tableContactEmail, tableContactPhone:
...
// revertColumn clears contact.title while it still says what was written.
//
...
func revertColumn(ctx context.Context, tx pgx.Tx, f appliedField) (bool, error) {
	if err := auth.EnsureRetractable(ctx, tx, entityContact, f.subject); err != nil {
		return false, err
	}
	if f.field != fieldTitle || f.value == nil {
		// A contact column this build cannot take back. It is left standing and
...
//
// By ROW ID, not by value. contact_social carries no source column, and the
...
func revertSocialHandle(ctx context.Context, tx pgx.Tx, f appliedField) (bool, error) {
	if err := auth.EnsureWritable(ctx, tx, entityContact, f.subject); err != nil {
		return false, err
	}
	if f.rowID == nil {
	...
	return true, touchRevertedContact(ctx, tx, f.subject)
}
...
// touchRevertedContact bumps the aggregate after a child row went, for an
// ARCHIVED contact as readily as a live one.
//
// touchContact refuses an archived subject, which is right for every writer that
...
func touchRevertedContact(ctx context.Context, tx pgx.Tx, contactID ids.UUID) error {
	if _, err := tx.Exec(ctx,
		`UPDATE contact SET updated_at = now() WHERE id = $1`, contactID); err != nil {
		return fmt.Errorf("contacts: bumping the contact a revert changed: %w", err)
	}
	return nil
}
