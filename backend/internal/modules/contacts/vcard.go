...
// Reading a .vcf file — the card a contact handed over, or exported from the
// address book they keep themselves.
//
// It lives in this package rather than in a shared one because there is exactly
// one caller: the import that turns a card into a contact. A format package with
...
// than a contact assembled out of the fields that happened to parse.
...
// silently skipped: an import that quietly drops a contact is worse than one
// that refuses, because the reader has no way to notice the contact who is
// missing.
func ParseVCards(r io.Reader) ([]VCardEntry, error) {
	...
	for _, line := range lines {
		...
		case "BEGIN":
			...
			// properties under it files a calendar's fields onto a contact —
			...
			if !strings.EqualFold(value, "VCARD") {
				return nil, fmt.Errorf("contacts: this file begins a %q, which is not a vCard", value)
			}
			...
			// 200, and the missing contact is the one who never gets followed
		...
		case "END":
			if !strings.EqualFold(value, "VCARD") {
				return nil, fmt.Errorf("contacts: this file ends a %q, which is not a vCard", value)
			}
...
func unfoldVCardLines(r io.Reader) ([]string, error) {
	...
	for scanner.Scan() {
		...
		if read > vcardMaxBytes {
			...
			return nil, fmt.Errorf("contacts: the vCard file exceeds %d bytes", vcardMaxBytes)
		}
		if len(lines) > vcardMaxLines {
			return nil, fmt.Errorf("contacts: the vCard file holds more than %d lines", vcardMaxLines)
		}
	...
	if err := scanner.Err(); err != nil {
		return nil, fmt.Errorf("contacts: reading the vCard file: %w", err)
	}
	return lines, nil
}
...
func applyVCardProperty(entry *VCardEntry, name string, params []string, raw string) {
	...
	case "N":
		// Only when FN said nothing: FN is the display name the contact chose,
...
func nameFromStructured(params []string, raw string) string {
	...
	var ordered []string
	// given, then family — the two components a contact is addressed by. The
	// prefixes and suffixes are dropped: "Dr." is not part of a name this
	// product matches on, and carrying it makes two records of one contact.
