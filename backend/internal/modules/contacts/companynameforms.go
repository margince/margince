...
// Company-name matching for the markets that write the legal form BEFORE the name.
//
...
//
// NOT a second normalizer for the company-name KEY: NormalizeCompanyName is unchanged
...
// (PO-PARAM-JW-2) and shared with contact and lead matching.
...
func stripLeadingMarkers(fields []string, markers []companyFormMarker, corroborated bool) []string {
	for {
		longest := 0
...
//
// NOT done inside NormalizeCompanyName, which also produces exact grouping keys —
...
// the same rule NormalizeCompanyName follows when a suffix IS the company.
...
func corroboratedMarket(raw string, fields []string, market marketForms) bool {
	...
	for _, closer := range market.closers {
		// Asked of the RAW name, because the suffix strip in NormalizeCompanyName
...
// companyNameForMatching is the name reduced to what could identify a company.
//
...
// evidence, never as a wildcard — which is the one thing NormalizeCompanyName may
// not do (namesim.go).
func companyNameForMatching(s string) string {
	name, _ := matchingFormOf(s)
	return name
}

// matchingFormOf is companyNameForMatching plus the market the name declared, which
...
func matchingFormOf(s string) (string, *marketForms) {
	...
	// (companynamecjk.go). Splitting them into words yields the whole name as one
	// token, so every path below would find nothing to strip and nothing to
	// compare.
	if name, isCJK := cjkNameForMatching(s); isCJK {
		return name, nil
	}
	fields := strings.FieldsFunc(NormalizeCompanyName(foldDStroke(s)), nameWordSeparators)
