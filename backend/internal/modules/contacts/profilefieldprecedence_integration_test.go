...
// The precedence rule of contact_profile_field, over a real Postgres, in BOTH
// directions.
//
...
// saying a contact's decision had been measured by a model that never saw it.
...
func storedConfidence(ctx context.Context, t *testing.T, e *dedupeEnv, contactID ids.ContactID, field string) *float64 {
	t.Helper()
	var got *float64
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`SELECT confidence FROM contact_profile_field WHERE contact_id = $1 AND field = $2`,
...
func fillFromSignature(ctx context.Context, t *testing.T, e *dedupeEnv, contactID ids.ContactID, f SignatureField) bool {
	t.Helper()
	return fillFromSignatureObserved(ctx, t, e, contactID, time.Now(), f)
}
...
func TestAMachineFillNeverReplacesWhatAHumanAccepted(t *testing.T) {
	...
	if _, err := e.store.SaveResearchClaims(ctx, contactID, []ResearchClaimInput{accepted}); err != nil {
	...
	if rows := claimRowsFor(ctx, t, e, contactID); rows != 1 {
		t.Errorf("rows under this contact = %d, want 1 — the fill landed a second row under a key the reader cannot see", rows)
	}
}
...
func TestAnAcceptanceReplacesAMachineFillAndItsScore(t *testing.T) {
	...
	if score := storedConfidence(ctx, t, e, contactID, "role"); score == nil || *score != 0.9 {
	...
	got := readStoredClaim(ctx, t, e, contactID, "role")
	...
	if score := storedConfidence(ctx, t, e, contactID, "role"); score != nil {
		t.Errorf("confidence = %v after a human's acceptance, want NULL: the row would otherwise say a "+
			"contact's decision had been scored by a model that never saw it", *score)
	}
}
...
// transaction, with the contact archived between the entry gate and the write.
...
func TestARefusedWriteReportsThatItDidNotLand(t *testing.T) {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		if _, err := tx.Exec(ctx,
			`UPDATE contact SET archived_at = now() WHERE id = $1`, contactID); err != nil {
			return err
		}
		var err error
		...
		t.Fatalf("run the write against an archived contact: %v", err)
	}
	if landed {
		t.Error("the writer reported a row landed on an archived contact")
	}
	if rows := claimRowsFor(ctx, t, e, contactID); rows != 0 {
		t.Errorf("rows for the archived contact = %d, want 0", rows)
	}
}
...
func TestAFillHoldsTheSubjectAgainstAConcurrentErasure(t *testing.T) {
	...
	erasure := e.store.tx(ctx, func(tx pgx.Tx) error {
		...
		_, err := tx.Exec(ctx, `UPDATE contact SET archived_at = now() WHERE id = $1`, contactID)
		return err
	})
	close(release)
	...
	if !errors.As(erasure, &pgErr) || pgErr.Code != "55P03" {
		t.Fatalf("the erasure got %v, want a lock timeout (55P03) — a fill in flight must hold the "+
			...
			"the row lands on a contact it had just cleared", erasure)
	}
}
