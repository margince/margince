...
// exists instead of minting a second one for the same contact.
//
// Routing alone is not enough, which is why the binding is asserted too: without
// it the contact is unreachable for a reply, invisible to a channel-keyed
...
func TestACorroboratingAddressAdoptsTheMailCapturedIncumbent(t *testing.T) {
	...
	incumbent := e.seedContact(e.as(), t, "Tuyen Dinh Quang", []string{email}, nil)
	...
	if res.ContactID != incumbent {
		t.Fatalf("ensure landed on %s, want the mail-captured incumbent %s — one human must not become two records", res.ContactID, incumbent)
	}
	if res.ContactCreated {
		t.Error("a second contact was created for a human the address already found")
	}
	if n := e.countInWorkspace(ctx, t,
		`SELECT count(*) FROM contact_channel_identity WHERE contact_id = $1 AND channel_user_id = $2 AND archived_at IS NULL`,
	...
	if n := e.countInWorkspace(ctx, t, `SELECT count(*) FROM contact_email WHERE contact_id = $1`, incumbent); n != 1 {
...
func TestAdoptingTheIncumbentTwiceWritesNothingTheSecondTime(t *testing.T) {
	...
	incumbent := e.seedContact(e.as(), t, "Dung Nguyen", []string{email}, nil)
	...
	auditsAfterFirst := e.countInWorkspace(ctx, t,
		`SELECT count(*) FROM audit_log WHERE entity_type = $1 AND entity_id = $2`, entityContact, incumbent)
	second, err := e.store.EnsureChannelCounterparty(ctx, e.corroboratedInput(t, ci, "Dung", email))
	if err != nil {
		t.Fatalf("second ensure: %v", err)
	}

	if second.ContactID != incumbent || second.ContactCreated {
		t.Fatalf("second ensure = %+v, want the incumbent %s reused", second, incumbent)
	}
	if n := e.countInWorkspace(ctx, t,
		`SELECT count(*) FROM contact_channel_identity WHERE contact_id = $1 AND archived_at IS NULL`, incumbent); n != 1 {
	...
	if n := e.countInWorkspace(ctx, t,
		...
		entityContact, incumbent); n != auditsAfterFirst {
...
// contact WITH it. An addressless record is one the next mail from the same human
// cannot match, so it mints a second record.
func TestAMintedChannelContactKeepsTheCorroboratingAddress(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.asChannelConnector()
	...
	if !res.ContactCreated {
		t.Fatal("no contact was created for an account nothing else knew")
	}
	var got string
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx, `SELECT email FROM contact_email WHERE contact_id = $1`, res.ContactID).Scan(&got)
	}); err != nil {
		t.Fatalf("reading the minted contact's address: %v", err)
	}
	if got != email {
...
func TestAnErasedAddressIsNotResurrectedByADirectMessage(t *testing.T) {
	...
	if n := e.countInWorkspace(ctx, t,
		`SELECT count(*) FROM contact_channel_identity WHERE channel_user_id = $1`, ci.ChannelUserID); n != 0 {
...
func TestLosingTheBindRaceStillRaisesTheIdentityReview(t *testing.T) {
	...
	incumbent := e.seedContact(e.as(), t, "Mail Captured", []string{email}, nil)
	rival := e.seedContact(e.as(), t, "Channel Captured", nil, nil)
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		return e.store.adoptEmailRoutedIncumbent(ctx, tx,
			...
			// What DedupeContact returns when the channel lane missed and the
	...
	if res.ContactID != rival {
		t.Fatalf("landed on %s, want the account's real owner %s — the binding is the arbiter, not the address", res.ContactID, rival)
	}
	if res.Conflict == nil {
		t.Fatal("no conflict reported — the address names one contact and the account another, and nothing downstream would ever surface it")
	}
...
// An address a connector's directory vouched for identifies the contact, and it
// must not also settle the MAIL ladder's question about that address.
//
// The ladder reads "a live contact holds this address" as a verdict of `real`,
// and the noise sweep refuses to touch an address a contact holds. That reading
...
func TestAVouchedAddressIsNotEvidenceOfCorrespondence(t *testing.T) {
	...
	// Collected: the contact is identified by it, which is what it is for.
	if n := e.countInWorkspace(ctx, t,
		`SELECT count(*) FROM contact_email WHERE contact_id = $1 AND email = $2`, res.ContactID, vouched); n != 1 {
		t.Fatalf("%d address rows on the minted contact, want the vouched address collected", n)
	}
	// But not as correspondence, which is what the mail ladder reads.
	if n := e.countInWorkspace(ctx, t,
		`SELECT count(*) FROM contact_email WHERE contact_id = $1 AND from_correspondence`, res.ContactID); n != 0 {
		t.Errorf("%d of the minted contact's addresses count as correspondence — one direct message would settle the mail ladder's verdict on a stranger's address", n)
	}
}
...
func TestAMailCapturedAddressStillCountsAsCorrespondence(t *testing.T) {
	...
	contact := e.seedContact(e.as(), t, "Real Contact", []string{known}, nil)

	if n := e.countInWorkspace(e.as(), t,
		`SELECT count(*) FROM contact_email WHERE contact_id = $1 AND from_correspondence`, contact); n != 1 {
		t.Errorf("%d of a normally created contact's addresses count as correspondence, want 1", n)
	}
}
