...
//
// contactname.go reads one message's header and is deliberately willing to
...
// app_user row with that address and a full name a human typed.
//
...
// workspace already holds their full name, typed by a contact.
func KnownHumanName(ctx context.Context, tx pgx.Tx, email string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("contacts: reading the human behind %s: %w", normalized, err)
	}
...
// holds their full name, typed by a contact: better evidence than a header, and
// not a guess.
//
// It only ever fills a blank. A CONFIDENT header parse stands, because the
// header is what that contact calls themselves in mail, and a stale app_user
...
func (s *Store) nameCounterparty(ctx context.Context, tx pgx.Tx, in EnsureCounterpartyInput) (ParsedName, error) {
	parsed := ParseContactName(in.DisplayName, in.Email)
	if parsed.Confident {
		return parsed, nil
	}
	...
	fromUser := ParseContactName(known, in.Email)
	if fromUser.Full == "" {
		return parsed, nil
	}
	return fromUser, nil
...
func addressAttestedTx(ctx context.Context, tx pgx.Tx, email string) (bool, error) {
	...
	if err := tx.QueryRow(ctx, `
		...
		return false, fmt.Errorf("contacts: reading whether %s was written to: %w", normalized, err)
	}
	return attested, nil
}
