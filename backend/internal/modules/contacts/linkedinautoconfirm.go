...
// erasure clears contact_social and then deletes the connection, and a confirm
...
// scanCandidates reads the (ghost, contact, owner, confirmable) rows both tiers
...
func scanCandidates(rows pgx.Rows) ([]matchCandidate, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ghost, &c.contact, &c.owner, &c.confirmable); err != nil {
			return nil, err
		}
		out = append(out, c)
	}
...
func confirmableContacts(cands []matchCandidate) []ids.UUID {
	...
	var out []ids.UUID
	for _, c := range cands {
		if c.confirmable && !seen[c.contact] {
			seen[c.contact] = true
...
// one ascending order. That order is the whole point: contact-before-connection
...
func holdConfirmContacts(ctx context.Context, tx pgx.Tx, sets ...[]matchCandidate) error {
	...
	for _, id := range contacts {
		if err := auth.HoldWritableLive(ctx, tx, entityContact, id); err != nil {
			return err
		}
	}
	return nil
...
func confirmOneMatch(ctx context.Context, tx pgx.Tx, c matchCandidate) (bool, error) {
	...
	if err := auth.HoldWritableLive(ctx, tx, entityContact, c.contact); err != nil {
		return false, err
	}
	...
	err := tx.QueryRow(ctx, `
		UPDATE linkedin_connection c
		   SET matched_contact_id = $2, match_status = 'confirmed', updated_at = now()
	...
	if err != nil {
		return false, fmt.Errorf("contacts: auto-confirming a LinkedIn match: %w", err)
	}
	if err := confirmMatchWriteTail(ctx, tx, c.ghost, c.owner, c.contact, wasStatus, wasContact); err != nil {
		return false, err
	}
	return true, nil
}
...
// no lock on the contact, and no audit on the contact, because a suggestion
...
func suggestOneMatch(ctx context.Context, tx pgx.Tx, c matchCandidate) (bool, error) {
	tag, err := tx.Exec(ctx, `
		UPDATE linkedin_connection
		   SET matched_contact_id = $2, match_status = 'suggested', updated_at = now()
		 WHERE id = $1 AND match_status = 'unmatched' AND tombstoned_at IS NULL`,
		c.ghost, c.contact)
	if err != nil {
		return false, fmt.Errorf("contacts: suggesting a LinkedIn match: %w", err)
	}
