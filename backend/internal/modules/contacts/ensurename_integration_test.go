...
//
// The unit table in contactname_test.go proves the parser reads a header
...
func (e *dedupeEnv) storedName(ctx context.Context, t *testing.T, id ids.ContactID) (full string, first, last *string) {
	t.Helper()
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`SELECT full_name, first_name, last_name FROM contact WHERE id = $1`, id).
...
func TestEnsureCounterpartyStoresAParsedName(t *testing.T) {
	...
	full, first, last := e.storedName(ctx, t, res.ContactID)
...
func TestEnsureCounterpartyLeavesSplitNamesNullWhenItCannotTell(t *testing.T) {
	...
	// A lone surname names the contact but says nothing about a given name, and
...
func TestEnsureCounterpartyFillsASplitNameItLearnsLater(t *testing.T) {
	...
	if err != nil || !first.ContactCreated {
		t.Fatalf("first ensure = %+v (err %v), want a created contact", first, err)
	}
	if _, f, l := e.storedName(ctx, t, first.ContactID); f != nil || l != nil {
	...
	if second.ContactID != first.ContactID {
		t.Fatalf("second ensure minted %s, want the incumbent %s", second.ContactID, first.ContactID)
	}
	_, f, l := e.storedName(ctx, t, first.ContactID)
...
// replaced by the contact's real name once we learn it.
//
...
func TestALearnedNameReplacesAnAddressBookLabel(t *testing.T) {
	...
	if full, _, _ := e.storedName(ctx, t, first.ContactID); full != "Bw" {
	...
	full, f, l := e.storedName(ctx, t, first.ContactID)
...
// And the one thing that outranks it: a contact who edited the name by hand.
//
// Driven through the real UpdateContact door rather than a raw UPDATE, because
...
func TestALearnedNameLeavesAHandEditedDisplayNameAlone(t *testing.T) {
	...
	chosen := "BW (Frankfurt)"
	if _, err := e.store.UpdateContact(ctx, res.ContactID, UpdateContactInput{FullName: &chosen}); err != nil {
	...
	full, f, l := e.storedName(ctx, t, res.ContactID)
	if full != chosen {
	...
	// display is the contact's to keep.
...
// authority on that contact, and no later header may rewrite it.
func TestEnsureCounterpartyNeverOverwritesANameAHumanSet(t *testing.T) {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `
			UPDATE contact SET full_name = $2, first_name = $3, last_name = $4 WHERE id = $1`,
			res.ContactID, "Wolfgang Schmitt-Rink", "Wolfgang", "Schmitt-Rink")
		return err
	}); err != nil {
		t.Fatal(err)
	}
	...
	if again.ContactID != res.ContactID {
		t.Fatalf("second ensure minted %s, want the incumbent %s", again.ContactID, res.ContactID)
	}
	full, first, last := e.storedName(ctx, t, res.ContactID)
...
// by a contact. The header alone would parse to "Lars" with no first or last,
...
// not overwrite what a contact signs their own mail with.
func TestAConfidentHeaderOutranksAKnownHumansStoredName(t *testing.T) {
	...
	if full, _, last := e.storedName(ctx, t, res.ContactID); full != "Anna Weber" || nameOrNull(last) != "Weber" {
...
// The record a contact LOOKS at has to change. A fill that writes the split
...
func TestFillingASplitNameAlsoFixesTheDisplayedName(t *testing.T) {
	...
	if full, _, _ := e.storedName(ctx, t, first.ContactID); full != "Lars" {
