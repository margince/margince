...
// would be destroying a contact's work to undo a purchase. So nothing is cleared
...
// set of contacts — and the eraser takes those rows subject-first, so it is also
...
func SubjectsWithProviderFills(ctx context.Context, tx pgx.Tx, providerName string) ([]ids.UUID, error) {
	rows, err := tx.Query(ctx,
		`SELECT DISTINCT contact_id FROM provider_applied_field WHERE provider = $1`, providerName)
	if err != nil {
		return nil, fmt.Errorf("contacts: reading whose records a purchase filled: %w", err)
	}
	defer rows.Close()
...
func RevertProviderFills(ctx context.Context, tx pgx.Tx, providerName string, subject ids.UUID) (RevertedSubject, error) {
	out := RevertedSubject{ContactID: subject}
	if err := auth.EnsureWritable(ctx, tx, entityContact, subject); err != nil {
		return out, err
	}
	...
	if _, err := storekit.LockRow(ctx, tx, entityContact, subject, storekit.IncludeArchived); err != nil {
		return out, err
	}
	...
	if _, err := tx.Exec(ctx,
		`DELETE FROM provider_applied_field WHERE provider = $1 AND contact_id = $2`,
		providerName, subject); err != nil {
		return out, fmt.Errorf("contacts: forgetting what a purchase filled: %w", err)
	}
	if len(out.Fields) == 0 {
		return out, nil
	}
...
func appliedFieldsFor(ctx context.Context, tx pgx.Tx, providerName string, subject ids.UUID) ([]appliedField, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("contacts: reading what a purchase filled: %w", err)
	}
	defer rows.Close()
...
func auditReverted(ctx context.Context, tx pgx.Tx, providerName string, r RevertedSubject) error {
	...
	auditID, err := storekit.AuditWithEvidence(ctx, tx, "update", entityContact, r.ContactID, before, after,
		map[string]any{auditKeyProvider: providerName, "fields": r.Fields})
	if err != nil {
		return err
	}
	return storekit.EmitEvent(ctx, tx, auditID, r.ContactID,
