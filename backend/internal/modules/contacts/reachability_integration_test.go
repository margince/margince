...
// The reachability projection (design §6.6) on the contact read: a live,
...
// reports unreachable. Both assert through the real GetContact path, the same
...
func (e *dedupeEnv) channelIdentityTimestamps(ctx context.Context, t *testing.T, ci connector.ChannelIdentity) (createdAt time.Time, blockedAt *time.Time) {
	...
	err := e.store.tx(ctx, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`SELECT created_at, blocked_at FROM contact_channel_identity
...
func TestContactReadReportsTelegramReachability(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	contact := e.seedContact(ctx, t, "Telegram Contact", nil, nil)
	...
	got, err := e.store.GetContact(ctx, contact, storekit.LiveOnly)
	if err != nil {
...
func TestBlockedIdentityReportsUnreachableButKeepsTheIdentity(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	contact := e.seedContact(ctx, t, "Blocked Telegram Contact", nil, nil)
...
// The SEND side of the same fact: the reply path asks where a contact can be
...
func TestReachableChannelIdentitiesOmitsABlockedAccountAndReturnsEveryLiveOne(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	contact := e.seedContact(ctx, t, "Two Account Contact", nil, nil)
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) (err error) {
		reachable, err = ReachableChannelIdentities(ctx, tx, contact, telegramProvider)
		return err
	}); err != nil {
...
// A contact with no binding at all is unreachable, and that is an ANSWER: an
// empty list, never an error. Reported as a fault it would reach the rep as a
// server problem instead of "this contact has not messaged you".
func TestReachableChannelIdentitiesAnswersEmptyForAContactWithNoBinding(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	contact := e.seedContact(ctx, t, "Mail Only Contact", nil, nil)
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) (err error) {
		...
		t.Fatalf("ReachableChannelIdentities on a contact with no binding → %v, want no error", err)
	}
	if len(reachable) != 0 {
