...
// has gone quiet, the contact nobody has tried is the only move that is not
// another follow-up.
func TestRankContactsOrdersWaitingThenAnsweredThenUntriedThenNoReply(t *testing.T) {
	...
	want := []ids.ContactID{waiting.ContactID, answered.ContactID, untried.ContactID, silent.ContactID}
	assertOrder(t, got, want)
}

// The silent contact here scores highest. If score led, it would sort first and
// the page would open on the one contact whose next move needs a reason.
func TestRankContactsPutsEngagementAboveScore(t *testing.T) {
	...
	assertOrder(t, got, []ids.ContactID{weakAnswered.ContactID, strongSilent.ContactID})
}
...
func TestRankContactsBreaksAScoreTieByStrengthThenID(t *testing.T) {
	...
	assertOrder(t, got, []ids.ContactID{tiedEarlyID.ContactID, strongerLateID.ContactID, weaker.ContactID})
}
...
func TestRankContactsIsStableAcrossInputOrder(t *testing.T) {
	...
	for i := range first {
		if first[i].ContactID != second[i].ContactID {
			t.Fatalf("position %d differs between input orders: %s vs %s",
				i, first[i].ContactID.UUID, second[i].ContactID.UUID)
		}
	}
}
...
func contactAt(t *testing.T, id string, rs RelationshipStrength) ContactStrength {
	...
	return ContactStrength{ContactID: ids.ContactID{UUID: parsed}, Strength: rs}
}

func assertOrder(t *testing.T, got []ContactStrength, want []ids.ContactID) {
	t.Helper()
	if len(got) != len(want) {
		t.Fatalf("got %d contacts, want %d", len(got), len(want))
	}
	for i := range want {
		if got[i].ContactID != want[i] {
			t.Fatalf("position %d: got %s, want %s", i, got[i].ContactID.UUID, want[i].UUID)
		}
	}
}
...
// contacts actually waiting on a reply — and a rank naming no state is a value
