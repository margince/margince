...
func TestEveryVatCheckStatementRunsAgainstTheRealSchema(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("seeding the company to check: %v", err)
	}
	...
	if _, err := e.store.VatCheckFor(ctx, companyID); err == nil {
	...
	stored, err := e.store.VatCheckFor(ctx, companyID)
	if err != nil {
	...
	after, err := e.store.VatCheckFor(ctx, companyID)
	if err != nil {
...
func TestOnlyAnUncheckedNumberIsWorthConsulting(t *testing.T) {
	...
	bare, err := e.store.CreateCompany(ctx, CreateCompanyInput{
	...
	if _, worth, err := e.store.VatNumberForCheck(ctx, ids.From[ids.CompanyKind](ids.UUID(bare.Id))); err != nil {
	...
	companyID, err := e.store.ApplyColdStartProfile(ctx, ApplyColdStartProfileInput{
	...
	if err := e.store.RecordVatCheck(ctx, VatCheck{
		CompanyID: companyID, Number: stated, Status: VatCheckValid, CheckedAt: consultedAt,
	}); err != nil {
		t.Fatalf("recording the consultation: %v", err)
	}
	if _, worth, err := e.store.VatNumberForCheck(ctx, companyID); err != nil {
...
func TestAnUnansweredConsultationIsAskedAgain(t *testing.T) {
	...
	if err := e.store.RecordVatCheck(ctx, VatCheck{
		CompanyID: companyID, Number: stated,
...
func TestWhitespaceDoesNotMakeANumberLookUnchecked(t *testing.T) {
	...
	if err := e.store.RecordVatCheck(ctx, VatCheck{
		CompanyID: companyID, Number: spaced,
