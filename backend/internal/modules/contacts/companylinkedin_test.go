...
// company_linkedin_url_key is unique on lower(linkedin_url) among live
// rows — two spellings of one company must not both be storable. And it has to
// satisfy company_linkedin_url_shape, the column CHECK that refuses
...
// columnShape mirrors company_linkedin_url_shape from migration 0191. A
...
func TestNormalizeCompanyLinkedInURLCanonicalizesWhatTheUniqueIndexCompares(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			got, err := normalizeCompanyLinkedInURL(tc.raw)
			if err != nil {
			...
			if !columnShape.MatchString(got) {
				t.Errorf("%q does not satisfy company_linkedin_url_shape — the store would hand the database a value it refuses", got)
			}
		})
	}
}

func TestNormalizeCompanyLinkedInURLRefusesWhatIsNotACompanyURL(t *testing.T) {
	for _, tc := range []struct {
		name string
		raw  string
		code string
	}{
		{"a contact profile", "https://www.linkedin.com/in/lars", "linkedin_url_not_company"},
...
// A cleared field is an edit, not a mistake: unlike the contact side, where the
// URL is the dedupe key, a company's LinkedIn URL is optional.
func TestCompanyLinkedInPatchValueTreatsAnEmptyInputAsAClear(t *testing.T) {
	for _, raw := range []string{"", "   "} {
		got, err := companyLinkedInPatchValue(raw)
		if err != nil {
...
func TestCompanyLinkedInPatchValueNormalizesBeforeItWrites(t *testing.T) {
	got, err := companyLinkedInPatchValue("  www.linkedin.com/company/voltaq?trk=nav  ")
	if err != nil {
...
func TestCompanyLinkedInPatchValueRefusesAContactProfile(t *testing.T) {
	if _, err := companyLinkedInPatchValue("https://www.linkedin.com/in/lars"); err == nil {
		t.Fatal("a contact profile stored on a company would collide two kinds of record in one unique index")
	}
}
