...
// Every production call to createContact is a door a contact can arrive
// through, and each must leave an acquisition record. The write lives inside
// createContact so this is true by construction — this proves the construction,
...
func TestEveryCreationDoorRecordsWhyTheContactExists(t *testing.T) {
	...
	var createContactCalls, recordsAcquisition int
	...
	if createContactCalls == 0 {
		t.Fatal("found no createContact calls — the scan is looking in the wrong place")
	}
	...
	if recordsAcquisition != 1 {
		t.Errorf("recordAcquisition is called %d times, want exactly 1 (inside createContact) — "+
...
// contact initiating contact. Recording the weaker case as the stronger would
...
func TestOnlyARepliedCounterpartyInitiatedContact(t *testing.T) {
	if got := acquiredFromCapture(true); got != AcquiredSubjectInitiated {
		t.Errorf("a reply is the contact writing to us: got %q", got)
	}
