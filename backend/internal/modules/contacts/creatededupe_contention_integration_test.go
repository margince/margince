...
// lives — past its probe and its contact_phone insert, before its commit —
...
// write identity and its contact_phone row is written but visible to nobody
...
// would hang instead.
func (e *dedupeEnv) beginHeldPhoneCreate(ctx context.Context, t *testing.T, name, phone string) (pgx.Tx, ids.ContactID, int) {
	t.Helper()
	...
	keys, err := phoneLaneKeys([]ContactPhoneInput{{Phone: phone}})
	if err != nil {
	...
	if _, err := tx.Exec(ctx,
		`INSERT INTO contact (id, full_name, source, captured_by) VALUES ($1, $2, 'manual', $3)`,
		id, name, by); err != nil {
		t.Fatalf("inserting the in-flight contact: %v", err)
	}
	if err := insertContactPhones(ctx, tx, id, "manual", by,
		[]ContactPhoneInput{{Phone: phone, PhoneType: "mobile", IsPrimary: true, Position: 1}}); err != nil {
		t.Fatalf("inserting the in-flight contact's number: %v", err)
	}
	return tx, id, pid
}
...
func (e *dedupeEnv) createInBackground(ctx context.Context, in CreateContactInput) <-chan racingCreate {
	done := make(chan racingCreate, 1)
	go func() {
		contact, err := e.store.CreateContact(ctx, in)
		done <- racingCreate{contact: contact, err: err}
	}()
	return done
}

// Two contacts can legitimately share a switchboard, so contact_phone has no
// cross-contact unique index and nothing structural stops two creates carrying
...
func TestASecondCreateSharingAPhoneStillRecordsTheReviewPair(t *testing.T) {
	...
	second := e.createInBackground(ctx, CreateContactInput{
		FullName: "Bruno Kellerwald", Source: "manual",
		Emails: []ContactEmailInput{{Email: "bruno@kellerwald.test", EmailType: "work", IsPrimary: true}},
		Phones: []ContactPhoneInput{{Phone: switchboard, PhoneType: "work", IsPrimary: true}},
	})
	...
	secondID := ids.From[ids.ContactKind](ids.UUID(res.contact.Id))
	...
	if n := e.countInWorkspace(ctx, t, `
		SELECT count(*) FROM contact_phone ph
		  JOIN contact p ON p.id = ph.contact_id
		 WHERE ph.phone = $1 AND ph.archived_at IS NULL AND p.archived_at IS NULL`,
		switchboard); n != 2 {
		t.Fatalf("%d live contacts hold %s, want 2 — both creates must have landed for this ordering to mean anything", n, switchboard)
	}

	rows := openCandidates(ctx, t, e, entityContact)
	if len(rows) != 1 {
		t.Fatalf("the review queue holds %d candidates, want exactly 1 — two live contacts share %s and no human will ever be asked about it",
...
// that has taken the lane's write identity and inserted its contact row, handed
...
// create waits on.
func (e *dedupeEnv) beginHeldNameCreate(ctx context.Context, t *testing.T, name string) (pgx.Tx, ids.ContactID, int) {
	t.Helper()
	tx, err := e.store.db.Pool().Begin(ctx)
	if err != nil {
		t.Fatalf("opening the in-flight create's transaction: %v", err)
...
// Two contacts can legitimately share a NAME, so contact has no unique index on it
...
// A lane that exists to stop a silent duplicate has to hold under two contacts
...
func TestASecondCreateSharingANameStillRecordsTheReviewPair(t *testing.T) {
	...
	if res.err != nil {
		t.Fatalf("a shared name must not refuse the second create — two contacts may share one: %v", res.err)
	}
	secondID := ids.From[ids.ContactKind](ids.UUID(res.contact.Id))
	...
	rows := openCandidates(ctx, t, e, entityContact)
	if len(rows) != 1 {
		t.Fatalf("the review queue holds %d candidates, want exactly 1 — two live contacts are written the same way and no human will ever be asked about it",
			len(rows))
	}
	pair := map[ids.UUID]bool{rows[0].LeftID: true, rows[0].RightID: true}
	if !pair[firstID.UUID] || !pair[secondID.UUID] {
