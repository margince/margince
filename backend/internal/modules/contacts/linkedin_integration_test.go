...
//   - re-importing updates rather than duplicating, because contacts re-export
...
//   - nothing ever becomes a contact.
...
func (e *dedupeEnv) ghostStatus(t *testing.T, name string) (status string, contact *ids.UUID) {
	t.Helper()
	ctx := e.as()
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`SELECT match_status, matched_contact_id FROM linkedin_connection WHERE full_name = $1`,
			name).Scan(&status, &contact)
	}); err != nil {
...
func TestAnAddressMatchConfirmsAndANameMatchOnlySuggests(t *testing.T) {
	...
	// disagree: whether two spellings are one contact is a human's judgement.
	...
	if status, contact := e.ghostStatus(t, "Dana Buyer"); status != "confirmed" || contact == nil || *contact != dana.UUID {
		t.Errorf("the address match is %q → %v, want confirmed → %s", status, contact, dana)
	}
	// A name plus an employer is plausible and no more. Auto-confirming it
	// would quietly attach a stranger to a customer record.
	if status, contact := e.ghostStatus(t, "Andreas Müller"); status != "suggested" || contact == nil || *contact != andreas.UUID {
		t.Errorf("the name+employer match is %q → %v, want suggested → %s", status, contact, andreas)
	}
...
func TestTwoContactsOfTheSameNameAtOneEmployerAreNotGuessedBetween(t *testing.T) {
	...
	first := e.seedVisibleContact(t, "Andreas Müller")
	second := e.seedVisibleContact(t, "Andreas Müller")
	...
	if status, contact := e.ghostStatus(t, "Andreas Müller"); status != "unmatched" || contact != nil {
		t.Errorf("an ambiguous name was resolved to %q → %v; picking one is a guess wearing a confirmation's clothes", status, contact)
	}
}

func TestImportingConnectionsCreatesNoContacts(t *testing.T) {
	e := setupDedupe(t)
	var before, after int
	ctx := e.as()
	count := func(into *int) {
		if err := e.store.tx(ctx, func(tx pgx.Tx) error {
			return tx.QueryRow(ctx, `SELECT count(*) FROM contact`).Scan(into)
		}); err != nil {
			t.Fatal(err)
		}
	}
	...
	count(&after)
	if after != before {
		t.Errorf("the import created %d contacts; a LinkedIn export is a list of third parties who never agreed to be in this CRM", after-before)
	}
}
...
func (e *dedupeEnv) seedEmail(t *testing.T, contact ids.ContactID, email string) {
	t.Helper()
	ctx := e.as()
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `
			INSERT INTO contact_email (contact_id, email, is_primary, source, captured_by)
			VALUES ($1, $2, true, 'manual', 'human:test')`, contact, email)
		return err
	}); err != nil {
		t.Fatalf("seeding email for %s: %v", contact, err)
	}
}

// employ puts a contact on an account's payroll, live.
func (e *dedupeEnv) employ(t *testing.T, contact ids.ContactID, company ids.CompanyID) {
	t.Helper()
	ctx := e.as()
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `
		...
		t.Fatalf("employing %s: %v", contact, err)
	}
}
