...
// name for itself is usually in the signature blocks of the contacts who work
...
// authority, however many of them agree — the contacts at one company share
...
const (
	...
	// CompanyNameCorroborationSignatures: two or more contacts at the company
	// signed with it. It outranks a lone signature but still stages for a human:
	// those contacts share one mail domain, so the agreement is one forgeable
	// source repeated, not two independent ones.
	CompanyNameCorroborationSignatures = "signatures"
	// CompanyNameCorroborationNone: one contact said so and nothing agrees — this
...
type CompanyNameVerdict struct {
	...
	// Contacts are the contacts whose signatures carry the name — the evidence a
...
func DecideCompanyName(c CompanyNameCandidate) (CompanyNameVerdict, bool) {
	...
	return CompanyNameVerdict{
		...
		Contacts:      sortedContactIDs(winner.contacts),
	}, true
}

// nameClaim is one company name several signatures agree on, with the spellings
// they used and the contacts who used them.
...
func groupSignatureNames(signatures []SignatureCompanyName, current string) map[string]*nameClaim {
	...
	for _, s := range signatures {
		...
		c, ok := claims[key]
		if !ok {
			c = &nameClaim{key: key, spelling: map[string]int{}, contacts: map[ids.ContactID]bool{}}
			claims[key] = c
		}
		c.spelling[s.Value]++
...
func sortedContactIDs(set map[ids.ContactID]bool) []ids.ContactID {
	out := make([]ids.ContactID, 0, len(set))
	for id := range set {
		out = append(out, id)
	}
...
func (s *Store) CompanyNameCandidates(ctx context.Context, after ids.CompanyID, limit int) ([]CompanyNameCandidate, error) {
		...
		rows, err := tx.Query(ctx, `
			...
			WHERE o.name_source = 'domain'
			  ...
			  AND EXISTS (
				...
				JOIN contact_profile_field f ON f.contact_id = p.id AND f.field = 'company_name'
	...
	if err != nil {
		return nil, fmt.Errorf("contacts: listing company-name promotion candidates: %w", err)
	}
	return out, nil
}
...
func (s *Store) PromoteCompanyNameTx(ctx context.Context, tx pgx.Tx, companyID ids.CompanyID, name, corroboration string) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("contacts: reading the company to promote: %w", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("contacts: promoting the company name: %w", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("contacts: auditing the company-name promotion: %w", err)
	}
	if err := storekit.EmitEvent(ctx, tx, auditID, companyID.UUID,
		...
		return false, fmt.Errorf("contacts: emitting company.updated for the promotion: %w", err)
	}
