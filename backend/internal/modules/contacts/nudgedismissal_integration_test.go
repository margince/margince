...
// aContact is one contact for a dismissal to be about. The name is fixed because
// no case here turns on it — what varies between cases is who dismissed whom.
func aContact(t *testing.T, e *dedupeEnv) ids.ContactID {
	t.Helper()
	contact, err := e.store.CreateContact(e.as(),
		CreateContactInput{FullName: "Dana Weiss", Source: "test"})
	if err != nil {
...
// predicate the projection embeds, run over the contact table.
//
...
func dismissedNow(
	ctx context.Context, t *testing.T, e *dedupeEnv, contacts []ids.ContactID, at time.Time,
) map[ids.UUID]bool {
	t.Helper()
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		...
		// The clause names `e.contact_id`, so the subject is aliased `e` here
		...
		rows, err := tx.Query(ctx, fmt.Sprintf(
			`SELECT e.contact_id FROM (SELECT unnest($%d::uuid[]) AS contact_id) e
...
func TestADismissedContactIsOffTheReadersLane(t *testing.T) {
	e := setupDedupe(t)
	contact := aContact(t, e)

	if err := e.store.DismissRelationshipNudge(e.as(), contact, 30); err != nil {
		t.Fatalf("dismissing: %v", err)
	}

	if !dismissedNow(e.as(), t, e, []ids.ContactID{contact}, time.Now())[contact.UUID] {
...
func TestADismissedContactReturnsWhenItsMomentPasses(t *testing.T) {
	...
	if err := e.store.DismissRelationshipNudge(e.as(), contact, 7); err != nil {
		t.Fatalf("dismissing: %v", err)
	}
	...
	if dismissedNow(e.as(), t, e, []ids.ContactID{contact}, past)[contact.UUID] {
	...
	if !dismissedNow(e.as(), t, e, []ids.ContactID{contact}, soon)[contact.UUID] {
...
func TestADismissalBindsOnlyTheReaderWhoMadeIt(t *testing.T) {
	...
	if dismissedNow(e.asOther(), t, e, []ids.ContactID{contact}, time.Now())[contact.UUID] {
...
func TestRestoringPutsTheContactBackImmediately(t *testing.T) {
	...
	if err := e.store.RestoreRelationshipNudge(e.as(), contact); err != nil {
...
func TestDismissingAgainReplacesTheMoment(t *testing.T) {
	e := setupDedupe(t)
	contact := aContact(t, e)
	if err := e.store.DismissRelationshipNudge(e.as(), contact, 3); err != nil {
		t.Fatalf("dismissing: %v", err)
	}
	...
	if !dismissedNow(e.as(), t, e, []ids.ContactID{contact}, between)[contact.UUID] {
...
func TestASpanOutsideTheRangeIsRefused(t *testing.T) {
	...
	for _, days := range []int{0, -1, nudgeDismissalMaxDays + 1} {
		if err := e.store.DismissRelationshipNudge(e.as(), contact, days); err == nil {
	...
	if err := e.store.DismissRelationshipNudge(e.as(), contact, nudgeDismissalMaxDays); err != nil {
...
func TestARedismissalNamesTheDeadlineItReplaced(t *testing.T) {
	...
	first := latestContactAudit(t, e, contact)["dismissed_until"]

	if err := e.store.DismissRelationshipNudge(e.as(), contact, 30); err != nil {
		t.Fatalf("re-dismissing: %v", err)
	}

	before, after := latestContactAuditPair(t, e, contact)
...
// latestContactAuditPair reads BOTH images of the newest update on one contact.
func latestContactAuditPair(t *testing.T, e *dedupeEnv, contact ids.ContactID) (before, after map[string]any) {
	t.Helper()
	var beforeJSON, afterJSON []byte
	if err := e.store.db.Pool().QueryRow(e.as(),
		`SELECT before, after FROM audit_log
		  WHERE entity_type = 'contact' AND entity_id = $1 AND action = 'update'
		  ORDER BY occurred_at DESC, id DESC LIMIT 1`, contact.UUID,
...
func TestTheDatabaseRefusesADismissalThatHasAlreadyLapsed(t *testing.T) {
	...
	_, err := e.store.db.Pool().Exec(e.as(), `
		INSERT INTO relationship_nudge_dismissal
		       (contact_id, reader_id, dismissed_until, set_by)
...
func TestADismissalWritesTheAuditAndTheAnnouncement(t *testing.T) {
	...
	if err := e.store.DismissRelationshipNudge(e.as(), contact, 14); err != nil {
		t.Fatalf("dismissing: %v", err)
	}

	after := latestContactAudit(t, e, contact)
	...
	typ, action := latestContactEvent(t, e, contact)
...
func TestRestoringALapsedDismissalAnnouncesNothing(t *testing.T) {
	e := setupDedupe(t)
	contact := aContact(t, e)
	if err := e.store.DismissRelationshipNudge(e.as(), contact, 1); err != nil {
		t.Fatalf("dismissing: %v", err)
	}
	...
	if _, err := e.store.db.Pool().Exec(e.as(), `
		UPDATE relationship_nudge_dismissal
		 ...
		 WHERE contact_id = $1`, contact.UUID); err != nil {
		t.Fatalf("ageing the dismissal: %v", err)
	}
	before := countContactEvents(t, e, contact)

	if err := e.store.RestoreRelationshipNudge(e.as(), contact); err != nil {
		t.Fatalf("restoring: %v", err)
	}

	if got := countContactEvents(t, e, contact); got != before {
	...
	var left int
	if err := e.store.db.Pool().QueryRow(e.as(),
		`SELECT count(*) FROM relationship_nudge_dismissal WHERE contact_id = $1`,
		contact.UUID).Scan(&left); err != nil {
...
// countContactEvents is how many announcements this contact has drawn.
func countContactEvents(t *testing.T, e *dedupeEnv, contact ids.ContactID) int {
	t.Helper()
	var n int
	if err := e.store.db.Pool().QueryRow(e.as(),
		`SELECT count(*) FROM event_outbox WHERE envelope->'entity'->>'id' = $1`,
		contact.UUID.String()).Scan(&n); err != nil {
		t.Fatalf("counting outbox rows: %v", err)
	}
	return n
}

// latestContactAudit reads the after-image of the newest update on one contact.
func latestContactAudit(t *testing.T, e *dedupeEnv, contact ids.ContactID) map[string]any {
	t.Helper()
	var raw []byte
	if err := e.store.db.Pool().QueryRow(e.as(),
...
// latestContactEvent reads the type and action of the newest outbox row.
func latestContactEvent(t *testing.T, e *dedupeEnv, contact ids.ContactID) (string, string) {
	t.Helper()
