...
// (PO-PARAM-JW-1) that contact dedupe, lead dedupe and site-contact matching all
// read, where a character metric is the right one: contacts carry no legal forms
// and no industry vocabulary.
func TestTheGateDoesNotReachContactNameSimilarity(t *testing.T) {
	// Two unrelated contacts whose names merely look alike still score high,
	// because that is the contact ladder's job to weigh against other evidence.
	if got := nameSimilarity("Jon Smith", "Jan Smith"); got < 0.8 {
		t.Errorf("contact name similarity = %.4f, want the unchanged metric", got)
	}
}
