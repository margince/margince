...
// leaves the field unset and the ensure ladder that names contacts never runs for
...
// repair, long after that sync (the second). Both feed completeContactName, whose
...
// FillParticipantNamesTx completes the names of every contact this activity's
...
// is not in question, and ContactsOwedACohortRepair already reads it the same way.
func FillParticipantNamesTx(ctx context.Context, tx pgx.Tx, activityID ids.ActivityID) error {
	...
	// keeps its local-part name forever. capture.meetingParticipantsWithContacts
	// resolves it the same way, for the same reason.
	//
	// Ordered by the row rather than left to the planner: one contact can be on
	// a meeting under two addresses spelled two ways, and completeContactName is
	// one-way, so which spelling wins must not depend on the query plan.
	//
	// The address is COALESCED, as its twin in fillContactNameFromAttendance
	...
	// and carries no address at all. ParseContactName reads it only as a
	...
	if err != nil {
		return fmt.Errorf("contacts: reading the names an invitation gave: %w", err)
	}
	...
	for _, attendee := range named {
		...
		if err := auth.EnsureLinkTarget(ctx, tx, entityContact, attendee.contact.UUID); err != nil {
			if errors.Is(err, apperrors.ErrNotFound) || errors.Is(err, apperrors.ErrPermissionDenied) {
				continue
			}
			return fmt.Errorf("contacts: checking the attendee %s a name was given for: %w", attendee.contact, err)
		}
		if _, err := completeContactName(ctx, tx, attendee.contact,
			ParseContactName(attendee.displayName, attendee.address)); err != nil {
			return err
		}
	}
	return nil
...
// scanNamedAttendees drains the query before any fill runs. completeContactName
...
func scanNamedAttendees(rows pgx.Rows) ([]namedAttendee, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&found.contact, &found.displayName, &found.address); err != nil {
			return nil, fmt.Errorf("contacts: reading the names an invitation gave: %w", err)
		}
...
// fillContactNameFromAttendance names ONE contact from the invitations they have
// been on, for the ordering where the meeting synced before they were a contact.
//
// The newest naming invitation wins. A contact's name does not change, but the
// spelling an organizer types for them does, and the most recent is the one
// somebody most recently confirmed by sending it.
func fillContactNameFromAttendance(ctx context.Context, tx pgx.Tx, contactID ids.ContactID) error {
	...
	err := tx.QueryRow(ctx, `
		SELECT ap.display_name, coalesce(ap.address, '')
		 ...
		 LIMIT 1`, contactID).Scan(&displayName, &address)
	if err != nil {
		...
		return fmt.Errorf("contacts: reading the name an invitation gave contact %s: %w", contactID, err)
	}
	_, err = completeContactName(ctx, tx, contactID, ParseContactName(displayName, address))
	return err
}
...
// one: a deployment assembling contacts without the timeline still repairs its
...
func (s *Store) rederiveAudiences(ctx context.Context, tx pgx.Tx, activityIDs []ids.UUID) error {
	...
	for _, id := range ordered {
		if err := s.recomputeAudience(ctx, tx, ids.From[ids.ActivityKind](id)); err != nil {
			return fmt.Errorf("contacts: re-deriving the audience of %s after filing it: %w", id, err)
		}
	}
	return nil
}
