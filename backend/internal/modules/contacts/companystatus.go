...
//
// The ordinary path is UpdateCompany, which takes a caller's sparse edit
...
// pinned to the company's version, so Redeem refuses a decision made
...
func (s *Store) SetCompanyStatusTx(
	...
	if err != nil {
		...
		return false, fmt.Errorf("contacts: reading the account's stage: %w", err)
	}
	...
	// audit row and a company.updated event describing a move that did
	...
	if err := storekit.EmitEvent(ctx, tx, auditID, companyID.UUID,
		...
		return false, fmt.Errorf("contacts: emitting company.updated for the stage change: %w", err)
	}
	return true, nil
}
