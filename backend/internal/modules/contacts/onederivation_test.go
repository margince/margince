...
// them are worked out differently, and the preview would name a contact the
...
const candidateType = "ContactCandidate"

// candidateBuilder is where that one place is.
const candidateBuilder = "leadContactCandidate"
...
func TestTheLadderCandidateIsDerivedInOnePlace(t *testing.T) {
	...
	t.Errorf("a %s is derived from a lead by %s as well as by %s.\n\nThe preview and the "+
		...
		"a contact the promotion does not land on — after a human read the preview and agreed. "+
...
// promotes into a contact with no name at all: a row nobody searching for that
// contact will ever match, created by a verb whose whole job is to name them.
	...
	forEachModuleFunc(t, func(parsed moduleFile, fn *ast.FuncDecl) {
		...
		if pos := readsFieldOf(fn, leadType, leadNameField); pos.IsValid() {
			t.Errorf("%s reads the lead's .%s directly at %s.\n\nThat is the second reading: "+
				...
				"and the lead that falls between them promotes into a contact with no name. "+
