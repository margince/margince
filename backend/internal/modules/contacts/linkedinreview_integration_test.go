...
func (e *dedupeEnv) linkedInHandle(t *testing.T, contact ids.ContactID) (string, bool) {
	t.Helper()
	ctx := e.as()
	var handle string
	found := true
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		...
		t.Fatalf("reading %s's LinkedIn handle: %v", contact, err)
	}
	return handle, found
}
...
func TestApplyingAnApprovedMatchPutsTheLinkedInURLOnTheContact(t *testing.T) {
	...
	// it, a string comparison there and a contact here.
	e := setupDedupe(t)
	company := e.seedAcmeCompany(t)
	andreas := e.seedVisibleContact(t, "Andreas Muller")
	...
	if status, contact := e.ghostStatus(t, suggestedGhost); status != "confirmed" || contact == nil || *contact != andreas.UUID {
		t.Fatalf("the connection is %q → %v, want confirmed → %s", status, contact, andreas)
	}
...
func TestTheMatchesAwaitingADecisionAreTheCallersOwnAndCarryTheExportsSpelling(t *testing.T) {
	...
	if pending[0].ContactName != "Andreas Muller" {
		t.Errorf("the candidate names contact %q, want the suggested contact", pending[0].ContactName)
	}
}
...
func TestCollapseNeverLetsAMachineGuessOverrideAHumanConfirmation(t *testing.T) {
	e := setupDedupe(t)
	company := e.seedAcmeCompany(t)
	guessed := e.seedVisibleContact(t, "Guessed Contact")
	confirmed := e.seedVisibleContact(t, "Confirmed Contact")
	...
	// a decision the member never made, about a contact they did not name.
	ctx := e.as()
			...
			if _, err := tx.Exec(ctx, `
				INSERT INTO linkedin_connection
				    (id, owner_user_id, full_name, normalized_name,
				     company_name, normalized_company, matched_contact_id, match_status,
				     source, synced_at)
				VALUES ($1,
				        $2, 'Dup Contact', 'dup contact', $3, $4, $5, $6, 'csv_export', $7::date)`,
				row.id, e.rep, row.company, NormalizeCompanyName(row.company),
				row.contact.UUID, row.status, row.syncedAt); err != nil {
				return err
			}
		}
		return nil
...
func TestAConnectionNeverMatchesAContactItsOwnerMayNotSee(t *testing.T) {
	...
	private := e.seedVisibleContact(t, "Andreas Müller")
	...
	if status, contact := e.ghostStatus(t, "Andreas Müller"); status != "unmatched" || contact != nil {
		t.Errorf("a ghost matched a contact its owner may not see: %q → %v", status, contact)
	}
}
...
//
// The guards on this path gate the CONTACT — the update grant and the target's
...
func TestApplyingAMatchRefusesAConnectionTheProposalDoesNotDescribe(t *testing.T) {
	...
	other := e.seedVisibleContact(t, "Someone Else")

	for _, tc := range []struct {
		name           string
		owner, contact ids.UUID
		what           string
	}{
		{
			name: "another member's connection", owner: ids.NewV7(), contact: andreas.UUID,
			what: "a member decides about their own imported network and nobody else's",
		},
		{
			name: "a pair the row does not name", owner: e.rep, contact: other.UUID,
		...
		t.Run(tc.name, func(t *testing.T) {
			if err := e.store.ApplyLinkedInMatch(e.as(), ghost, tc.owner, tc.contact); !errors.Is(err, apperrors.ErrNotFound) {
	...
	// away earlier, by the pair clause or by the contact lock — and neutralising
	...
	if status, contact := e.ghostStatus(t, suggestedGhost); status != "confirmed" ||
		contact == nil || *contact != andreas.UUID {
