...
const (
	// linkedInCompanyPrefix is the one path shape a company URL takes. A contact
...
// normalizeCompanyLinkedInURL reduces a LinkedIn *company* URL to its stored
...
func normalizeCompanyLinkedInURL(raw string) (string, error) {
	...
	// company_linkedin_url_shape, whose slug pattern admits no slash.
...
// companyLinkedInPatchValue resolves what the guarded patch should write for a
// supplied LinkedIn URL: nil to clear the column, or the normalized spelling.
// Clearing is a legitimate edit — unlike the contact path, where the URL is the
// dedupe key, a company's LinkedIn URL is an optional identity field.
func companyLinkedInPatchValue(raw string) (*string, error) {
	if strings.TrimSpace(raw) == "" {
		return nil, nil //nolint:nilnil // a cleared optional field is a NULL write, not an error
	}
	normalized, err := normalizeCompanyLinkedInURL(raw)
	if err != nil {
		return nil, err
	}
...
// hold, matching company_linkedin_url_shape's `([a-z]{2,3}\.)?linkedin\.com`
