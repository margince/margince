...
func validateSiteReadOutcome(in FinishSiteReadInput) error {
	if in.Status != siteReadStatusFailed {
		if in.StatusCode != "" || in.StatusDetail != "" || in.NextAttemptAt != nil {
			return fmt.Errorf(
				"contacts: a %s site read carries no diagnosis (status_code/status_detail/next_attempt_at are for a failure)",
				in.Status)
		}
		return nil
	}
	if _, known := SiteReadFailureCodes[in.StatusCode]; !known {
		return fmt.Errorf("contacts: %q is not a site-read failure code", in.StatusCode)
	}
	if in.StatusDetail == "" {
		return errors.New("contacts: a failed site read needs a status_detail a human can act on")
	}
	return nil
}
...
func (s *Store) FinishSiteRead(ctx context.Context, readID ids.UUID, in FinishSiteReadInput) error {
	if !finishedSiteReadStatuses[in.Status] {
		return fmt.Errorf("contacts: %q is not a terminal site-read status (done|partial|failed|cancelled)", in.Status)
	}
	if in.StoppedReason != nil && !siteReadStopReasons[*in.StoppedReason] {
		return fmt.Errorf("contacts: %q is not a site-read stop reason (budget|page_cap|byte_cap|deadline)", *in.StoppedReason)
	}
	...
	if err != nil {
		return fmt.Errorf("contacts: site-read pages: %w", err)
	}
	skipped, err := marshalSiteReadList(in.Skipped)
	if err != nil {
		return fmt.Errorf("contacts: site-read skips: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("contacts: site-read profile fields: %w", err)
	}
	facts, err := marshalSiteReadList(in.Facts)
	if err != nil {
		return fmt.Errorf("contacts: site-read facts: %w", err)
	}
	contacts, err := marshalSiteReadList(in.Contacts)
	if err != nil {
		return fmt.Errorf("contacts: site-read contacts: %w", err)
	}
	entities, err := marshalSiteReadList(in.LegalEntities)
	if err != nil {
		return fmt.Errorf("contacts: site-read legal entities: %w", err)
	}
	warnings, err := marshalSiteReadList(in.Warnings)
	if err != nil {
		return fmt.Errorf("contacts: site-read warnings: %w", err)
	}
	...
	return s.tx(ctx, func(tx pgx.Tx) error {
		finished, err := scanSiteRead(tx.QueryRow(ctx, `
			...
			SET status = $2, pages = $3, skipped = $4, stopped_reason = $5,
			    ...
			    contacts = $10, warnings = $11, proposal_hash = $12,
			    legal_entities = $15,
			...
			profileFields, facts, contacts, warnings, in.ProposalHash, len(in.Pages), grounded, entities,
