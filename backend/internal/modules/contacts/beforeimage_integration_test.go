...
// real keys cannot take field ownership away from a contact.
//
...
func TestACompletedNameRecordsWhatEachColumnHeld(t *testing.T) {
	...
	// A lone surname names the contact and says nothing about a given name, so
	...
	if !second.NameFilled || second.ContactID != first.ContactID {
		t.Fatalf("second ensure = %+v, want the incumbent %s completed", second, first.ContactID)
	}

	before, after := auditImagesHolding(ctx, t, e.store, entityContact, first.ContactID.UUID, "first_name")
...
func TestACompletedNameNeverRecordsAColumnAHumanTyped(t *testing.T) {
	...
	if n := countAuditRowsHolding(ctx, t, e.store, entityContact, res.ContactID.UUID, "first_name"); n != 0 {
...
func TestASiteContactFillRecordsTheEmptyFields(t *testing.T) {
	...
	before, after := auditImagesHolding(ctx, t, e.store, entityContact, contactID.UUID, "role")
...
func TestASignatureFillRecordsTheEmptyColumn(t *testing.T) {
	...
	before, after := auditImagesHolding(ctx, t, e.store, entityContact, contactID.UUID, "title")
	wantImage(t, before, "before", "title", nil)
...
func TestASiteContactFillNeverRecordsAFieldAlreadyAnswered(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	contactID, companyID := e.seedEmployedContact(ctx, t,
		"Mira Halvorsen", "mira@voltaq.test", "Voltaq Systems GmbH", "voltaq.test")

	if _, err := e.store.ApplySignatureFields(ctx, contactID, e.openSignatureSource(ctx, t), []SignatureField{{
	...
	if _, err := e.store.ApplySiteContactFields(ctx, companyID, SiteContactFields{
		Name: "Mira Halvorsen", Role: "Head of Platform",
		PublishedEmail:  "mira@voltaq.test",
		EvidenceSnippet: "Mira Halvorsen, Head of Platform",
		SourceURL:       "https://voltaq.test/team",
	...
	if n := countAuditRowsHolding(ctx, t, e.store, entityContact, contactID.UUID, "role"); n != 1 {
		t.Fatalf("%d audit rows claim role, want only the signature's — the site fill declined", n)
	}
	_, after := auditImagesHolding(ctx, t, e.store, entityContact, contactID.UUID, "role")
