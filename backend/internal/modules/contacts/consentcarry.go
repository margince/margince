...
// record survives it. Three paths need it — a contact merge, a lead merge, and
// a lead's promotion to a contact — and they ask the same four questions in the
// same order:
//
...
// consentSubject is the column naming whose consent a contact_consent or
...
const (
	consentSubjectContact consentSubject = "contact_id"
...
const (
	consentCarryContactMerge consentCarryKind = iota
...
type consentCarrySpec struct {
	...
	// contact would destroy the only record of when it was given and to whom.
	//
	...
	// round-trip is contact-keyed (consent.Store.resolveDOIConfirmation — "promote
...
//
// Held by: TestTheConsentCarryIsSpelledOnceInContacts (backend/gates/oneconsentcarry_test.go)
var consentCarries = map[consentCarryKind]consentCarrySpec{
	consentCarryContactMerge: {
		name: "a contact merge",
		from: consentSubjectContact, to: consentSubjectContact,
	...
	consentCarryLeadPromotion: {
		...
		from: consentSubjectLead, to: consentSubjectContact,
...
func flipCarriedWithdrawals(ctx context.Context, tx pgx.Tx, spec consentCarrySpec,
	...
	_, err := tx.Exec(ctx, storekit.SQLf(`
		WITH flipped AS (
		  UPDATE contact_consent b SET state = 'withdrawn', captured_at = $3, source = $5
		  FROM contact_consent a
...
func dropCollidingConsent(ctx context.Context, tx pgx.Tx, spec consentCarrySpec, fromID, toID ids.UUID) error {
	_, err := tx.Exec(ctx, storekit.SQLf(`
		DELETE FROM contact_consent a
		WHERE a.%[1]s = $1 AND EXISTS (
		  SELECT 1 FROM contact_consent b
...
// (contact_consent.lead_id cascades on lead deletion).
func repointCarriedConsent(ctx context.Context, tx pgx.Tx, spec consentCarrySpec, fromID, toID ids.UUID) error {
	...
	_, err := tx.Exec(ctx, storekit.SQLf(
		`UPDATE contact_consent SET %[2]s = $2%[3]s WHERE %[1]s = $1`, spec.from, spec.to, clearOldKey),
		fromID, toID)
	if err != nil {
