...
//
// `phones` was a contact field on create and no field at all on update, so a
...
func TestContactUpdateInputCarriesTheNumbersItIsGiven(t *testing.T) {
...
func TestAnAbsentPhoneListIsNotAnEmptyOne(t *testing.T) {
	...
	if got := contactUpdateInput(absent, nil).Phones; got != nil {
	...
	if got := contactUpdateInput(cleared, nil).Phones; got == nil {
