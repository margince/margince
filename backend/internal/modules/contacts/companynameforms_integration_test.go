...
func TestVietnameseCompaniesResolveThroughTheLadder(t *testing.T) {
	...
	// Seeded WITHOUT domains on purpose. exactCompanyByDomain is tier 1 and returns
	...
	for _, name := range []string{
		...
		m := e.dedupeCompanyInTx(ctx, t, CompanyCandidate{DisplayName: name})
	...
	ascii := e.dedupeCompanyInTx(ctx, t, CompanyCandidate{
	...
	longForm := e.dedupeCompanyInTx(ctx, t, CompanyCandidate{
	...
	short := e.dedupeCompanyInTx(ctx, t, CompanyCandidate{DisplayName: "小米科技"})
	...
	withDomain, err := e.store.CreateCompany(ctx, CreateCompanyInput{
		DisplayName: "TẬP ĐOÀN ĐIỆN LỰC VIỆT NAM", Source: "manual",
		Domains: []CompanyDomainInput{{Domain: "evn.com.vn", IsPrimary: true}},
	})
	if err != nil {
		t.Fatalf("seeding the domain-carrying company: %v", err)
	}
	sharedDomain := e.dedupeCompanyInTx(ctx, t, CompanyCandidate{
	...
	if sharedDomain.CompanyID.String() != withDomain.Id.String() {
		t.Fatalf("the domain tier named %s, want the company holding the domain (%s)",
			sharedDomain.CompanyID, withDomain.Id)
	}
}
