...
func TestLeadPromotedPayload_WithEvidence(t *testing.T) {
	...
	payload := leadPromotedPayload(contactID, "created", "inbound_reply", &evidenceID, nil)
	...
	if payload.PromotedContactId != openapi_types.UUID(contactID.UUID) {
		t.Errorf("got %v, want %v", payload.PromotedContactId, openapi_types.UUID(contactID.UUID))
	}
...
func TestLeadPromotedPayload_CarriesTheActivitiesItMoved(t *testing.T) {
	...
	payload := leadPromotedPayload(contactID, "merged", "human_qualify", nil, moved)

	if payload.CarriedActivityIds == nil {
		t.Fatal("a promotion that moved two activities named none of them, so a " +
			"consumer has only the contact id — which on a merge is the wrong set")
	}
...
func TestLeadPromotedPayload_OmitsAnEmptyCarry(t *testing.T) {
	...
	payload := leadPromotedPayload(contactID, "created", "human_qualify", nil, nil)
...
func TestLeadPromotedPayload_MergedNoEvidence(t *testing.T) {
	...
	payload := leadPromotedPayload(contactID, "merged", "human_qualify", nil, nil)
