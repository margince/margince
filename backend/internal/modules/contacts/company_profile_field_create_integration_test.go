...
// companies a contact would want to check.
//
...
// bareCompany seeds a company with NO sidecar rows at all — the state a crawl that
// found no imprint leaves, and the one evidenceCompany deliberately does not model.
func bareCompany(ctx context.Context, t *testing.T, e *dedupeEnv) ids.CompanyID {
	t.Helper()
	company, err := e.store.CreateCompany(ctx, CreateCompanyInput{
		DisplayName: "Halden Kraft GmbH", Source: "manual",
		Domains: []CompanyDomainInput{{Domain: "halden-kraft.test", IsPrimary: true}},
	})
	if err != nil {
...
func profileFieldSource(
	ctx context.Context, t *testing.T, e *dedupeEnv, companyID ids.CompanyID, field string,
) string {
	t.Helper()
	var source string
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx, `
			SELECT source FROM company_profile_field
			 WHERE company_id = $1 AND field = $2`, companyID, field).Scan(&source)
	}); err != nil {
...
// The case the product could not do: a contact states a fact about a company
// whose site never published it.
func TestARepCanStateAProfileFieldTheMachineNeverProposed(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	companyID := bareCompany(ctx, t, e)

	stated := "DE811907980"
	out, err := e.store.UpdateCompanyProfileField(ctx, companyID, "register_vat",
	...
	// A created row belongs to the contact who created it, on both columns the
	...
	if string(out.Source) != "human" {
		t.Errorf("source = %q, want human — the contact IS the evidence on this path", out.Source)
	}
	if got := profileFieldSource(ctx, t, e, companyID, "register_vat"); got != "human" {
...
func TestConfirmingAProfileFieldNobodyProposedIsStillNotFound(t *testing.T) {
	...
	_, err := e.store.ConfirmCompanyProfileField(ctx, companyID, "register_vat",
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx, `
			SELECT count(*) FROM company_profile_field
			 WHERE company_id = $1 AND field = 'register_vat'`, companyID).Scan(&rows)
	}); err != nil {
...
func TestStatingAVatNumberQueuesItsConsultation(t *testing.T) {
	...
	var queued []ids.CompanyID
	e.store.WithVatCheckEnqueue(func(_ context.Context, _ pgx.Tx, id ids.CompanyID, requested bool) error {
		if requested {
			t.Error("a write's own consultation was marked as a contact's request")
		}
	...
	stated := "DE811907980"
	if _, err := e.store.UpdateCompanyProfileField(ctx, companyID, "register_vat",
	...
	if queued[0] != companyID {
		t.Errorf("queued the consultation for %v, want %v", queued[0], companyID)
	}
}

// The agent principal is dedupequeue's own `asAgent` — it already carries the
// company update grant, so the only thing separating it from `as()` here
...
func TestAnAgentCannotOriginateAClaimNobodyProposed(t *testing.T) {
	e := setupDedupe(t)
	companyID := bareCompany(e.as(), t, e)

	stated := "DE811907980"
	_, err := e.store.UpdateCompanyProfileField(e.asAgent(), companyID, "register_vat",
		ProfileFieldWriteInput{Value: &stated})
	if !errors.Is(err, apperrors.ErrNotFound) {
...
func TestAnAgentStillCorrectsAClaimThatAlreadyExists(t *testing.T) {
	...
	if _, err := e.store.UpdateCompanyProfileField(e.asAgent(), companyID, "icp",
		ProfileFieldWriteInput{Value: &corrected}); err != nil {
		t.Fatalf("an agent correcting an existing claim: %v", err)
	}
	if got := readProfileFieldValue(ctx, t, e, companyID, "icp"); got != corrected {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx, `
			SELECT action, before::text, after
			  FROM audit_log
			 WHERE entity_type = 'company_profile_field'
...
func TestRepsStatingTheSameFieldAtOnceAllSucceed(t *testing.T) {
	...
	got := readProfileFieldValue(ctx, t, e, companyID, "register_vat")
	if !slices.Contains(stated, got) {
		t.Errorf("value = %q, want one of the stated numbers — the seeded empty row survived", got)
	}
}

// The registry address is the other field a contact can now state, and it must
...
func TestStatingTheRegisteredAddressLeavesTheOperatingAddressAlone(t *testing.T) {
	...
	if _, err := e.store.UpdateCompanyProfileField(ctx, companyID, "registered_address",
		ProfileFieldWriteInput{Value: &stated}); err != nil {
		t.Fatalf("state the registered address: %v", err)
	}

	for _, column := range []string{"address_line1", "address_city", "address_postal_code"} {
		if got := companyColumn(ctx, t, e, companyID, column); got != "" {
			t.Errorf("%s = %q, want empty — the registry address is not the operating address", column, got)
		}
	}
	if got := readProfileFieldValue(ctx, t, e, companyID, "registered_address"); got != stated {
