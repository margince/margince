...
//   - A DATED STATEMENT BY THE CONTACT replaces what is older than it. A mail
//     signature and a business card are the contact saying so themselves, on a
...
type contactProfileFieldPrecedence int
...
const (
	...
	claimUnanswered contactProfileFieldPrecedence = iota
	// replaceOnAcceptance is a human's decision, which outranks whatever a
	// pass put there.
	replaceOnAcceptance
	// supersedeOnNewerObservation is the contact's own dated statement, and it
...
// contactProfileFieldRow is one evidence row: a fact about who this contact is, and
...
type contactProfileFieldRow struct {
	Field           string
	Value           string
	...
	// table's own — a title a human typed straight onto the contact, which
...
// point. contact_profile_field is a declared PII table and Art. 17 erasure clears
// its rows while stamping the contact archived, so a fill decided before that
...
//
// Locking CONTACT and not the field row: the field row may not exist yet, and
// what has to be serialized is the subject's liveness. Every caller that goes
// on to write a contact COLUMN takes the same row next, so the order is
// contact-then-contact and no new deadlock edge is introduced.
func writeContactProfileField(ctx context.Context, tx pgx.Tx, contactID ids.ContactID, row contactProfileFieldRow, precedence contactProfileFieldPrecedence) (bool, error) {
	if err := auth.LockSubjectLive(ctx, tx, "contact", contactID.UUID); err != nil {
	...
	if landed && row.Field == fieldLinkedin {
		if err := fillEmptyLinkedinSlot(ctx, tx, contactID, row.Value); err != nil {
			return false, err
		}
	}
	return landed, nil
...
// slot the rest of the product reads. contact_social is where the contact rail,
...
func fillEmptyLinkedinSlot(ctx context.Context, tx pgx.Tx, contactID ids.ContactID, value string) error {
	...
	// through writeContactProfileField, whose entry points each probe the row,
	// but this write reaches contact_social and the contact row itself, and the
	...
	_, landed, err := insertSocialHandle(ctx, tx, contactID.UUID, socialLinkedIn, value)
	if err != nil || !landed {
		return err
	}
	// The slot is part of the contact aggregate: the bump invalidates If-Match
	// tokens held by browsers that never saw it, for touchContact's reason.
	...
	return auditLinkedInHandleGained(ctx, tx, contactID.UUID)
}
