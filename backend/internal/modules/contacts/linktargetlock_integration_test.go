...
func TestAProbedLinkTargetMakesTheArchiveWait(t *testing.T) {
	...
	go func() {
		holding <- e.store.tx(ctx, func(tx pgx.Tx) error {
			if err := auth.EnsureLinkTarget(ctx, tx, entityContact, contactID.UUID); err != nil {
				return err
			}
...
// exclusive: a contact mid-ingest is referenced by every message captured for
...
func TestTwoReferencesOntoOneRecordDoNotWaitForEachOther(t *testing.T) {
	...
	second := e.store.tx(ctx, func(tx pgx.Tx) error {
		...
		return auth.EnsureLinkTarget(ctx, tx, entityContact, contactID.UUID)
	})
	close(release)
	if err := <-holding; err != nil {
