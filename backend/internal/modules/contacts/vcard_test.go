...
func TestParseVCardsReadsTheCardsExportersWrite(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			want: VCardEntry{FullName: "Postal Contact", Address: "Hauptstr. 1, Berlin, 10115, Germany"},
		},
		{
		...
		},
		{
			...
			want: VCardEntry{FullName: "Linked Contact", URL: "https://example.com/team/jdoe"},
		},
	}
...
func TestParseVCardsKeepsAnEscapedSemicolonInsideAComponent(t *testing.T) {
	...
	// boundary, and the contact's own identity — what dedupe matches on —
...
func TestParseVCardsReadsEveryCardInAMultiCardFile(t *testing.T) {
	...
	for i, want := range []string{"First Contact", "Second Contact", "Third Contact"} {
...
func TestParseVCardsRefusesAFileItCannotRead(t *testing.T) {
	cases := []struct {
		...
		// An import that quietly drops a contact is worse than one that
...
func TestParseVCardsKeepsEveryChannelACardStates(t *testing.T) {
	...
	// Both of each: a card listing two addresses is a contact reachable two
	// ways, and keeping one would drop a fact the contact themselves stated.
