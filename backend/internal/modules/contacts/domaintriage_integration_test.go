...
// openTriage puts a domain in the state the ensure ladder leaves it: the contact
...
func TestCompanyVerdictCreatesTheCompanyAndWiresEveryoneWaitingOnIt(t *testing.T) {
	...
	// Two colleagues wrote in while the question was open. Both have contacts
	...
	// Both waiting contacts get their edge, not only the one that happened to
	// trigger the crawl.
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		...
		return tx.QueryRow(ctx, `
			...
			res.CompanyID, []ids.ContactID{first.ContactID, second.ContactID}).Scan(&employed)
	}); err != nil {
		t.Fatal(err)
	}
	...
	if employed != 2 {
		t.Fatalf("%d of the waiting contacts were employed, want 2", employed)
	}
...
func TestPersonalVerdictRefusesTheCompanyForGood(t *testing.T) {
	...
	if again.TriagePending || again.CompanyID != nil {
		t.Fatalf("ensure after a personal verdict = %+v, want contact only, no company, no new question", again)
	}
	if !again.ContactCreated {
		t.Fatal("refusing the company must not refuse the contact")
	}
}
...
func TestListDueDomainsOffersOnlyTheQuestionsWorthAsking(t *testing.T) {
	...
	e.openTriageFirst(ctx, t, "a@waiting.test", "A Contact", "waiting.test")
	e.openTriageFirst(ctx, t, "b@inflight.test", "B Contact", "inflight.test")
	e.openTriageFirst(ctx, t, "c@settled.test", "C Contact", "settled.test")
	e.openTriageFirst(ctx, t, "d@exhausted.test", "D Contact", "exhausted.test")
...
func TestMarkTriageQueuedSpendsAnAttemptAndBacksOff(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	e.openTriageFirst(ctx, t, "a@backoff.test", "A Contact", "backoff.test")
...
func TestASuppressedDomainNeverBecomesACompany(t *testing.T) {
	...
	// The CONTACT is still created — refusing the company is not refusing the human.
	if res.ContactID.IsZero() {
		t.Fatal("the contact was refused; only the company is suppressed")
	}
	if res.CompanyID != nil {
...
func TestAHumanAdmissionOutranksEveryLaterMachineRefusal(t *testing.T) {
	...
	if admission != DomainAdmitted || source != AdmissionSourceHuman {
		t.Fatalf("admission = %q/%q, want admitted/human — a machine must not overturn a contact", admission, source)
	}
...
func TestUnblockingADomainReopensTheCompanyQuestion(t *testing.T) {
	...
	// A newsletter arrives first: the contact lands, the domain is refused.
	...
	// And the company lands when the crawl comes back, with the contact who was
