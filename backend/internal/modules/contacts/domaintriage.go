...
const (
	...
	// DomainPersonal — a natural contact's own domain. A contact, never a company.
...
// still-uncommitted contact. That contact ends up with the company but no edge
...
func readDispositionTx(ctx context.Context, tx pgx.Tx, domain string) (DomainDisposition, bool, error) {
	...
	if err != nil {
		return DomainDisposition{}, false, fmt.Errorf("contacts: reading the disposition of %s: %w", domain, err)
	}
	if source != nil {
		d.Source = *source
	}
...
// known about: the contact is created, the company is not, and this row is
...
func (s *Store) ListDueDomains(ctx context.Context, limit int) ([]DueDomain, error) {
	...
	err := s.db.Tx(ctx, func(tx pgx.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("contacts: listing domains due for triage: %w", err)
		}
		defer rows.Close()
		for rows.Next() {
			var d DueDomain
			if err := rows.Scan(&d.Domain); err != nil {
				return fmt.Errorf("contacts: scanning a domain due for triage: %w", err)
			}
			out = append(out, d)
		}
		return rows.Err()
...
func MarkTriageQueuedTx(ctx context.Context, tx pgx.Tx, domain string) error {
	if _, err := tx.Exec(ctx, `
		...
		return fmt.Errorf("contacts: arming the triage cursor for %s: %w", domain, err)
	}
	return nil
}
...
// later message will re-ask — a contact permanently without a company, and
...
func (s *Store) ExhaustedDomains(ctx context.Context, limit int) ([]DueDomain, error) {
	...
	err := s.db.Tx(ctx, func(tx pgx.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("contacts: listing domains that ran out of triage attempts: %w", err)
		}
		defer rows.Close()
		for rows.Next() {
			var d DueDomain
			if err := rows.Scan(&d.Domain); err != nil {
				return fmt.Errorf("contacts: scanning an exhausted domain: %w", err)
			}
			out = append(out, d)
		}
		return rows.Err()
...
func (s *Store) RetireStaleTriageRead(ctx context.Context, domain string) error {
	return s.db.Tx(ctx, func(tx pgx.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("contacts: retiring the stale triage dossier for %s: %w", domain, err)
		}
...
// OnDomain lists the live contacts the workspace records at a mail domain
// (contacts.OnDomain to a caller — the package already says which record)
...
func OnDomain(ctx context.Context, tx pgx.Tx, domain string) ([]DomainContact, error) {
	rows, err := tx.Query(ctx, `
	...
	if err != nil {
		return nil, fmt.Errorf("contacts: listing the contacts on %s: %w", domain, err)
	}
	defer rows.Close()
	// Grouped by CONTACT, not by address: two mailboxes on somebody's own domain
	// are one human, and counting them as two would demand that each explain
	// the domain independently.
	byContact := map[ids.UUID]int{}
	var out []DomainContact
	for rows.Next() {
		var id ids.UUID
		var name, local string
		if err := rows.Scan(&id, &name, &local); err != nil {
			return nil, fmt.Errorf("contacts: scanning a contact on %s: %w", domain, err)
		}
		at, seen := byContact[id]
		if !seen {
			byContact[id] = len(out)
			out = append(out, DomainContact{FullName: name})
			at = len(out) - 1
		}
