...
// The review arm leaks too, and differently: it writes no contact, but what it
...
// The create arm mints the contact BEFORE it may take the source lock, so a
// refusal has to roll that write back. This asserts the rollback: a refused card
// leaves no contact, no email and no phone, not merely a "skipped" outcome.
//
...
func TestARefusedCreateLeavesNoHalfWrittenContact(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	...
	var contacts, emails, phones int
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		if err := tx.QueryRow(ctx,
			`SELECT count(*) FROM contact WHERE full_name = 'Petra Post'`).Scan(&contacts); err != nil {
			return err
		}
		if err := tx.QueryRow(ctx,
			`SELECT count(*) FROM contact_email WHERE email = 'petra@briefkasten.example'`).Scan(&emails); err != nil {
			return err
		}
		return tx.QueryRow(ctx,
			`SELECT count(*) FROM contact_phone WHERE phone = '+49305556677'`).Scan(&phones)
	}); err != nil {
		t.Fatalf("reading back what the refused card left: %v", err)
	}
	if contacts != 0 || emails != 0 || phones != 0 {
		t.Errorf("a refused card left %d contact, %d email and %d phone row(s) — the outcome "+
			"says skipped while the record it denies writing is queryable, and the email "+
			"row would claim an address no contact here owns", contacts, emails, phones)
	}
}
