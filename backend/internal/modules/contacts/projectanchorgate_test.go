...
// the contact exists, the project exists, the write succeeds.
//
...
// method here is a project verb and takes the row anchor, or it is a contact
// read and takes the contact's own object grant. Something that is neither is
...
// contactReadGate is what the file's other kind of exported method owes: a name
...
const contactReadGate = "Require"

// contactReadPackage is where that grant lives. Named, because the gate matches
// on the receiver as well as the method: `auth.Require` is the grant and
// something else's Require is not.
const contactReadPackage = "auth"
...
func TestEveryProjectStakeholderVerbTakesTheRowAnchorGate(t *testing.T) {
	...
	for _, decl := range file.Decls {
		...
		if !takesAProject(fn, carriers) {
			...
			// grant.
			if callsAny(fn, contactReadPackage, map[string]bool{contactReadGate: true}) {
				continue
			}
			t.Errorf("%s in %s names no project, by parameter or by input struct, and takes no "+
				...
				fn.Name.Name, stakeholderFile, contactReadGate)
			continue
		}
		verbs++
