...
// is this contact accountable for" a query rather than a note.
...
// SetProjectStakeholder attaches a contact to a project with a role. It is
// a PUT because it is idempotent per contact: re-stating an existing edge
...
func (h Handlers) SetProjectStakeholder(w http.ResponseWriter, r *http.Request, id crmcontracts.Id, _ crmcontracts.SetProjectStakeholderParams) {
	...
	rel, err := h.store.SetProjectStakeholder(r.Context(), SetProjectStakeholderInput{
		ProjectID: pathID[ids.ProjectKind](id),
		ContactID: pathID[ids.ContactKind](req.ContactId),
		Role:      role,
	})
	if err != nil {
		writeStoreErr(w, r, err)
...
// RemoveProjectStakeholder detaches a contact from a project by archiving
// the edge — detaching is not deleting.
func (h Handlers) RemoveProjectStakeholder(w http.ResponseWriter, r *http.Request, id crmcontracts.Id, contactID openapi_types.UUID, _ crmcontracts.RemoveProjectStakeholderParams) {
	err := h.store.RemoveProjectStakeholder(r.Context(),
		pathID[ids.ProjectKind](id), ids.From[ids.ContactKind](ids.UUID(contactID)))
	if err != nil {
		writeStoreErr(w, r, err)
		return
	}
