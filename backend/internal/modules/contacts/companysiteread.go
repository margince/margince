...
var ErrSiteReadAlreadyConfirmed = errors.New("contacts: the website read was already confirmed")
...
var ErrSiteReadNotConfirmable = errors.New("contacts: the website read is not ready to confirm")
...
// StageSiteReadContacts stages the dossier's published contacts after the anchor
// exists. The callback runs inside the company confirmation transaction.
type StageSiteReadContacts func(context.Context, pgx.Tx, ids.CompanyID, SiteRead, []SiteReadContact) ([]ids.UUID, error)
...
// updates the anchor, writes the selected profile/facts, stages contacts
...
// because a contact's own mark already holds that slot, so the caller collects
...
func (s *Store) ConfirmCompanySiteRead(ctx context.Context, in ConfirmCompanySiteReadInput, stageContacts StageSiteReadContacts) (Company, []string, error) {
	...
	err = s.tx(ctx, func(tx pgx.Tx) error {
		out, unadoptedLogos, err = s.confirmCompanySiteReadTx(ctx, tx, in, by, stageContacts)
		return err
	})
	if err != nil {
...
func (s *Store) confirmCompanySiteReadTx(
	...
	by string,
	stageContacts StageSiteReadContacts,
	...
	confirmation.proposalIDs, err = stageConfirmedSiteReadContacts(ctx, tx, confirmation.target.id, read, stageContacts)
	if err != nil {
		return Company{}, nil, err
	}
...
func stageConfirmedSiteReadContacts(
	ctx context.Context,
	tx pgx.Tx,
	companyID ids.CompanyID,
	read SiteRead,
	...
	if stageContacts == nil || len(read.Contacts) == 0 {
		return nil, nil
	}
	proposalIDs, err := stageContacts(ctx, tx, companyID, read, read.Contacts)
	if err != nil {
		return nil, fmt.Errorf("stage website contacts: %w", err)
	}
...
func recordSiteReadConfirmation(ctx context.Context, tx pgx.Tx, read SiteRead, confirmation siteReadConfirmation) error {
	...
	// one. A read of a site that names no contactable contact stages nothing
...
// seam.
//
//nolint:ireturn // dispatches to PublicEventCompanyCreated vs Updated by confirmation.created; tested directly via the interface in contact_company_payload_test.go
...
func selectSiteReadFacts(proposed []DeepReadFact, selected []string) ([]DeepReadFact, error) {
	...
	for _, key := range selected {
		if seen[key] {
			return nil, fmt.Errorf("contacts: selected fact key %q appears more than once", key)
		}
		fact, ok := byKey[key]
		if !ok {
			return nil, fmt.Errorf("contacts: selected fact key %q is not in the inspected website draft: %w", key, apperrors.ErrVersionSkew)
		}
		seen[key] = true
		out = append(out, fact)
	}
