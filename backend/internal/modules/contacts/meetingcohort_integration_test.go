...
// A meeting reaching the contact who attended it, when the contact arrived after
// the meeting did.
//
...
func TestTheRepairFilesAMeetingUnderAnAttendeeWhoArrivedLater(t *testing.T) {
	...
	if !containsContact(owed, res.ContactID) {
		t.Fatalf("the sweep does not see %s, who attended a meeting they are not filed under", res.ContactID)
	}

	if _, err := e.store.RepairContactCohort(ctx, res.ContactID); err != nil {
		t.Fatalf("repair: %v", err)
	}
	if linked, named := e.cohortStateOf(ctx, t, meeting, res.ContactID); !linked || !named {
	...
	if containsContact(after, res.ContactID) {
		t.Errorf("%s is still owed a repair after one ran — the selector asks a different "+
			"question than the write answers, so every tick redoes this work", res.ContactID)
	}
}

// Two attendees, two links. The mail arm refuses an activity that any contact is
...
// it, so the guard has to be per-contact or the second attendee is locked out by
// the first.
func TestAMeetingReachesEveryAttendeeNotOnlyTheFirst(t *testing.T) {
	...
	if _, err := e.store.RepairContactCohort(ctx, one.ContactID); err != nil {
		t.Fatalf("repair one: %v", err)
	}
	if _, err := e.store.RepairContactCohort(ctx, two.ContactID); err != nil {
		t.Fatalf("repair two: %v", err)
	}

	if linked, _ := e.cohortStateOf(ctx, t, meeting, one.ContactID); !linked {
		t.Errorf("the first attendee is not filed under the meeting they attended")
	}
	if linked, _ := e.cohortStateOf(ctx, t, meeting, two.ContactID); !linked {
...
// naming the retired contact id, with the survivor live beside it.
//
...
func TestAMeetingReachesTheSurvivorWhenItsAttendeeRowNamesAMergedRecord(t *testing.T) {
	...
	// pre-fix ordering, where the merge moved the contact but not the row.
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		if _, err := tx.Exec(ctx, `
			UPDATE activity_participant SET contact_id = $1, address = NULL
			 WHERE activity_id = $2`, retired.ContactID, meeting); err != nil {
			return err
		}
		_, err := tx.Exec(ctx, `
			UPDATE contact SET merged_into_id = $1, archived_at = now() WHERE id = $2`,
			survivor.ContactID, retired.ContactID)
		return err
	}); err != nil {
	...
	if !containsContact(owed, survivor.ContactID) {
		t.Fatalf("the sweep does not offer the survivor %s — it selected the retired id, the "+
			"liveness join dropped it, and the meeting is owed to nobody", survivor.ContactID)
	}

	if _, err := e.store.RepairContactCohort(ctx, survivor.ContactID); err != nil {
		t.Fatalf("repair: %v", err)
	}
	if linked, _ := e.cohortStateOf(ctx, t, meeting, survivor.ContactID); !linked {
