...
func sarSections(pkg *SARPackage, contactID ids.ContactID, emails []string, leads, identities []ids.UUID) []sarSection {
	...
	sections = append(sections, sarJudgementSections(pkg, contactID, emails)...)
...
func sarProvenanceSections(pkg *SARPackage) []sarSection {
	return []sarSection{
		...
		// a limited one. An original can outlive the record it was read from —
		// the poll stores it before the activity exists, an erasure can destroy
		// the activity and leave the original for its own retention rule, and an
		// internal-only drop leaves one that never became a record at all. Under
		// a `NOT EXISTS (… audience <> 'workspace')` test every one of those
		// cases reads exactly like permission, so the payload of a message with
		// no surviving row to speak for it would be disclosed in full. Absence
		// is not consent: an original nothing vouches for stays withheld, and
		// that is the whole of what an unset reference means here.
		{&pkg.RawCapture, `SELECT rc.source_system, rc.source_id, rc.received_at,
		       EXISTS (SELECT 1 FROM activity a
		                WHERE a.raw_capture_id = rc.id
