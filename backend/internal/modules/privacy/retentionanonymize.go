...
// retentionactions.go: that file is the dispatch table and the authorable
// set, one concept, and this is the second — everything anonymizeContact
// (retentionactions.go) calls into to strip a subject's identifying fields
// without deleting the record, kept together because
// TestErasingAndAnonymizingClearTheSameTables (backend/gates/contactscrub_test.go)
// reads this act as one unit against the eraser's.
