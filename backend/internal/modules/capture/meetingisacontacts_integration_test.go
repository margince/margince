...
// A meeting is with a contact. A company is not somebody you can meet: it is
// reached through the contact who was in the room.
//
...
func TestACompanyCannotBeMetOrCalled(t *testing.T) {
	...
	for _, kind := range []string{"meeting", "call"} {
		t.Run(kind, func(t *testing.T) {
			...
			if !strings.Contains(err.Error(), "is with a contact") {
...
// TestAMeetingWithAContactIsUntouched proves the rule forbids only the thing it
// means to. The ordinary case — a meeting linked to the contact who was there —
// must still write.
func TestAMeetingWithAContactIsUntouched(t *testing.T) {
	ctx, db := meetingWorkspace(t)

	activityID, _ := seedActivityAndCompany(ctx, t, db, "meeting")
	contactID, ownerID := ids.NewV7(), ids.NewV7()
	...
	if err := db.Tx(ctx, func(tx pgx.Tx) error {
		...
		t.Fatalf("a meeting with the contact who was there must write: %v", err)
	}
}
