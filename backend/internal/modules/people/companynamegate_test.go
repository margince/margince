...
func TestUnrelatedCompaniesAreNotScoredAgainstEachOther(t *testing.T) {
	...
	for _, pair := range unrelated {
		got := bestCompanyNamePairing(pair[0], "", pair[1], "").Confidence
...
func TestAShortNameIsStillAName(t *testing.T) {
	for _, name := range []string{"3M", "HP", "GE", "Box", "Sun", "IBM"} {
		if got := distinctiveCompanyTokens(name); len(got) == 0 {
...
// counts — and it runs inside DedupeCompanyForCreate, which holds the
// workspace-wide company-name write lock. `display_name` is `text` with no
// maxLength in the contract, so one create can hand it a megabyte; unbounded,
// that would pin every company-name writer in the workspace behind it.
//
...
func TestTheTokenLoopIsBounded(t *testing.T) {
	...
	if got := len(distinctiveCompanyTokens(huge)); got > companyGateTokenBudget {
		t.Errorf("a 20,000-word name yields %d tokens, past the %d budget — the "+
			...
			got, companyGateTokenBudget)
	}
	// And the bound must not cost a real name: the longest in the measured
	// corpus was six words.
	if got := len(distinctiveCompanyTokens("Dynamic Air Quality Solutions Pty Ltd")); got == 0 {
...
func TestClassesThatOnlyDomainEvidenceCanAnswer(t *testing.T) {
	...
	for _, pair := range unreachable {
		if got := nameSimilarity(NormalizeCompanyName(pair[0]), NormalizeCompanyName(pair[1])); got >= dedupeReviewThreshold {
