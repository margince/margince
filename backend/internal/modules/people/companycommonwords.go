...
// companyCommonNameWords are ordinary words that many unrelated companies put in
// their names.
//
// DISCOUNTED, NOT DELETED, which is why this is not more entries in
// companyNameStopwords. A stopword is removed from the name because it is never a
...
var companyCommonNameWords = map[string]bool{
...
func weakEvidenceWord(word string) bool {
	return ambiguousFormWord[word] || companyCommonNameWords[word]
}
