...
// "AS Roma"; companyformtables_test.go holds the rest.

// companyFormMarker is one legal form as it appears after folding, split into the
...
// market it claims — see corroboratedMarket in companynameforms.go. An unambiguous
// form needs no corroboration: no company is called "ООО Something" by accident.
type companyFormMarker struct {
	tokens    []string
	ambiguous bool
}
...
type marketForms struct {
	name          string
	prefixes      []companyFormMarker
	continuations []companyFormMarker
	fillers       []companyFormMarker
	...
	// trailing-suffix pass in NormalizeCompanyName; these say what that suffix
...
//
// Hand-written and versioned, for the reason companyNameStopwords is. Seeded from
...
func unambiguous(forms ...[]string) []companyFormMarker {
	markers := make([]companyFormMarker, 0, len(forms))
	for _, tokens := range forms {
		markers = append(markers, companyFormMarker{tokens: tokens})
	}
	return markers
}
...
func ambiguous(forms ...[]string) []companyFormMarker {
	markers := make([]companyFormMarker, 0, len(forms))
	for _, tokens := range forms {
		markers = append(markers, companyFormMarker{tokens: tokens, ambiguous: true})
	}
	return markers
}
...
var arabic = marketForms{
	...
	// census in companynameforms_test.go fails the build over.
	prefixes: unambiguous(
