...
// dossier-backed signatures each buy a company still named by its
// domain.
func TestDecideCompanyNameWeighsCorroboration(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			candidate: CompanyNameCandidate{
				DisplayName: "Gitex",
				Signatures:  []SignatureCompanyName{{PersonID: alice, Value: "Gitex Global GmbH"}},
			},
			wantVerdict:       true,
			wantName:          "Gitex Global GmbH",
			wantCorroborated:  false,
			wantCorroboration: CompanyNameCorroborationNone,
		},
		{
			// Two people at one company are two mailboxes on ONE mail
			...
			wantCorroborated:  false,
			wantCorroboration: CompanyNameCorroborationSignatures,
		},
		{
		...
		},
		{
			...
			wantCorroboration: CompanyNameCorroborationDossier,
		},
		{
		...
		},
		{
			...
			candidate: CompanyNameCandidate{
				DisplayName: "Gitex",
				Signatures:  []SignatureCompanyName{{PersonID: alice, Value: "   "}},
			},
			wantVerdict: false,
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, ok := DecideCompanyName(tc.candidate)
...
// company back every night.
func TestDecideCompanyNameIsDeterministicAcrossEqualEvidence(t *testing.T) {
	alice := ids.New[ids.PersonKind]()
	bob := ids.New[ids.PersonKind]()
	candidate := CompanyNameCandidate{
		DisplayName: "Gitex",
	...
	first, ok := DecideCompanyName(candidate)
	if !ok {
...
func TestOnlyTheDossierAuthorizesAnUnattendedRename(t *testing.T) {
	...
	candidate := CompanyNameCandidate{DisplayName: "Gitex", Signatures: signatures}
	...
	if got.Corroboration != CompanyNameCorroborationSignatures {
		t.Errorf("corroboration = %q, want %q — agreement is still recorded, it just does not authorize the write",
			got.Corroboration, CompanyNameCorroborationSignatures)
	}
	...
	if !got.Corroborated || got.Corroboration != CompanyNameCorroborationDossier {
		t.Errorf("dossier-backed verdict = (corroborated %v, %q), want (true, %q)",
			got.Corroborated, got.Corroboration, CompanyNameCorroborationDossier)
	}
}
...
func TestDecideCompanyNameKeyIsTheNormalizedClaim(t *testing.T) {
	alice := ids.New[ids.PersonKind]()
	bob := ids.New[ids.PersonKind]()

	spelled, ok := DecideCompanyName(CompanyNameCandidate{
		DisplayName: "Gitex",
		Signatures:  []SignatureCompanyName{{PersonID: alice, Value: "Gitex Global GmbH"}},
	})
	if !ok {
	...
	restyled, ok := DecideCompanyName(CompanyNameCandidate{
		DisplayName: "Gitex",
		Signatures:  []SignatureCompanyName{{PersonID: bob, Value: "  GITEX   GLOBAL  GMBH "}},
	})
	if !ok {
...
func TestDecideCompanyNameReportsTheSigningPersons(t *testing.T) {
	alice := ids.New[ids.PersonKind]()
	bob := ids.New[ids.PersonKind]()
	carol := ids.New[ids.PersonKind]()
	got, ok := DecideCompanyName(CompanyNameCandidate{
		DisplayName: "Gitex",
		Signatures: []SignatureCompanyName{
			{PersonID: alice, Value: "Gitex Global GmbH"},
			{PersonID: bob, Value: "Gitex Global"},
