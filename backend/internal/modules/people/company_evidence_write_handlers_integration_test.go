...
// The transport for the company-360 evidence writes: a correction and a
...
func TestUpdateCompanyProfileFieldHandler(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	...
	req := httptest.NewRequest(http.MethodPatch, "/companies/x/profile-fields/icp",
		strings.NewReader(`{"value":"Utilities"}`)).WithContext(ctx)
	h.UpdateCompanyProfileField(rec, req, crmcontracts.Id(companyID.UUID), "icp",
		crmcontracts.UpdateCompanyProfileFieldParams{})
	...
	// addressing it by company alone would rewrite the whole profile.
	fields, err := e.store.ListCompanyProfileFields(ctx, companyID)
	if err != nil {
		t.Fatal(err)
	}
...
func TestConfirmCompanyProfileFieldHandler(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	companyID := seedCompanyWithEvidence(ctx, t, e)
	h := Handlers{store: e.store}
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/companies/x/profile-fields/icp/confirm", nil).WithContext(ctx)
	h.ConfirmCompanyProfileField(rec, req, crmcontracts.Id(companyID.UUID), "icp",
		crmcontracts.ConfirmCompanyProfileFieldParams{})
	if rec.Code != http.StatusOK {
	...
	listed, err := e.store.ListCompanyProfileFields(ctx, companyID)
	if err != nil {
	...
	// A foreign/absent company is existence-hidden rather than refused.
	rec = httptest.NewRecorder()
	h.ConfirmCompanyProfileField(rec, req, crmcontracts.Id(ids.NewV7()), "icp",
		crmcontracts.ConfirmCompanyProfileFieldParams{})
	if rec.Code != http.StatusNotFound {
		t.Fatalf("absent-company confirm status = %d, want 404", rec.Code)
	}
}

func TestUpdateCompanyFactHandler(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	companyID := seedCompanyWithEvidence(ctx, t, e)
	h := Handlers{store: e.store}
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPatch, "/companies/x/facts/phone:",
		strings.NewReader(`{"value":"+49 30 9999"}`)).WithContext(ctx)
	h.UpdateCompanyFact(rec, req, crmcontracts.Id(companyID.UUID), "phone:",
		crmcontracts.UpdateCompanyFactParams{})
	if rec.Code != http.StatusOK {
	...
	facts, err := e.store.ListCompanyFacts(ctx, companyID)
	if err != nil {
		t.Fatal(err)
	}
	for _, f := range facts {
...
func TestConfirmCompanyFactHandler(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	companyID := seedCompanyWithEvidence(ctx, t, e)
	h := Handlers{store: e.store}
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/companies/x/facts/confirm", nil).WithContext(ctx)
	h.ConfirmCompanyFact(rec, req, crmcontracts.Id(companyID.UUID), "certification:iso 9001",
		crmcontracts.ConfirmCompanyFactParams{})
	if rec.Code != http.StatusOK {
