...
// employerOf answers which company a person's live employment edge names.
...
func TestTheSweepAttachesACompanysPeopleThatNoWriteReaches(t *testing.T) {
	...
	// creates the person and opens the company question rather than
	...
	for _, person := range []struct {
		...
		if *employer != ids.UUID(company.Id) {
			t.Errorf("%s contact works at %s, want the company just created %s", person.name, *employer, company.Id)
		}
	}
	...
	for _, d := range after {
		if d.CompanyID.UUID == ids.UUID(company.Id) {
			t.Errorf("the sweep still offers %s after attaching its people; the selection does not drain", company.Id)
		}
	}
}
...
//
// uq_rel_employment admits one live employment per (person, company), so
...
func TestTheSweepSkipsADomainWhosePeopleThePlantWillRefuse(t *testing.T) {
	...
	if err := e.store.tx(ctx, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `
			...
			person.PersonID, ids.UUID(company.Id))
		return err
	}); err != nil {
		t.Fatal(err)
	}
