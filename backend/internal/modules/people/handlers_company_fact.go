...
// and confirm verbs in handlers_company.go rather than in it: that file
...
// CreateCompanyFact records a fact a person states about a company.
//
...
// happened to find.
func (h Handlers) CreateCompanyFact(w http.ResponseWriter, r *http.Request,
	id crmcontracts.Id, _ crmcontracts.CreateCompanyFactParams,
) {
	var req crmcontracts.CreateCompanyFactRequest
	if !httperr.Decode(w, r, &req) {
		return
	}
	out, err := h.store.CreateCompanyFact(r.Context(),
		pathID[ids.CompanyKind](id),
		FactCreateInput{
...
func (h Handlers) DeleteCompanyFact(w http.ResponseWriter, r *http.Request,
	id crmcontracts.Id, factKey crmcontracts.FactKey,
	_ crmcontracts.DeleteCompanyFactParams,
) {
	ifVersion, ok := httperr.IfMatchVersion(w, r)
	if !ok {
		return
	}
	if err := h.store.DeleteCompanyFact(r.Context(),
		pathID[ids.CompanyKind](id), factKey,
