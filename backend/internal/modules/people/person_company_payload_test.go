...
// The person + company family: drives the payload-builder functions
// the person/company emit sites call — promotedPersonPayload
...
func TestCompanySaveEventPayload_Created(t *testing.T) {
	...
	if !reflect.DeepEqual(payload.EventType(), "company.created") {
		t.Errorf("got %v, want %v", payload.EventType(), "company.created")
	}
	created, ok := payload.(crmcontracts.PublicEventCompanyCreated)
	if !ok {
		t.Error("expected the condition to be true")
	}
	...
	var decoded crmcontracts.PublicEventCompanyCreated
	if json.Unmarshal(raw, &decoded) != nil {
		t.Fatalf("unexpected error: %v", json.Unmarshal(raw, &decoded))
	}
	if !reflect.DeepEqual(decoded, created) {
...
func TestCompanySaveEventPayload_Updated(t *testing.T) {
	...
	if !reflect.DeepEqual(payload.EventType(), "company.updated") {
		t.Errorf("got %v, want %v", payload.EventType(), "company.updated")
	}
	updated, ok := payload.(crmcontracts.PublicEventCompanyUpdated)
	if !ok {
		t.Error("expected the condition to be true")
	}
...
func TestColdStartApplyPayload_CreatedFromDomainFallback(t *testing.T) {
	...
	// empty name — matching the value resolveOrCreateColdStartCompany stores.
	in := ApplyColdStartProfileInput{
	...
	created, ok := payload.(crmcontracts.PublicEventCompanyCreated)
	if !ok {
		t.Fatal("expected a company.created payload")
	}
...
func TestRelationshipUpdatedPayload_PerAnchor(t *testing.T) {
	...
	companyPayload := relationshipUpdatedPayload("company", delta)
	if !reflect.DeepEqual(companyPayload.EventType(), "company.updated") {
		t.Errorf("got %v, want %v", companyPayload.EventType(), "company.updated")
	}
	if !reflect.DeepEqual(companyPayload.(crmcontracts.PublicEventCompanyUpdated).ChangedFields, delta) {
		t.Errorf("got %v, want %v", companyPayload.(crmcontracts.PublicEventCompanyUpdated).ChangedFields, delta)
	}
}
