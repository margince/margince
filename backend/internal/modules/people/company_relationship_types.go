...
// of the split that retired company.classification.
//
...
// rest.
//
// The rows are an owned child of the company, written only through the
// company's own gated paths — the patch below and the partner upsert —
...
const relationshipTypePartner = string(crmcontracts.CompanyRelationshipTypesPartner)
...
var validRelationshipTypes = map[string]bool{
	string(crmcontracts.CompanyRelationshipTypesCustomer):         true,
	string(crmcontracts.CompanyRelationshipTypesPartner):          true,
	string(crmcontracts.CompanyRelationshipTypesSupplier):         true,
	string(crmcontracts.CompanyRelationshipTypesInvestor):         true,
	string(crmcontracts.CompanyRelationshipTypesPortfolioCompany): true,
	string(crmcontracts.CompanyRelationshipTypesCompetitor):       true,
	string(crmcontracts.CompanyRelationshipTypesOther):            true,
}
...
// attachCompanyRelationshipTypes fills the wire field for a page of companies
// in one query, the same batch shape attachCompanyDomains uses.
//
// A company with no types gets an EMPTY slice rather than a nil one: the
// reader asked what this company is to us and the answer is "nothing recorded",
// which is a fact, not a missing field.
func attachCompanyRelationshipTypes(ctx context.Context, tx pgx.Tx, companies []crmcontracts.Company) error {
	if len(companies) == 0 {
		return nil
	}
	...
	for i := range companies {
		...
		empty := []crmcontracts.CompanyRelationshipTypes{}
		companies[i].RelationshipTypes = &empty
	}

	rows, err := tx.Query(ctx,
		`SELECT company_id, relationship_type
		 FROM company_relationship_type
		 WHERE company_id = ANY($1) AND archived_at IS NULL
		 ORDER BY relationship_type`, companyIDs)
	if err != nil {
		return err
	}
	defer rows.Close()
	for rows.Next() {
		...
		var relType string
		if err := rows.Scan(&companyID, &relType); err != nil {
			return err
		}
		o, ok := idx[openapi_types.UUID(companyID)]
		if !ok {
			continue
		}
		*o.RelationshipTypes = append(*o.RelationshipTypes, crmcontracts.CompanyRelationshipTypes(relType))
	}
	return rows.Err()
}
...
func readLiveRelationshipTypes(ctx context.Context, tx pgx.Tx, companyID ids.CompanyID) ([]string, error) {
	rows, err := tx.Query(ctx,
		`SELECT relationship_type FROM company_relationship_type
		 WHERE company_id = $1 AND archived_at IS NULL
		 ORDER BY relationship_type`, companyID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// hasPartnerRow reports whether the company carries the partner program
// extension — the other half of the invariant this file guards.
func hasPartnerRow(ctx context.Context, tx pgx.Tx, companyID ids.CompanyID) (bool, error) {
	var exists bool
	// LIVE rows only, matching what the partner API itself calls a partner
	// (partner.go:298). Counting archived ones would force a company to
	...
	err := tx.QueryRow(ctx,
		`SELECT EXISTS (SELECT 1 FROM partner
		                 WHERE company_id = $1 AND archived_at IS NULL)`, companyID).Scan(&exists)
	return exists, err
}

// reconcileCompanyRelationshipTypes applies the replace-set: insert what is new,
...
func reconcileCompanyRelationshipTypes(
	ctx context.Context,
	tx pgx.Tx, companyID ids.CompanyID, source, by string, desired []string,
) ([]string, error) {
	...
	isPartner, err := hasPartnerRow(ctx, tx, companyID)
	if err != nil {
		return nil, err
	}
	switch {
	...
	for _, t := range desired {
		...
		if _, err := tx.Exec(ctx,
			`INSERT INTO company_relationship_type (company_id, relationship_type, source, captured_by)
			 VALUES ($1, $2, $3, $4)`,
			companyID, t, source, by); err != nil {
	...
	for _, t := range live {
		...
		if _, err := tx.Exec(ctx,
			`UPDATE company_relationship_type SET archived_at = now()
			 WHERE company_id = $1 AND relationship_type = $2 AND archived_at IS NULL`,
			companyID, t); err != nil {
...
// ensureCompanyRelationshipType asserts one type without disturbing the others.
...
func ensureCompanyRelationshipType(
	ctx context.Context, tx pgx.Tx, companyID ids.CompanyID, relType, source, by string,
) error {
	_, err := tx.Exec(ctx,
		`INSERT INTO company_relationship_type
		   (company_id, relationship_type, source, captured_by)
		 SELECT $1, $2, $3, $4
		 WHERE NOT EXISTS (
		   SELECT 1 FROM company_relationship_type
		   WHERE company_id = $1 AND relationship_type = $2 AND archived_at IS NULL)`,
		companyID, relType, source, by)
	if err != nil {
...
// moveCompanyRelationshipTypes re-homes the loser's types onto the survivor in a
...
// unique index admits one live row per (company, type) and the survivor's
// own row is the one with the older provenance worth keeping.
func moveCompanyRelationshipTypes(ctx context.Context, tx pgx.Tx, from, to ids.CompanyID) error {
	if _, err := tx.Exec(ctx,
		`UPDATE company_relationship_type l SET archived_at = now()
		 WHERE l.company_id = $1 AND l.archived_at IS NULL
		   AND EXISTS (SELECT 1 FROM company_relationship_type s
		               WHERE s.company_id = $2 AND s.relationship_type = l.relationship_type
		                 AND s.archived_at IS NULL)`, from, to); err != nil {
		return fmt.Errorf("archive duplicate relationship types on merge: %w", err)
	}
	if _, err := tx.Exec(ctx,
		`UPDATE company_relationship_type SET company_id = $2
		 WHERE company_id = $1 AND archived_at IS NULL`, from, to); err != nil {
