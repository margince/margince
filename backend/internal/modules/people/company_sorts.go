...
// the canonical fact. This picks the same row attachCompanyDomains renders from —
...
func orderByPrimaryDomain(context.Context, func(any) int) (string, error) {
	return `(SELECT wd.domain FROM company_domain wd
	          WHERE wd.company_id = company.id
...
func orderByOpenDealCount(ctx context.Context, arg func(any) int) (string, error) {
	...
	return storekit.SQLf(
		`(SELECT pipeline_sort.open_deal_count
		    FROM company_open_pipeline_rollup($%d) pipeline_sort
		   WHERE pipeline_sort.company_id = company.id)`, arg(rollupAsOf())), nil
}
