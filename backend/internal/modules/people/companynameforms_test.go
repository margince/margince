...
func TestEveryVietnameseNameReducesToItsBrand(t *testing.T) {
	for _, company := range vietnameseCorpus {
		if got := companyNameForMatching(company.name); got != company.brand {
...
func TestNoVietnameseCompanyMatchesAnyOther(t *testing.T) {
	...
	for i, left := range vietnameseCorpus {
		for _, right := range vietnameseCorpus[i+1:] {
			...
			pairs++
			got := bestCompanyNamePairing(left.name, "", right.name, "").Confidence
...
func TestABrandIsNotDeletedBecauseItLooksLikeBoilerplate(t *testing.T) {
	...
	for _, k := range kept {
		if got := companyNameForMatching(k.name); got != k.brand {
		...
		if got, want := distinctiveCompanyTokens(k.name), strings.Fields(k.brand); len(got) != len(want) {
...
func TestTheVietnameseRulesDoNotReachOtherMarkets(t *testing.T) {
	...
	for _, k := range []struct{ name, want string }{
		...
		if got := companyNameForMatching(k.name); got != k.want {
	...
	for _, p := range [][2]string{
		...
		if got := bestCompanyNamePairing(p[0], "", p[1], "").Confidence; got < dedupeReviewThreshold {
	...
	for _, name := range []string{"Bank of the West", "Bank of the East"} {
		if got := distinctiveCompanyTokens(name); len(got) != 2 {
...
// the same rule bestCompanyNamePairing already applies to two blank legal names.
func TestAllBoilerplateNamesDoNotMatchEachOther(t *testing.T) {
	...
	for _, name := range boilerplate {
		if got := companyNameForMatching(name); got != "" {
	...
	for i, left := range boilerplate {
		for _, right := range boilerplate[i+1:] {
			...
			got := bestCompanyNamePairing(left, "", right, "").Confidence
			if got >= dedupeReviewThreshold {
...
func TestPhraseTableEntriesMatchTheirOwnNormalization(t *testing.T) {
	...
	for _, market := range prefixMarkets {
		groups := map[string][]companyFormMarker{
		...
		for group, markers := range groups {
			for _, marker := range markers {
				...
				if got := NormalizeCompanyName(foldDStroke(joined)); got != joined {
...
func TestEveryMarketStripsItsOwnForms(t *testing.T) {
	for _, market := range prefixMarkets {
		for _, marker := range market.prefixes {
			...
			if got := companyNameForMatching(name); got != "zzbrandzz" {
...
//
// companyNameForMatching only differs from NormalizeCompanyName when a Vietnamese
...
func TestWesternNamesAreUnchanged(t *testing.T) {
	...
	for _, name := range western {
		...
		got := strings.FieldsFunc(companyNameForMatching(name), nameWordSeparators)
		want := strings.FieldsFunc(NormalizeCompanyName(name), nameWordSeparators)
