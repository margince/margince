...
func TestACommonWordIsEvidenceWhenItIsTheWholeName(t *testing.T) {
	...
	if got := bestCompanyNamePairing("Union Pacific Bank", "", "Pacific Union Bank", "").Confidence; got < dedupeReviewThreshold {
...
func TestCommonNameWordsMatchTheirOwnNormalization(t *testing.T) {
	if len(companyCommonNameWords) == 0 {
		t.Fatal("no common words declared — the census would pass against nothing")
	}
	for word := range companyCommonNameWords {
		if got := NormalizeCompanyName(word); got != word {
