...
// no", and existence-hides a foreign company as 404.
...
func TestGetCompanyVatCheckHandler(t *testing.T) {
	e := setupDedupe(t)
	ctx := e.as()
	h := Handlers{store: e.store}
	req := httptest.NewRequest(http.MethodGet, "/companies/x/vat-check", nil).WithContext(ctx)
	...
	h.GetCompanyVatCheck(rec, req, company.Id)
	...
	var got crmcontracts.CompanyVatCheck
	...
	// A foreign or absent company is existence-hidden as 404, same as every other
	// row-scoped read.
	rec = httptest.NewRecorder()
	h.GetCompanyVatCheck(rec, req, crmcontracts.Id(ids.NewV7()))
	if rec.Code != http.StatusNotFound {
		t.Fatalf("foreign company status = %d, want 404", rec.Code)
	}
}
