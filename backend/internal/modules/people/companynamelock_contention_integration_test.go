...
// Which company writers actually wait on the name lock, judged at the CALL
...
// holdCompanyNameLock opens a transaction holding the workspace's company-name
...
func (e *dedupeEnv) holdCompanyNameLock(ctx context.Context, t *testing.T) (pgx.Tx, int) {
	t.Helper()
	...
	// established rather than assumed. assertParkedBeforeTheCompanyRow
	...
	if err := lockCompanyNameWrites(ctx, tx); err != nil {
		t.Fatalf("taking the company-name write identity: %v", err)
	}
	return tx, pid
}
...
// company row, so it cannot hold that row while wanting the lock — the
...
func TestAnEvidenceApplyClearingANameWaitsOnTheNameLock(t *testing.T) {
	...
	company, err := e.store.CreateCompany(ctx, CreateCompanyInput{
		...
		Domains: []CompanyDomainInput{{Domain: "kranz.test", IsPrimary: true}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	go func() {
			...
			_, err = applyEvidenceFieldsWithOverwrite(ctx, tx, workspaceID(ctx), companyID, "site_read", by,
				[]ColdStartFieldInput{{
	...
	if waited, finished := waitUntilBlockedBy(t, holder, pid, done); !waited {
		t.Fatalf("the apply completed without ever waiting on the name lock (err=%v) — "+
			"it can take this company's row lock first and deadlock against a rename", finished)
	}
...
// serialize behind every company write in the installation. Enrichment and
...
func TestAnEvidenceApplyWithoutANameDoesNotWaitOnTheNameLock(t *testing.T) {
	...
	company, err := e.store.CreateCompany(ctx, CreateCompanyInput{
		...
		Domains: []CompanyDomainInput{{Domain: "vogel.test", IsPrimary: true}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
// assertParkedBeforeTheCompanyRow states the invariant DIRECTLY, by asking
...
// lock on `company` says it had not touched the row first. Either alone
// passes over the defect: an apply parked on the row lock is also "waiting", and
// an apply that never reached the lock holds no company lock either.
func assertParkedBeforeTheCompanyRow(ctx context.Context, t *testing.T, holder pgx.Tx, holderPID int) {
	t.Helper()
	...
	// company lookup below resolves an OID local to THIS database, so it
	...
	// belongs to the name lock. holdCompanyNameLock establishes that at the source:
	...
	// Which locks that backend holds on the company table. AccessShareLock
	...
	rows, err := holder.Query(ctx, `
		SELECT mode FROM pg_locks
		 WHERE pid = $1 AND locktype = 'relation' AND granted
		   AND relation = 'company'::regclass
		   AND mode <> 'AccessShareLock'
		 ORDER BY mode`, waiter)
	if err != nil {
		t.Fatalf("reading backend %d's locks on the company table: %v", waiter, err)
	}
	defer rows.Close()
	var held []string
	for rows.Next() {
	...
	if len(held) > 0 {
		t.Fatalf("the apply is parked on the name lock while already holding %v on the company table — "+
