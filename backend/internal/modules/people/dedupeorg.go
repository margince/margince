...
// PO-F-2, the company half of the one dedupe implementation. It lives
...
// CompanyCandidate is the input to PO-F-2.
type CompanyCandidate struct {
	...
	Domains []string
	// ExcludeID drops one company from both tiers. The rename re-check
...
//
// The name axis has no unique index — two companies may legitimately share
...
func DedupeCompanyForCreate(ctx context.Context, tx pgx.Tx, c CompanyCandidate) (CompanyMatch, error) {
	if err := lockCompanyNameWrites(ctx, tx); err != nil {
		return CompanyMatch{}, err
	}
	return DedupeCompany(ctx, tx, c)
}
...
// expression index by syntax, and idx_company_legal_name_trgm is built on the bare
...
func fuzzyCompany(ctx context.Context, tx pgx.Tx, c CompanyCandidate) (CompanyMatch, error) {
	...
	// Tier 1 (exactCompanyByDomain, above) keeps the anchor deliberately: a domain
	...
	rows, err := tx.Query(ctx, `
		SELECT id, display_name, coalesce(legal_name, '') FROM company
	...
	if err != nil {
		return CompanyMatch{}, fmt.Errorf("dedupe company candidate set: %w", err)
	}
	defer rows.Close()

	var ranked []CompanyCandidateScore
	for rows.Next() {
		var id ids.CompanyID
		var name, rowLegal string
		if err := rows.Scan(&id, &name, &rowLegal); err != nil {
			return CompanyMatch{}, fmt.Errorf("scan company candidate: %w", err)
		}
		score := bestCompanyNamePairing(
	...
	if err := rows.Err(); err != nil {
		return CompanyMatch{}, fmt.Errorf("drain company candidates: %w", err)
	}
	if len(ranked) == 0 {
		return CompanyMatch{Decision: DecisionNoMatch}, nil
	}
	...
	slices.SortFunc(ranked, func(a, b CompanyCandidateScore) int {
...
// repeated arm buys nothing but a longer query under the company-name
// write lock.
func searchAxes(c CompanyCandidate) []string {
	var axes []string
	for _, name := range []string{c.DisplayName, c.LegalName} {
		for _, axis := range []string{NormalizeCompanyName(name), companyNameForMatching(name)} {
...
// sequentially scan the table — on the common case, since most companies
...
func companyTrigramArms(args *[]any, axes ...string) []string {
	var arms []string
	for _, axis := range axes {
		...
		*args = append(*args, axis)
		...
		// workspace-wide company-name write lock is held.
		containment := len(distinctiveCompanyTokens(axis)) > 0
...
// bestCompanyNamePairing scores every pairing of the two name axes and reports
...
// (companynamegate.go). Jaro-Winkler cannot see a word boundary, and on company
...
//
// The gate and the score read the SAME string (companyNameForMatching), so a word
...
func bestCompanyNamePairing(candidateDisplay, candidateLegal, rowDisplay, rowLegal string) CompanyCandidateScore {
	sides := []struct {
		value string
		field string
