...
// partner_company_id and partner_attribution are one fact stored in two columns,
...
// companyIDPtr builds the id argument a caller supplies when naming a partner.
func companyIDPtr(t *testing.T) *ids.CompanyID {
	t.Helper()
...
func dealNamingPartner(attribution string) crmcontracts.Deal {
	partner := openapi_types.UUID(ids.New[ids.CompanyKind]().UUID)
	claim := crmcontracts.DealPartnerAttribution(attribution)
	return crmcontracts.Deal{PartnerCompanyId: &partner, PartnerAttribution: &claim}
}
...
func samePartnerRestated(attribution string) struct {
	...
	same := ids.From[ids.CompanyKind](ids.UUID(*current.PartnerCompanyId))
	return struct {
		current crmcontracts.Deal
		in      UpdateDealInput
	}{current: current, in: UpdateDealInput{PartnerCompanyID: &same}}
}
...
func TestWhatADealClaimsAboutThePartnerItNames(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"a bare partner link is the sourced motion": {
			current: crmcontracts.Deal{},
			in:      UpdateDealInput{PartnerCompanyID: companyIDPtr(t)},
			want:    attributionSourced,
		},
		"an explicit claim wins over the default": {
			current: crmcontracts.Deal{},
			in:      UpdateDealInput{PartnerCompanyID: companyIDPtr(t), PartnerAttribution: &influenced},
...
func TestWhatANewbornDealClaimsAboutThePartnerItNames(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"a bare partner link is the sourced motion": {
			in:   CreateDealInput{PartnerCompanyID: companyIDPtr(t)},
			want: &[]string{attributionSourced}[0],
		},
		"an explicit claim wins over the default": {
			in:   CreateDealInput{PartnerCompanyID: companyIDPtr(t), PartnerAttribution: &influenced},
			want: &influenced,
		},
...
func TestANewbornDealsUnknownAttributionIsRefusedBeforeTheDatabaseSeesIt(t *testing.T) {
	...
	_, err := birthAttribution(CreateDealInput{PartnerCompanyID: companyIDPtr(t), PartnerAttribution: &bogus})
...
func TestACreateBodyCarriesItsPartnerThroughToTheStore(t *testing.T) {
	...
	if ids.UUID(partner) != in.PartnerCompanyID.UUID {
		t.Errorf("partner = %v, want %v", in.PartnerCompanyID.UUID, ids.UUID(partner))
	}
	...
	for _, key := range []string{"partner_company_id", partnerAttributionField} {
...
// still carried partner_company_id into ApplyClears would set one column and earn a
...
func TestThePairedClearLeavesTheSingleColumnPath(t *testing.T) {
	...
	rest, partner := splitDealClears(p,
		[]string{"wait_until", filterPartnerCompanyID, filterCompanyID}, current)

	if !partner {
		t.Error("the partner clear was not recognised")
	}
	if len(rest) != 2 || rest[0] != "wait_until" || rest[1] != filterCompanyID {
...
func TestEitherHalfsNameForgetsTheWholePair(t *testing.T) {
	for _, named := range []string{filterPartnerCompanyID, partnerAttributionField} {
...
// deal is created without one — so the contract's nullable company_id must
