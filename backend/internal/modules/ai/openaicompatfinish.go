...
// compatFinishError is this wire's terminal for an upstream that failed after
// the 200 went out.
const compatFinishError = "error"

// terminalCode matches a finish reason spelled as a code. The upstream's own
// terminal is a string a remote party chose, so it is kept only in that shape
// and anything else is stored as the normalized terminal instead.
var terminalCode = regexp.MustCompile(`^[A-Za-z][A-Za-z0-9_]{0,63}$`)
...
func (c openAICompatChoice) terminal(ctx context.Context) (string, error) {
	...
	if c.Message.Refusal != "" || c.FinishReason == finishContentFilter {
		reason := c.NativeFinishReason
		switch {
		case terminalCode.MatchString(reason):
		case c.Message.Refusal != "":
			reason = finishRefusal
		default:
			reason = finishContentFilter
		}
		return "", withheldError{wire: "openai-compat", reason: reason, detail: safeProviderText(ctx, c.Message.Refusal)}
	}
