...
//
// It is the ONE defaulting site for a parsed or stored config: ParseRouting and
// FromStored both end here, so no serving role can construct a router with an
// out-of-range or undefaulted width. NewLocalRouter's callers build by struct
// literal and never reach it, so that constructor applies the upstream default
// itself. The digest is taken last, over the DEFAULTED value, which is what
// makes an omitted width and an explicitly-written default the same binding.
