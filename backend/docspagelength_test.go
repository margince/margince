...
// sees it in the diff and can ask why a new page needs exempting. That is what
// makes "the list only shrinks" a checked statement rather than a hopeful one.
...
func isGeneratedDoc(t *testing.T, path string) bool {
	...
	for i := 0; scanner.Scan() && i < 5; i++ {
		...
		// The marker has to look like a MARKER, not merely contain the phrase: a
		// hand-written page that happens to say "do not edit by hand" in its
		// opening lines would otherwise exempt itself from the budget, silently.
		// Every generator in this tree writes the marker inside an HTML comment.
		if !strings.HasPrefix(line, "<!--") {
			continue
		}
		if strings.Contains(line, generatedDocMarker) {
			return true
		}
	}
