...
// auto-enrich lane beside it already argues: contact.created reaches the outbox
...
// startContactDataEnrich subscribes the consumer that buys a newly created
// contact's data from the connected provider.
//
...
func startContactDataEnrich(
	...
	consumer := compose.NewContactDataEnrich(pool, store, logger)
	_, _ = fmt.Fprintln(stdout, "worker enriching newly created contacts from the connected data provider (cg:contact-data)")
	background.Go(func() { runSubscriber(ctx, rdb, "cg:contact-data", consumer.HandleEvent, logger, 0) })
	return nil
}
