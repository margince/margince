...
func rotateSetupToken(ctx context.Context, dsn string, stdout io.Writer) error {
	...
	if errors.Is(err, identity.ErrAlreadyProvisioned) {
		return errors.New("migrate setup-token: this installation already has a workspace — there is nothing left to claim; use `migrate reset-password` to recover an account")
	}
	if err != nil {
		return err
	}
