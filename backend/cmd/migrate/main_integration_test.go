...
// ensure_template string-compares. workspace-exists is the same kind of contract for
...
// TestWorkspaceExistsAnswersTheEntrypointsQuestion pins the three states the deploy
...
func TestWorkspaceExistsAnswersTheEntrypointsQuestion(t *testing.T) {
	maint, base, withDB := testDSNs(t)
	...
	if out := mustMigrate(t, "workspace-exists", "--dsn", dsn); out != "false\n" {
		t.Fatalf("workspace-exists against a migrated but unbootstrapped installation printed %q, want %q", out, "false\n")
	}
	...
	if out := mustMigrate(t, "workspace-exists", "--dsn", dsn); out != "true\n" {
		t.Fatalf("workspace-exists against a bootstrapped installation printed %q, want %q", out, "true\n")
	}
	...
	if out := mustMigrate(t, "workspace-exists", "--dsn", dsn); out != "false\n" {
		t.Fatalf("workspace-exists counted an ARCHIVED workspace as present (printed %q); the api's boot count ignores it, so the two disagree", out)
	}
}
