...
// the company-exists answer and captures the literal it expects.
var entrypointProvisionedPattern = regexp.MustCompile(`\[ "\$provisioned" = "([^"]*)" \]`)

// TestCompanyExistsAnswerMatchesTheEntrypointComparison pins the other half of a
...
func TestCompanyExistsAnswerMatchesTheEntrypointComparison(t *testing.T) {
	...
	// The exact call companyExists makes to report a provisioned installation. The
	...
	if got := strings.TrimSpace(out.String()); got != want {
		t.Errorf("company-exists prints %q for a provisioned installation but %s branches on %q — the entrypoint would write a plaintext bootstrap credential onto a live installation; change both together", got, script, want)
	}
}
