...
var predicateEdgeReads = gatekit.Waive(map[string]string{
	...
	"internal/compose/projectattribution.go:LiveProjectsReached":               "the candidate arm of the project-attribution ladder, which runs as the CAPTURE principal — a connector holding activity, person, organization, project and deal, and no relationship grant, because reading edges is not what capture does. The edge decides which projects a message could be about; no edge column or endpoint pair reaches anyone, and the project id it yields is staged as an approval that re-checks project.read against the DECIDING human. Gating this on relationship.read refused every candidate for every connector, which reads as the ladder having nothing to propose rather than as a refusal",
