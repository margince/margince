...
var predicateEdgeReads = gatekit.Waive(map[string]string{
	...
	"internal/modules/projects/surface.go:ListProjectsForOrganizationTx": "the company arm of the company page's project list: the edge decides WHICH projects the page lists, and no edge column or endpoint pair reaches the caller — the project cards do, over the caller's own project row scope. Removing the arm would widen the list to every project, never narrow it. The cost is that a card's presence weakly reflects that the company is on that project, which is what the page is FOR. Keyed to this function rather than the file because its neighbour, ListProjectsForPersonTx, carries the real gate",
	"internal/compose/projectattribution.go":                             "the candidate arm of the project-attribution ladder: the edge decides which live projects a captured message could belong to, and the answer is a proposal a human decides — no edge column reaches a caller, and the projects that do carry the caller's own project row scope. The cost is that a proposal weakly reflects that the sender's company is on that project",
